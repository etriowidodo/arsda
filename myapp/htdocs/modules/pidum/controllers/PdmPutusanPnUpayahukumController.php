...
use app\modules\pidum\models\PdmPutusanPnSearch;
...
*/
    ...
    public function actionUpdate()
    {
        ...
        if(!isset($no_akta)){
            return $this->redirect(['//pidum/pdm-rp11/index']);
        }
        $rp11   = PdmRp11::findOne(['no_akta'=>$no_akta]);

        $model = PdmPutusanPn::findOne(['no_akta'=>$no_akta, 'status_yakum'=>$rp11->id_status_yakum]);
        
        if($model == NULL){
            $model = PdmPutusanPn::find()->where(['no_register_perkara'=>$no_register_perkara])
                    ->andWhere(['is','status_yakum', NULL])->One();
        }
        
        $searchModelBerkas = new PdmGridTahap2Search();
        $dataProviderBerkas= $searchModelBerkas->search(Yii::$app->request->queryParams);
        $tersangka         = VwTerdakwaT2::findAll(['no_register_perkara'=>$no_register_perkara, 'no_reg_tahanan'=>$no_reg_tahanan]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try{    
                    ...
                    if($model->status_yakum !== NULL){
                        $model = new PdmPutusanPn();
                    }

                    $model->no_surat             = $_POST['PdmPutusanPn']['no_surat'];
                    ...
                    $model->status_yakum         = $rp11->id_status_yakum;
                    ...
                    if($model->save()){
                        ...
                        foreach ($terdakwa as $key => $value2) {
                                $modelTerdakwa = new PdmPutusanPnTerdakwa();
                                $modelTerdakwa->status_yakum        = $rp11->id_status_yakum;
                                $modelTerdakwa->no_akta             = $no_akta;
                                $modelTerdakwa->no_register_perkara = $no_register_perkara;                    
                                $modelTerdakwa->no_surat            = $no_surat;
                                $modelTerdakwa->no_reg_tahanan      = $no_reg_tahanan ;
                                $modelTerdakwa->status_rentut       = 3;
