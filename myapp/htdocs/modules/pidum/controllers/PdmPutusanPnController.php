 ...
 */
class PdmPutusanPnController extends Controller
{
...
//                            echo '<pre>';print_r($model_pn);exit();
        ...
        //                    $transaction->rollBack();
                            echo '<pre>';print_r($model_pn->getErrors().'wtf');exit;
                            Yii::$app->getSession()->setFlash('success', [
                                'type' => 'danger',
                                'duration' => 3000,
                                'icon' => 'glyphicon glyphicon-ok-sign', //String
...
*/
    ...
    public function actionUpdate()
    {
        ...
        $model = PdmPutusanPn::find()->where(['no_register_perkara'=>$no_register_perkara])->andWhere(['is','status_yakum',NULL])->One();
        if($model == NULL){
...
//                        echo '<pre>';print_r($pasal);exit();
                        ...
                        foreach ($terdakwa as $key => $value2) {
                           ...
                           //{
                                ...
                                $modelTerdakwa->no_urut_tersangka = $key;
                                $modelTerdakwa->no_reg_tahanan = VwTerdakwaT2::findOne(['no_register_perkara'=>$no_register_perkara, 'no_urut_tersangka'=>$key])->no_reg_tahanan;
                                $modelTerdakwa->status_rentut  = 3;
