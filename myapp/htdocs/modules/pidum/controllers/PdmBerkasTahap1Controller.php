...
use app\modules\pidum\models\PdmBerkasTahap1GridSearch;
...
//        echo $berkas.'|'.$no_pengantar;exit();
        if($no_pengantar == ''){
            ...
            return $this->redirect(['../pidum/pdm-berkas-tahap1/pendapat?id_berkas='.$berkas.'&id_pengantar='.$no_pengantar.'&id_ceklist='.$berkasdanpengantar]);
...
//        echo $id_pengantar.' '.$id_berkas.' '.$id_ceklist;exit();
        ...
        if ($modelCeklis->load(Yii::$app->request->post())) {
      	        ...
      	        $filename = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) .'/ceklist'.$jml_ceklist['jml'].'.odt';
...
//                        echo $_POST['PdmCeklistTahap1']['file_upload'];exit();
            ...
            if ($upload != false) {
                ...
                if (file_exists($filename)) {
                    ...
                    $path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/ceklist'.$jml_ceklist['jml'].'.' . $upload->extension;  //penamaan file ke folder pidum surat
                    $upload->saveAs($path);
                }
            }
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post())) {
              ...
              $hapus_pengantar = $_POST['hapuspengantar'];
                           if(isset($hapus_pengantar))
                           {
                                ...
                                if($count_hapus_pengantar>0){
                                    ...
                                    return $this->redirect(['../pidum/pdm-berkas-tahap1/index']);
                                }
            ...
            try {
                ...
                return $this->redirect(['../pidum/pdm-berkas-tahap1/update?id='.$model->id_berkas]);
            } catch (Exception $e) {
                $transaction->rollBack();
                Yii::$app->getSession()->setFlash('success', [
                    'type' => 'danger',
...
//                    return $this->render(['update', 'id' => $no_berkas]);    
                    return $this->redirect(['../pidum/pdm-berkas-tahap1/update?id='.$no_berkas]);
