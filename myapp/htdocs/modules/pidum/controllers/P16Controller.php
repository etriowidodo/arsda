...
use app\modules\pidum\models\PdmP16Search;
    ...
    public function actionUnggah(){
        ...
        if ($files != false && !empty($files) ) {
            ...
            $files->saveAs($path);
                ...
                $path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p16_'.$jml_pt['jml'].'.'. $files->extension;
                $files->saveAs($path);
            }
    ...
    public function actionCreate() {
        ...
        // print_r($files);exit;
				if ($files != false && !empty($files) ) {
					...
					$path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p16_'.$jml_pt['jml'].'.'. $files->extension;
					$files->saveAs($path);
				}
...
#Jaksa Peneliti
        ...
        $odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pidum/p16.odt");
	//print_r($odf);exit;
...
//        }else{
//            $odf = new \Odf(Yii::$app->params['report-path']."modules/pidum/template/p16.odt");
