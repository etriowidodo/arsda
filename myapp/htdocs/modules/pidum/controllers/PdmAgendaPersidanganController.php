...
use Yii;
use app\modules\pidum\models\PdmAgendaPersidangan;
use app\modules\pidum\models\PdmAgendaPersidanganSearch;
...
class PdmAgendaPersidanganController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $majelis_hak    = "select * from pidum.pdm_agenda_persidangan where no_register_perkara='$no_register' order by no_agenda desc limit 1";
...
//                echo '<pre>';print_r($model);exit();
    ...
    public function actionUpdate($no_agenda)
    {
        ...
        $kode_kejari    = $session->get('kode_kejari');
        $kode_cabjari   = $session->get('kode_cabjari');
        $model          = PdmAgendaPersidangan::findOne(['no_agenda'=>$no_agenda, 'no_register_perkara'=>$no_register]);
        $modelsdg       = PdmMsStatusData::findAll(['is_group'=> 'P-39 ']);
...
//                $text2                      = json_encode(array($_POST['txt_nama_surat2'],$_POST['txt_nama_surat2_2']));
    ...
    public function actionDelete()
    {
        ...
        $id_tahap = $_POST['hapusIndex'][0];
            ...
            if($id_tahap=='all'){
                    $id_tahapx=PdmAgendaPersidangan::find()->select("no_agenda")->where(['no_register_perkara'=>$no_register_perkara])->asArray()->all();
        ...
        $count = 0;
           foreach($id_tahap AS $key_delete => $delete){
            //echo '<pre>';print_r($delete);exit;
             try{
                    PdmAgendaPersidangan::deleteAll(['no_register_perkara' => $no_register_perkara, 'no_agenda'=>$delete]);
