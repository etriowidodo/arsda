...
use app\modules\pidum\models\PdmP22Search;
...
class PdmP22Controller extends Controller {
   ...
   public function actionIndex() {
        ...
        if ($model->load(Yii::$app->request->post())) {
			...
			if ($files != false && !empty($files) ) {
				if($file_lama !=''){
					...
					$path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p22_'.$file_lama.'.'.$files->extension;
					$files->saveAs($path);
				}else{
					$model->file_upload = preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p22_'.$jml_pt['jml'].'.'. $files->extension;
					$path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p22_'.$jml_pt['jml'].'.'.$files->extension;
					$files->saveAs($path);
				}
			}else{
				$model->file_upload = $file_lama;
    ...
    public function actionCetak($id_p22) {
		$id_perkara=Yii::$app->session->get('id_perkara');
        $odf = new Odf(Yii::$app->params['report-path'] . "web/template/pidum/p22.odt");
