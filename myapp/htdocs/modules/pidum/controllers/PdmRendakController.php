...
use app\modules\pidum\models\PdmRendakSearch;
...
//                $modelPengantar = PdmPengantarTahap1::findOne(['id_pengantar' => $id_pengantar]);
        ...
        if ($model->load(Yii::$app->request->post()) ) {
			...
			if ($files != false && !empty($files) ) {
				if($file_lama !=''){
					...
					$path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/rendak_'.$file_lama.'.'.$files->extension;
					$files->saveAs($path);
				}else{
					$model->file_upload = preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/rendak_'.$jml_pt['jml'].'.'. $files->extension;
					$path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/rendak_'.$jml_pt['jml'].'.'.$files->extension;
					$files->saveAs($path);
				}
			}else{
	...
	public function actionCetakblanko($id_berkas) {
		...
		$odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pidum/rendak.odt");
