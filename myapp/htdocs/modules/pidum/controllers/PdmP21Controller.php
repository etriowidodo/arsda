...
use app\modules\pidum\models\PdmP21Search;
...
//            echo $berkas;exit();
            ...
            if ($id_pengantar == ""){
                ...
                return $this->redirect(['../pidum/pdm-p24/index']);
            }
//            echo $id_hasil;exit();
	       ...
	       $transaction = Yii::$app->db->beginTransaction();
            try{
				...
				if ($files != false && !empty($files) ) {
					...
					$path = Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p21_'.$jml_pt['jml'].'.'. $files->extension;
					$files->saveAs($path);
				}
...
//				'id_hasil' => $id_hasil,
    ...
    {	
		...
		PdmTembusan::deleteAll(['id_table' => $model1->id_p21, 'kode_table' => GlobalConstMenuComponent::P21]);
				...
				$filename = Yii::$app->basePath . '/web/template/pidum_surat/' .preg_replace('/[^A-Za-z0-9\-]/', '',$model1->id_perkara). '/p21_'.$model1->id_p21.'.pdf';
    ...
    {	
        ...
        public function actionCetak($id_pengantar,$idp21){
            ...
            $odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pidum/p21.odt");
