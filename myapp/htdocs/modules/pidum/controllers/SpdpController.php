...
use app\modules\pidum\models\PdmSpdpSearch;
    ...
    public function actionUpdate2($id)
    {
        ...
        $modelPasalUpdate = $this->findModelPasal($id);
    	 ...
    	 if ($model->load(Yii::$app->request->post())) {
			...
			rename(Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id), Yii::$app->basePath . '/web/template/pidum_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',substr($model->id_perkara,0,6).$_POST['PdmSpdp']['no_surat']));
...
where b.status = 'SPDP') ";
				...
				foreach($get_perkara as $perkara){
					$dirname = Yii::$app->basePath ."/web/template/pidum_surat/".preg_replace('/[^A-Za-z0-9\-]/', '',$perkara['id_perkara']);
				...
				$dirname = Yii::$app->basePath ."/web/template/pidum_surat/".preg_replace('/[^A-Za-z0-9\-]/', '',$id_tersangka[$i]);
    ...
    public function actionDrop(){
     ...
     public function actionSyncFileClient(){
        ...
        $client->run('http://localhost:90/pidum/spdp/sync-file-server'); //connect to server and start sync
    }
