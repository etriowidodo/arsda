...
class PdmP18Controller extends Controller
{
   ...
   public function actionCreate()
   {
    ...
    public function actionUpdate($id_p18,$id_pengantar)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($files != false && !empty($files))
            {
                ...
                $path = Yii::$app->basePath . '/web/template/pidum_surat/' .preg_replace('/[^A-Za-z0-9\-]/', '',$id).'/p18.' . $files->extension;                        
                $files->saveAs($path);
            }
			
			if(file_exists(Yii::$app->basePath . '/web/template/pidum_surat/' .preg_replace('/[^A-Za-z0-9\-]/', '',$id).'/p18.pdf'))
			{
	...
	public function actionCetak($id_p18,$id_pengantar){
        ...
        $odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pidum/p18.odt");
        $id = Yii::$app->session->get('id_perkara');
