<?php
    ...
    $docx   = new CreateDocxFromTemplate('../web/template/pidum/P-41.docx');
    $satker = Yii::$app->globalfunc->getNamaSatker($spdp->wilayah_kerja)->inst_nama;


    //echo '<pre>';print_r(Yii::$app->globalfunc->JenisPidana());exit;
    ...
    $nota='   ';
    if($p41->kepada=='kejari'){
        $nota = 'NOTA DINAS';
    }

    $nop16a = Yii::$app->globalfunc->GetLastP16a();
    $docx->replaceVariableByText(array('nota'=>$nota), array('parseLineBreaks'=>true));

    $docx->replaceVariableByText(array('nomor_p39'=>$agenda['no_surat_p39']), array('parseLineBreaks'=>true));
    $docx->replaceVariableByText(array('tanggal_p39'=>Yii::$app->globalfunc->ViewIndonesianFormat($agenda['tgl_dikeluarkan'])), array('parseLineBreaks'=>true));
...
//    echo '<pre>';print_r($pasal);exit();
           ...
           $tersangka1 .= '<tr><td width="9%" height="0%" style="font-family:Times New Roman; font-size:12pt;" >'.$i.'.</td>
                            ...
                            <tr><td style="font-family:Times New Roman; font-size:12pt;" ></td>
                                ...
                                <td style="font-family:Times New Roman; font-size:12pt;">'.$rowqry_p29["is_pendidikan"].'</td></tr>
                                    <tr><td></td></tr>
                            <tr><td style="font-family:Times New Roman; font-size:12pt;" ></td>
                                <td colspan="3" style="font-family:Times New Roman; font-size:12pt;" >Telah sampai pada tahap tuntutan pidana (Requisitoir) yang akan dibacakan oleh jaksa penuntut umum pada : hari '.Yii::$app->globalfunc->GetNamaHari($p41["tgl_dikeluarkan"]).' tanggal '.Yii::$app->globalfunc->ViewIndonesianFormat($p41["tgl_dikeluarkan"]).'</td></tr>
                                    <tr><td></td></tr>
                                   ';
           $i++;
        }
    ...
    if (count($p41_tsk) != 0) {
        ...
        foreach ($p41_tsk as $rowtsk => $valtsk){
            //if($rowtsk==0){
                $memberatkan .= '<table border="0"  style="font-family:Times New Roman; font-size:12pt;" width="100%"><tr><td colspan="2"><strong>'.$valtsk['nama'].' :<br>Memberatkan</strong></tr>';
...
//        print_r($udg->undang[0]);exit();
    ...
    $docx->createDocx('../web/template/pidum_surat/P-41');
    $file = '../web/template/pidum_surat/P-41.docx';
    if (file_exists($file)) {
