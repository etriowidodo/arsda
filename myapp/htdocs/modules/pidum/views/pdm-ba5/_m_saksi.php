...
<div class="modal-content" style="width: 780px;margin: 30px auto;">    
    ...
    <div class="modal-footer">
            <a id="pilih-saksi" class="btn btn-warning">Pilih</a>
    </div>
...
$this->registerJs(\yii\web\View::POS_BEGIN);
        $js = <<< JS
        $('#pilih-saksi').click(function () {
            
            $('#saksi-grid input:checkbox:checked').each(function (index) {
                var value = $(this).val();
                ...
                if(jQuery.inArray(data[4], nipBaruValue) > -1) {
                    ...
                    $('#table_saksi_penerima').append(
                        '<tr id="trsaksi-'+data[0]+'">' +
                        '<td><input type="text" name="saksi-no_urut_news[]" style="width: 50px;" class="form-control" value="'+(trCount)+'"></td>' +
                        '<td><input type="text" class="form-control" name="saksi-peg_nip_news[]" readonly="true" value="' + data[4] + '"> </td>' +
                        '<td><input type="text" class="form-control" name="saksi-peg_nama_news[]" readonly="true" value="' + data[1] + '"> </td>' +
                        '<td><input type="text" class="form-control" name="saksi-pangkat_news[]" readonly="true" value="' + data[2] + '"> </td>' +
                        '<td><input type="text" class="form-control" name="saksi-jabatan_news[]" readonly="true" value="' + data[3] + '"> </td>' +
                        '<td id="tdJPU"><a class="btn btn-danger delete" data-id="'+ data[4] +'" id="btn_hapus_saksi"></a></td>' +
                        '<input type="hidden" name="saksi-peg_nip_news[]" value="' + data[0] + '">' +
                        '</tr>'
                    );
                    $('#m_saksi').modal('hide');
                }
