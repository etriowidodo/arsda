...
<?php
	...
	if($(".empty").text()=='Tidak ada data yang ditemukan.'){
        ...
        if(idp21==0)
        {
            var url = window.location.protocol + "//" + window.location.host + "/pidum/pdm-p21/create?id_pengantar="+idpengantar+"&&idp21="+idp21;
            $(location).attr('href',url);
        }
		else
        {
            var url = window.location.protocol + "//" + window.location.host + "/pidum/pdm-p21/update?id_pengantar="+idpengantar+"&&idp21="+idp21;
            $(location).attr('href',url);
        }
	...
	$('#idUbah').click (function (e) {
        ...
        var p21         =$('.checkHapusIndex:checked').attr('data-id');
            if(p21==0)
            {
               var url = window.location.protocol + "//" + window.location.host + "/pidum/pdm-p21/create?id_pengantar="+idpengantar;
                $(location).attr('href',url); 
            }
            else
            {
                var url = window.location.protocol + "//" + window.location.host + "/pidum/pdm-p21/update?id_pengantar="+idpengantar;
                $(location).attr('href',url);                 
    		}
        }
    });
