...
<div class="pdm-rendak-index">
    ...
    <?php
       $form = \kartik\widgets\ActiveForm::begin([
            'id' => 'hapus-index',
            'action' => '/pidum/pdm-rendak/delete/'
        ]);  
    ?>  
...
<?php
	...
	if($(".empty").text()=='Tidak ada data yang ditemukan.'){
        ...
        {
        var url = window.location.protocol + "//" + window.location.host + "/pidum/pdm-rendak/update?id=" + idrendak+"&id_berkas=" +idberkas;
	...
	$('#idUbah').click (function (e) {
        ...
        var url = window.location.protocol + "//" + window.location.host + "/pidum/pdm-rendak/update?id=" + id+"&id_berkas=" +idberkas;
        $(location).attr('href',url);
		//alert(count);
		}
    });
	...
	$('.btnPrintCheckboxIndex').click(function(){
		...
		if (count != 1 )
		{
			...
			if(id=='0'){
				...
				var cetak   = window.location.protocol + "//" + window.location.host + "/pidum/pdm-rendak/cetakblanko?id_berkas="+id; 
