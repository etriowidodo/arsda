...
HTML;
      ...
      <div class="box-footer" style="text-align: center;">
        ...
        <?= Html::a('Batal', $model->isNewRecord ? ['index'] : ['../pidum/pdm-p16a/index'], ['class' => 'btn btn-danger']) ?>
        <!-- END CMS_PIDUM001_16 --> 
        </div>
        <div id="hiddenId">
...
$script1 = <<< JS
// $('#m_alert').load('/pidum/pdm-p16a/alert-sukses');
    ...
    $('#printToSave').click(function(){
        ...
        if(print!=''&&tempat!=''&&tgl!=''&&tandaTangan!='')
        {
            if(jumlahJaksa<1){
                ...
                $.ajax({
                    type        : 'POST',
                    url         :'/pidum/pdm-p16a/cek-no-surat-p16a',
                    data        : 'no_surat_p16a='+print,                                
                    success     : function(data)
                            {
                                if(data>0){
                                    ...
                                    $('input[name="printToSave"]').val('1');
                                         $.ajax({
                                            type: "POST",
                                            async:    false,
                                            url: '/pidum/pdm-p16a/create',
                                            data: $("form").serialize(),
                                            success:function(data){ 
                                            $('.box-footer').hide(); 
                                            var cetak   = '/pidum/pdm-p16a/cetak?id='+data;  
        ...
        {      
         $('input[name="printToSave"]').val('2');
             $.ajax({
                type: "POST",
                async:    false,
                url: '/pidum/pdm-p16a/update?id='+'$model->no_surat_p16a',
                data: $("form").serialize(),
                success:function(data){
