    ...
    <div class="content-wrapper-1">
            ...
            <div class="col-md-12" style="padding-right: 0px;padding-left: 0px;">
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="control-label col-md-3" style="width: 140px;">No SK 
</label>
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="col-sm-12">
                                     <?= $form->field($model, 'no_sk_was_2b')->textInput()->label(false); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_sk_was_2b',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
      ...
      <div class="panel panel-primary">
          <div class="panel-heading">Penandatangan</div>
            ...
            <?php if(!$model->isNewRecord){ ?>
            <div class="col-md-12" style="padding-top: 15px;">
                <label>Unggah Berkas Sk-Was-2b : 
                     <?php if (substr($model->upload_file,-3)!='pdf'){?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_2b'].'&id2='.$model['id_sk_was_2b'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-image-o"></i></span></a>' :'') ?> <?= $model['upload_file']?>
                    <?php } else{?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_2b'].'&id2='.$model['id_sk_was_2b'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-pdf-o"></i></span></a>' :'') ?>  <?= $model['upload_file']?>
                    <?php } ?>
                </label>
 ...
 <?php    
       if (!$model->isNewRecord){
        // if($result_expire=='1'){
          ...
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-2b/index'], ['id' => 'KembaliSkWas2b', 'class' => 'btn btn-primary']); 
        //  }
       }else{
          echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-save"></i> Simpan' : '<i class="fa fa-save"></i> Simpan', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ;
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-2b/index'], ['id' => 'KembaliSkWas2b', 'class' => 'btn btn-primary']);
...
#grid-penandatangan_surat.loading .modal-loading-new {display: block;}
    ...
    .field-skwas2bpelapor-nama_pelapor , .field-skwas2bpelapor-alamat_pelapor , .field-skwas2bpelapor-email_pelapor
    , .field-skwas2bpelapor-telp_pelapor, .field-skwas2bpelapor-pekerjaan_pelapor, .field-skwas2bpelapor-kewarganegaraan_pelapor
    , .field-skwas2bpelapor-agama_pelapor, .field-skwas2bpelapor-pendidikan_pelapor, .field-skwas2bpelapor-nama_kota_pelapor
    , .field-skwas2bpelapor-tanggal_lahir_pelapor, .field-skwas2bpelapor-tempat_lahir_pelapor, .field-skwas2bpelapor-sumber_lainnya
    , .field-skwas2bpelapor-id_sumber_laporan ,.field-skwas2b-nip_pegawai_terlapor ,.field-skwas2b-nama_pegawai_terlapor
    ,.field-skwas2b-pangkat_pegawai_terlapor ,.field-skwas2b-golongan_pegawai_terlapor ,.field-skwas2b-jabatan_pegawai_terlapor
    ,.field-skwas2bpemeriksa-nip_pemeriksa ,.field-skwas2bpemeriksa-nrp_pemeriksa ,.field-skwas2bpemeriksa-nama_pemeriksa
    ,.field-skwas2bpemeriksa-pangkat_pemeriksa ,.field-skwas2bpemeriksa-jabatan_pemeriksa ,.field-skwas2bpemeriksa-golongan_pemeriksa{
        display: none;
    }
 
/* #skwas2b-tgl_sk_was_2b-disp{
    width: 405px;
 }   */
</style>
...
$(document).on('click','#addtembusan',function() {
        ...
        $(document).on('click','#tambah_penandatangan',function() {
          ...
          if(jml=>1 && jml<2){
             $('#skwas2b-nip_penandatangan').val(data.nip);
             $('#skwas2b-nama_penandatangan').val(data.nama);
             $('#skwas2b-jabatan_penandatangan').val(data.nama_jabatan);
             $('#skwas2b-golongan_penandatangan').val(data.gol_kd);
             $('#skwas2b-pangkat_penandatangan').val(data.gol_pangkat2);
             $('#skwas2b-jbtn_penandatangan').val(data.jabtan_asli);
 ...
 /*/////////PENANDATANGAN LOADING GRID//////////////*/
    ...
    $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sk-was-2b/getttd",function(responseText, statusText, xhr)
            {
