            ...
            <div class="col-md-12" style="padding-right: 0px;padding-left: 0px;">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="col-sm-12">
                                     <?= $form->field($model, 'no_sk_was_3c')->textInput()->label(false); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_sk_was_3c',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
            ...
            <div class="col-md-12" style="padding-top: 15px;">
                <label>Unggah Berkas Sk-Was-3c : 
                     <?php if (substr($model->upload_file,-3)!='pdf'){?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_3c'].'&id2='.$model['id_sk_was_3c'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-image-o"></i></span></a>' :'') ?> <?= $model['upload_file']?>
                    <?php } else{?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_3c'].'&id2='.$model['id_sk_was_3c'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-pdf-o"></i></span></a>' :'') ?>  <?= $model['upload_file']?>
                    <?php } ?>
                </label>
 ...
 <?php    
       if (!$model->isNewRecord){
        // if($result_expire=='1'){
          ...
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-3c/index'], ['id' => 'KembaliSkWas3c', 'class' => 'btn btn-primary']);
        //  }
       }else{
          echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-save"></i> Simpan' : '<i class="fa fa-save"></i> Simpan', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ;
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-3c/index'], ['id' => 'KembaliSkWas3c', 'class' => 'btn btn-primary']);
...
#grid-penandatangan_surat.loading .modal-loading-new {display: block;}
    ...
    .field-skwas3cpelapor-nama_pelapor , .field-skwas3cpelapor-alamat_pelapor , .field-skwas3cpelapor-email_pelapor
    , .field-skwas3cpelapor-telp_pelapor, .field-skwas3cpelapor-pekerjaan_pelapor, .field-skwas3cpelapor-kewarganegaraan_pelapor
    , .field-skwas3cpelapor-agama_pelapor, .field-skwas3cpelapor-pendidikan_pelapor, .field-skwas3cpelapor-nama_kota_pelapor
    , .field-skwas3cpelapor-tanggal_lahir_pelapor, .field-skwas3cpelapor-tempat_lahir_pelapor, .field-skwas3cpelapor-sumber_lainnya
    , .field-skwas3cpelapor-id_sumber_laporan ,.field-skwas3c-nip_pegawai_terlapor ,.field-skwas3c-nama_pegawai_terlapor
    ,.field-skwas3c-pangkat_pegawai_terlapor ,.field-skwas3c-golongan_pegawai_terlapor ,.field-skwas3c-jabatan_pegawai_terlapor
    ,.field-skwas3cpemeriksa-nip_pemeriksa ,.field-skwas3cpemeriksa-nrp_pemeriksa ,.field-skwas3cpemeriksa-nama_pemeriksa
    ,.field-skwas3cpemeriksa-pangkat_pemeriksa ,.field-skwas3cpemeriksa-jabatan_pemeriksa ,.field-skwas3cpemeriksa-golongan_pemeriksa{
        display: none;
    }
 
/* #skwas3c-tgl_sk_was_3c-disp{
    width: 405px;
 } */  
</style>
...
$(document).on('click','#addtembusan',function() {
        ...
        $(document).on('click','#tambah_penandatangan',function() {
          ...
          if(jml=>1 && jml<2){
             $('#skwas3c-nip_penandatangan').val(data.nip);
             $('#skwas3c-nama_penandatangan').val(data.nama);
             $('#skwas3c-jabatan_penandatangan').val(data.nama_jabatan);
             $('#skwas3c-golongan_penandatangan').val(data.gol_kd);
             $('#skwas3c-pangkat_penandatangan').val(data.gol_pangkat2);
             $('#skwas3c-jbtn_penandatangan').val(data.jabtan_asli);
    ...
    $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sk-was-3c/getttd",function(responseText, statusText, xhr)
            {
