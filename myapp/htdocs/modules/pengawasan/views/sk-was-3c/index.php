...
<div class="sk-was3c-index">

    <h4><?php ?></h4>
    <?php //  echo $this->render('_search', ['model' => $searchModel]); ?>
<!-- href="/pengawasan/sk-was-3c/create" -->
    <p>
        <div class="btn-toolbar">
              <a class="btn btn-primary btn-sm pull-right" id="cetak_skwas3c"><i class="glyphicon glyphicon-print"> </i> Cetak </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="hapus_skwas3c"><i class="glyphicon glyphicon-trash"> </i> Hapus </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="ubah_skwas3c"><i class="glyphicon glyphicon-pencil"> </i> Ubah </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="create_skwas3c" ><i class="glyphicon glyphicon-plus"> </i> SK-WAS-3c</a>
            </div>
    </p>
    <style type="text/css">
...
legend.group-border {
    ...
    font-size: 14px;
     ...
     <fieldset class="group-border">
        <legend class="group-border">Daftar SK-WAS-3c</legend>
    <?= GridView::widget([
...
//         $("#create_skwas3c").addClass("disabled");
//         $("#ubah_skwas3c").addClass("disabled");
//         $("#hapus_skwas3c").addClass("disabled");
//         $("#cetak_skwas3c").addClass("disabled");
//       }else{
//         $(this).find('.checkbox-row').prop('checked',true);
//         $("#create_skwas3c").removeClass("disabled");
//         $("#ubah_skwas3c").removeClass("disabled");
//         $("#hapus_skwas3c").removeClass("disabled");
//         $("#cetak_skwas3c").removeClass("disabled");
//       }
//       // alert(z);
// });
...
$("#create_skwas3c").addClass("disabled");
  ...
  function ConditionOfButton(n){
        if(n == 1){
           $('#cetak_skwas3c,#ubah_skwas3c,#create_skwas3c, #hapus_skwas3c').removeClass('disabled');
        } else if(n > 1){
           $('#hapus_skwas3c').removeClass('disabled');
           $('#cetak_skwas3c,#ubah_skwas3c,#create_skwas3c').addClass('disabled');
        } else{
           $('#cetak_skwas3c,#ubah_skwas3c,#create_skwas3c, #hapus_skwas3c').addClass('disabled');
        }
    }

    $('#create_skwas3c').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.nip_trlp != null){
            ...
            location.href='/pengawasan/sk-was-3c/create?id='+check.nip_terlapor+'&id2='+check.pasal;
           } 
         }
    });

    $('#ubah_skwas3c').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.id_sk_was_3c == null){
               bootbox.alert({
                      message:"Data Belum Bisa Di Rubah Nomor Harus Di Isi",
                      size:'small'
                  });
             }else{
                location.href='/pengawasan/sk-was-3c/update?id='+check.id_sk_was_3c+'&id2='+check.nip_pegawai_terlapor+'&id3='+check.pasal;
             }
         }
    });

    $(document).on('click', '#cetak_skwas3c', function () {

          // alert()
          var x=$(".checkbox-row:checked").length;
          var link=$(".checkbox-row:checked").val();
          var check=JSON.parse($('.checkbox-row:checked').attr('json'));
         // alert(check.id_sk_was_3c);
          ...
          if(check.id_sk_was_3c == null){
             bootbox.alert({
              ...
              location.href='/pengawasan/sk-was-3c/cetak?id='+check.id_sk_was_3c;
           }
          //alert(nilai);
        });

$(document).on('click','#hapus_skwas3c',function(){
                                ...
                                var checkValues = $('.checkbox-row:checked').map(function()
                                {
                                 ...
                                 var jml=$('.checkbox-row:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/sk-was-3c/delete',
                                            data:'id='+data.id_sk_was_3c+'&jml='+jml,
