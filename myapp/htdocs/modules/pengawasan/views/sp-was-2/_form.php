...
<section class="content" style="padding: 10px;">
  <div class="content-wrapper-1">
    ...
    <div class="box box-primary" style="overflow:hidden;padding:15px 0px 8px 0px;">
      ...
      if($model->isNewRecord) {
        ...
        <div class="alert alert-warning" style="margin:0 15px 15px 15px;">
            <strong>Peringatan!</strong> . Sp.WAS-2 Masih Aktif!. Anda Tidak Bisa Membuat Sp.WAS-2 lagi
        </div>
       ...
       <?php  if(!$model->isNewRecord) {?>
        <div class="col-md-7" style="padding-bottom: 15px;">
            <fieldset class="scheduler-border">
                <legend class="scheduler-border">No Sprint</legend>
                <div class="form-group row">
                    <div class="col-md-8">
                        <?php $nomor_sp2=explode("/H/Hjw/", $model->nomor_sp_was2); 
                        $x1= substr($nomor_sp2[0],5);
                         echo $nomor_sp2[1];
                        ?>
                        <label for="no_sp_was_2" style="padding:0px">No Sprint</label>
                          <div class="form-group field-spwas2-nomor_sp_was2">
                          <div class="col-md-12">
                            <div class="col-sm-4" style="padding-left: 0px;">
                              <input id="spwas2-nomor_sp_was2" class="form-control" name="no_print" value="PRIN-" readonly="readonly" type="text">
                            ...
                            <?php
                                ...
                                // if($model->nomor_sp_was2==''){
                                //   $model->nomor_sp_was2=$default_nomor;
                                // }
                                $model->nomor_sp_was2=$x1;
                                echo $form->field($model, 'nomor_sp_was2')->textInput();
                                ?>
                            </div>
                            <div class="col-sm-4"  style="padding-left: 0px;">
                              <input class="form-control" name="no_hwj" value="<?= ($nomor_sp2[1]=='' ? $default_nomor: '/H/Hjw/'.$nomor_sp2[1] )?>" readonly="readonly" type="text"></input>
                    ...
                    <div class="col-md-4"  style="padding-left: 0px;">
                        ...
                        <?=
                        $form->field($model, 'tanggal_sp_was2',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
                            'type' => DateControl::FORMAT_DATE,
                            'ajaxConversion' => false,
                            'options' => [
                                'pluginOptions' => [
                                    'autoclose' => true,
                                    'startDate' => date("d/m/Y", strtotime($model->tanggal_mulai_sp_was2)),
                                    'endDate' => date("d/m/Y", strtotime($model->tanggal_akhir_sp_was2)),
                                ]
                            ]
                        ]);
                        ?>
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <label for="tanggal_sp_was2" style="padding:0px">Dari Tanggal</label>
                <?php
                if($model->tanggal_mulai_sp_was2!=''){
                  $model->tanggal_mulai_sp_was2=date("d-m-Y", strtotime($model->tanggal_mulai_sp_was2));
                }
               echo  $form->field($model, 'tanggal_mulai_sp_was2',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]]);
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <label for="tanggal_sp_was2" style="padding:0px">Sampai Tanggal</label>
                <?php
                if($model->tanggal_akhir_sp_was2!=''){
                  $model->tanggal_akhir_sp_was2=date("d-m-Y", strtotime($model->tanggal_akhir_sp_was2));
                }
                echo $form->field($model, 'tanggal_akhir_sp_was2',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]]);
      ...
      <?php
            ...
            ?>
      <div class="tampungan dasar_surat<?php echo $dasar_surat['id_dasar_sp_was2'] ?>">
      <div class="col-sm-11" style="margin-top:10px;"><!-- ini default dari LAPDU -->
       <label class="control-label" for="inputsm">Dasar Surat</label>
          <?php//= $form->field($model, 'isi_dasar_sp_was_2[]')->textArea(['value'=>$dasar_surat['id_dasar_surat_sp_was2']]) ?>
          <textarea class="form-control" name="isi_dasar_sp_was_2[]"><?php echo $dasar_surat['isi_dasar_sp_was2'] ?></textarea>
      </div>
        <div class="col-sm-1 hapus" style="margin-top:30px;width:59px;padding:0px"><a  rel="<?php echo $dasar_surat['id_dasar_sp_was2'] ?>" class="btn btn-primary btn-sm pull-right hapus_dasar" id="hapus_dasar"><i class="glyphicon glyphicon-trash"></i></a></div>
      </div>
        <?php
            }
          }else{
          ...
          //             where a.no_register='".$_SESSION['was_register']."' and a.id_tingkat='".$_SESSION['kode_tk']."' and a.id_kejati='".$_SESSION['kode_kejati']."' and a.id_kejari='".$_SESSION['kode_kejari']."' and a.id_cabjari='".$_SESSION['kode_cabjari']."' and a.id_pemeriksa='".$var[2]."'";
          //  $result_dasar1 = $connection->createCommand($sqldasar1)->queryOne();
           $sqldasar10     ="select string_agg(a.nama_pegawai_terlapor,', ') as nama from was.pegawai_terlapor_was10_inspeksi a
          ...
          $sqldasarPegTer ="select string_agg(a.nama_pegawai_terlapor,', ') as nama from was.pegawai_terlapor a
                            ...
                            and a.id_level1='".$_SESSION['id_level_1']."' and a.id_level2='".$_SESSION['id_level_2']."'
                            and a.id_level3='".$_SESSION['id_level_3']."' and a.id_level4='".$_SESSION['id_level_4']."'
                            and a.id_sp_was1=(select distinct max(id_sp_was1) 
     ...
     <div class="col-sm-12" style="margin-bottom: 15px;"><!-- ini default dari LAPDU -->
         <label for="inputsm">Dasar Surat</label>
             <textarea class="form-control" name="isi_dasar_sp_was_2[]"><?=$dasar1?></textarea>
     </div>
     <div class="col-sm-12" style="margin-bottom: 15px;"><!-- ini default dari LAPDU -->
         <!-- <label for="inputsm">Dasar Surat</label> -->
             <textarea class="form-control" name="isi_dasar_sp_was_2[]"><?= $dasar2?></textarea>
     </div>
       <?php 
     // } 
     }
    ...
    <div class="col-sm-12 for_dasar_surat">
    </div>
            ...
            <div class="">
                <div class="">
                    ...
                    <div class="control-group" id="fields">
                        ...
                        <label class="control-label" for="field1">Dasar Surat</label>
                           ...
                           /* if(!$model->isNewRecord){
                            ...
                            ?>
                             <form class="input-append">
                                <div id="field"><input autocomplete="off" class="input" id="field1" type="text" name="isi_dasar_sp_was2[]" value="<?php echo $dasar_surat['isi_dasar_sp_was2'] ?>" /><button id="b1" class="btn add-more btn-flat" type="button"><i class="fa fa-plus" aria-hidden="true"></i></button></div>
                             </form>
                          <?php
                              }
                            }else{
                           ?>
                             <form class="input-append">
                                <div id="field"><input autocomplete="off" class="input" id="field1" type="text" name="isi_dasar_sp_was2[]"/><button id="b1" class="btn add-more btn-flat" type="button"><i class="fa fa-plus" aria-hidden="true"></i></button></div>
                             </form>
                           <? } */?>   
                        </div>
                    </div>
            ...
            <table class="table table-bordered">
                    ...
                    <tbody class="bd_pemeriksa_tmp">
                        ...
                        foreach ($modelPemeriksa as $value_pemeriksa) { ?>
                            <tr class="pemeriksa<?php echo $value_pemeriksa['nip_pemeriksa']; ?>" id="<?= $value_pemeriksa['nip_pemeriksa'] ?>" nilai="<?= $value_pemeriksa['nip_pemeriksa'] ?>" >
                            <td style="text-align:center;"><?= $no_1 ?></td>
                            <td><?= $value_pemeriksa['nip_pemeriksa'].($value_pemeriksa['nrp_pemeriksa']==''?'':'/'.$value_pemeriksa['nrp_pemeriksa'])?></td>
                            <td><?= $value_pemeriksa['nama_pemeriksa']?></td>
                            <td><?= $value_pemeriksa['jabatan_pemeriksa']?></td>
                            <td><?= $value_pemeriksa['pangkat_pemeriksa']?></td>
                  <?php
                            echo "<td width='2%'><input class='selection_one' type='checkbox' name='ck_pr_ubah' rel='pemeriksa".$value_pemeriksa['id_pemeriksa_sp_was2']."' value='".$value_pemeriksa['nip_pemeriksa']."'enama='".$value_pemeriksa['nama_pemeriksa']. "'epangkat='".$value_pemeriksa['pangkat_pemeriksa']."'enip='".$value_pemeriksa['nip_pemeriksa']."'ejabatan='".$value_pemeriksa['jabatan_pemeriksa']."'egolongan='".$value_pemeriksa['golongan_pemeriksa']."'enrp='".$value_pemeriksa['nrp_pemeriksa']."'></td>";
                              ?>
                            <input type="hidden" name="nip_pemeriksa[]" class="nip" value="<?= $value_pemeriksa['nip_pemeriksa']?>">
                            <input type="hidden" name="nama_pemeriksa[]" class="nama_pemeriksa" value="<?= $value_pemeriksa['nama_pemeriksa']?>">
                            <input type="hidden" name="jabatan_pemeriksa[]" class="jabatan" value="<?= $value_pemeriksa['jabatan_pemeriksa'] ?>">
                            <input type="hidden" name="pangkat_pemeriksa[]" class="pangkat" value="<?= $value_pemeriksa['pangkat_pemeriksa']?>">
                            <input type="hidden" name="nrp_pemeriksa[]" class="nrp" value="<?= $value_pemeriksa['nrp_pemeriksa'] ?>">
                            <input type="hidden" name="golongan_pemeriksa[]" class="golongan" value="<?= $value_pemeriksa['golongan_pemeriksa']?>">
                          </tr>
                        <?php 
                        $no_1++;
                            }
                 ...
                 if($model->isNewRecord){/*jika dalam keadaan create ambil terlapor dari terlapor awal*/
                        ...
                        ?>
                          <tr id="trterlapor<?php echo $terlapor['no_urut'] ?>">
                              ...
                              <td class="td_pangkat"><?= ($terlapor['golongan_terlapor_awal']==''?'':$terlapor['golongan_terlapor_awal'].($terlapor['pangkat_terlapor_awal']!=''?'('.$terlapor['pangkat_terlapor_awal'].')':'')) ?></td>
                              <td class="td_nip"><?= ($terlapor['nip_pegawai_terlapor']==''?'' : $terlapor['nip_pegawai_terlapor'].($terlapor['nrp_pegawai_terlapor']!=''?'/'.$terlapor['nrp_pegawai_terlapor']:'')) ?></td>
                              ...
                              <input name="nipTerlapor[]" class="nipTerlapor" value="<?= $terlapor['nip_pegawai_terlapor']?>" type="hidden">
                              ...
                              <input class="selection_one1" type="checkbox" value="" name="ck_tr" rel='<?=$tmp ?>' atrTerlapor="<?= $terlapor['nama_terlapor_awal'].'#'.$terlapor['jabatan_terlapor_awal'].'#'.$terlapor['satker_terlapor_awal'].'####'?>"></td>
                        ...
                        $no=1;
                         ...
                         ?>
                          <tr id="trterlapor<?php echo $terlapor['no_urut'] ?>">
                              ...
                              <input name="jabatanTerlapor[]" class="jabatanTerlapor" value="<?= $terlapor['jabatan_terlapor_awal']?>" type="hidden">
                              <input name="satkerTerlapor[]" class="satkerTerlapor" value="<?= $terlapor['satker_terlapor_awal']?>" type="hidden">
                              <input class="selection_one1" type="checkbox" value="<?= $terlapor['nip']?>" name="ck_tr" rel='<?=$tmp ?>' atrTerlapor="<?= $terlapor['nama_terlapor_awal'].'#'.$terlapor['jabatan_terlapor_awal'].'#'.$terlapor['satker_terlapor_awal'].'#'.$terlapor['nip'].'#'.$terlapor['nrp_pegawai_terlapor'].'#'.$terlapor['pangkat_terlapor_awal'].'#'.$terlapor['golongan_terlapor_awal']?>"></td>
 ...
 <?php
    ...
    // if(!$model->isNewRecord){
    //     $query="select string_agg(nip_pemeriksa,',') as nip_pemeriksa from was.pemeriksa_sp_was2 where no_register='".$_GET['id']."' and id_sp_was2='".$_GET['id_sp_was2']."' and id_tingkat='".$_SESSION['kode_tk']."' and id_kejati='".$_SESSION['kode_kejati']."' and id_kejari='".$_SESSION['kode_kejari']."' and id_cabjari='".$_SESSION['kode_cabjari']."' ";
    //     $resultNip=$connection->createCommand($query)->queryOne();
    // }

    // if(!$model->isNewRecord){
    //     $sql="select string_agg(nip,',') as nip_terlapor from was.pegawai_terlapor_sp_was2 where no_register='".$_GET['id']."' and id_sp_was2='".$_GET['id_sp_was2']."' and id_tingkat='".$_SESSION['kode_tk']."' and id_kejati='".$_SESSION['kode_kejati']."' and id_kejari='".$_SESSION['kode_kejari']."' and id_cabjari='".$_SESSION['kode_cabjari']."' and nip<>''";
...
if(!$model->isNewRecord){
    $sql="select string_agg(nip,',') as nip_terlapor from was.pegawai_terlapor where no_register='".$_GET['id']."' and id_sp_was1='".$_GET['id_sp_was2']."' and id_tingkat='".$_SESSION['kode_tk']."' and id_kejati='".$_SESSION['kode_kejati']."' and id_kejari='".$_SESSION['kode_kejari']."' and id_cabjari='".$_SESSION['kode_cabjari']."' and nip<>''";
    $resultNip_terlapor=$connection->createCommand($sql)->queryOne();
}
                   ...
                   <div class="col-md-4">
                      <div class="form-group">
                          ...
                          <div class="col-md-10" style="width:75%">
                            <?php
                          if(!$model->isNewRecord){
                          echo $form->field($model, 'nip_penandatangan',['addon' => ['append' => ['content'=>Html::button('Pilih', ['class'=>'btn btn-primary cari_ttd','id'=>'pilih_pegawai','data-toggle'=>'modal','data-target'=>'#penandatangan2']), 
...
<!-- ///////////////////////////////////////// -->
    ...
    //  print_r($_SESSION);
      ...
      ?>
       <div class="col-md-12" style="padding-top: 15px;padding-bottom: 15px;">
                <label>Unggah Berkas SPWAS-2 : 
                     <?php if (substr($model['file_sp_was2'],-3)!='pdf'){?>
                     <?= ($model->file_sp_was2!='' ? '<a href="viewpdf?id='.$model['id_sp_was2'].'&id_register='.$model['no_register'].'&id_tingkat='.$model['id_tingkat'].'&id_kejati='.$model['id_kejati'].'&id_kejari='.$model['id_kejari'].'&id_cabjari='.$model['id_cabjari'].'" ><span style="cursor:pointer;font-size:28px;">
                    <i class="fa fa-file-image-o"></i></span></a>' :'') ?>
                    <?php } else{?>
                     <?= ($model->file_sp_was2!='' ? '<a href="viewpdf?id='.$model['id_sp_was2'].'&id_register='.$model['no_register'].'&id_tingkat='.$model['id_tingkat'].'&id_kejati='.$model['id_kejati'].'&id_kejari='.$model['id_kejari'].'&id_cabjari='.$model['id_cabjari'].'" target="_blank"><span style="cursor:pointer;font-size:28px;">
                    <i class="fa fa-file-pdf-o"></i></span></a>' :'') ?> <?php } ?>
                </label>
                <!-- <input type="file" name="file_sp_was2" /> -->
                <div class="fileupload fileupload-new" data-provides="fileupload">
                <span class="btn btn-primary btn-file fa fa-folder-open"><span class="fileupload-new"> Upload File </span>
                <span class="fileupload-exists "> Ubah File</span>         <input type="file" name="file_sp_was2" id="file_sp_was2" /></span>
     ...
     <?php } ?>
        ...
        <div class="box-footer" style="text-align: center;"> 
            <!-- <button class="btn btn-primary btn-sm" type="submit" name="simpan" id="simpan" value="simpan">Simpan</button> -->
         <!--    <a class="btn btn-primary btn-sm" href="">Batal</a> -->
            <?php 
			//echo $result_expire;
                if (!$model->isNewRecord){
         
                   if($result_expire=='1' && $model->tanggal_sp_was2!=''){
            ...
            <?php
              if (!$model->isNewRecord) {
            ?>    
                <!--  Html::a('<i class="fa fa-print"></i> Cetak', ['/pengawasan/sp-was-2/cetakdoc', 'id_sp_was2' => $model->id_sp_was2], ['class' => 'btn btn-primary']); -->
               <!--  echo " ".Html::a('<i class ="fa fa-times"></i> Hapus', ['/pengawasan/sp-was-1/hapus', 'id' => $model->id_sp_was1], ['id' => 'hapusSpwas2', 'class' => 'btn btn-primary']); -->
                <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['/pengawasan/sp-was-2/cetakdocx?no_register='.$model->no_register.'&id='. $model['id_sp_was2'].'&id_tingkat='. $model['id_tingkat'].'&id_kejati='. $model['id_kejati'].'&id_kejari='. $model['id_kejari'].'&id_cabjari='. $model['id_cabjari']])?>"><i class="fa fa-print"></i> Cetak</a>
            <?php  
              }
              
               echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sp-was-2/index'], ['id' => 'KembaliSpWas2', 'class' => 'btn btn-primary']);
             ?>
            <!-- <a class="btn btn-primary btn-sm" target="_blank" href="">Cetak</a>     -->
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>
                        ...
                        <?php
                            $searchModelWas9 = new SpWas2Search();
...
<div id="TerlaporModalUbah" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lebar">
    ...
    <div class="clearfix"></div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" id="Madd_terlapor">Pilih</button>
        <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
      </div>
    </div>
...
// $('#spwas1-tanggal_akhir_sp_was1').datepicker();
  ...
  $( function() {
    var dateFormat = "mm/dd/yy",
      from = $( "#spwas2-tanggal_mulai_sp_was2" )
        .datepicker({
          minDate: -hari,
          changeMonth: true,
        })
      ...
      to = $( "#spwas2-tanggal_akhir_sp_was2" ).datepicker({
        changeMonth: true,
      })
      .on( "change", function() {
    ...
    function getDate( element ) {
      ...
      localStorage.setItem('nip_terlapor_spwas2', JSON.stringify(nipTerlapor));
...
if(asal=='ubah'){/*jika button yang di tekan ubah maka masuk kan data*/
  ...
  var json = JSON.parse(localStorage["nip_terlapor_spwas2"]);
	 if(Mnip!=''){/* jika nip ada maka masukan data dan set localStorage*/
    if(jQuery.inArray(Mnip,json)==-1){
    // alert(Mnama_terlapor);
       $('.selection_one1:checked').closest('tr').find('.td_nip').html(gabung_nipNRP);
       $('.selection_one1:checked').closest('tr').find('.td_nama').html(Mnama_terlapor);
       $('.selection_one1:checked').closest('tr').find('.td_jabatan').html(Mjabatan);
       $('.selection_one1:checked').closest('tr').find('.td_satker').html(Msatker);
       $('.selection_one1:checked').closest('tr').find('.td_pangkat').html(gabung_golPANGKAT);


       $('.selection_one1:checked').closest('tr').find('.nipTerlapor').val(Mnip);
       $('.selection_one1:checked').closest('tr').find('.nrpTerlapor').val(Mnrp);
       $('.selection_one1:checked').closest('tr').find('.namaTerlapor').val(Mnama_terlapor);
       $('.selection_one1:checked').closest('tr').find('.jabatanTerlapor').val(Mjabatan);
       $('.selection_one1:checked').closest('tr').find('.golonganTerlapor').val(Mgolongan);
       $('.selection_one1:checked').closest('tr').find('.pangkatTerlapor').val(Mpangkat);
       $('.selection_one1:checked').closest('tr').find('.satkerTerlapor').val(Msatker);
       $('.selection_one1:checked').val(Mnip);
       $('.selection_one1:checked').attr('atrTerlapor',tmp);
  //   }
   /*masukan ke table*/
   localStorage.removeItem("nip_terlapor_spwas2");
   var checkValues = $('.selection_one1').map(function()
                                {
                                    return $(this).val();
                                }).get();
   
   
    localStorage.setItem('nip_terlapor_spwas2', JSON.stringify(checkValues));
   }
  }else{
    $('.selection_one1:checked').closest('tr').find('.td_nama').html(Mnama_terlapor);
    $('.selection_one1:checked').closest('tr').find('.td_jabatan').html(Mjabatan);
  ...
  }else{/*jika button yang di click tambah maka apend data*/
   
   var json = JSON.parse(localStorage["nip_terlapor_spwas2"]);
   if(Mnip!=''){
   if(jQuery.inArray(Mnip,json)==-1){/*jika ada nip maka masukan data dan set localstorage*/
    $('#tbody_terlapor').append('<tr>'+
                      ...
                      '<input class="selection_one1" type="checkbox" value="'+Mnip+'" name="ck_tr" rel="2" atrTerlapor="'+tmp+'"></td>'+
                  '</tr>');
    }
  localStorage.removeItem("nip_terlapor_spwas2");
    var checkValues = $('.selection_one1').map(function()
...
//}
  ...
  // alert('aaa');
   /*jika button yang di rubah tambah maka apend data*/
    
  //   var cek = JSON.parse(localStorage["nip_terlapor"]);
  //   alert(cek);
  //    if(Mnip!=''){
  //    alert(json);
  //    if(jQuery.inArray(Mnip,cek)==-1){/*jika tidak ada maka masukan data dan set localstorage*/
  //     $('#tbody_terlapor').append('<tr>'+
  //                       '<td style="text-align:center;"><input name="noTerlapor[]" class="noTerlapor" value="" type="hidden"></td>'+
  //                       '<td class="td_nama">'+Mnama_terlapor+'</td>'+
  //                       '<td class="td_pangkat">'+gabung_golPANGKAT+'</td>'+
...
////Isi Field Kolom Terlapor
      ...
      // $(document).on('click','.MPenandatangan_selection_one',function() {
      //    var data=JSON.parse($(this).attr('json'));
      //   //alert(data.jabatan);
      //    $('#Mnip_pnd').val(data.nip);
      //    $('#Mnama_pnd').val(data.nama_penandatangan);
      //    $('#Mjabt_pnd').val(data.jabatan);
      //    $('#Mgol_pnd').val(data.golongan_penandatangan);
      //    $('#Mpgkt_pnd').val(data.pangkat_penandatangan);
      ...
      // }); 
        ...
        $(".add-more").click(function(e){
            ...
            next = next + 1;
            var newIn = '<input autocomplete="off" class="input" id="field' + next + '" name="isi_dasar_sp_was2[]" type="text">';
            var newInput = $(newIn);
  ...
  $(document).on('click','#ubah_terlapor',function(){
    ...
    var data=JSON.parse($(".selection_one1:checked").attr("rel"));
    var x =$(".selection_one1:checked").attr("atrTerlapor");
    // alert(data.nip);
    var pecah=x.split('#');
    // alert(x);
   //exit();
...
// $(document).on('change','#hapus_all_tr',function() {
//       if ( $(this).is(':checked') ) {
//     $('.selection_one').prop('checked','checked');
// } 
// else {
//     $('.selection_one').prop('checked','');
// }
...
$(document).on('click','#hapus_terlapor',function() {
  $(".selection_one1:checked").closest('tr').remove();
});


$(document).on('change','#select_all',function() {
    var c = this.checked ? true : false;
    $('.selection_one1').prop('checked',c);
    var x=$('.selection_one1:checked').length;
    ConditionOfButtonTr(x);
});
    
$(document).on('change','.selection_one1',function() {
...
$(document).on('click','#hapus_terlapor',function() {
  ...
  // $(document).on('click','#plh_peg',function(){
  //  // alert('aaaa');
  //  // $('#asal').val('tambah');
  //   // $('#TerlaporModalUbah').modal('show');
...
$(document).on('click','#tambah_penandatangan',function() {
    var data=JSON.parse($(".selection_one:checked").attr("json"));
       $('#spwas2-nip_penandatangan').val(data.nip);
       $('#spwas2-nama_penandatangan').val(data.nama);
       $('#spwas2-jabatan_penandatangan').val(data.nama_jabatan);
       $('#spwas2-golongan_penandatangan').val(data.gol_kd);
       $('#spwas2-pangkat_penandatangan').val(data.gol_pangkat2);
       $('#spwas2-jbtn_penandatangan').val(data.jabtan_asli);
  ...
  $(document).on('click','#Mtambah_pemeriksa',function(){
    var jml=$(".Mselection_one:checked").length;
      ...
      if(jQuery.inArray(trans.peg_nip_baru,cek)==-1 && jQuery.inArray(trans.peg_nip_baru,cek_onDB)==-1){/*jika kondisi tidak sama maka tambahkan data*/
        $('.bd_pemeriksa_tmp').append('<tr class="pemeriksa'+trans.peg_nip_baru+'">'+
          '<td><input type="hidden" name="nrp_pemeriksa[]" class="nrp" value="'+trans.peg_nrp+'"></td>'+
          '<td><input type="hidden" name="nip_pemeriksa[]" class="nip" value="'+trans.peg_nip_baru+'">'+trans.peg_nip_baru+'</td>'+
          '<td><input type="hidden" name="nama_pemeriksa[]" class="nama_pemeriksa" value="'+trans.nama+'">'+trans.nama+'</td>'+
          '<td><input type="hidden" name="jabatan_pemeriksa[]" class="jabatan" value="'+trans.jabatan+'">'+trans.jabatan+'</td>'+
          '<td><input type="hidden" name="golongan_pemeriksa[]" class="golongan" value="'+trans.gol_kd+'"><input type="hidden" name="pangkat_pemeriksa[]" class="pangkat" value="'+trans.gol_pangkat2+'">'+trans.gol_pangkat2+'</td>'+
          '<td class="ck_box" width="2%">'+
          '<input class="selection_one" type="checkbox" value="'+trans.peg_nip_baru+'" name="ck_tr" rel="2" atrTerlapor="'+trans.peg_nip_baru+'"></td>'+
        '</tr>');
      }
       }
	//alert(arrnip);
         ...
         var checkValues = $('.selection_one:checked').map(function()
            {
                ...
                for (var i = 0; i < cek; i++) {
                    $('.pemeriksa'+checkValues[i]).remove();
                };
...
$(document).on('click','#tambah_dasar',function() {
        $('.for_dasar_surat').append('<div class=\"tampungan\"><div class=\"col-sm-11\" style=\"margin-top:10px;padding: 0px\"><textarea class=\"form-control\" name=\"isi_dasar_sp_was_2[]\"></textarea></div><div class=\"col-sm-1 hapus\" style=\"margin-top:10px;width:59px;padding:0px\"><a  rel=\"\" class=\"btn btn-primary btn-sm pull-right hapus_dasar\" class=\"hapus_dasar\"><i class=\"glyphicon glyphicon-trash\"></i></a></div></div>');
...
/*////////////reload grid penandatangan surat/////////////////*/
     $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sp-was-2/getttd",function(responseText, statusText, xhr)
            {
     ...
     $(document).on('click','.cari_riksa',function() { 
      $('#grid-pemeriksa_surat').addClass('loading');
        $("#grid-pemeriksa_surat").load("/pengawasan/sp-was-2/getpemeriksa",function(responseText, statusText, xhr)
            {
                if(statusText == "success")
