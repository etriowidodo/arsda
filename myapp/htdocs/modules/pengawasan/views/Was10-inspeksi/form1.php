...
$this->title = 'WAS-10 Inspeksi Surat Permintaan Keterangan Terlapor';
...
<div class="dipa-master-index">
    ...
    <p>
        <div class="btn-toolbar">
            <a class="btn btn-primary btn-sm pull-right" id="Madd_hapus_terlapor"><i class="glyphicon glyphicon-trash">  </i> Hapus</a>&nbsp;
            ...
            <a class="btn btn-primary btn-sm pull-right"  data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#Mcreate_terlapor"><i class="glyphicon glyphicon-plus"> </i> Terlapor</a>
...
<?=  $form->field($model, 'nrp_pegawai_terlapor')->hiddenInput()->label(false) ?>
    ...
    $sql="select string_agg(nip_pegawai_terlapor,',') as nip from was.pegawai_terlapor_was10_inspeksi where  no_register='".$_SESSION['was_register']."' AND id_tingkat = '".$_SESSION['kode_tk']."' AND id_kejati = '".$_SESSION['kode_kejati']."' AND id_kejari = '".$_SESSION['kode_kejari']."' AND id_cabjari = '".$_SESSION['kode_cabjari']."'";
    $result_terlapor = $connection->createCommand($sql)->queryOne();
    ?>
    <input type="hidden" name="tmp_nip" value="<?=$result_terlapor['nip']?>" id="tmp_nip">
    <p>
        <div class="btn-toolbar">
            <a class="btn btn-primary btn-sm pull-right" href="/pengawasan/was10-inspeksi/index"><i class="fa fa-arrow-left" aria-hidden="true"></i>&nbsp;&nbsp;Batal</a>
                ...
                <div class="panel with-nav-tabs panel-default">
                    <div class="panel-heading single-project-nav">
                        <ul class="nav nav-tabs"> 
                            <li class="active">
                                <a href="#spwas" data-toggle="tab" id="Mspwas">SP-Was-2</a>
                            </li>
                            <li>
    ...
    window.onload=function(){
        ...
        if(asal==1){/*jika mengambil data dari pegawai terlapor*/
            ...
            $('#body_tmp_terlapor').append('<tr>'+
              ...
              '<td><input type="hidden" name="golongan[]" class="golongan" value="'+trans['golongan_pegawai_terlapor']+'"><input type="hidden" name="pangkat[]" class="pangkat" value="'+trans['pangkat_pegawai_terlapor']+'">'+trans['golongan_pegawai_terlapor']+'/'+trans['jabatan_pegawai_terlapor']+'</td>'+
              '<td style="text-align: center;"><input type="hidden" name="satker[]" class="satker" value="'+trans['satker_pegawai_terlapor']+'"><input type="hidden" name="id_sp_was2[]" class="id_sp_was2" value="'+trans['id_sp_was2']+'"  ><input class="selection_one" name="selection[]" value="'+trans['nip']+'"  type="checkbox"></td>'+
            '</tr>');
            }
                }
            var names = arrnip;
            ...
            $('#body_tmp_terlapor').append('<tr>'+
              ...
              '<td style="text-align: center;"><input type="hidden" name="satker[]" class="satker" value="'+trans['instansi']+'"><input type="hidden" name="id_sp_was2[]" class="id_sp_was2" value="'+trans['id_sp_was2']+'"  ><input class="selection_one" name="selection[]" value="'+trans['peg_nip_baru']+'" type="checkbox"></td>'+
            '</tr>');
            }else{
