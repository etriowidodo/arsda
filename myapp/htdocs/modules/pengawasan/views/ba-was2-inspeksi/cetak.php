...
require_once('/wordtest/classes/CreateDocx.inc');
        ...
        $docx = new CreateDocxFromTemplate('../modules/pengawasan/template/ba_was_2_inspeksi.docx');
...
//         /*-----------------------------end tandatangan-----------------------------------*/
        ...
        /*noSpWas1*/
        $no_sp_was_2=$model['nomor_sp_was_2'];
...
// // //         $noSpWas=$sp_was_1[0]['nomor_sp_was1'];
        ...
        $pemeriksa="";
        $no_pemeriksa=1;
        $pemeriksa .="<table width='100%' style='margin-top:20px; font-family:arial;'>";
        foreach ($modelPemeriksa as $rowPemeriksa) {
        	if(count($modelPemeriksa)==0){
        	$pemeriksa='<p></p>';
        	}else if(count($modelPemeriksa)<=1){
            $pemeriksa .="<tr>
...
//         $pertanyaan .="</table>";
        ...
        $saksi_int="";
        $no_saksi_int=1;
        $saksi_int .="<table width='100%' style='margin-top:20px; font-family:arial;'>";
        foreach ($modelSaksiIn as $rowSaksiInt) {
        	if(count($modelSaksiIn)==0){
        	$saksi_int='<p></p>';
        	}else if(count($modelSaksiIn)==1){
            $saksi_int .="<tr>
        ...
        $saksi_eks="";
        $no_saksi_eks=1;
        $saksi_eks .="<table width='100%' style='margin-top:20px; font-family:arial;'>";
        foreach ($modelSaksiEk as $rowSaksiEks) {
        	if(count($modelSaksiEk)==0){
        	$saksi_eks='<p></p>';
        	}else if(count($modelSaksiEk)==1){
            $saksi_eks .="<tr>
                          ...
                          <tr>
                            ...
                            <td width='2%'>:</td>
                            <td>".$rowSaksiEks['tempat_lahir_saksi_eksternal'].' '.\Yii::$app->globalfunc->ViewIndonesianFormat($rowSaksiEks['tanggal_lahir_saksi_eksternal'])."</td>
                          </tr>
                          <tr>
        ...
        $pertanyaan="";
        $no_pertanyaan=1;
        $pertanyaan .="<table width='100%' style='margin-top:20px; font-family:arial;'>
        				<thead>
        					<th>No</th>
        					<th>Hasil Wawancara</th>
        				</thead>";

        foreach ($modelKesimpulan as $rowPertanyaan) {
        	if(count($modelKesimpulan)==0){
        	$pertanyaan='<p></p>';
        	}else if(count($modelKesimpulan)==1){
            $pertanyaan .="<tr>
            				<td width='5%' style='vertical-align:top'></td>
                            <td>".$rowPertanyaan['kesimpulan_ba_was_2']."</td>
                          </tr>";
        	}else{
        	$pertanyaan .="<tr>
        ...
        $jabatan_ttd=$model['jbtn_penandatangan'];;
        ...
        $docx->replaceVariableByText(array('jabatan_ttd'=>$jabatan_ttd), array('parseLineBreaks'=>true));
        ...
        $docx->replaceVariableByText(array('tgl_sp_was_2'=>$tgl_spwas2), array('parseLineBreaks'=>true));
        ...
        $docx->createDocx('template/pengawasan/ba_was_2_inspeksi');
		
        $file = 'template/pengawasan/ba_was_2_inspeksi.docx';
