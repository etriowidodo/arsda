    ...
    <div class="content-wrapper-1">
            ...
            <div class="col-md-12" style="padding-right: 0px;padding-left: 0px;">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="col-sm-12">
                                     <?= $form->field($model, 'no_sk_was_3a')->textInput()->label(false); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_sk_was_3a',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
      ...
      <div class="panel panel-primary">
          <div class="panel-heading">Penandatangan</div>
            ...
            <?php if(!$model->isNewRecord){ ?>
            <div class="col-md-12" style="padding-top: 15px;">
                <label>Unggah Berkas Sk-Was-3c : 
                     <?php if (substr($model->upload_file,-3)!='pdf'){?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_3a'].'&id2='.$model['id_sk_was_3a'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-image-o"></i></span></a>' :'') ?> <?= $model['upload_file']?>
                    <?php } else{?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_3a'].'&id2='.$model['id_sk_was_3a'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-pdf-o"></i></span></a>' :'') ?>  <?= $model['upload_file']?>
                    <?php } ?>
                </label>
 ...
 <?php    
       if (!$model->isNewRecord){
        // if($result_expire=='1'){
          ...
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-3c/index'], ['id' => 'KembaliSkWas3a', 'class' => 'btn btn-primary']);
        //  }
       }else{
          echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-save"></i> Simpan' : '<i class="fa fa-save"></i> Simpan', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ;
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-3c/index'], ['id' => 'KembaliSkWas3a', 'class' => 'btn btn-primary']);
...
#grid-penandatangan_surat.loading .modal-loading-new {display: block;}
    ...
    .field-skwas3apelapor-nama_pelapor , .field-skwas3apelapor-alamat_pelapor , .field-skwas3apelapor-email_pelapor
    , .field-skwas3apelapor-telp_pelapor, .field-skwas3apelapor-pekerjaan_pelapor, .field-skwas3apelapor-kewarganegaraan_pelapor
    , .field-skwas3apelapor-agama_pelapor, .field-skwas3apelapor-pendidikan_pelapor, .field-skwas3apelapor-nama_kota_pelapor
    , .field-skwas3apelapor-tanggal_lahir_pelapor, .field-skwas3apelapor-tempat_lahir_pelapor, .field-skwas3apelapor-sumber_lainnya
    , .field-skwas3apelapor-id_sumber_laporan ,.field-skwas3a-nip_pegawai_terlapor ,.field-skwas3a-nama_pegawai_terlapor
    ,.field-skwas3a-pangkat_pegawai_terlapor ,.field-skwas3a-golongan_pegawai_terlapor ,.field-skwas3a-jabatan_pegawai_terlapor
    ,.field-skwas3apemeriksa-nip_pemeriksa ,.field-skwas3apemeriksa-nrp_pemeriksa ,.field-skwas3apemeriksa-nama_pemeriksa
    ,.field-skwas3apemeriksa-pangkat_pemeriksa ,.field-skwas3apemeriksa-jabatan_pemeriksa ,.field-skwas3apemeriksa-golongan_pemeriksa{
        display: none;
    }
 
/* #skwas3a-tgl_sk_was_3a-disp{
    width: 405px;
 } */  
</style>
...
$(document).on('click','#addtembusan',function() {
        ...
        $(document).on('click','#tambah_penandatangan',function() {
          ...
          if(jml=>1 && jml<2){
             $('#skwas3a-nip_penandatangan').val(data.nip);
             $('#skwas3a-nama_penandatangan').val(data.nama);
             $('#skwas3a-jabatan_penandatangan').val(data.nama_jabatan);
             $('#skwas3a-golongan_penandatangan').val(data.gol_kd);
             $('#skwas3a-pangkat_penandatangan').val(data.gol_pangkat2);
             $('#skwas3a-jbtn_penandatangan').val(data.jabtan_asli);
    ...
    $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sk-was-3a/getttd",function(responseText, statusText, xhr)
            {
