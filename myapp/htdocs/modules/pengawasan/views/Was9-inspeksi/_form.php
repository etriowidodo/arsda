...
<?php $this->registerJs("
  ...
  $(document).ready(function(){
    ...
    $('#jenis_saksi1').click(function(){
      $('#was9_inspeksi-nama_pegawai_terlapor').val('');
    ...
    $('#was9_inspeksi-nama_pegawai_terlapor').val(nama);
   ...
   $('#was9_inspeksi_saksieksternal').modal('hide');
});
...
function datePickerSetup () {
    ...
    $('#was9_inspeksi-tanggal_pemeriksaan_was9').datepicker({
      inline: true,
      //minDate: new Date(),
      ...
      onSelect: function(date) {
        ...
        var weekday = {Sunday:'Minggu',Monday:'Senin',Tuesday:'Selasa',Wednesday:'Rabu',Thursday:'Kamis',Friday:'Jumat',Saturday:'Sabtu'};
         ...
         $('#was9_inspeksi-hari_pemeriksaan_was9').val(dayOfWeek);
...
<div class="box box-primary">
 ...
 <!-- <div class="box box-primary" style="overflow:hidden;padding:15px 0px 8px 0px;"> -->
    ...
    if($_GET['jns']=='Eksternal'){
      $fitternya="id_saksi_eksternal";
      }else{
      $fitternya="id_saksi_internal";
      }
      $connection = \Yii::$app->db;
      $sqlnomor="select*from was.was9_inspeksi where no_register='".$_SESSION['was_register']."' and  $fitternya='".$_GET['id_saksi']."' and jenis_saksi='".$_GET['jns']."' and nomor_surat_was9=''";
      
      $cekN = $connection->createCommand($sqlnomor)->queryOne();
     if($cekN!= null){
      echo "<div class='alert alert-warning' style='margin:0 15px 15px 15px;'>
            <strong>Perhatian!</strong> . Surat ini harus diberi nomor!
        </div>";
     }
   // }
      ...
      <?php  if(!$model->isNewRecord){ ?>   
         <div class="col-md-12">
             <div class="form-group">
                ...
                <div class="col-md-9">
                <?= $form->field($model, 'nomor_surat_was9')->textInput(['maxlength' => true,'readonly'=>true]) ?>
                </div>
            </div>
        </div>
      <?php  }else{?>
                ...
                <?php
                    echo $form->field($model, 'tanggal_was9',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
                        ...
                        'options' => [
                            'pluginOptions' => [
                                'autoclose' => true,
                                'startDate' => date("d-m-Y",strtotime($spwas2['tanggal_mulai_sp_was2'])),
                                'endDate' => date("d-m-Y",strtotime($spwas2['tanggal_akhir_sp_was2'])),
...
<?php  if (!$model->isNewRecord){ ?>   
    <div class="col-md-12" style="padding-top: 15px;padding-bottom: 15px;">
                <label>Unggah Berkas WAS-9 Inspeksi : 
                     <?php if (substr($model['was9_file'],-3)!='pdf'){?>
                     <?= ($model->was9_file!='' ? '<a href="viewpdf?id='.$_GET['id_saksi'].'&no='.$model['nomor_surat_was9'].'&id_register='.$model['no_register'].'&id_tingkat='.$model['id_tingkat'].'&id_kejati='.$model['id_kejati'].'&id_kejari='.$model['id_kejari'].'&id_cabjari='.$model['id_cabjari'].'" ><span style="cursor:pointer;font-size:28px;">
                    <i class="fa fa-file-image-o"></i></span></a>' :'') ?>
                    <?php } else{?>
                     <?= ($model->was9_file!='' ? '<a href="viewpdf?id='.$_GET['id_saksi'].'&no='.$model['nomor_surat_was9'].'&id_register='.$model['no_register'].'&id_tingkat='.$model['id_tingkat'].'&id_kejati='.$model['id_kejati'].'&id_kejari='.$model['id_kejari'].'&id_cabjari='.$model['id_cabjari'].'" target="_blank"><span style="cursor:pointer;font-size:28px;">
      ...
      if (!$model->isNewRecord){
        ...
        if (!$model->isNewRecord){ ?>
         <?php if($_GET['jns']=='Internal'){ ?>
            <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['was9-inspeksi/cetak?no_register='.$model->no_register.'&id='. $model['id_surat_was9'].'&id_tingkat='. $model['id_tingkat'].'&id_kejati='. $model['id_kejati'].'&id_kejari='. $model['id_kejari'].'&id_cabjari='. $model['id_cabjari']])?>">Cetak</a>       
         <?php }else {?>
            <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['was9-inspeksi/cetak2?no_register='.$model->no_register.'&id='. $model['id_surat_was9'].'&id_tingkat='. $model['id_tingkat'].'&id_kejati='. $model['id_kejati'].'&id_kejari='. $model['id_kejari'].'&id_cabjari='. $model['id_cabjari']])?>">Cetak</a>       
         <?php }?>
      <?php }?>
    <?php
       // echo Html::a('<i class="fa fa-print"></i> Cetak', ['/pengawasan/was9/cetak', 'id' => $model->id_surat_was9], ['class' => 'btn btn-primary']) ;
       echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal ', ['/pengawasan/was9-inspeksi/index'], ['id' => 'KembaliSpWas2', 'class' => 'btn btn-primary']) ;
     ?>
    </div>
<?php // }?>
    <!-- </div> -->
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>
                        ...
                        <?php
                            $searchModelWas9 = new Was9_InspeksiSearch();
...
$(document).on('click','#tambah_penandatangan',function() {  
    var data=JSON.parse($(".selection_one:checked").attr("json"));
       $('#was9_inspeksi-nip_penandatangan').val(data.nip);
       $('#was9_inspeksi-nama_penandatangan').val(data.nama);
       $('#was9_inspeksi-jabatan_penandatangan').val(data.nama_jabatan);
       $('#was9_inspeksi-jbtn_penandatangan').val(data.jabtan_asli);
       $('#was9_inspeksi-pangkat_penandatangan').val(data.gol_pangkat2);
       $('#was9_inspeksi-golongan_penandatangan').val(data.gol_kd);
...
$(document).on('click','.cari_ttd',function() { 
  $('#grid-penandatangan_surat').addClass('loading');
    $("#grid-penandatangan_surat").load("/pengawasan/was9-inspeksi/getttd",function(responseText, statusText, xhr)
        {
