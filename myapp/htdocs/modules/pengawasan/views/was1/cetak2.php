...
require_once('/wordtest/classes/CreateDocx.inc');
        ...
        // print_r(strlen($data_satker['inst_lokinst']));
        $cek_spasi=explode(' ', $data_satker['inst_lokinst']);
        $x='';
        if(count($cek_spasi)<=1){
            $beri_spasi=strlen($data_satker['inst_lokinst']);
            for ($i=0; $i <$beri_spasi ; $i++) { 
            $x .=$data_satker['inst_lokinst'][$i].' ';
            }
            $lokasi=strtoupper($x);
        }
    ...
    //    $data=($model['data']!=''?$model['data']:'<p></p>');
          /*Isi Data*/
        $isiD='<style="font-family: arial;">'.$model['data'].'</style>';

        $analisa=($model['was1_analisa']!=''?$model['was1_analisa']:'<p></p>');
     //   $saran=($model['isi_saran_was1']!=''?$model['isi_saran_was1']:'<p></p>');
        $saran=($model['was1_uraian']!=''?$model['was1_uraian']:'<p></p>');
        $kesimpulan=($model['was1_kesimpulan']!=''?$model['was1_kesimpulan']:'<p></p>');
        $var=str_split($_SESSION['is_inspektur_irmud_riksa']);
        
		$sts =(substr($model['id_jabatan'],0,1));
		/* print_r($sts);
		exit(); */
		
		if($sts=='0'){ //jabatansebenernya
            // $nip=($model['nip_1']!=''?substr($model['nip_1'],0,7).' '.substr($model['nip_1'],8,6).' '.substr($model['nip_1'],14,1).' '.substr($model['nip_1'],15,3):'<p></p>');
			$nip=($model['nip_penandatangan']!=''?substr($model['nip_penandatangan'],0,7).' '.substr($model['nip_penandatangan'],8,6).' '.substr($model['nip_penandatangan'],14,1).' '.substr($model['nip_penandatangan'],15,3):'<p></p>');
			...
			//$nip=($model['nip_penandatangan']!=''?$model['nip_penandatangan']:'<p></p>');
			$nip=($model['nip_penandatangan']!=''?substr($model['nip_penandatangan'],0,7).' '.substr($model['nip_penandatangan'],8,6).' '.substr($model['nip_penandatangan'],14,1).' '.substr($model['nip_penandatangan'],15,3):'<p></p>');
			$nama_penandatangan=($model['nama_penandatangan']!=''?$model['nama_penandatangan']:'<p></p>');
			$jabatan_alias=($model['jabatan_penandatangan']!=''?$model['jabatan_penandatangan']:'<p></p>');
			$golongan_penandatangan=($model['pangkat_penandatangan']!=''?$model['pangkat_penandatangan']:'<p></p>');
        ...
        $narasi=($model['no_surat']!=''?$model['was1_narasi_awal']:'<p></p>');
        $tglsurat=($model['was1_tgl_surat']!=''?$tgl:'<p></p>');
...
/* 		print_r(explode(' ',$model['jabatan_penandatangan']));
        ...
        $docx->replaceVariableByHTML('data', 'inline', $isiD, array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));
        ...
        $docx->replaceVariableByHTML('analisa', 'inline','<div style="margin-left:10px">'.$analisa.'</div>', array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));
        ...
        $docx->replaceVariableByHTML('kesimpulan', 'inline','<p style="text-align: justify; margin-left: 20px;">'.$kesimpulan.'</p>', array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));
        ...
        $docx->replaceVariableByHTML('narasi', 'inline',$narasi, array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));
		$docx->replaceVariableByHTML('lokasi', 'inline','<h2 style="text-align: center;"><p>'.$lokasi.'</p></h2>', array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));
        // header nota dinas
