...
require_once('/wordtest/classes/CreateDocx.inc');
        ...
        $docx = new CreateDocxFromTemplate('../modules/pengawasan/template/ba_was_3_int_inspeksi.docx');
...
//          /*permintaan kang putut lokasi harus ada spasi*/
        ...
        /*tempat Panggilan*/
        $tempat=$data_satker['inst_lokinst'];
...
// //         $noSpWas=$sp_was_1[0]['nomor_sp_was1'];
        ...
        foreach ($modelTerlapor as $rowTerlapor) {
            $saksi_int .="<tr>
                          ...
                          <tr>
                            <td width='30%'>Nama Saksi Internal</td>
                            <td width='2%'>:</td>
                            <td>".$rowTerlapor['nama_pegawai_terlapor']."</td>
                          </tr>
                          <tr>
                            <td width='30%'>Golongan Saksi Internal</td>
                            <td width='2%'>:</td>
                            <td>".$rowTerlapor['golongan_pegawai_terlapor']."</td>
                          </tr>
                          <tr>
                            <td width='30%'>Pangkat Saksi Internal</td>
                            <td width='2%'>:</td>
                            <td>".$rowTerlapor['pangkat_pegawai_terlapor']."</td>
                          </tr>
                          <tr>
                            <td width='30%'>Jabatan Saksi Internal</td>
                            <td width='2%'>:</td>
        ...
        foreach ($modelPertanyaan as $rowPertanyaan) {
            $pertanyaan .="<tr>
                            <td>".$no_pertanyaan.'. '.$rowPertanyaan['pertanyaan'].'<p style="margin: 0px 0px 0px 20px;">'.$no_pertanyaan.'. '.$rowPertanyaan['jawaban']."</p></td>
                          </tr>";
            $no_pertanyaan++;
        }
        $pertanyaan .="</table>";
        ...
        $docx->replaceVariableByHTML('terlapor', 'block', $saksi_int, array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));
        $docx->replaceVariableByHTML('pertanyaan', 'block', $pertanyaan, array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));

        $docx->createDocx('template/pengawasan/ba_was_3_int_inspeksi');
		
        $file = 'template/pengawasan/ba_was_3_int_inspeksi.docx';
