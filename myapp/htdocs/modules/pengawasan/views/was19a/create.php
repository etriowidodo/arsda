...
?>
<script>
  var url3 = '<?php echo Url::to('update'); ?>';
...
<div class="was19a-create">
 ...
 <div>
  ...
  <div class="col-md-12" style="border:1px solid #cecece;border-radius:4px;padding:10px 0px;background-image: linear-gradient(to bottom, rgba(255, 255, 255, 1) 0%, rgba(246, 246, 246, 1) 47%, rgba(237, 237, 237, 1) 100%);margin-top: -15px;">
       <div class="form-group">
              <div class="col-lg-10"> <span class="pull-left" style="margin-left: 15px;"> <a class="btn btn-primary" data-toggle="modal" data-target="#p_was19a"><i class="glyphicon glyphicon-user"></i> Tambah Identitas Terlapor</a> </span> </div>
              <button id="btnCetak" class="btn btn-primary" type="button"><i class="fa fa-print"></i> Cetak</button>&nbsp;
              <button id="btnHapus" class="deleteTable btn btn-primary" type="button"><i class="fa fa-times"></i> Hapus</button>
            </div>
    </div>
            ...
            'columns' => [
                ...
                [
                    ...
                    'value'=>function ($model, $key, $index, $widget) use($was_register){
                       ...
                       return ($model['id_terlapor'] == ''? '-' : app\modules\pengawasan\models\VTerlapor::find()->where('id_register = :id',[':id'=>$was_register])->one()->peg_nip);
                            },
       // 'attribute'=>'tempat',
        'label' => 'NIP ',
...
<?php
    Modal::begin([
     ...
     <div id="update-19a">
            <?php echo $this->render('_form', ['model'=> $model,'modelTembusan'=>$modelTembusan]); ?></div>
	<?php
Modal::end();?>
<?php
    Modal::begin([
