...
$this->title = 'Daftar SK-WAS-4D';
$this->params['breadcrumbs'][] = $this->title;
$this->params['ringkasan_perkara'] = $_SESSION['was_register'];
// $this->params['ringkasan_perkara'] = $_SESSION['was_register'];
?>
<?php// print_r($_SESSION); ?>
<div class="sk-was4d-index">

    <h4><?php ?></h4>
    <?php //  echo $this->render('_search', ['model' => $searchModel]); ?>
<!-- href="/pengawasan/sk-was-4d/create" -->
    <p>
        <div class="btn-toolbar">
			  <a class="btn btn-primary btn-sm pull-right" id="cetak_skwas4d"><i class="glyphicon glyphicon-print"> </i> Cetak </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="hapus_skwas4d"><i class="glyphicon glyphicon-trash"> </i> Hapus </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="ubah_skwas4d"><i class="glyphicon glyphicon-pencil"> </i> Ubah </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="create_skwas4d" ><i class="glyphicon glyphicon-plus"> </i> SK-WAS-4D</a>
            </div>
    </p>
    <style type="text/css">
...
legend.group-border {
    ...
    font-size: 14px;
     ...
     <fieldset class="group-border">
        <legend class="group-border">Daftar SK-WAS-4D</legend>
    <?= GridView::widget([
...
$("#create_skwas4d").addClass("disabled");
    $("#ubah_skwas4d").addClass("disabled");
    $("#hapus_skwas4d").addClass("disabled");
    $("#cetak_skwas4d").addClass("disabled");
    /*permintaan pa putut*/
  ...
  function ConditionOfButton(n){
        if(n == 1){
           $('#cetak_skwas4d,#ubah_skwas4d,#create_skwas4d, #hapus_skwas4d').removeClass('disabled');
        } else if(n > 1){
           $('#hapus_skwas4d').removeClass('disabled');
           $('#cetak_skwas4d,#ubah_skwas4d,#create_skwas4d').addClass('disabled');
        } else{
           $('#cetak_skwas4d,#ubah_skwas4d,#create_skwas4d, #hapus_skwas4d').addClass('disabled');
        }
    }
...
//         $("#create_skwas4d").removeClass("disabled");
//         $("#ubah_skwas4d").removeClass("disabled");
//         $("#hapus_skwas4d").removeClass("disabled");
// 		$("#cetak_skwas4d").removeClass("disabled");
//       }
//       // alert(z);
// });
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.nip_trlp != null){
            ...
            location.href='/pengawasan/sk-was-4d/create?id='+check.nip_terlapor+'&id2='+check.pasal;
           } 
         }
    });

    $('#ubah_skwas4d').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
          ...
          if(check.id_sk_was_4d == null){
             bootbox.alert({
                    message:"Data Belum Bisa Di Rubah Nomor Harus Di Isi",
                    size:'small'
                });
           }else{
            location.href='/pengawasan/sk-was-4d/update?id='+check.id_sk_was_4d+'&id2='+check.nip_pegawai_terlapor+'&id3='+check.pasal;
            //alert('isi');
            //location.href='/pengawasan/sk-was-4d/index';
           }
           
         }
    });

    $(document).on('click', '#cetak_skwas4d', function () {

          // alert()
          var x=$(".checkbox-row:checked").length;
          var link=$(".checkbox-row:checked").val();
          var check=JSON.parse($('.checkbox-row:checked').attr('json'));
         // alert(check.id_sk_was_4d);
          ...
          if(check.id_sk_was_4d == null){
             bootbox.alert({
              ...
              location.href='/pengawasan/sk-was-4d/cetak?id='+check.id_sk_was_4d; 
           }
...
$(document).on('click','#hapus_skwas4d',function(){
                                ...
                                var checkValues = $('.checkbox-row:checked').map(function()
                                {
                                 ...
                                 var jml=$('.checkbox-row:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/sk-was-4d/delete',
                                            data:'id='+data.id_sk_was_4d+'&jml='+jml,
