...
<div class="sk-was4b-index">

    <h4><?php ?></h4>
    <?php //  echo $this->render('_search', ['model' => $searchModel]); ?>
<!-- href="/pengawasan/sk-was-4b/create" -->
    <p>
        <div class="btn-toolbar">
              <a class="btn btn-primary btn-sm pull-right" id="cetak_skwas4b"><i class="glyphicon glyphicon-print"> </i> Cetak </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="hapus_skwas4b"><i class="glyphicon glyphicon-trash"> </i> Hapus </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="ubah_skwas4b"><i class="glyphicon glyphicon-pencil"> </i> Ubah </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="create_skwas4b" ><i class="glyphicon glyphicon-plus"> </i> SK-WAS-4B</a>
            </div>
    </p>
    <style type="text/css">
...
legend.group-border {
    ...
    font-size: 14px;
     ...
     <fieldset class="group-border">
        <legend class="group-border">Daftar SK-WAS-4B</legend>
    <?= GridView::widget([
...
//         $("#create_skwas4b").addClass("disabled");
//         $("#ubah_skwas4b").addClass("disabled");
//         $("#hapus_skwas4b").addClass("disabled");
//         $("#cetak_skwas4b").addClass("disabled");
//       }else{
//         $(this).find('.checkbox-row').prop('checked',true);
//         $("#create_skwas4b").removeClass("disabled");
//         $("#ubah_skwas4b").removeClass("disabled");
//         $("#hapus_skwas4b").removeClass("disabled");
//         $("#cetak_skwas4b").removeClass("disabled");
//       }
//       // alert(z);
// });
...
$("#create_skwas4b").addClass("disabled");
  ...
  function ConditionOfButton(n){
        if(n == 1){
           $('#cetak_skwas4b,#ubah_skwas4b,#create_skwas4b, #hapus_skwas4b').removeClass('disabled');
        } else if(n > 1){
           $('#hapus_skwas4b').removeClass('disabled');
           $('#cetak_skwas4b,#ubah_skwas4b,#create_skwas4b').addClass('disabled');
        } else{
           $('#cetak_skwas4b,#ubah_skwas4b,#create_skwas4b, #hapus_skwas4b').addClass('disabled');
        }
    }

    $('#create_skwas4b').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.nip_trlp != null){
            ...
            location.href='/pengawasan/sk-was-4b/create?id='+check.nip_terlapor+'&id2='+check.pasal;
           } 
         }
    });

    $('#ubah_skwas4b').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
            ...
            if(check.id_sk_was_4b == null){
               bootbox.alert({
                      message:"Data Belum Bisa Di Rubah Nomor Harus Di Isi",
                      size:'small'
                  });
             }else{
                location.href='/pengawasan/sk-was-4b/update?id='+check.id_sk_was_4b+'&id2='+check.nip_pegawai_terlapor+'&id3='+check.pasal; 
             }
         }
    });

    $(document).on('click', '#cetak_skwas4b', function () {

          // alert()
          var x=$(".checkbox-row:checked").length;
          var link=$(".checkbox-row:checked").val();
          var check=JSON.parse($('.checkbox-row:checked').attr('json'));
         // alert(check.id_sk_was_4b);
          ...
          if(check.id_sk_was_4b == null){
             bootbox.alert({
              ...
              location.href='/pengawasan/sk-was-4b/cetak?id='+check.id_sk_was_4b;
           }
          //alert(nilai);
        });

$(document).on('click','#hapus_skwas4b',function(){
                                ...
                                var checkValues = $('.checkbox-row:checked').map(function()
                                {
                                 ...
                                 var jml=$('.checkbox-row:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/sk-was-4b/delete',
                                            data:'id='+data.id_sk_was_4b+'&jml='+jml,
