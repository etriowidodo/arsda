...
<div class="was-form">
        ...
        <div class="col-md-12">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?= $form->field($model, 'no_surat_was12')->textInput(['readonly'=>'readonly'])?>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
    ...
    <div class="panel panel-primary">
        <div class="panel-heading">Terlapor</div>
            <div class="panel-body">
                <div class="btn-toolbar" style="margin-bottom:10px">
                  ...
                  <a class="btn btn-primary btn-sm pull-right" id="Tambah_terlapor"><i class="glyphicon glyphicon-plus"> </i> Tambah</a>
                </div>
                   ...
                   // print_r($modelTerlapor);
                    ...
                    foreach ($modelTerlapor as $value_terlapor) { ?>
                        ...
                        echo "<td class='ck_bok' width='2%' style='text-align:center'><input class='td_terlapor' type='checkbox' name='ck_pr_ubah' rel='trterlpaor".$value_terlapor['id_terlapor']."' value='".$value_terlapor['id_terlapor']."'enama='".$value_terlapor['nama_pegawai_terlapor']. "'epangkat='".$value_terlapor['pangkat_pegawai_terlapor']."'enip='".$value_terlapor['nip']."'ejabatan='".$value_terlapor['jabatan_pegawai_terlapor']."'egolongan='".$value_terlapor['golongan_pegawai_terlapor']."'enrp='".$value_terlapor['nrp_pegawai_terlapor']."'></td>";
                          ?>
                        <input type="hidden" name="id_was10[]" value="<?= $value_terlapor['id_surat_was10']?>">
                        ...
                        <input type="hidden" name="id_pegawai_terlapor[]" class="id_pegawai_terlapor" value="<?= $value_terlapor['id_pegawai_terlapor']?>">
                      </tr>
                    <?php 
                    $no_1++;
                        }
     ...
     /*simpan data terlapor pada local storage*/
         if(!$model->isNewRecord){
            ...
            $sql = "select string_agg(nip_pegawai_terlapor,',') as nip_pegawai_terlapor 
                    from was.was12_inspeksi_terlapor where no_register='".$_SESSION['was_register']."'
    ...
    <div class="col-md-12" style="padding-top: 15px;padding-bottom: 15px;">
        <label>Unggah Berkas WAS-12 Inspeksi : 
        ...
        <span class="fileupload-exists "> Rubah File</span>         <input type="file" name="was12_file" id="was12_file" /></span>
    ...
    ?>
      ...
      <?php if(!$model->isNewRecord){ ?>  
        <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['was12-inspeksi/cetakdocx?no_register='.$model->no_register.'&id='. $model['id_was_12'].'&id_tingkat='. $model['id_tingkat'].'&id_kejati='. $model['id_kejati'].'&id_kejari='. $model['id_kejari'].'&id_cabjari='. $model['id_cabjari']])?>"><i class="fa fa-print"></i> Cetak</a>
      <?php } ?>
        <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['was12-inspeksi/index'])?>"><i class="fa fa-arrow-left"></i> Kembali</a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Undang Penandatangan</h4>
                </div>
                <div class="modal-body">
                    <p>
                        <?php $form = ActiveForm::begin([
...
$(document).on('click','#hapus_tembusan',function() {
 ...
 $('#hapus_terlapor').click(function(){
         var cek = $('.ck_bok:checked').length;
         alert(cek);
        //  $('.trterlpaor:checked').closest('tr').remove();
    });

/*part localstorage*/
localStorage.removeItem("was12inspeksinip_terlapor");
/*ambil dari database*/
...
localStorage.removeItem("was12inspeksinip_terlapor_OnDB");
localStorage.setItem("was12inspeksinip_terlapor_OnDB", JSON.stringify(temp));
...
$(document).on('click','#Tambah_terlapor',function(){
    ...
    $(document).on('click','#Mtambah_terlapor',function(){
            ...
            var names=[];
            var cek = JSON.parse(localStorage.getItem("was12inspeksinip_terlapor"));
            var cek_OnDB = JSON.parse(localStorage.getItem("was12inspeksinip_terlapor_OnDB"));
            // var data=JSON.parse($('.Mselection_one:checked').attr('json'));
            for (var i = 0; i < jml; i++) {
                var trans=JSON.parse(checkValues[i]);
            if(jQuery.inArray(trans.nip_pegawai_terlapor,cek)==-1 && jQuery.inArray(trans.nip_pegawai_terlapor,cek_OnDB)==-1){/*jika kondisi tidak sama maka tambahkan data*/
                $('.bd_terlapor_tmp').append('<tr>'+
                        '<td></td>'+
                        '<td>'+trans.nip_pegawai_terlapor+'</td>'+
                        '<td>'+trans.nama_pegawai_terlapor+'</td>'+
                        '<td>'+trans.jabatan_pegawai_terlapor+'</td>'+
                        '<td>'+trans.pangkat_pegawai_terlapor+'</td>'+
                        '<td><input class="td_terlapor" name="selection[]" value="'+trans.nip_pegawai_terlapor+'"  type="checkbox">'+
                        '<input type="hidden" name="nip_pegawai_terlapor[]" value="'+trans.nip_pegawai_terlapor+'">'+
                        ...
                        '<input type="hidden" name="id_pegawai_terlapor[]" value="'+trans.id_pegawai_terlapor+'">'+
                        '</td>'+
                        '</tr>');
                // names.push(trans.nip,arrnip);
            }
            };
            var arrnip = $('.td_terlapor').map(function()
                                            {
                                                return $(this).val();
                                            }).get();
                localStorage.setItem("was12inspeksinip_terlapor", JSON.stringify(arrnip));
...
$(document).on('click','#tambah_penandatangan',function() {
    ...
    if(jml=>1 && jml<2){
       $('#was12inspeksi-nip_penandatangan').val(data.nip);
       $('#was12inspeksi-nama_penandatangan').val(data.nama);
       $('#was12inspeksi-jabatan_penandatangan').val(data.nama_jabatan);
       $('#was12inspeksi-jbtn_penandatangan').val(data.jabtan_asli);
       $('#was12inspeksi-pangkat_penandatangan').val(data.gol_pangkat2);
       $('#was12inspeksi-golongan_penandatangan').val(data.gol_kd);
...
$(document).on('click','.cari_ttd',function() { 
    $('#grid-penandatangan_surat').addClass('loading');
    $("#grid-penandatangan_surat").load("/pengawasan/was12-inspeksi/getttd",function(responseText, statusText, xhr)
        {
