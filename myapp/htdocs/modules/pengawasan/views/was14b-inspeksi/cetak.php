...
require_once('/wordtest/classes/CreateDocx.inc');
        $docx = new CreateDocxFromTemplate('../modules/pengawasan/template/was14b_inspeksi.docx');
        ...
        $nomor         =($model['no_was14b']!=''?$model['no_was14b']:'<p></p>');

        if($model['sifat_was14b']=='0'){
            $sifat="Biasa";
            $akr="B";
         }else if($model['sifat_was14b']=='1'){
            $sifat="Segera";
            $akr="S";
         }else if($model['sifat_was14b']=='2'){
            $sifat="Rahasia";
            $akr="R";
         }
        $lampiran         		=$model['lampiran_was14b'];
        $perihal          		=$model['perihal_was14b'];
        $kepada           		=$model['kepada_was14b'];
        $tempat           		=$lokasi_surat;
        $tanggal_surat   	 	=\Yii::$app->globalfunc->ViewIndonesianFormat($model['tgl_was14b']);
        $di 			  		=$model['di_was14b'];
    ...
    //     $detail_uraian="";
    //     $no=1;
    //     foreach ($modelwas14b as $row_uraian) {
    ...
    //     	$no++;
	   ...
	   //  $detail_uraian=$detail_uraian;
        $detail_uraian="";
        $no_uraian=1;
        $uraian_was14b_inspeksi .="<table width='100%' style='font-family:Arial;'>";
        foreach ($modelwas14b as $rowUraian) {
            if(count($modelwas14b)>=2){
            $uraian_was14b_inspeksi .="<tr>
                            <td width='1%'>".$no_uraian."</td>
       ...
       $docx->replaceVariableByText(array('jabtan_penandatangan'=>ucwords(strtolower($jabtan_penandatangan))), array('parseLineBreaks'=>true));
       ...
       $docx->replaceVariableByHTML('uraian_was14b_inspeksi', 'block',$uraian_was14b_inspeksi, array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));

       $docx->replaceVariableByHTML('tembusan', 'block',$tembusan, array('isFile' => false, 'parseDivsAsPs' => true, 'downloadImages' => false));

 		$docx->createDocx('template/pengawasan/was14b_inspeksi');
		
        $file = 'template/pengawasan/was14b_inspeksi.docx';
