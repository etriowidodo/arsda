    ...
    <div class="content-wrapper-1">
            ...
            <div class="col-md-12" style="padding-right: 0px;padding-left: 0px;">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="col-sm-12">
                                     <?= $form->field($model, 'no_sk_was_4c')->textInput()->label(false); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_sk_was_4c',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
             ...
             <?php if(!$model->isNewRecord){ ?>
                 <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_diterima_sk_was_4c',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
                                    'type' => DateControl::FORMAT_DATE,
                                    'ajaxConversion' => false,
                                    'options' => [
                                        'pluginOptions' => [
      ...
      <div class="panel panel-primary">
          <div class="panel-heading">Penandatangan</div>
            ...
            <?php if(!$model->isNewRecord){ ?>
            <div class="col-md-12" style="padding-top: 15px;">
                <label>Unggah Berkas Sk-Was-4c : 
                     <?php if (substr($model->upload_file,-3)!='pdf'){?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_4c'].'&id2='.$model['id_sk_was_4c'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-image-o"></i></span></a>' :'')?> <?= $model['upload_file']?>
                    <?php } else{?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_4c'].'&id2='.$model['id_sk_was_4c'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-pdf-o"></i></span></a>' :'')?> <?= $model['upload_file']?> 
                    <?php } ?>
                </label>
 ...
 <?php    
       if (!$model->isNewRecord){
        // if($result_expire=='1'){
          ...
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-4c/index'], ['id' => 'KembaliSkWas4c', 'class' => 'btn btn-primary']);
        //  }
       }else{
          echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-save"></i> Simpan' : '<i class="fa fa-save"></i> Simpan', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ;
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-4c/index'], ['id' => 'KembaliSkWas4c', 'class' => 'btn btn-primary']);
...
#grid-penandatangan_surat.loading .modal-loading-new {display: block;}
    ...
    .field-skwas4cpelapor-nama_pelapor , .field-skwas4cpelapor-alamat_pelapor , .field-skwas4cpelapor-email_pelapor
    , .field-skwas4cpelapor-telp_pelapor, .field-skwas4cpelapor-pekerjaan_pelapor, .field-skwas4cpelapor-kewarganegaraan_pelapor
    , .field-skwas4cpelapor-agama_pelapor, .field-skwas4cpelapor-pendidikan_pelapor, .field-skwas4cpelapor-nama_kota_pelapor
    , .field-skwas4cpelapor-tanggal_lahir_pelapor, .field-skwas4cpelapor-tempat_lahir_pelapor, .field-skwas4cpelapor-sumber_lainnya
    , .field-skwas4cpelapor-id_sumber_laporan ,.field-skwas4c-nip_pegawai_terlapor ,.field-skwas4c-nama_pegawai_terlapor
    ,.field-skwas4c-pangkat_pegawai_terlapor ,.field-skwas4c-golongan_pegawai_terlapor ,.field-skwas4c-jabatan_pegawai_terlapor
    ,.field-skwas4cpemeriksa-nip_pemeriksa ,.field-skwas4cpemeriksa-nrp_pemeriksa ,.field-skwas4cpemeriksa-nama_pemeriksa
    ,.field-skwas4cpemeriksa-pangkat_pemeriksa ,.field-skwas4cpemeriksa-jabatan_pemeriksa ,.field-skwas4cpemeriksa-golongan_pemeriksa{
        display: none;
    }
 
 /*#skwas4c-tgl_sk_was_4c-disp{
    width: 405px;
 } */  
</style>
...
$(document).on('click','#addtembusan',function() {
        ...
        $(document).on('click','#tambah_penandatangan',function() {
          ...
          if(jml=>1 && jml<2){
             $('#skwas4c-nip_penandatangan').val(data.nip);
             $('#skwas4c-nama_penandatangan').val(data.nama);
             $('#skwas4c-jabatan_penandatangan').val(data.nama_jabatan);
             $('#skwas4c-golongan_penandatangan').val(data.gol_kd);
             $('#skwas4c-pangkat_penandatangan').val(data.gol_pangkat2);
             $('#skwas4c-jbtn_penandatangan').val(data.jabtan_asli);
    ...
    $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sk-was-4c/getttd",function(responseText, statusText, xhr)
            {
