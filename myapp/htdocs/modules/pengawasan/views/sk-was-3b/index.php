...
<div class="sk-was3b-index">

    <h4><?php ?></h4>
    <?php //  echo $this->render('_search', ['model' => $searchModel]); ?>
<!-- href="/pengawasan/sk-was-3b/create" -->
    <p>
        <div class="btn-toolbar">
              <a class="btn btn-primary btn-sm pull-right" id="cetak_skwas3b"><i class="glyphicon glyphicon-print"> </i> Cetak </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="hapus_skwas3b"><i class="glyphicon glyphicon-trash"> </i> Hapus </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="ubah_skwas3b"><i class="glyphicon glyphicon-pencil"> </i> Ubah </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="create_skwas3b" ><i class="glyphicon glyphicon-plus"> </i> SK-WAS-3b</a>
            </div>
    </p>
    <style type="text/css">
...
legend.group-border {
    ...
    font-size: 14px;
     ...
     <fieldset class="group-border">
        <legend class="group-border">Daftar SK-WAS-3b</legend>
    <?= GridView::widget([
...
//         $("#create_skwas3b").addClass("disabled");
//         $("#ubah_skwas3b").addClass("disabled");
//         $("#hapus_skwas3b").addClass("disabled");
//         $("#cetak_skwas3b").addClass("disabled");
//       }else{
//         $(this).find('.checkbox-row').prop('checked',true);
//         $("#create_skwas3b").removeClass("disabled");
//         $("#ubah_skwas3b").removeClass("disabled");
//         $("#hapus_skwas3b").removeClass("disabled");
//         $("#cetak_skwas3b").removeClass("disabled");
//       }
//       // alert(z);
// });
...
$("#create_skwas3b").addClass("disabled");
  ...
  function ConditionOfButton(n){
        if(n == 1){
           $('#cetak_skwas3b,#ubah_skwas3b,#create_skwas3b, #hapus_skwas3b').removeClass('disabled');
        } else if(n > 1){
           $('#hapus_skwas3b').removeClass('disabled');
           $('#cetak_skwas3b,#ubah_skwas3b,#create_skwas3b').addClass('disabled');
        } else{
           $('#cetak_skwas3b,#ubah_skwas3b,#create_skwas3b, #hapus_skwas3b').addClass('disabled');
        }
    }

    $('#create_skwas3b').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.nip_trlp != null){
            ...
            location.href='/pengawasan/sk-was-3b/create?id='+check.nip_terlapor+'&id2='+check.pasal;
           } 
         }
    });

    $('#ubah_skwas3b').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
            ...
            if(check.id_sk_was_3b == null){
               bootbox.alert({
                      message:"Data Belum Bisa Di Rubah Nomor Harus Di Isi",
                      size:'small'
                  });
             }else{
                location.href='/pengawasan/sk-was-3b/update?id='+check.id_sk_was_3b+'&id2='+check.nip_pegawai_terlapor+'&id3='+check.pasal;
             }
         }
    });

    $(document).on('click', '#cetak_skwas3b', function () {

          // alert()
          var x=$(".checkbox-row:checked").length;
          var link=$(".checkbox-row:checked").val();
          var check=JSON.parse($('.checkbox-row:checked').attr('json'));
         // alert(check.id_sk_was_3b);
          ...
          if(check.id_sk_was_3b == null){
             bootbox.alert({
              ...
              location.href='/pengawasan/sk-was-3b/cetak?id='+check.id_sk_was_3b;
           }
          //alert(nilai);
        });

$(document).on('click','#hapus_skwas3b',function(){
                                ...
                                var checkValues = $('.checkbox-row:checked').map(function()
                                {
                                 ...
                                 var jml=$('.checkbox-row:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/sk-was-3b/delete',
                                            data:'id='+data.id_sk_was_3b+'&jml='+jml,
