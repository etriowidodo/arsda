...
<div class="sk-was2c-index">

    <h4><?php ?></h4>
    <?php //  echo $this->render('_search', ['model' => $searchModel]); ?>
<!-- href="/pengawasan/sk-was-2c/create" -->
    <p>
        <div class="btn-toolbar">
              <a class="btn btn-primary btn-sm pull-right" id="cetak_skwas2c"><i class="glyphicon glyphicon-print"> </i> Cetak </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="hapus_skwas2c"><i class="glyphicon glyphicon-trash"> </i> Hapus </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="ubah_skwas2c"><i class="glyphicon glyphicon-pencil"> </i> Ubah </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="create_skwas2c" ><i class="glyphicon glyphicon-plus"> </i> SK-WAS-2c</a>
            </div>
    </p>
    <style type="text/css">
...
legend.group-border {
    ...
    font-size: 14px;
     ...
     <fieldset class="group-border">
        <legend class="group-border">Daftar SK-WAS-2c</legend>
    <?= GridView::widget([
...
//         $("#create_skwas2c").addClass("disabled");
//         $("#ubah_skwas2c").addClass("disabled");
//         $("#hapus_skwas2c").addClass("disabled");
//         $("#cetak_skwas2c").addClass("disabled");
//       }else{
//         $(this).find('.checkbox-row').prop('checked',true);
//         $("#create_skwas2c").removeClass("disabled");
//         $("#ubah_skwas2c").removeClass("disabled");
//         $("#hapus_skwas2c").removeClass("disabled");
//         $("#cetak_skwas2c").removeClass("disabled");
//       }
//       // alert(z);
// });
...
$("#create_skwas2c").addClass("disabled");
  ...
  function ConditionOfButton(n){
        if(n == 1){
           $('#cetak_skwas2c,#ubah_skwas2c,#create_skwas2c, #hapus_skwas2c').removeClass('disabled');
        } else if(n > 1){
           $('#hapus_skwas2c').removeClass('disabled');
           $('#cetak_skwas2c,#ubah_skwas2c,#create_skwas2c').addClass('disabled');
        } else{
           $('#cetak_skwas2c,#ubah_skwas2c,#create_skwas2c, #hapus_skwas2c').addClass('disabled');
        }
    }

    $('#create_skwas2c').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.nip_trlp != null){
            ...
            location.href='/pengawasan/sk-was-2c/create?id='+check.nip_terlapor+'&id2='+check.pasal;
           } 
         }
    });

    $('#ubah_skwas2c').click(function(){
         ...
         return false
            ...
            if(check.id_sk_was_2c == null){
               bootbox.alert({
                ...
                location.href='/pengawasan/sk-was-2c/update?id='+check.id_sk_was_2c+'&id2='+check.nip_pegawai_terlapor+'&id3='+check.pasal;
             } 
         }
    });

    $(document).on('click', '#cetak_skwas2c', function () {

          // alert()
          var x=$(".checkbox-row:checked").length;
          var link=$(".checkbox-row:checked").val();
          var check=JSON.parse($('.checkbox-row:checked').attr('json'));
         // alert(check.id_sk_was_2c);
          ...
          if(check.id_sk_was_2c == null){
             bootbox.alert({
              ...
              location.href='/pengawasan/sk-was-2c/cetak?id='+check.id_sk_was_2c;
           }
          
          //alert(nilai);
        });

$(document).on('click','#hapus_skwas2c',function(){
                                ...
                                var checkValues = $('.checkbox-row:checked').map(function()
                                {
                                 ...
                                 var jml=$('.checkbox-row:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/sk-was-2c/delete',
                                            data:'id='+data.id_sk_was_2c+'&jml='+jml,
