    ...
    <div class="content-wrapper-1">
            ...
            <div class="col-md-12" style="padding-right: 0px;padding-left: 0px;">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="col-sm-12">
                                     <?= $form->field($model, 'no_sk_was_2c')->textInput()->label(false); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_sk_was_2c',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
      ...
      <div class="panel panel-primary">
          <div class="panel-heading">Penandatangan</div>
            ...
            <?php if(!$model->isNewRecord){ ?>
            <div class="col-md-12" style="padding-top: 15px;">
                <label>Unggah Berkas Sk-Was-2c : 
                     <?php if (substr($model->upload_file,-3)!='pdf'){?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_2c'].'&id2='.$model['id_sk_was_2c'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-image-o"></i></span></a>' :'') ?> <?= $model['upload_file']?>
                    <?php } else{?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_2c'].'&id2='.$model['id_sk_was_2c'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-pdf-o"></i></span></a>' :'') ?>  <?= $model['upload_file']?>
                    <?php } ?>
                </label>
 ...
 <?php    
       if (!$model->isNewRecord){
        // if($result_expire=='1'){
          ...
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-2c/index'], ['id' => 'KembaliSkWas2c', 'class' => 'btn btn-primary']);
        //  }
       }else{
          echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-save"></i> Simpan' : '<i class="fa fa-save"></i> Simpan', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ;
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-2c/index'], ['id' => 'KembaliSkWas2c', 'class' => 'btn btn-primary']);
...
#grid-penandatangan_surat.loading .modal-loading-new {display: block;}
    ...
    .field-skwas2cpelapor-nama_pelapor , .field-skwas2cpelapor-alamat_pelapor , .field-skwas2cpelapor-email_pelapor
    , .field-skwas2cpelapor-telp_pelapor, .field-skwas2cpelapor-pekerjaan_pelapor, .field-skwas2cpelapor-kewarganegaraan_pelapor
    , .field-skwas2cpelapor-agama_pelapor, .field-skwas2cpelapor-pendidikan_pelapor, .field-skwas2cpelapor-nama_kota_pelapor
    , .field-skwas2cpelapor-tanggal_lahir_pelapor, .field-skwas2cpelapor-tempat_lahir_pelapor, .field-skwas2cpelapor-sumber_lainnya
    , .field-skwas2cpelapor-id_sumber_laporan ,.field-skwas2c-nip_pegawai_terlapor ,.field-skwas2c-nama_pegawai_terlapor
    ,.field-skwas2c-pangkat_pegawai_terlapor ,.field-skwas2c-golongan_pegawai_terlapor ,.field-skwas2c-jabatan_pegawai_terlapor
    ,.field-skwas2cpemeriksa-nip_pemeriksa ,.field-skwas2cpemeriksa-nrp_pemeriksa ,.field-skwas2cpemeriksa-nama_pemeriksa
    ,.field-skwas2cpemeriksa-pangkat_pemeriksa ,.field-skwas2cpemeriksa-jabatan_pemeriksa ,.field-skwas2cpemeriksa-golongan_pemeriksa{
        display: none;
    }
 
/* #skwas2c-tgl_sk_was_2c-disp{
    width: 405px;
 }   */
</style>
...
$(document).on('click','#addtembusan',function() {
        ...
        $(document).on('click','#tambah_penandatangan',function() {
          ...
          if(jml=>1 && jml<2){
             $('#skwas2c-nip_penandatangan').val(data.nip);
             $('#skwas2c-nama_penandatangan').val(data.nama);
             $('#skwas2c-jabatan_penandatangan').val(data.nama_jabatan);
             $('#skwas2c-golongan_penandatangan').val(data.gol_kd);
             $('#skwas2c-pangkat_penandatangan').val(data.gol_pangkat2);
             $('#skwas2c-jbtn_penandatangan').val(data.jabtan_asli);
 ...
 /*/////////PENANDATANGAN LOADING GRID//////////////*/
    ...
    $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sk-was-2c/getttd",function(responseText, statusText, xhr)
            {
