...
<div class="box box-primary" style="padding: 15px 0px;">
    <div class="col-md-12">
        <div class="col-md-4">
            <div class="form-group">
                ...
                <div class="col-md-8">
                <?= $form->field($model, 'no_was_11')->textInput(['maxlength' => true]) ?>
                </div>
   	        </div>
        </div>
            ...
            <div class="form-group">
                ...
                <?= $form->field($model, 'tgl_was_11',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
                    ...
                    'options' => [
                        'pluginOptions' => [
                            'autoclose' => true,
                            'startDate' => date("d/m/Y", strtotime($modelSpwas1['tanggal_mulai_sp_was1'])),
    ...
    <div class="col-md-12">
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <?= $form->field($model, 'jenis_saksi')->hiddenInput(['maxlength' => true,'value'=>$_GET['jns']]) ?>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="panel-heading">Saksi <?= $_GET['jns']?></div>
                <div class="panel-body">
                   <div class="btn-toolbar role">
                        <a class="btn btn-primary btn-sm pull-right" id="hapus_saksi"><i class="glyphicon glyphicon-trash"> </i> Hapus</a>&nbsp;
                    ...
                    <tbody class="bd_saksi_in_tmp">
                      ...
                      foreach ($modelSaksiIn_trans as $key_saksi_in) {
                        ...
                        <tr classs="saksi_int">
                          ...
                          <input value="<?= $key_saksi_in['id_was_9'] ?>" type="hidden" class="form-control" id="nip" name="nip[]"  readonly>
                          <input value="<?= $key_saksi_in['nama_saksi_internal'] ?>" type="hidden" class="form-control" id="nama_saksi_in[]" name="nama_saksi_in[]" readonly>
                          <input value="<?= $key_saksi_in['jabatan_saksi_internal'] ?>" type="hidden" class="form-control" id="jabatan" name="jabatan[]"  readonly>
                          <input value="<?= $key_saksi_in['pangkat_saksi_internal'] ?>" type="hidden" class="form-control" id="pangkat" name="pangkat[]"  readonly>
                          <input value="<?= $key_saksi_in['golongan_saksi_internal'] ?>" type="hidden" class="form-control" id="golongan" name="golongan[]"  readonly>
                          </td>
                          ...
                          <td><input class="selection_one_saksi_in" value="<?= $key_saksi_in['nip_saksi_internal'] ?>" type="checkbox" name="selection_one_saksi_in" id="<?= $key_saksi_in['nip_saksi_internal'] ?>"></td>
                        </tr>
                      <?php
                      $no_saksi_in++;
                      }
                    ...
                    <tbody class="bd_saksi_ek_tmp">
                      ...
                      foreach ($modelSaksiIn_trans as $key_saksi_ek) {
                        ...
                        <tr classs="saksi_eks">
                          ...
                          <input value="<?= $key_saksi_ek['id_was_9'] ?>" type="hidden" class="form-control" id="Mid_saksi_eksternal" name="Mid_saksi_eksternal[]"  readonly>
                          <input value="<?= $key_saksi_ek['nama_saksi_eksternal'] ?>" type="hidden" class="form-control" id="nama_saksi_eks" name="nama_saksi_eks[]"  readonly>
                          <input value="<?= $key_saksi_ek['alamat_saksi_eksternal'] ?>" type="hidden" class="form-control" id="alamat_saksi_eks[]" name="alamat_saksi_eks[]" readonly>
                          <input value="<?= $key_saksi_ek['nama_kota_saksi_eksternal'] ?>" type="hidden" class="form-control" id="kota_saksi_eks" name="kota_saksi_eks[]"  readonly>
                          </td>
                          ...
                          <td><input class="selection_one_saksi_ek" value="<?= $key_saksi_ek['id_was_11_detil_ek'] ?>" type="checkbox" name="hapus_tr_saksi_ek" id="<?= $key_saksi_ek['id_was_11_detil_ek'] ?>"></td>
                        </tr>
                      <?php
                      $no_saksi_ek++;
                      }
    ...
    <div class="col-md-12" style="padding-top: 15px;padding-bottom: 15px;">
        ...
        <span class="btn btn-primary btn-file fa fa-folder-open"><span class="fileupload-new"> Pilih Berkas </span>
    ...
    <div class="col-md-12">
        ...
        <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['was11/index'])?>"><i class="fa fa-arrow-left"></i> Batal</a>
        <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['was11/cetakdocx?no_register='.$model->no_register.'&id='. $model['id_surat_was11'].'&id_tingkat='. $model['id_tingkat'].'&id_kejati='. $model['id_kejati'].'&id_kejari='. $model['id_kejari'].'&id_cabjari='. $model['id_cabjari']])?>"><i class="fa fa-print"></i> Cetak</a>
        </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Pilih Saksi</h4>
                </div>
              ...
              ['class' => 'yii\grid\CheckboxColumn',
               ...
               'contentOptions'=>['style'=>'text-align:center; width:5%'],
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default" id="tambah_saksi_int">Pilih</button>
                </div>
            </div>
        </div>
</div>
...
/*end js upload
  ...
  /*remove localstorage*/
    ...
    $('#hapus_saksi').click(function(){
        ...
        var cek1 = $('.selection_one_saksi_in:checked').length;
        $('.selection_one_saksi_in:checked').closest('tr').remove();
       
    }); $

    $('#hapus_saksi_ek').click(function(){
        var cek = $('.hapus_tr_saksi_ek:checked').length;
      // alert(cek);
         var checkValues = $('.hapus_tr_saksi_ek:checked').map(function()
            {
                return $(this).val();
            }).get();
                for (var i = 0; i < cek; i++) {
...
window.onload=function(){
    ...
    $(document).on('click','#tambah_penandatangan',function() {
      ...
      if(jml=>1 && jml<2){
         $('#was11-nip_penandatangan').val(data.nip);
         $('#was11-nama_penandatangan').val(data.nama);
         $('#was11-jabatan_penandatangan').val(data.nama_jabatan);
         $('#was11-pangkat_penandatangan').val(data.gol_pangkat2);
         $('#was11-golongan_penandatangan').val(data.gol_kd);
         $('#was11-jbtn_penandatangan').val(data.jabtan_asli);
    ...
    $(document).on('click','#tambah_saksi_int',function() {
      ...
      for (var i = 0; i <jml; i++) {
        ...
        $('.bd_saksi_in_tmp').append('<tr>'+
                            ...
                            '<input type="hidden" name="nip[]" value="'+data.id_surat_was9+'">'+
                            '<input type="hidden" name="Mnrp_saksi[]" value="'+data.pangkat_saksi_internal+'">'+
    ...
    $(document).on('click','#tambah_saksi_eks',function() {
          ...
          for (var i = 0; i <jml; i++) {
            ...
            $('.bd_saksi_ek_tmp').append('<tr>'+
                                ...
                                '<input type="hidden" name="Mid_saksi_eksternal[]" value="'+data.id_surat_was9+'">'+
    ...
    $(document).on('click','.cari_ttd',function() { 
        $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/was11/getttd",function(responseText, statusText, xhr)
            {
