...
$this->title = 'BA-WAS 6';
$this->subtitle = 'SURAT PERNYATAAN MENERIMA/MENOLAK TERHADAP SURAT KEPUTUSAN PHD';
   ...
   <?php// echo $this->render('_search', ['model' => $searchModel]); ?>
    ...
    <div id="divHapus">
		<div class="pull-right">
			<a class="btn btn-primary" href="/pengawasan/ba-was6/create"><i class="fa fa-plus"></i> Tambah</a>
	...
	<?= GridView::widget([
            ...
            'rowOptions'   => function ($model, $key, $index, $grid) {
                return ['data-id' => $model['id_ba_was_6']];
            },
				...
				['class' => 'yii\grid\CheckboxColumn',
                     ...
                     'contentOptions'=>['style'=>'text-align:center; width:5%','class'=>'aksinya'],
                               'checkboxOptions' => function ($data) {
                                ...
                                return ['value' => $data['id_ba_was_6'],'class'=>'selection_one','json'=>$result];
                                },
                        ],
...
window.onload=function(){		
	...
	$(document).on('click','#btnUbah',function(){
		var data=JSON.parse($('.selection_one:checked').attr('json'));
           // alert(data.id_was_16b);
            location.href='/pengawasan/ba-was6/update?id='+data.id_ba_was_6;  
	});

	 $(document).on('click', '#btnCetak', function () {
          var nilai=$('.selection_one:checked').val();
          location.href='/pengawasan/ba-was6/cetak?id='+nilai;
        });
                                ...
                                var checkValues = $('.selection_one:checked').map(function()
                                {
                                 ...
                                 var jml=$('.selection_one:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/ba-was6/delete',
                                            data:'id='+data.id_ba_was_6+'&jml='+jml,
  ...
  $(document).on('dblclick', 'tr td:not(.aksinya)', function () {
         ...
         // var data=JSON.parse($('.selection_one:checked').attr('json'));
            location.href='/pengawasan/was20a/update?id='+check.id_ba_was_6; 
...
//             var id = $(this).find('.selection_one').val();
//          // var data=JSON.parse($('.selection_one:checked').attr('json'));
//                   location.href='/pengawasan/ba-was6/update?id='+id; 
//         });
//  }); 
</script>
