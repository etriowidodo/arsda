...
<div class="sk-was2a-index">

    <h4><?php ?></h4>
    <?php //  echo $this->render('_search', ['model' => $searchModel]); ?>
<!-- href="/pengawasan/sk-was-2a/create" -->
    <p>
        <div class="btn-toolbar">
              <a class="btn btn-primary btn-sm pull-right" id="cetak_skwas2a"><i class="glyphicon glyphicon-print"> </i> Cetak </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="hapus_skwas2a"><i class="glyphicon glyphicon-trash"> </i> Hapus </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="ubah_skwas2a"><i class="glyphicon glyphicon-pencil"> </i> Ubah </a>&nbsp;
              <a class="btn btn-primary btn-sm pull-right" id="create_skwas2a" ><i class="glyphicon glyphicon-plus"> </i> SK-WAS-2A</a>
            </div>
    </p>
    <style type="text/css">
...
legend.group-border {
    ...
    font-size: 14px;
     ...
     <fieldset class="group-border">
        <legend class="group-border">Daftar SK-WAS-2A</legend>
    <?= GridView::widget([
...
//         $("#create_skwas2a").addClass("disabled");
//         $("#ubah_skwas2a").addClass("disabled");
//         $("#hapus_skwas2a").addClass("disabled");
//         $("#cetak_skwas2a").addClass("disabled");
//       }else{
//         $(this).find('.checkbox-row').prop('checked',true);
//         $("#create_skwas2a").removeClass("disabled");
//         $("#ubah_skwas2a").removeClass("disabled");
//         $("#hapus_skwas2a").removeClass("disabled");
//         $("#cetak_skwas2a").removeClass("disabled");
//       }
//       // alert(z);
// });
...
//     });
  ...
  function ConditionOfButton(n){
        if(n == 1){
           $('#cetak_skwas2a,#ubah_skwas2a,#create_skwas2a, #hapus_skwas2a').removeClass('disabled');
        } else if(n > 1){
           $('#hapus_skwas2a').removeClass('disabled');
           $('#cetak_skwas2a,#ubah_skwas2a,#create_skwas2a').addClass('disabled');
        } else{
           $('#cetak_skwas2a,#ubah_skwas2a,#create_skwas2a, #hapus_skwas2a').addClass('disabled');
        }
    }

    $('#create_skwas2a').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
           ...
           if(check.nip_trlp != null){
            ...
            location.href='/pengawasan/sk-was-2a/create?id='+check.nip_terlapor+'&id2='+check.pasal;
           } 
         }
    });

    $('#ubah_skwas2a').click(function(){
       ...
       if(x>=2 || x<=0){
         ...
         return false
            ...
            if(check.id_sk_was_2a == null){
               bootbox.alert({
                      message:"Data Belum Bisa Di Rubah Nomor Harus Di Isi",
                      size:'small'
                  });
             }else{
                location.href='/pengawasan/sk-was-2a/update?id='+check.id_sk_was_2a+'&id2='+check.nip_pegawai_terlapor+'&id3='+check.pasal;
             }
           
         }
    });

    $(document).on('click', '#cetak_skwas2a', function () {

          // alert()
          var x=$(".checkbox-row:checked").length;
          var link=$(".checkbox-row:checked").val();
          var check=JSON.parse($('.checkbox-row:checked').attr('json'));
         // alert(check.id_sk_was_2a);
          ...
          if(check.id_sk_was_2a == null){
             bootbox.alert({
              ...
              location.href='/pengawasan/sk-was-2a/cetak?id='+check.id_sk_was_2a;
           }
          
          //alert(nilai);
        });

$(document).on('click','#hapus_skwas2a',function(){
                                ...
                                var checkValues = $('.checkbox-row:checked').map(function()
                                {
                                 ...
                                 var jml=$('.checkbox-row:checked').length;
                                    for (var i = 0; i<jml; i++) {
                                        ...
                                        $.ajax({
                                            type:'POST',
                                            url:'/pengawasan/sk-was-2a/delete',
                                            data:'id='+data.id_sk_was_2a+'&jml='+jml,
