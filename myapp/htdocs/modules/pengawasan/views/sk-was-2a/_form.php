    ...
    <div class="content-wrapper-1">
        <div class="box-header with-border">
            <div class="col-md-12" style="padding-right: 0px;padding-left: 0px;">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="control-label col-md-4" style="width: 140px;">No SK 
</label>
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="col-sm-12">
                                     <?= $form->field($model, 'no_sk_was_2a')->textInput()->label(false); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <?=
                                $form->field($model, 'tgl_sk_was_2a',['addon' => ['prepend' => ['content'=>'<i class="glyphicon glyphicon-calendar"></i>']]])->widget(DateControl::className(), [
           ...
           <div class="col-md-12" style="padding-top: 15px;">
                <div class="panel panel-primary">
                    <div class="panel-heading">Uraian Mengingat</div>
                    <div class="panel-body">
                        <div class="form-group">
                         <div class="col-sm-12" ></div>
                            <?php 
      ...
      <div class="panel panel-primary">
          <div class="panel-heading">Penandatangan</div>
            ...
            <?php if(!$model->isNewRecord){ ?>
            <div class="col-md-12" style="padding-top: 15px;">
                <label>Unggah Berkas Sk-Was-2a : 
                     <?php if (substr($model->upload_file,-3)!='pdf'){?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_2a'].'&id2='.$model['id_sk_was_2a'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-image-o"></i></span></a>' :'') ?> <?= $model['upload_file']?>
                    <?php } else{?>
                        <?= ($model['upload_file']!='' ? '<a href="viewpdf?id='.$model['id_sk_was_2a'].'&id2='.$model['id_sk_was_2a'].'" target="_blank"><span style="cursor:pointer;font-size:28px;"><i class="fa fa-file-pdf-o"></i></span></a>' :'') ?>  <?= $model['upload_file']?>
                    <?php } ?>
                </label>
 ...
 <?php    
       if (!$model->isNewRecord){
        // if($result_expire=='1'){
         ...
         echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-2a/index'], ['id' => 'KembaliSkWas2a', 'class' => 'btn btn-primary']);
        //  }
       }else{
          echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-save"></i> Simpan' : '<i class="fa fa-save"></i> Simpan', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ;
          echo " ".Html::a('<i class ="fa fa-arrow-left"></i> Batal', ['/pengawasan/sk-was-2a/index'], ['id' => 'KembaliSkWas2a', 'class' => 'btn btn-primary']);
...
<style type="text/css">
    ...
    .control-group.error .fileupload .uneditable-input{color:#b92a48;border-color:#b92a48;}
    .control-group.error .fileupload .fileupload-preview{color:#b92a48;}
    .control-group.error .fileupload .thumbnail{border-color:#b92a48;}
    .control-group.success .fileupload .uneditable-input{color:#468847;border-color:#468847;}
    .control-group.success .fileupload .fileupload-preview{color:#468847;}
    .control-group.success .fileupload .thumbnail{border-color:#468847;}
    .cke{margin:15px!important;}
    .field-skwas2apelapor-nama_pelapor , .field-skwas2apelapor-alamat_pelapor , .field-skwas2apelapor-email_pelapor
    , .field-skwas2apelapor-telp_pelapor, .field-skwas2apelapor-pekerjaan_pelapor, .field-skwas2apelapor-kewarganegaraan_pelapor
    , .field-skwas2apelapor-agama_pelapor, .field-skwas2apelapor-pendidikan_pelapor, .field-skwas2apelapor-nama_kota_pelapor
    , .field-skwas2apelapor-tanggal_lahir_pelapor, .field-skwas2apelapor-tempat_lahir_pelapor, .field-skwas2apelapor-sumber_lainnya
    , .field-skwas2apelapor-id_sumber_laporan ,.field-skwas2a-nip_pegawai_terlapor ,.field-skwas2a-nama_pegawai_terlapor
    ,.field-skwas2a-pangkat_pegawai_terlapor ,.field-skwas2a-golongan_pegawai_terlapor ,.field-skwas2a-jabatan_pegawai_terlapor
    ,.field-skwas2apemeriksa-nip_pemeriksa ,.field-skwas2apemeriksa-nrp_pemeriksa ,.field-skwas2apemeriksa-nama_pemeriksa
    ,.field-skwas2apemeriksa-pangkat_pemeriksa ,.field-skwas2apemeriksa-jabatan_pemeriksa ,.field-skwas2apemeriksa-golongan_pemeriksa{
        display: none;
    }
 
/* #skwas2a-tgl_sk_was_2a-disp{
    width: 405px;
 }  */ 
</style>
...
$(document).on('click','#addtembusan',function() {
        ...
        $(document).on('click','#tambah_penandatangan',function() {
          ...
          if(jml=>1 && jml<2){
             $('#skwas2a-nip_penandatangan').val(data.nip);
             $('#skwas2a-nama_penandatangan').val(data.nama);
             $('#skwas2a-jabatan_penandatangan').val(data.nama_jabatan);
             $('#skwas2a-golongan_penandatangan').val(data.gol_kd);
             $('#skwas2a-pangkat_penandatangan').val(data.gol_pangkat2);
             $('#skwas2a-jbtn_penandatangan').val(data.jabtan_asli);
    ...
    $(document).on('click','.cari_ttd',function() { 
      $('#grid-penandatangan_surat').addClass('loading');
        $("#grid-penandatangan_surat").load("/pengawasan/sk-was-2a/getttd",function(responseText, statusText, xhr)
            {
