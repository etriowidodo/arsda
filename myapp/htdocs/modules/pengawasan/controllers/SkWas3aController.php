...
use Yii;
use app\modules\pengawasan\models\SkWas3a;
use app\modules\pengawasan\models\SkWas3aPelapor;
use app\modules\pengawasan\models\SkWas3aPemeriksa;
use app\modules\pengawasan\models\SkWas3aUraian;
use app\modules\pengawasan\models\SkWas3aSearch;
...
use app\modules\pengawasan\models\TembusanSkWas3a;
...
/**
 * SkWas3aController implements the CRUD actions for SkWas3a model.
 */
class SkWas3aController extends Controller
{
    ...
    /**
     * Displays a single SkWas3a model.
     * @param string $id
     * @return mixed
     */
    public function actionView($id)
    {
     ...
     public function actionViewpdf($id,$id2){
        $file_upload=$this->findModel($id,$id2);
          ...
          $filepath = '../modules/pengawasan/upload_file/sk_was_3a/'.$file_upload['upload_file'];
    ...
    {
        $model = new SkWas3a();
        $modelPemeriksa = new SkWas3aPemeriksa();
        $modelPelapor = new SkWas3aPelapor();
        ...
        $modelTembusanMaster = TembusanWas::find()->where("for_tabel=:condition1 OR for_tabel=:condition2", [":condition1" => 'sk_was_3a','condition2'=>'master'])->orderBy('id_tembusan desc')->all();
        // print_r($dataLapdu);
        // exit();
        $queryp = new Query;
        ...
        $modelTerlapor="select a.*,b.*,c.*,a.bentuk_pelanggaran as keterangan_pelanggaran,d.* 
                        ...
                        where b.saran_dari='Jamwas' and b.sk='SK-WAS3-A' 
        ...
        if ($model->load(Yii::$app->request->post())){
          ...
          // exit();
            ...
            // $model->id_terlapor = $_POST['SkWas3a']['id_terlapor']; 
            // $model->ttd_sk_was_3a = $_POST['SkWas3a']['ttd_sk_was_3a'];
            // $model->no_sk_was_3a = "KEP-".$_POST['no_sk_was_3a'];
          ...
          //  try {
             ...
             // exit();
                   $jml=count($_POST['uraian']);
                    for ($i=0; $i < $jml; $i++) { 
                        $modelsk13aUaraian=new SkWas3aUraian();
                        $modelsk13aUaraian->no_register = $_SESSION['was_register'];
                        $modelsk13aUaraian->id_tingkat  = $_SESSION['kode_tk'];
                        $modelsk13aUaraian->id_kejati   = $_SESSION['kode_kejati'];
                        $modelsk13aUaraian->id_kejari   = $_SESSION['kode_kejari'];
                        $modelsk13aUaraian->id_cabjari  = $_SESSION['kode_cabjari'];
                        $modelsk13aUaraian->id_sp_was2  = $model->id_sp_was2;
                        $modelsk13aUaraian->id_ba_was2  = $model->id_ba_was2;
                        $modelsk13aUaraian->id_l_was2   = $model->id_l_was2;
                        $modelsk13aUaraian->id_was15    = $model->id_was15;
                        $modelsk13aUaraian->id_sk_was_3a= $model->id_sk_was_3a;
                        $modelsk13aUaraian->isi_uraian  = $_POST['uraian'][$i];
                        $modelsk13aUaraian->created_by  =\Yii::$app->user->identity->id;
                        $modelsk13aUaraian->created_ip  =$_SERVER['REMOTE_ADDR'];
                        $modelsk13aUaraian->created_time=date('Y-m-d H:i:s');
                      //  print_r( $modelsk13aUaraian);
                       // print_r($model->id_sk_was_3a);
                        // print_r($jml);
                        //  exit();
                        $modelsk13aUaraian->save();
                    }
//SkWas3aPelapor[nama_pelapor]
                    $jmlPelapor=count($_POST['nama_pelapor']);

                    for ($i=0; $i < $jmlPelapor; $i++) { 
                        $modelsk13aPelapor=new SkWas3aPelapor();
                        $modelsk13aPelapor->no_register = $_SESSION['was_register'];
                        $modelsk13aPelapor->id_tingkat  = $_SESSION['kode_tk'];
                        $modelsk13aPelapor->id_kejati   = $_SESSION['kode_kejati'];
                        $modelsk13aPelapor->id_kejari   = $_SESSION['kode_kejari'];
                        $modelsk13aPelapor->id_cabjari  = $_SESSION['kode_cabjari'];
                        $modelsk13aPelapor->id_sp_was2  = $model->id_sp_was2;
                        $modelsk13aPelapor->id_ba_was2  = $model->id_ba_was2;
                        $modelsk13aPelapor->id_l_was2   = $model->id_l_was2;
                        $modelsk13aPelapor->id_was15    = $model->id_was15;
                        $modelsk13aPelapor->id_sk_was_3a= $model->id_sk_was_3a;
                        $modelsk13aPelapor->nama_pelapor= $_POST['nama_pelapor'][$i];
                        $modelsk13aPelapor->alamat_pelapor    = $_POST['alamat_pelapor'][$i];
                        $modelsk13aPelapor->email_pelapor     = $_POST['email_pelapor'][$i];
                        $modelsk13aPelapor->telp_pelapor      = $_POST['telp_pelapor'][$i];
                        $modelsk13aPelapor->pekerjaan_pelapor = $_POST['pekerjaan_pelapor'][$i];
                        $modelsk13aPelapor->sumber_lainnya    = $_POST['sumber_lainnya'][$i];
                        $modelsk13aPelapor->id_sumber_laporan = $_POST['id_sumber_laporan'][$i];
                        $modelsk13aPelapor->tempat_lahir_pelapor   = $_POST['tempat_lahir_pelapor'][$i];
                        $modelsk13aPelapor->tanggal_lahir_pelapor  = $_POST['tanggal_lahir_pelapor'][$i];
                        $modelsk13aPelapor->kewarganegaraan_pelapor= $_POST['kewarganegaraan_pelapor'][$i];
                        $modelsk13aPelapor->agama_pelapor          = $_POST['agama_pelapor'][$i];
                        $modelsk13aPelapor->pendidikan_pelapor     = $_POST['pendidikan_pelapor'][$i];
                        $modelsk13aPelapor->nama_kota_pelapor      = $_POST['nama_kota_pelapor'][$i];
                        $modelsk13aPelapor->created_by  =\Yii::$app->user->identity->id;
                        $modelsk13aPelapor->created_ip  =$_SERVER['REMOTE_ADDR'];
                        $modelsk13aPelapor->created_time=date('Y-m-d H:i:s');
                      //  print_r( $modelsk13aPelapor);
                       // print_r($model->id_sk_was_3a);
                       // print_r($modelsk13aPelapor);
                       // exit();
                        $modelsk13aPelapor->save();
                    }
//[nama_pemeriksa]
                    ...
                    for ($i=0; $i < $jmlPemeriksa; $i++) { 
                        $modelsk13aPemeriksa=new SkWas3aPemeriksa();
                        $modelsk13aPemeriksa->no_register = $_SESSION['was_register'];
                        $modelsk13aPemeriksa->id_tingkat  = $_SESSION['kode_tk'];
                        $modelsk13aPemeriksa->id_kejati   = $_SESSION['kode_kejati'];
                        $modelsk13aPemeriksa->id_kejari   = $_SESSION['kode_kejari'];
                        $modelsk13aPemeriksa->id_cabjari  = $_SESSION['kode_cabjari'];
                        $modelsk13aPemeriksa->id_sp_was2  = $model->id_sp_was2;
                        $modelsk13aPemeriksa->id_ba_was2  = $model->id_ba_was2;
                        $modelsk13aPemeriksa->id_l_was2   = $model->id_l_was2;
                        $modelsk13aPemeriksa->id_was15    = $model->id_was15;
                        $modelsk13aPemeriksa->id_sk_was_3a= $model->id_sk_was_3a;
                        $modelsk13aPemeriksa->nip_pemeriksa = $_POST['nip_pemeriksa'][$i];
                        $modelsk13aPemeriksa->nrp_pemeriksa     = $_POST['nrp_pemeriksa'][$i];
                        $modelsk13aPemeriksa->nama_pemeriksa    = $_POST['nama_pemeriksa'][$i];
                        $modelsk13aPemeriksa->pangkat_pemeriksa = $_POST['pangkat_pemeriksa'][$i];
                        $modelsk13aPemeriksa->jabatan_pemeriksa = $_POST['jabatan_pemeriksa'][$i];
                        $modelsk13aPemeriksa->golongan_pemeriksa= $_POST['golongan_pemeriksa'][$i];
                        $modelsk13aPemeriksa->created_by  =\Yii::$app->user->identity->id;
                        $modelsk13aPemeriksa->created_ip  =$_SERVER['REMOTE_ADDR'];
                        $modelsk13aPemeriksa->created_time=date('Y-m-d H:i:s');
                        // print_r($modelsk13aPemeriksa);
                        // exit();
                        $modelsk13aPemeriksa->save();
                    }

                     $pejabat = $_POST['pejabat'];
                    TembusanWas2::deleteAll(['from_table'=>'Sk-Was-3a','no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'], 'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],'pk_in_table'=>strrev($model->id_sk_was_3a),'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
                        for($z=0;$z<count($pejabat);$z++){
                            $saveTembusan = new TembusanWas2;
                            $saveTembusan->from_table = 'Sk-Was-3a';
                            $saveTembusan->pk_in_table = strrev($model->id_sk_was_3a);
                    ...
                    // if ($files != false) {
                    // $path = \Yii::$app->params['uploadPath'].'sk_was_3a/'.$files->name;
                    // $files->saveAs($path);
                    // }
                 // for($i=0;$i<count($tembusan);$i++){
                 //        $saveTembusan = new TembusanSkWas3a();
                 //        $saveTembusan->id_sk_was_3a = $model->id_sk_was_3a;
    ...
    public function actionUpdate($id,$id2)
    {
      ...
      //  $dataPemeriksa = PemeriksaSpWas2::findAll(["id_sp_was2"=>$modelSpWas2->id_sp_was2,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari']]);
       ...
       // $modelPelapor = SkWas3aPelapor::findAll(["id_sk_was_3a"=>$id,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
        // print_r($dataPemeriksa);
        // exit();
        $dataLapdu = Lapdu::findOne(['no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari']]);
      //  $modelTembusanMaster = TembusanWas::find()->where("for_tabel=:condition1 OR for_tabel=:condition2", [":condition1" => 'was_16b','condition2'=>'master'])->orderBy('id_tembusan desc')->all();
        ...
        $modelTerlapor="select a.*,b.*,c.*,a.bentuk_pelanggaran as keterangan_pelanggaran,d.* 
                        ...
                        a.id_cabjari=d.id_cabjari and 
                        a.no_register=d.no_register 
                        where b.saran_dari='Jamwas' and b.sk='SK-WAS3-A' and b.nip_terlapor='".$_GET['id2']."'";
        $dataTerlapor = $connection->createCommand($modelTerlapor)->queryOne();
        // print_r($dataTerlapor);
        // exit();
        ...
        if ($model->load(Yii::$app->request->post())){
            ...
            try {
                ...
                $model->pelanggaran = $dataTerlapor['keterangan_pelanggaran'];
            if($model->save()){
                if($OldFile!='' && file_exists($file_tmp) && file_exists(\Yii::$app->params['uploadPath'].'sk_was_3a/'.$OldFile)) {
                      unlink(\Yii::$app->params['uploadPath'].'sk_was_3a/'.$OldFile);
                  }

                $jml=count($_POST['uraian']);
                SkWas3aUraian::deleteAll(['id_sk_was_3a'=>$model->id_sk_was_3a,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
                    for ($i=0; $i < $jml; $i++) { 
                          $modelsk13aUaraian=new SkWas3aUraian();
                          $modelsk13aUaraian->no_register = $_SESSION['was_register'];
                          $modelsk13aUaraian->id_tingkat  = $_SESSION['kode_tk'];
            ...
            move_uploaded_file($file_tmp,\Yii::$app->params['uploadPath'].'sk_was_3a/'.$rename_file);
            $transaction->commit();
     ...
     public function actionCetak($id){
        ...
        // print_r($model);
        // exit();
        $modelwasUraian1=SkWas3aUraian::findAll(['id_sk_was_3a'=>$model->id_sk_was_3a,'no_register'=>$_SESSION['was_register'],
        ...
        $modelwasUraian2=SkWas3aUraian::findAll(['id_sk_was_3a'=>$model->id_sk_was_3a,'no_register'=>$_SESSION['was_register'],
                            'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                            'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
                            'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],
                            'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],
        ...
        $modelwasUraian3=SkWas3aUraian::findAll(['id_sk_was_3a'=>$model->id_sk_was_3a,'no_register'=>$_SESSION['was_register'],
                            'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                            'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
                            'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],
                            'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],
        ...
        $modelwasUraian4=SkWas3aUraian::findAll(['id_sk_was_3a'=>$model->id_sk_was_3a,'no_register'=>$_SESSION['was_register'],
                            'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                            'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
                            'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],
                            'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],
        ...
        $modelTembusan=TembusanWas2::findAll(['pk_in_table'=>$model->id_sk_was_3a,'from_table'=>'Sk-Was-3a',
                            'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],
                            'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],
        ...
        $tgl_sk      =\Yii::$app->globalfunc->ViewIndonesianFormat($model['tgl_sk_was_3a']);
      //  $tgl_diterima=\Yii::$app->globalfunc->ViewIndonesianFormat($model['tgl_diterima_sk_was_3a']);
     ...
     $id_sk_was_3a = $_POST['id'];
     $jumlah = $_POST['jml'];
     
   //  echo $id_bawas3;
        for ($i=0; $i <$jumlah ; $i++) { 
            $pecah=explode(',', $id_sk_was_3a);
           // echo $pecah[$i];
          //  $this->findModel($pecah[$i])->delete();
            SkWas3a::deleteAll(['id_wilayah'=>$_SESSION['was_id_wilayah'],
                               'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],
                               'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4'],
                               'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],
                               'id_kejati'=>$_SESSION['kode_kejati'], 'id_kejari'=>$_SESSION['kode_kejari'],
                               'id_cabjari'=>$_SESSION['kode_cabjari'],'id_sk_was_3a'=>$pecah[$i]]);
            
            TembusanWas2::deleteAll(['from_table'=>'Sk-Was-3a','no_register'=>$_SESSION['was_register'],
                                     'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                                     'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
    ...
    public function actionDelete_old() {
        ...
        for ($i = 0; $i < count($arr); $i++) {
          if (SkWas3a::updateAll(["flag" => 3], "id_sk_was_3a ='" . $arr[$i] . "'")) {
            TembusanSkWas3a::deleteAll('id_sk_was_3a=:del', [':del'=>$arr[$i]]);
            Yii::$app->getSession()->setFlash('success', [
              'type' => 'success',
              'duration' => 3000,
              'icon' => 'fa fa-users',
    ...
    /**
     * Finds the SkWas3a model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return SkWas3a the loaded model
    ...
    $lokasi = $qryLokasi->select('a.inst_lokinst')
        ...
        ->innerJoin('was.sk_was_3a b', 'a.inst_satkerkd=b.inst_satkerkd')
        ->where('length(a.inst_satkerkd)=:inst_satkerkd AND b.id_sk_was_3a=:id_sk_was_3a',
            [':inst_satkerkd' => 2, ':id_sk_was_3a' => $model->id_sk_was_3a])->one();

    $keputusan = VPejabatPimpinan::find()->where('id_jabatan_pejabat=:id_jabatan_pejabat', [':id_jabatan_pejabat' => $model->ttd_sk_was_3a])->one();
    ...
    $odf = new Odf(Yii::$app->params['reportPengawasan'] . "sk_was_3a.odt");

    $odf->setVars('nm_kejaksaan', $instansi['inst_nama']);
    $odf->setVars('keputusan', $keputusan->jabatan);
    $odf->setVars('nmr_sk_was_3a', $model->no_sk_was_3a);
    ...
    $odf->setVars('tgl_ditetapkan', GlobalFuncComponent::tglToWord($model->tgl_sk_was_3a));
    ...
    $query->select('a.jabatan')
        ...
        ->innerJoin('was.tembusan_sk_was_3a b', 'a.id_pejabat_tembusan = b.id_pejabat_tembusan')
        ->innerJoin('was.sk_was_3a c', 'b.id_sk_was_3a = c.id_sk_was_3a')
        ->where("c.id_sk_was_3a=:id_sk_was_3a", [':id_sk_was_3a' => $model->id_sk_was_3a]);
...
select c.id_terlapor,c.id_register,c.id_h_jabatan,e.peg_nama,e.peg_nip,e.peg_nip_baru,y.jml_terlapor from was.terlapor c
    ...
    inner join kepegawaian.kp_pegawai e on (c.peg_nik=e.peg_nik)
        ...
        $odf->exportAsAttachedFile("SK.WAS-3A ".$dugaan['terlapor'].'.odt');   
   }

public function actionGetttd(){
       
   $searchModelskwas3a = new SkWas3aSearch();
   $dataProviderPenandatangan = $searchModelskwas3a->searchPenandatangan(Yii::$app->request->queryParams);
   ...
   echo GridView::widget([
    ...
    {
        if (($model = SkWas3a::findOne(['id_sk_was_3a'=>$id,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']])) !== null) {
            return $model;
        } else {
