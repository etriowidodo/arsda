...
use Yii;
use app\modules\pengawasan\models\SkWas2c;
use app\modules\pengawasan\models\SkWas2cPelapor;
use app\modules\pengawasan\models\SkWas2cPemeriksa;
use app\modules\pengawasan\models\SkWas2cUraian;
use app\modules\pengawasan\models\SkWas2cSearch;
...
use app\modules\pengawasan\models\TembusanSkWas2c;
...
/**
 * SkWas2cController implements the CRUD actions for SkWas2c model.
 */
class SkWas2cController extends Controller
{
    ...
    /**
     * Displays a single SkWas2c model.
     * @param string $id
     * @return mixed
     */
    public function actionView($id)
    {
     ...
     public function actionViewpdf($id,$id2){
        $file_upload=$this->findModel($id,$id2);
          ...
          $filepath = '../modules/pengawasan/upload_file/sk_was_2c/'.$file_upload['upload_file'];
    ...
    {
        $model = new SkWas2c();
        $modelPemeriksa = new SkWas2cPemeriksa();
        $modelPelapor = new SkWas2cPelapor();
        ...
        $modelTembusanMaster = TembusanWas::find()->where("for_tabel=:condition1 OR for_tabel=:condition2", [":condition1" => 'sk_was_2c','condition2'=>'master'])->orderBy('id_tembusan desc')->all();
        // print_r($dataLapdu);
        // exit();
        $queryp = new Query;
        ...
        $modelTerlapor="select a.*,b.*,c.*,a.bentuk_pelanggaran as keterangan_pelanggaran,d.* 
                        ...
                        where b.saran_dari='Jamwas' and b.sk='SK-WAS2-C' 
        ...
        if ($model->load(Yii::$app->request->post())){
          ...
          // exit();
            ...
            // $model->id_terlapor = $_POST['SkWas2c']['id_terlapor']; 
            // $model->ttd_sk_was_2c = $_POST['SkWas2c']['ttd_sk_was_2c'];
            // $model->no_sk_was_2c = "KEP-".$_POST['no_sk_was_2c'];
          ...
          //  try {
             ...
             // exit();
                   $jml=count($_POST['uraian']);
                    for ($i=0; $i < $jml; $i++) { 
                        $modelsk12cUaraian=new SkWas2cUraian();
                        $modelsk12cUaraian->no_register = $_SESSION['was_register'];
                        $modelsk12cUaraian->id_tingkat  = $_SESSION['kode_tk'];
                        $modelsk12cUaraian->id_kejati   = $_SESSION['kode_kejati'];
                        $modelsk12cUaraian->id_kejari   = $_SESSION['kode_kejari'];
                        $modelsk12cUaraian->id_cabjari  = $_SESSION['kode_cabjari'];
                        $modelsk12cUaraian->id_sp_was2  = $model->id_sp_was2;
                        $modelsk12cUaraian->id_ba_was2  = $model->id_ba_was2;
                        $modelsk12cUaraian->id_l_was2   = $model->id_l_was2;
                        $modelsk12cUaraian->id_was15    = $model->id_was15;
                        $modelsk12cUaraian->id_sk_was_2c= $model->id_sk_was_2c;
                        $modelsk12cUaraian->isi_uraian  = $_POST['uraian'][$i];
                        $modelsk12cUaraian->created_by  =\Yii::$app->user->identity->id;
                        $modelsk12cUaraian->created_ip  =$_SERVER['REMOTE_ADDR'];
                        $modelsk12cUaraian->created_time=date('Y-m-d H:i:s');
                      //  print_r( $modelsk12cUaraian);
                       // print_r($model->id_sk_was_2c);
                        // print_r($jml);
                        //  exit();
                        $modelsk12cUaraian->save();
                    }
//SkWas2cPelapor[nama_pelapor]
                    $jmlPelapor=count($_POST['nama_pelapor']);

                    for ($i=0; $i < $jmlPelapor; $i++) { 
                        $modelsk12cPelapor=new SkWas2cPelapor();
                        $modelsk12cPelapor->no_register = $_SESSION['was_register'];
                        $modelsk12cPelapor->id_tingkat  = $_SESSION['kode_tk'];
                        $modelsk12cPelapor->id_kejati   = $_SESSION['kode_kejati'];
                        $modelsk12cPelapor->id_kejari   = $_SESSION['kode_kejari'];
                        $modelsk12cPelapor->id_cabjari  = $_SESSION['kode_cabjari'];
                        $modelsk12cPelapor->id_sp_was2  = $model->id_sp_was2;
                        $modelsk12cPelapor->id_ba_was2  = $model->id_ba_was2;
                        $modelsk12cPelapor->id_l_was2   = $model->id_l_was2;
                        $modelsk12cPelapor->id_was15    = $model->id_was15;
                        $modelsk12cPelapor->id_sk_was_2c= $model->id_sk_was_2c;
                        $modelsk12cPelapor->nama_pelapor= $_POST['nama_pelapor'][$i];
                        $modelsk12cPelapor->alamat_pelapor    = $_POST['alamat_pelapor'][$i];
                        $modelsk12cPelapor->email_pelapor     = $_POST['email_pelapor'][$i];
                        $modelsk12cPelapor->telp_pelapor      = $_POST['telp_pelapor'][$i];
                        $modelsk12cPelapor->pekerjaan_pelapor = $_POST['pekerjaan_pelapor'][$i];
                        $modelsk12cPelapor->sumber_lainnya    = $_POST['sumber_lainnya'][$i];
                        $modelsk12cPelapor->id_sumber_laporan = $_POST['id_sumber_laporan'][$i];
                        $modelsk12cPelapor->tempat_lahir_pelapor   = $_POST['tempat_lahir_pelapor'][$i];
                        $modelsk12cPelapor->tanggal_lahir_pelapor  = $_POST['tanggal_lahir_pelapor'][$i];
                        $modelsk12cPelapor->kewarganegaraan_pelapor= $_POST['kewarganegaraan_pelapor'][$i];
                        $modelsk12cPelapor->agama_pelapor          = $_POST['agama_pelapor'][$i];
                        $modelsk12cPelapor->pendidikan_pelapor     = $_POST['pendidikan_pelapor'][$i];
                        $modelsk12cPelapor->nama_kota_pelapor      = $_POST['nama_kota_pelapor'][$i];
                        $modelsk12cPelapor->created_by  =\Yii::$app->user->identity->id;
                        $modelsk12cPelapor->created_ip  =$_SERVER['REMOTE_ADDR'];
                        $modelsk12cPelapor->created_time=date('Y-m-d H:i:s');
                      //  print_r( $modelsk12cPelapor);
                       // print_r($model->id_sk_was_2c);
                       // print_r($modelsk12cPelapor);
                       // exit();
                        $modelsk12cPelapor->save();
                    }
//[nama_pemeriksa]
                    ...
                    for ($i=0; $i < $jmlPemeriksa; $i++) { 
                        $modelsk12cPemeriksa=new SkWas2cPemeriksa();
                        $modelsk12cPemeriksa->no_register = $_SESSION['was_register'];
                        $modelsk12cPemeriksa->id_tingkat  = $_SESSION['kode_tk'];
                        $modelsk12cPemeriksa->id_kejati   = $_SESSION['kode_kejati'];
                        $modelsk12cPemeriksa->id_kejari   = $_SESSION['kode_kejari'];
                        $modelsk12cPemeriksa->id_cabjari  = $_SESSION['kode_cabjari'];
                        $modelsk12cPemeriksa->id_sp_was2  = $model->id_sp_was2;
                        $modelsk12cPemeriksa->id_ba_was2  = $model->id_ba_was2;
                        $modelsk12cPemeriksa->id_l_was2   = $model->id_l_was2;
                        $modelsk12cPemeriksa->id_was15    = $model->id_was15;
                        $modelsk12cPemeriksa->id_sk_was_2c= $model->id_sk_was_2c;
                        $modelsk12cPemeriksa->nip_pemeriksa = $_POST['nip_pemeriksa'][$i];
                        $modelsk12cPemeriksa->nrp_pemeriksa     = $_POST['nrp_pemeriksa'][$i];
                        $modelsk12cPemeriksa->nama_pemeriksa    = $_POST['nama_pemeriksa'][$i];
                        $modelsk12cPemeriksa->pangkat_pemeriksa = $_POST['pangkat_pemeriksa'][$i];
                        $modelsk12cPemeriksa->jabatan_pemeriksa = $_POST['jabatan_pemeriksa'][$i];
                        $modelsk12cPemeriksa->golongan_pemeriksa= $_POST['golongan_pemeriksa'][$i];
                        $modelsk12cPemeriksa->created_by  =\Yii::$app->user->identity->id;
                        $modelsk12cPemeriksa->created_ip  =$_SERVER['REMOTE_ADDR'];
                        $modelsk12cPemeriksa->created_time=date('Y-m-d H:i:s');
                        // print_r($modelsk12cPemeriksa);
                        // exit();
                        $modelsk12cPemeriksa->save();
                    }

                     $pejabat = $_POST['pejabat'];
                    TembusanWas2::deleteAll(['from_table'=>'Sk-Was-2c','no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'], 'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],'pk_in_table'=>strrev($model->id_sk_was_2c),'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
                        for($z=0;$z<count($pejabat);$z++){
                            $saveTembusan = new TembusanWas2;
                            $saveTembusan->from_table = 'Sk-Was-2c';
                            $saveTembusan->pk_in_table = strrev($model->id_sk_was_2c);
                    ...
                    // if ($files != false) {
                    // $path = \Yii::$app->params['uploadPath'].'sk_was_2c/'.$files->name;
                    // $files->saveAs($path);
                    // }
                 // for($i=0;$i<count($tembusan);$i++){
                 //        $saveTembusan = new TembusanSkWas2c();
                 //        $saveTembusan->id_sk_was_2c = $model->id_sk_was_2c;
    ...
    public function actionUpdate($id,$id2)
    {
        ...
        $modelTembusan=TembusanWas2::find()->where(['pk_in_table'=>$model->id_sk_was_2c,'from_table'=>'Sk-Was-2c','no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']])->orderBy('is_order desc')->all();
        $modelSpWas2 = SpWas2::findOne(['no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4'],'trx_akhir'=>1]);
      //  $dataPemeriksa = PemeriksaSpWas2::findAll(["id_sp_was2"=>$modelSpWas2->id_sp_was2,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari']]);
        $dataPemeriksa = was10Inspeksi::findOne(["id_sp_was2"=>$modelSpWas2->id_sp_was2,
          ...
          'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari'],
          'nip_pegawai_terlapor'=>$_GET['id'],'trx_akhir'=>'1']);
        $modelPemeriksa = SkWas2cPemeriksa::findAll(["id_sk_was_2c"=>$id,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
        $modelUraian = SkWas2cUraian::findAll(["id_sk_was_2c"=>$id,
                                              'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],
                                              'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],
                                              'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari'],
       ...
       // $modelPelapor = SkWas2cPelapor::findAll(["id_sk_was_2c"=>$id,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
        // print_r($dataPemeriksa);
        // exit();
        $dataLapdu = Lapdu::findOne(['no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_kejati'=>$_SESSION['kode_kejati'],'id_cabjari'=>$_SESSION['kode_cabjari']]);
      //  $modelTembusanMaster = TembusanWas::find()->where("for_tabel=:condition1 OR for_tabel=:condition2", [":condition1" => 'was_16b','condition2'=>'master'])->orderBy('id_tembusan desc')->all();
        ...
        $modelTerlapor="select a.*,b.*,c.*,a.bentuk_pelanggaran as keterangan_pelanggaran,d.* 
                        ...
                        a.id_cabjari=d.id_cabjari and 
                        a.no_register=d.no_register 
                        where b.saran_dari='Jamwas' and b.sk='SK-WAS2-C' and b.nip_terlapor='".$_GET['id2']."'";
        $dataTerlapor = $connection->createCommand($modelTerlapor)->queryOne();
        // print_r($dataTerlapor);
        // exit();
        ...
        if ($model->load(Yii::$app->request->post())){
            ...
            try {
                ...
                $model->pelanggaran = $dataTerlapor['keterangan_pelanggaran'];
            if($model->save()){
                if($OldFile!='' && file_exists($file_tmp) && file_exists(\Yii::$app->params['uploadPath'].'sk_was_2c/'.$OldFile)) {
                      unlink(\Yii::$app->params['uploadPath'].'sk_was_2c/'.$OldFile);
                  }

                $jml=count($_POST['uraian']);
                SkWas2cUraian::deleteAll(['id_sk_was_2c'=>$model->id_sk_was_2c,'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4']]);
                    for ($i=0; $i < $jml; $i++) { 
                          $modelsk12cUaraian=new SkWas2cUraian();
                          $modelsk12cUaraian->no_register = $_SESSION['was_register'];
                          $modelsk12cUaraian->id_tingkat  = $_SESSION['kode_tk'];
            ...
            move_uploaded_file($file_tmp,\Yii::$app->params['uploadPath'].'sk_was_2c/'.$rename_file);
            $transaction->commit();
     ...
     public function actionCetak($id){
        ...
        // print_r($model);
        // exit();
        $modelwasUraian1=SkWas2cUraian::findAll(['id_sk_was_2c'=>$model->id_sk_was_2c,'no_register'=>$_SESSION['was_register'],
        ...
        $modelwasUraian2=SkWas2cUraian::findAll(['id_sk_was_2c'=>$model->id_sk_was_2c,'no_register'=>$_SESSION['was_register'],
                            'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                            'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
                            'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],
                            'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],
        ...
        $modelwasUraian3=SkWas2cUraian::findAll(['id_sk_was_2c'=>$model->id_sk_was_2c,'no_register'=>$_SESSION['was_register'],
                            'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                            'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
                            'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],
                            'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],
        ...
        $modelwasUraian4=SkWas2cUraian::findAll(['id_sk_was_2c'=>$model->id_sk_was_2c,'no_register'=>$_SESSION['was_register'],
                            'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                            'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
                            'id_wilayah'=>$_SESSION['was_id_wilayah'],'id_level1'=>$_SESSION['was_id_level1'],
                            'id_level2'=>$_SESSION['was_id_level2'],'id_level3'=>$_SESSION['was_id_level3'],
        ...
        $modelTembusan=TembusanWas2::findAll(['pk_in_table'=>$model->id_sk_was_2c,'from_table'=>'Sk-Was-2c',
                            'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],
                            'id_kejati'=>$_SESSION['kode_kejati'],'id_kejari'=>$_SESSION['kode_kejari'],
        ...
        $tgl_sk      =\Yii::$app->globalfunc->ViewIndonesianFormat($model['tgl_sk_was_2c']);
    //    $tgl_diterima=\Yii::$app->globalfunc->ViewIndonesianFormat($model['tgl_diterima_sk_was_2c']);
   ...
   public function actionDelete() {
     $id_sk_was_2c = $_POST['id'];
     $jumlah = $_POST['jml'];
     
   //  echo $id_bawas3;
        for ($i=0; $i <$jumlah ; $i++) { 
            $pecah=explode(',', $id_sk_was_2c);
           // echo $pecah[$i];
          //  $this->findModel($pecah[$i])->delete();
            SkWas2c::deleteAll(['id_wilayah'=>$_SESSION['was_id_wilayah'],
                               'id_level1'=>$_SESSION['was_id_level1'],'id_level2'=>$_SESSION['was_id_level2'],
                               'id_level3'=>$_SESSION['was_id_level3'],'id_level4'=>$_SESSION['was_id_level4'],
                               'no_register'=>$_SESSION['was_register'],'id_tingkat'=>$_SESSION['kode_tk'],
                               'id_kejati'=>$_SESSION['kode_kejati'], 'id_kejari'=>$_SESSION['kode_kejari'],
                               'id_cabjari'=>$_SESSION['kode_cabjari'],'id_sk_was_2c'=>$pecah[$i]]);
            
            TembusanWas2::deleteAll(['from_table'=>'Sk-Was-2c','no_register'=>$_SESSION['was_register'],
                                     'id_tingkat'=>$_SESSION['kode_tk'],'id_kejati'=>$_SESSION['kode_kejati'],
                                     'id_kejari'=>$_SESSION['kode_kejari'],'id_cabjari'=>$_SESSION['kode_cabjari'],
    ...
    public function actionDelete_old() {
        ...
        for ($i = 0; $i < count($arr); $i++) {
          if (SkWas2c::updateAll(["flag" => 3], "id_sk_was_2c ='" . $arr[$i] . "'")) {
            TembusanSkWas2c::deleteAll('id_sk_was_2c=:del', [':del'=>$arr[$i]]);
            Yii::$app->getSession()->setFlash('success', [
              'type' => 'success',
              'duration' => 3000,
              'icon' => 'fa fa-users',
    ...
    /**
     * Finds the SkWas2c model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return SkWas2c the loaded model
    ...
    $lokasi = $qryLokasi->select('a.inst_lokinst')
        ...
        ->innerJoin('was.sk_was_2c b', 'a.inst_satkerkd=b.inst_satkerkd')
        ->where('length(a.inst_satkerkd)=:inst_satkerkd AND b.id_sk_was_2c=:id_sk_was_2c',
            [':inst_satkerkd' => 2, ':id_sk_was_2c' => $model->id_sk_was_2c])->one();

    $keputusan = VPejabatPimpinan::find()->where('id_jabatan_pejabat=:id_jabatan_pejabat', [':id_jabatan_pejabat' => $model->ttd_sk_was_2c])->one();
    ...
    $odf = new Odf(Yii::$app->params['reportPengawasan'] . "sk_was_2c.odt");

    $odf->setVars('nm_kejaksaan', $instansi['inst_nama']);
    $odf->setVars('keputusan', $keputusan->jabatan);
    $odf->setVars('nmr_sk_was_2c', $model->no_sk_was_2c);
    ...
    $odf->setVars('tgl_ditetapkan', GlobalFuncComponent::tglToWord($model->tgl_sk_was_2c));
    ...
    $query->select('a.jabatan')
        ...
        ->innerJoin('was.tembusan_sk_was_2c b', 'a.id_pejabat_tembusan = b.id_pejabat_tembusan')
        ->innerJoin('was.sk_was_2c c', 'b.id_sk_was_2c = c.id_sk_was_2c')
        ->where("c.id_sk_was_2c=:id_sk_was_2c", [':id_sk_was_2c' => $model->id_sk_was_2c]);
...
select c.id_terlapor,c.id_register,c.id_h_jabatan,e.peg_nama,e.peg_nip,e.peg_nip_baru,y.jml_terlapor from was.terlapor c
    ...
    inner join kepegawaian.kp_pegawai e on (c.peg_nik=e.peg_nik)
        ...
        $odf->exportAsAttachedFile("SK.WAS-2C ".$dugaan['terlapor'].'.odt');   
   }

public function actionGetttd(){
       
   $searchModelskwas2c = new SkWas2cSearch();
   $dataProviderPenandatangan = $searchModelskwas2c->searchPenandatangan(Yii::$app->request->queryParams);
