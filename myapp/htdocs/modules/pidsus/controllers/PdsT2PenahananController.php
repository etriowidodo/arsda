...
class PdsT2PenahananController extends Controller{
    public function actionIndex(){
        $searchModel = new PdsT2Penahanan;
        $dataProvider = $searchModel->searchPer(Yii::$app->request->get());
        return $this->render('index', ['dataProvider' => $dataProvider]);
    }

    public function actionCreate(){
        $model  = new PdsT2Penahanan;	
    ...
    public function actionUpdate($nop16){
		...
		$model 	= PdsT2Penahanan::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
