...
class PdsP10UmumController extends Controller{
    public function actionIndex(){
        unset($_SESSION['pidsus_no_p10_umum']);
		$searchModel = new PdsP10Umum;
        $dataProvider = $searchModel->searchPer(Yii::$app->request->get());
        return $this->render('index', ['dataProvider' => $dataProvider]);
    }
    
    public function actionCreate(){
        $model  = new PdsP10Umum;	
		return $this->render('create', ['model'=>$model]);
    }
	
    public function actionCekpdsp10umum(){
		if (Yii::$app->request->isAjax) {
			$model = new PdsP10Umum;
			$nilai = $model->cekPdsP10Umum(Yii::$app->request->post());
    ...
    public function actionUpdate($id1){
        ...
        $no_p10_umum = rawurldecode($id1);
		$_SESSION['pidsus_no_p10_umum'] = $no_p10_umum;
		...
		$sqlnya = "
			...
			where ".$whereDef." and a.no_p10_umum = '".$no_p10_umum."'";
		$model 	= PdsP10Umum::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
	...
	public function actionSimpan(){
		...
		if($param['isNewRecord']){
			...
			if($sukses){
				...
				return $this->redirect(['update', 'id1' => rawurlencode($_SESSION['pidsus_no_p10_umum'])]);
			}
		}
    }
