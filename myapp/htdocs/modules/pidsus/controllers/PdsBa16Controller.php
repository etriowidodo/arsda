...
class PdsBa16Controller extends Controller{
    public function actionIndex(){
        $searchModel = new PdsBa16;
    ...
    public function actionGetsita(){
        if (Yii::$app->request->isAjax) {
            $searchModel = new PdsBa16;
            $dataProvider = $searchModel->searchPer(Yii::$app->request->get());
            return $this->renderAjax('_tambah_sita',['dataProvider'=>$dataProvider]);
        }
    }
    ...
    public function actionUpdate($nop16){
		...
		$model 	= PdsBa16::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
    ...
    public function actionCekp16(){
		if (Yii::$app->request->isAjax) {
			$model = new PdsBa16;
			$nilai = $model->cekPdsBa16(Yii::$app->request->post());
