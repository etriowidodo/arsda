...
class PdsP21aController extends Controller{
    public function actionIndex(){
        if(!$_SESSION["no_berkas"]){
			$searchModel = new PdsP21a;
			$dataProvider = $searchModel->searchPer(Yii::$app->request->get());
			return $this->render('index', ['dataProvider' => $dataProvider]);
		} else{
			Yii::$app->response->redirect(['pidsus/pds-p21a/create', 'id1'=>$_SESSION["no_berkas"], 'id2'=>""]);
		}
    }
    ...
    public function actionCreate($id1, $id2){
		...
		if(!$_SESSION["no_berkas"]){
			$sqlnya = "
				select a.id_kejati, a.id_kejari, a.id_cabjari, a.no_spdp, a.tgl_spdp, a.no_berkas, a.tgl_berkas, b.no_pengantar, b.tgl_terima as tgl_terima, f.no_surat as no_p21, 
				f.tgl_dikeluarkan as tgl_p21, g.no_pengantar as id_p21a, g.no_surat, g.sifat, g.lampiran, g.tgl_dikeluarkan, g.dikeluarkan, g.kepada, g.di_kepada, 
				g.penandatangan_nama, g.penandatangan_nip, g.penandatangan_jabatan_pejabat, g.penandatangan_gol, g.penandatangan_pangkat, g.penandatangan_status_ttd, 
				g.penandatangan_jabatan_ttd, g.file_upload_p21a    
		...
		$model 	= PdsP21a::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
	
    public function actionCekp21a(){
		if (Yii::$app->request->isAjax) {
			$model = new PdsP21a;
			$nilai = $model->cekP21a(Yii::$app->request->post());
