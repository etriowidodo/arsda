...
class PdsP22Controller extends Controller{
    public function actionIndex(){
        if(!$_SESSION["no_berkas"]){
			$searchModel = new PdsP22;
			$dataProvider = $searchModel->searchPer(Yii::$app->request->get());
			return $this->render('index', ['dataProvider' => $dataProvider]);
		} else{
			Yii::$app->response->redirect(['pidsus/pds-p22/create', 'id1'=>$_SESSION["no_berkas"]]);
		}
    }
    ...
    public function actionCreate($id1){
		...
		$sqlnya = "
			...
			i.no_pengantar as id_p22, i.no_surat, i.sifat, i.lampiran, i.tgl_dikeluarkan, i.dikeluarkan, i.kepada, i.di_kepada, i.penandatangan_nama, i.penandatangan_nip, 
			i.penandatangan_jabatan_pejabat, i.penandatangan_gol, i.penandatangan_pangkat, i.penandatangan_status_ttd, i.penandatangan_jabatan_ttd, i.file_upload_p22, 
			...
			left join pidsus.pds_p22 i on f.id_kejati = i.id_kejati and f.id_kejari = i.id_kejari and f.id_cabjari = i.id_cabjari 
		...
		$model 	= PdsP22::findBySql($sqlnya)->asArray()->one();
		return $this->render('create', ['model' => $model]);
    }

    public function actionCekp22(){
		if (Yii::$app->request->isAjax) {
			$model = new PdsP22;
			$nilai = $model->cekP22(Yii::$app->request->post());
    ...
    public function actionHapus(){
		...
		if($hasil){
			...
			Yii::$app->session->setFlash('success', ['type'=>'danger', 'message'=>'Maaf, data tidak dapat dihapus, jika ada data P-23']);
    ...
    public function actionCetak($id1){
        ...
        $sqlnya = "
                ...
                i.no_pengantar as id_p22, i.no_surat, i.sifat, i.lampiran, i.tgl_dikeluarkan, i.dikeluarkan, i.kepada, i.di_kepada, i.penandatangan_nama, i.penandatangan_nip, 
                i.penandatangan_jabatan_pejabat, i.penandatangan_gol, i.penandatangan_pangkat, i.penandatangan_status_ttd, i.penandatangan_jabatan_ttd, i.file_upload_p22, 
                h.nama as instansi_penyidik, j.no_surat as no_surat_p19, j.tgl_dikeluarkan tgl_p19
                from pidsus.pds_terima_berkas a
                join pidsus.pds_spdp g on a.id_kejati = g.id_kejati and a.id_kejari = g.id_kejari and a.id_cabjari = g.id_cabjari 
                        and a.no_spdp = g.no_spdp and a.tgl_spdp = g.tgl_spdp 
                join pidsus.ms_inst_penyidik h on g.id_asalsurat = h.kode_ip 
                ...
                left join pidsus.pds_p22 i on f.id_kejati = i.id_kejati and f.id_kejari = i.id_kejari and f.id_cabjari = i.id_cabjari 
                        and f.no_spdp = i.no_spdp and f.tgl_spdp = i.tgl_spdp and f.no_berkas = i.no_berkas and f.no_pengantar = i.no_pengantar 
                left join pidsus.pds_p19 j on f.id_kejati = j.id_kejati and f.id_kejari = j.id_kejari and f.id_cabjari = j.id_cabjari 
                        and f.no_spdp = j.no_spdp and f.tgl_spdp = j.tgl_spdp and f.no_berkas = j.no_berkas and f.no_pengantar = j.no_pengantar
                where ".$where." and a.no_berkas = '".$id1."'";
        $model 	= PdsP22::findBySql($sqlnya)->asArray()->one();
