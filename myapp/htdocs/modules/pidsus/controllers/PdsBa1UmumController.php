...
class PdsBa1UmumController extends Controller{
    public function actionIndex(){
        $searchModel = new PdsBa1Umum;
        $dataProvider = $searchModel->searchPer(Yii::$app->request->get());
        return $this->render('index', ['dataProvider' => $dataProvider]);
    }
    
    public function actionCreate(){
        $model  = new PdsBa1Umum;	
    ...
    public function actionUpdate($id1){
		...
		where ".$where." and a.no_ba1_umum = '".$idnya."'";
		$model 	= PdsBa1Umum::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
