...
class PdsBa2KhususController extends Controller{
    public function actionIndex(){
        $searchModel = new PdsBa2Khusus;
        $dataProvider = $searchModel->searchPer(Yii::$app->request->get());
        return $this->render('index', ['dataProvider' => $dataProvider]);
    }
    
    public function actionCreate(){
        $model  = new PdsBa2Khusus;	
    ...
    public function actionGetba1khusus(){
        if (Yii::$app->request->isAjax){
			$searchModel = new PdsBa2Khusus;
			$dataProvider = $searchModel->searchBa1Khusus(Yii::$app->request->get());
			return $this->renderAjax('_getba1khusus', ['dataProvider' => $dataProvider]);
        }
    }
    ...
    public function actionUpdate($id1){
		...
		$sqlnya = "
		select a.*, b.tgl_p8_khusus, c.nama as kebangsaan, d.nama as pendidikan, e.nama as agama 
		from pidsus.pds_ba2_khusus a 
		...
		where ".$where." and a.no_ba2_khusus = '".$idnya."'";
		$model 	= PdsBa2Khusus::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
	...
	public function actionSimpan(){
		...
		if($param['isNewRecord']){
			...
			if($sukses){
				...
				return $this->redirect(['update', 'id1' => $param['no_ba2_khusus']]);
			}
		}
    }
