...
use app\modules\pidsus\models\PdsPidsus6;

class PdsPidsus6Controller extends Controller{
    public function actionIndex(){
        $searchModel = new PdsPidsus6;
    ...
    public function actionGettsk(){
        if (Yii::$app->request->isAjax) {
            return $this->renderAjax('_tambah_tsk');
        }
    }
    

    public function actionCreate(){
        $model  = new PdsPidsus6;	
    ...
    public function actionUpdate($nop16){
		...
		$model 	= PdsPidsus6::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
    ...
    public function actionCekp16(){
		if (Yii::$app->request->isAjax) {
			$model = new PdsPidsus6;
			$nilai = $model->cekPdsPidsus6(Yii::$app->request->post());
