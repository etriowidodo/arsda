...
class PdsBa3Controller extends Controller{
    public function actionIndex(){
        $searchModel = new PdsBa3;
    ...
    public function actionGetsaksi(){
        if (Yii::$app->request->isAjax) {
            return $this->renderAjax('_tambah_saksi');
        }
    }
    ...
    public function actionUpdate($nop16){
		...
		$model 	= PdsBa3::findBySql($sqlnya)->asArray()->one();
        return $this->render('create', ['model' => $model]);
    }
    ...
    public function actionCekp16(){
		if (Yii::$app->request->isAjax) {
			$model = new PdsBa3;
			$nilai = $model->cekPdsBa3(Yii::$app->request->post());
