...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_p10_umum  = new Date(tgl_auto($("#tgl_p10_umum").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_umum > tgl_p10_umum){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p10_umum").html('<i style="color:#dd4b39; font-size:12px;">Tanggal P-10 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_p10_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_p10_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p10_umum").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal P-10 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_p10_umum"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p10-umum/cekpdsp10umum'; ?>',
