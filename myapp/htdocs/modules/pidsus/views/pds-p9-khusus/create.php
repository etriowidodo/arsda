...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_p9_khusus  = new Date(tgl_auto($("#tgl_p9_khusus").val()));
		var tgl_pemanggilan = new Date(tgl_auto($("#tgl_pemanggilan").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_khusus > tgl_p9_khusus){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p9_khusus").html('<i style="color:#dd4b39; font-size:12px;">Tanggal P-9 Khusus lebih kecil dari tanggal P-8 Khusus</i>');
			setErrorFocus($("#error_custom_tgl_p9_khusus"), $("#role-form"), false);
			return false;
		} else if(tgl_p9_khusus > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p9_khusus").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal P-9 Khusus adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_p9_khusus"), $("#role-form"), false);
			return false;
		} else if(tgl_p9_khusus > tgl_pemanggilan){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pemanggilan").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pemanggilan lebih kecil dari tanggal P-9 Khusus</i>');
			setErrorFocus($("#error_custom_tgl_pemanggilan"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p9-khusus/cekpdsp9khusus'; ?>',
