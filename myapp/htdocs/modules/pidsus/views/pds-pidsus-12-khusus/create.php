...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_pidsus12_khusus = new Date(tgl_auto($("#tgl_pidsus12_khusus").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_khusus > tgl_pidsus12_khusus){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus12_khusus").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus 12 Khusus lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_pidsus12_khusus"), $("#role-form"), false);
			return false;
		} else if(tgl_pidsus12_khusus > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus12_khusus").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus 12 Khusus adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_pidsus12_khusus"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-12-khusus/cekpidsus12khusus'; ?>',
