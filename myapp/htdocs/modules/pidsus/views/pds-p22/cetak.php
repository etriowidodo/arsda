<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/pidsus/template/P-22.docx');
	...
	$tanggalP19     = date('d-m-Y',strtotime($model['tgl_p19']));
	...
	$res = PdsP22::findBySql($sql)->asArray()->all();
	...
	$sql2 = "select a.no_urut as no_urut, a.tembusan as tembusan from pidsus.pds_p22_tembusan a
		where ".$whereDefault." and a.no_berkas = '".$model['no_berkas']."' order by a.no_urut";
	$res2 = PdsP22::findBySql($sql2)->asArray()->all();
	if(count($res2) > 0){
		$nom2 = 0;
	...
	$docx->replaceVariableByText(array('di_tempat'=>$model['di_kepada']), array('parseLineBreaks'=>true));
        ...
        $docx->replaceVariableByText(array('nomorP19'=>$model['no_surat_p19']), array('parseLineBreaks'=>true));
        $docx->replaceVariableByText(array('nomorP19'=>$model['no_surat_p19']), array('parseLineBreaks'=>true));
        $docx->replaceVariableByText(array('tanggalP19'=>tgl_indo($tanggalP19)), array('parseLineBreaks'=>true));
        $docx->replaceVariableByText(array('tglterimaberkasMax'=>tgl_indo($tgl_terima)), array('parseLineBreaks'=>true));
	...
	$docx->createDocx('template/pidsus/P-22');
	$file = 'template/pidsus/P-22.docx';
	if (file_exists($file)) {
