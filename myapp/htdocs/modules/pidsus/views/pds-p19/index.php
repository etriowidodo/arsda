...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-p19/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_berkas'])."#".rawurlencode($data['no_p19']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_berkas'].'</p>'.$data['tgl_berkas'];
					}, 
				],
				'2'=>[
					'headerOptions'=>['style'=>'width:30%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal P-18', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_p18'].'</p>'.$data['tgl_p18'];
					}, 
				],
				'3'=>[
					'headerOptions'=>['style'=>'width:27%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal P-19', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_p19'].'</p>'.$data['tgl_p19'];
					}, 
				],
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p19/update?id1="+tm[0]+"&id2="+tm[1];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p19/cetak?id1="+tm[0]+"&id2="+tm[1];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p19/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
