<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/pidsus/template/P-19.docx');
	...
	$tanggalP18     = date('d-m-Y',strtotime($model['tgl_p18']));
	...
	$res = PdsP19::findBySql($sql)->asArray()->all();
	...
	$sql2 = "select no_urut as no_urut, tembusan from pidsus.pds_p19_tembusan a
		where ".$whereDefault." and a.no_berkas = '".$model['no_berkas']."' and a.no_surat = '".$model['no_surat']."' order by a.no_urut";
	$res2 = PdsP19::findBySql($sql2)->asArray()->all();
	if(count($res2) > 0){
		$nom2 = 0;
	...
	$docx->replaceVariableByText(array('tanggal_surat'=>tgl_indo($tgl_surat)), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('tersangka_lampiran'=>$nama_tersangka), array('parseLineBreaks'=>true));
        ...
        $docx->replaceVariableByText(array('nomorP18'=>$model['no_p18']), array('parseLineBreaks'=>true));
        $docx->replaceVariableByText(array('tanggalP18'=>tgl_indo($tanggalP18)), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('tersangka_lampiran'=>$nama_tersangka), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('nomorpengirimanberkas'=>$model['no_berkas']), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('tanggalpengirimanberkas'=>tgl_indo($tgl_berkas)), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('tanggalterimapengirimanberkas'=>tgl_indo($tgl_terima)), array('parseLineBreaks'=>true));
	$docx->replaceVariableByHTML('petunjuk', 'block', $model['petunjuk'], $arrDocnya);
	...
	$docx->createDocx('template/pidsus/P-19');
	$file = 'template/pidsus/P-19.docx';
	if (file_exists($file)) {
