<?php
	...
	$this->title = 'Jabatan (Tandatangan)';
?>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/ttd-jabatan/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				//$idnya = rawurlencode($data['$id_kejati']).'|#|'.rawurlencode($data['$id_kejari']).'|#|'.rawurlencode($data['$id_cabjari']).'|#|'.rawurlencode($data['kode']);
				$idnya = rawurlencode($data['kode']);
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/ttd-jabatan/update?kode="+tm[0];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url	: "<?php echo Yii::$app->request->baseUrl.'/pidsus/ttd-jabatan/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
