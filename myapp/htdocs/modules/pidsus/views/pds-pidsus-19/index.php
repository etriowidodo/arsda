				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn" name="btnSearch" id="btnSearch"><i class="fa fa-search jarak-kanan"></i>Cari</button>
                                                <a href="/pidsus/pds-pidsus-19/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-pidsus-19/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
...
//				$idnya = rawurlencode($data['no_p8_umum']);
...
//					'value'=>function($data){
//						return '<p style="margin-bottom:5px;">'.$data['no_p8_umum'].'</p>'.date("d-m-Y", strtotime($data['tgl_p8_umum']));
//					}, 
//				],
//				'2'=>[
//					'headerOptions'=>['style'=>'width:30%', 'class'=>'text-center'],
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-pidsus-19/update?no_p8_umum="+id;
			$(location).attr("href", url);
		});
                
                $("#idCetak").on("click", function(){
			var id 	= $(".selection_one:checked").val();
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-pidsus-19/cetak?no_p8_umum="+id;
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-19/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
