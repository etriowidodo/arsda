<?php
	...
	$linkBatal		= '/pidsus/pds-p11-umum/index';
	$linkCetak		= '/pidsus/pds-p11-umum/cetak?id1='. rawurlencode($model['no_p11_umum']);
	if($isNewRecord){
		$sqlCek = "select no_p8_umum, tgl_p8_umum from pidsus.pds_p8_umum where ".$whereDefault;
		$model 	= PdsP11Umum::findBySql($sqlCek)->asArray()->one();
	}
...
?>
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-p11-umum/simpan" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Nomor P-11 Umum</label>
                    <div class="col-md-8">
                        <input type="text" name="no_p11_umum" id="no_p11_umum" class="form-control" value="<?php echo $model['no_p11_umum']; ?>" maxlength="50" required data-error="Nomor P-11 Umum belum diisi" />
                        <div class="help-block with-errors" id="error_custom_no_p11_umum"></div>
                    </div>
            	</div>
            </div>
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <input type="text" name="tgl_p11_umum" id="tgl_p11_umum" class="form-control datepicker" value="<?php echo $tgl_p11_umum;?>" required data-error="Tanggal P-11 Umum belum diisi" />
                        </div>
                        <div class="help-block with-errors" id="error_custom_tgl_p11_umum"></div>
                    </div>
                </div>
            </div>
		</div>
                ...
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-md-8">
                        <select name="sifat" id="sifat" class="select2" style="width:100%" required data-error="Sifat surat belum diisi">
                            <option></option>
                            <?php 
                                $resOpt = PdsP11Umum::findBySql("select distinct id, nama from ms_sifat_surat order by id")->asArray()->all();
            ...
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" id="table-saksi">
                        ...
                        <tbody>
                        <?php 
                            $sqlnya = "select * from pidsus.pds_p11_umum_saksi where ".$whereDefault." and no_p11_umum = '".$model['no_p11_umum']."' order by no_urut_saksi";
                            $hasil 	= PdsP11Umum::findBySql($sqlnya)->asArray()->all();
                            if(count($hasil) == 0)
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['p11_umum'].$model['file_upload'];
                        $labelFile 	= 'Unggah Pidsus P-11 Umum';
...
$(document).ready(function(){
	...
	$(".form-buat-pemberi-kuasa").on("click", "#btn_add_saksi", function(){
		...
		if(perihal){			
			...
			$("#tambah_saksi_modal").find(".modal-body").load("/pidsus/pds-p11-umum/getsaksi?perihal="+encodeURIComponent(perihal), function(){
		...
		$.post("/pidsus/pds-p11-umum/getsaksi", objk, function(data){
	...
	$("#tambah_saksi_modal").on('show.bs.modal', function(e){
		...
		if(arrnya['nurec_penggeledahan'] == 1){
			...
			$.post("/pidsus/pds-p11-umum/setsaksi", frmnya, function(data){
				if(isNaN(rwNom)){
					rwTbl.remove();
		...
		if(perihal){
			...
			$("#list_saksi_modal").find(".modal-body").load("/pidsus/pds-p11-umum/getlistsaksi?perihal="+encodeURIComponent(perihal), function(){
				if(perihal == 'Saksi'){ 
