<?php
	...
	if(!$model['id_p12_umum']){
		$this->title = 'Pidsus-12 Umum';
		$isNewRecord = 1;
	} else{
		$this->title = 'Pidsus-12 Umum';
		$isNewRecord = 0;
...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_pidsus12_umum 	= new Date(tgl_auto($("#tgl_pidsus12_umum").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_umum > tgl_pidsus12_umum){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus12_umum").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus 12 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_pidsus12_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_pidsus12_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus12_umum").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus 12 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_pidsus12_umum"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-12-umum/cekpidsus12umum'; ?>',
