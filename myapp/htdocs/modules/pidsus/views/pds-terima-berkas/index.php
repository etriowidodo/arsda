...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-terima-berkas/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
		...
		echo GridView::widget([
			...
			'columns' => [
				...
				'3'=>[
					...
					'value'=>function($data){
						$temp = explode('#', $data['undang_pasal']);
						$text = '';
						if(count($temp) > 0){
							foreach($temp as $idx=>$res){
								$isi = explode("--", $res);
								if($isi[0] && $isi[1] && $isi[2]){
									$text .= '<div style="margin-bottom: 5px; width: 100%; display: table;">';
									$text .= '<div style="display: table-cell; width: 23px;">'.$isi[0].'.</div><div style="display: table-cell;">'.$isi[1].'</div><div style="display: table-cell;">'.$isi[2].'</div>';
									$text .= '</div>';
								}
							}
						}
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-terima-berkas/update?id1="+tm[0]+"&id2="+tm[1]+"&id3="+tm[2];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-terima-berkas/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
