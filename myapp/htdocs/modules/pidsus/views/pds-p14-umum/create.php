...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_dikeluarkan = new Date(tgl_auto($("#tgl_dikeluarkan").val()));
                var alasan          = $("input:checkbox[name='alasan[]']:checked").length;
                var tgl_pidsus7     = $("#tgl_pidsus7_umum").val();
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			$("body").removeClass("loading");
			$("#error_custom_no_pidsus7_umum").html('<i style="color:#dd4b39; font-size:12px;">Pidsus-7 Umum belum dipilih</i>');
			setErrorFocus($("#error_custom_no_pidsus7_umum"), $("#role-form"), false);
			return false;
		} else if(alasan == 0){
			$("body").removeClass("loading");
			$("#error_custom_alasan").html('<i style="color:#dd4b39; font-size:12px;">Alasan belum dipilih</i>');
			setErrorFocus($("#error_custom_alasan"), $("#role-form"), false);
			return false;
		} else if(tgl_p8_umum > tgl_dikeluarkan){
			$("body").removeClass("loading");
			$("#error_custom_tgl_dikeluarkan").html('<i style="color:#dd4b39; font-size:12px;">Tanggal dikeluarkan lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_dikeluarkan"), $("#role-form"), false);
			return false;
		} else if(tgl_dikeluarkan > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_dikeluarkan").html('<i style="color:#dd4b39; font-size:12px;">Maximal dikeluarkan adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_dikeluarkan"), $("#role-form"), false);
			return false;
			...
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p14-umum/cekpdsp14umum'; ?>',
