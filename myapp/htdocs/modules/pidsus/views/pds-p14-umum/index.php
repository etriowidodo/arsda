				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn" name="btnSearch" id="btnSearch"><i class="fa fa-search jarak-kanan"></i>Cari</button>
                                                <a href="/pidsus/pds-p14-umum/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-p14-umum/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_p14_umum']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor/Tanggal P-14 Umum', 
					'value'=>function($data){
						return $data['no_p14_umum']."<br />".date("d-m-Y", strtotime($data['tgl_dikeluarkan']));
					}, 
				],
				'2'=>[
				...
				'3'=>[
					...
					'header'=>'Alasan', 
					'value'=>function($data){
                                                $a = explode(",", $data['alasan']);
                                                $alasan = ((in_array("1", $a))?'Tidak terdapat cukup bukti<br />':'').
                                                          ((in_array("2", $a))?'Peristiwa yang dilakukan bukan merupakan tindak pidana<br />':'').
                                                          ((in_array("3", $a))?'Penyidikan harus ditutup demi hukum<br />':'');
						return $alasan;
					}, 
				],
				'aksi'=>[
...
$(document).ready(function(){
	...
	$("#idUbah").on("click", function(){
		...
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p14-umum/update?id1="+tm[0];
	...
	$("#idCetak").on("click", function(){
		var id 	= $(".selection_one:checked").val();
		var tm 	= id.toString().split("#");
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p14-umum/cetak?id1="+tm[0]+"&&id2=0";
		$(location).attr("href", url);
	});
		...
		bootbox.confirm({
			...
			callback: function (result) {
				if(result){
					...
					$.ajax({
						type	: "POST",
						url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p14-umum/hapusdata'; ?>",
						data	: { 'id' : id },
						cache	: false,
						dataType: "json",
