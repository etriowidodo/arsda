<?php
	...
	$this->title = 'P-8 Umum';
	$this->subtitle = 'Surat Perintah Penyidikan (Umum)';
	$whereDefault 	= "id_kejati = '".$_SESSION["kode_kejati"]."' and id_kejari = '".$_SESSION["kode_kejari"]."' and id_cabjari = '".$_SESSION["kode_cabjari"]."'";
	$linkBatal		= '/pidsus/pds-p8-umum/index';
	$linkCetak		= '/pidsus/pds-p8-umum/cetak?no_p8_umum='.$model['no_p8_umum'];
...
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-p8-umum/simpan" enctype="multipart/form-data">
<input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />	
<div class="box box-primary" style="margin-bottom:30px;">
    <div class="box-header with-border">
        <h3 class="box-title">P-6 Laporan Terjadinya Tindak Pidana</h3>
    </div>
    <div class="box-body">
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-md-4">
                        <?php if(!$model['no_urut_p6']){?>
                        <div class="input-group input-group-sm">
                            <input type="hidden" name="no_urut_p6" id="no_urut_p6" value="<?php echo $model['no_urut_p6'];?>" />
                            <input type="text" name="tgl_p6" id="tgl_p6" class="form-control" value="<?php echo $tgl_p6;?>" readonly />
                            <span class="input-group-btn"><button class="btn" type="button" id="pilih_p6"><i class="fa fa-search"></i></button></span>
                        </div>
                        <?php } else{ ?>
                        <div class="input-group input-group-sm">
                        ...
                        <?php }?>
                        <div class="help-block with-errors" id="error_custom_tgl_p6"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Jaksa Pembuat Laporan</label>        
                    <div class="col-md-8">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="pangkat" id="pangkat" class="form-control" value="<?php echo $model['pangkat_jaksa'];?>" readonly />
                        <div class="help-block with-errors"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="jabatan" id="jabatan" class="form-control" value="<?php echo $model['jabatan_jaksa'];?>" readonly />
                        <div class="help-block with-errors"></div>
                    </div>
                </div>
            </div>
...
<div class="row">
    <div class="col-md-6">
        <div class="form-group form-group-sm">
            ...
            <div class="col-md-8">
                <input type="text" name="no_p8_umum" id="no_p8_umum" class="form-control" value="<?php echo $model['no_p8_umum'];?>" required data-error="Nomor P-8 Umum belum diisi" maxlength="50" />
        ...
        <div class="form-group form-group-sm">
            ...
            <div class="col-md-4">
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input type="text" name="tgl_p8_umum" id="tgl_p8_umum" class="form-control  datepicker" value="<?php echo $tgl_p8_umum;?>" required data-error="Tanggal P-8 Umum belum diisi" />
                </div>
                <div class="help-block with-errors" id="error_custom_tgl_p8_umum"></div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="box-body">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-jpn-modal">
                ...
                <tbody>
                <?php 
                    $sqlnya = "select * from pidsus.pds_p8_umum_jaksa where ".$whereDefault." and no_p8_umum = '".$model['no_p8_umum']."' order by no_urut";
                    $hasil 	= PdsP8Umum::findBySql($sqlnya)->asArray()->all();
                    if(count($hasil) == 0)
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <?php
                        	if($model['no_p8_umum'] == ''){
                        		$sqlx = "select no_urut, tembusan from pidsus.ms_template_surat_tembusan where kode_template_surat = 'P-8-Umum' order by no_urut";
                        		$resx = PdsP8Umum::findBySql($sqlx)->asArray()->all();
                        	} else{
                        		$sqlx = "select no_urut as no_urut, tembusan from pidsus.pds_p8_umum_tembusan 
										where ".$whereDefault." and no_p8_umum = '".$model['no_p8_umum']."' order by no_urut";
                        		$resx = PdsP8Umum::findBySql($sqlx)->asArray()->all();
                        	}
                        	$no = 1;
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['p8umum'].$model['file_upload'];
                        $labelFile 	= 'Unggah File P-8 Umum';
                        if($model['file_upload'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah File P-8 Umum';
...
<div class="modal fade" id="p6_modal" role="dialog" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" style="width:1000px;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Daftar P-6</h4>
            </div>
...
$(document).ready(function(){
	/* START AMBIL P-6 */
	$("#pilih_p6").on('click', function(e){
		$("#p6_modal").find(".modal-body").html("");
		$("#p6_modal").find(".modal-body").load("/pidsus/pds-p8-umum/getp6");
		$("#p6_modal").modal({backdrop:"static"});
	});
	$("#p6_modal").on('show.bs.modal', function(e){
		$("body").addClass("loading");
	}).on('shown.bs.modal', function(e){
		$("body").removeClass("loading");
	}).on("dblclick", "#tabel-p6-modal td:not(.aksinya)", function(){
		var index = $(this).closest("tr").data("id");
		var param = index.toString().split('|#|');
		$("#no_urut_p6").val(decodeURIComponent(param[0]));
		$("#tgl_p6").val(decodeURIComponent(param[1]));
		$("#nip_jaksa").val(decodeURIComponent(param[2]));
		$("#nama_jaksa").val(decodeURIComponent(param[3]));
		$("#pangkat").val(decodeURIComponent(param[4]));
		$("#jabatan").val(decodeURIComponent(param[5]));
		$("#laporan_pidana").val(decodeURIComponent(param[6]));
		$("#p6_modal").modal("hide");
	}).on('click', "#idPilihP6Modal", function(){
		var modal = $("#p6_modal").find("#tabel-p6-modal");
		var index = modal.find(".pilih-p6-modal:checked").val();
		var param = index.toString().split('|#|');
		$("#no_urut_p6").val(decodeURIComponent(param[0]));
		$("#tgl_p6").val(decodeURIComponent(param[1]));
		$("#nip_jaksa").val(decodeURIComponent(param[2]));
		...
		$("#laporan_pidana").val(decodeURIComponent(param[6]));
		$("#p6_modal").modal("hide");
	}).on('click','#idBatalP6Modal', function(){
		$("#p6_modal").modal("hide");
	});
	/* END AMBIL P-6 */
	...
	$(".form-buat-pemberi-kuasa").on("click", "#btn_popjpn", function(){
		...
		$("#tambah_jaksa_modal").find(".modal-body").load("/pidsus/pds-p8-umum/getjp");
