                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <input type="text" name="lampiran" id="lampiran" class="form-control" value="<?php echo $model['lampiran'];?>" <?php echo ($model['lampiran'])?'readonly':'';?> required data-error="Lampiran belum diisi" maxlength="50" />
                                <div class="help-block with-errors" id="error_custom_no_p16"></div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover table-geledah-modal">
                        <thead>
                            <tr>
                                <th class="text-center" width="5%"><input type="checkbox" name="allCheckGeledah" id="allCheckGeledah" class="allCheckGeledah" /></th>
                                <th class="text-center" width="5%">No</th>
                                <th class="text-center" width="15%">Subyek/Obyek</th>
                    ...
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <?php
                        	if($model['no_p16'] == ''){
                        		...
                        		$resx = PdsPidsus16::findBySql($sqlx)->asArray()->all();
                        	} else{
...
$(document).ready(function(){
    /* START PENGGELEDAHAN */
		...
		$("#tambah_geledah_modal").find(".modal-body").load("/pidsus/pds-pidsus-16/getgeledah");
	...
	$(".tambah_geledah_modal").find(".table-jpn-modal tr[data-id]").each(function(k, v){
		var idnya = $(v).data("id");
		formValues[idnya] = idnya;
	});
	localStorage.setItem("formValues", JSON.stringify(formValues));
        
	$(".tambah_geledah_modal").on("click", "#btn_tambahjpn", function(){
		...
		var id 		= [];
		var tabel 	= $(".tambah_geledah_modal").find(".table-jpn-modal");
		tabel.find(".hRowJpn:checked").each(function(k, v){
			var idnya = $(v).val();
			tabel.find("tr[data-id='"+idnya+"']").remove();
			if(tabel.find("tr").length == 1){
				var nRow = $(".tambah_geledah_modal").find(".table-jpn-modal > tbody");
	...
	$(".tambah_geledah_modal").on("ifChecked", ".table-jpn-modal input[name=allCheckJpn]", function(){
	...
	/* END AMBIL JPN */
		...
		$("#tambah_sita_modal").find(".modal-body").load("/pidsus/pds-pidsus-16/getsita");
	...
	$(".tambah_sita_modal").find(".table-jpn-modal tr[data-id]").each(function(k, v){
		var idnya = $(v).data("id");
		formValuesSita[idnya] = idnya;
	});
	localStorage.setItem("formValuesSita", JSON.stringify(formValuesSita));
        
	$(".tambah_sita_modal").on("click", "#btn_tambahjpn", function(){
		$("#jpn_modal_sita").find(".modal-body").html("");
		$("#jpn_modal_sita").find(".modal-body").load("/pidsus/get-jpu/index");
		$("#jpn_modal_sita").modal({backdrop:"static"});
	}).on("click", "#btn_hapusjpn", function(){
		var id 		= [];
		var tabel 	= $(".tambah_sita_modal").find(".table-jpn-modal");
		tabel.find(".hRowJpn:checked").each(function(k, v){
			var idnya = $(v).val();
			tabel.find("tr[data-id='"+idnya+"']").remove();
			if(tabel.find("tr").length == 1){
				var nRow = $(".tambah_sita_modal").find(".table-jpn-modal > tbody");
				nRow.append('<tr><td colspan="4">Tidak ada dokumen</td></tr>');
			}
		});
		tabel.find(".frmnojpn").each(function(i,v){$(this).text(i+1);});				
	...
	function insertToRoleSita(myvar, index){
		var tabel 	= $(".tambah_sita_modal").find(".table-jpn-modal");
		var rwTbl	= tabel.find('tbody > tr:last');
		var rwNom	= parseInt(rwTbl.find("span.frmnojpn").data('rowCount'));
		var newId 	= (isNaN(rwNom))?1:parseInt(rwNom + 1);
	...
	$(".tambah_sita_modal").on("ifChecked", ".table-jpn-modal input[name=allCheckJpn]", function(){
		$(".hRowJpn").not(':disabled').iCheck("check");
	}).on("ifUnchecked", ".table-jpn-modal input[name=allCheckJpn]", function(){
		$(".hRowJpn").not(':disabled').iCheck("uncheck");
	}).on("ifChecked", ".table-jpn-modal .hRowJpn", function(){
