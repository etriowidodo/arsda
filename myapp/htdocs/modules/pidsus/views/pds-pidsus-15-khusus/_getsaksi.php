...
<div class="modal-pds14u-index">
	...
	<?php
        ...
        echo GridView::widget([
            'tableOptions' => ['class' => 'table table-bordered table-hover explorer', 'id'=>'tabel-pidsus14-khusus-modal'],
			'layout' => "{summary}\n{items}\n<div class='text-center'>{pager}</div>",
			'dataProvider' => $dataProvider,
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['nama']).'|#|'.rawurlencode($data['jabatan']);
			...
			'columns' => [
				...
				'aksi'=>[
					...
					'value'=>function($data, $index){
						$idnya = rawurlencode($data['nama']).'|#|'.rawurlencode($data['jabatan']);
						return '<input type="radio" name="pilih-pidsus14-khusus-modal[]" id="pilih-pidsus14-khusus-modal'.($index+1).'" value="'.$idnya.'" class="pilih-pidsus14-khusus-modal" />';
					}, 
				],
            ],
        ]);
...
$(document).ready(function(){
	...
	$(".modal-pds14u-index").on("ifChecked", ".pilih-pidsus14-khusus-modal", function(){
