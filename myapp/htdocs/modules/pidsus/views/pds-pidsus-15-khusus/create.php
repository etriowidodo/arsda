...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_pidsus15_umum 	= new Date(tgl_auto($("#tgl_pidsus15_umum").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			setErrorFocus($("#error_custom_file_template"), $("#role-form"), false);
			return false;
		} else if($("#nama_saksi").val() == "" ){
			$("body").removeClass("loading");
			$("#error_custom_nama_saksi").html('<i style="color:#dd4b39; font-size:12px;">Kolom [Atas nama] belum dipilih</i>');
			setErrorFocus($("#error_custom_nama_saksi"), $("#role-form"), false);
			return false;
			...
			return false;
		} else if(tgl_p8_umum > tgl_pidsus15_umum){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus15_umum").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus-15 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_pidsus15_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_pidsus15_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus15_umum").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus-15 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_pidsus15_umum"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-15-umum/cekpdspidsus15umum'; ?>',
