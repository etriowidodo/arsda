<?php   use app\modules\pidsus\models\PdsBa2Umum;
	require_once('./function/tgl_indo.php');
	require_once('./wordtest/classes/CreateDocx.inc');
	$docx = new CreateDocxFromTemplate('../modules/pidsus/template/BA-2-Umum.docx');
	...
	$almtSatker     = Yii::$app->inspektur->getLokasiSatker()->alamat;
        ...
        $no_ba2_umum    = ($model['isDraft'] && $model['no_ba2_umum']=="")?'null':"'".$model['no_ba2_umum']."'";
        $sql = "select * from pidsus.pds_ba2_umum_saksi where ".$whereDefault." and no_ba2_umum = ".$no_ba2_umum." order by no_urut_saksi";
	$res = PdsBa2Umum::findBySql($sql)->asArray()->all();
	...
	$ttdsaksi .= '</tbody></table>';
        ...
        $hari_ba2       = ($model['isDraft'] && $model['tgl_ba2_umum']=="")?'........................................':Yii::$app->globalfunc->GetNamaHari($model['tgl_ba2_umum']);
        $tgl_ba2_umum   = ($model['isDraft'] && $model['tgl_ba2_umum']=="")?'........................................':tgl_indo($tgl_ba2_umum);
        $jam_ba2_umum   = ($model['isDraft'] && $model['jam_ba2_umum']=="")?'........................................':$model['jam_ba2_umum'];
	...
	$docx->replaceVariableByText(array('tanggal_ba2'=>$tgl_ba2_umum), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('jam_ba2'=>$jam_ba2_umum), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('jml_saksi'=>$nom1), array('parseLineBreaks'=>true));
        ...
        $docx->createDocx('template/pidsus/BA-2-Umum');
	$file = 'template/pidsus/BA-2-Umum.docx';
	if (file_exists($file)) {
