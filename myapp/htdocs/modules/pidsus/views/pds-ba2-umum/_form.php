<?php
	...
	$linkBatal		= '/pidsus/pds-ba2-umum/index';
	$linkCetak		= '/pidsus/pds-ba2-umum/cetak?id1='.rawurlencode($model['no_ba2_umum']);
	if($isNewRecord){
		$sqlCek = "select no_p8_umum, tgl_p8_umum from pidsus.pds_p8_umum where ".$whereDefault;
		$model 	= PdsBa2Umum::findBySql($sqlCek)->asArray()->one();
	}
	$tgl_p8_umum 	= ($model['tgl_p8_umum'])?date('d-m-Y',strtotime($model['tgl_p8_umum'])):'';
	$tgl_ba1_umum 	= ($model['tgl_keterangan'])?date('d-m-Y',strtotime($model['tgl_keterangan'])):'';
	$tgl_ba2_umum 	= ($model['tgl_ba2_umum'])?date('d-m-Y',strtotime($model['tgl_ba2_umum'])):'';
	$tgl_lahir 		= ($model['tgl_lahir'])?date('d-m-Y',strtotime($model['tgl_lahir'])):'';
?>
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-ba2-umum/simpan" enctype="multipart/form-data">
        ...
        <div class="box box-primary">
            <div class="box-body">
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-4">
                                <div class="input-group">
                                    <input type="text" name="tgl_ba1_umum" id="tgl_ba1_umum" class="form-control" value="<?php echo $tgl_ba1_umum;?>" readonly />
                                    <div class="input-group-btn"><a class="btn btn-pidsus btn-sm" id="btn-cari-ba1-umum"><i class="fa fa-search"></i></a></div>
                                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group form-group-sm">
                            <label class="control-label col-md-4">Tanggal/Jam BA-2 Umum</label>        
                            <div class="col-md-4">
                                <div class="input-group">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                    <input type="text" name="tgl_ba2_umum" id="tgl_ba2_umum" class="form-control datepicker" value="<?php echo $tgl_ba2_umum;?>" required data-error="Tanggal BA-2 Umum belum diisi" />
                                </div>
                                <div class="help-block with-errors" id="error_custom_tgl_ba2_umum"></div>
                            </div>
                            <div class="col-md-4">
                                <div class="input-group bootstrap-timepicker">
                                    <div class="input-group-addon picker" style="border-right:0px;"><i class="fa fa-clock-o"></i></div>
                                    <input type="text" name="jam_ba2_umum" id="jam_ba2_umum" class="form-control timepicker" value="<?php echo $model['jam_ba2_umum']; ?>" required data-error="Jam BA-2 Umum belum diisi" />
                                </div>
                                <div class="help-block with-errors" id="error_custom_jam_ba2_umum"></div>
                            </div>
                        </div>
                    </div>
				</div>
                            ...
                            <div class="col-md-8">
                                <select name="nama_jaksa_penyidik" id="nama_jaksa_penyidik" class="select2" style="width:100%" required data-error="Nama Jaksa belum dipilih">
                                    ...
                                    <?php
										...
										$resOpt1 = PdsBa2Umum::findBySql($sqlOpt1)->asArray()->all();
            ...
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover table-jpn-modal">
                        ...
                        <tbody>
                        <?php 
							$sqlnya = "select * from pidsus.pds_ba2_umum_saksi where ".$whereDefault." and no_ba2_umum = '".$model['no_ba2_umum']."' order by no_urut_saksi";
                            $hasil 	= ($model['no_ba2_umum'])?PdsBa2Umum::findBySql($sqlnya)->asArray()->all():array();
                            if(count($hasil) == 0)
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['ba2_umum'].$model['file_upload'];
                        $labelFile 	= 'Unggah BA-2 Umum';
                        if($model['file_upload'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah BA-2 Umum';
...
<div class="box-footer" style="text-align: center;"> 
    ...
    <input type="hidden" name="no_ba2_umum" id="no_ba2_umum" value="<?php echo $model['no_ba2_umum']; ?>" />
...
<div class="modal fade" id="get_ba1_umum_modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" style="width:1000px;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">BA-1 Umum</h4>
            </div>
            <div class="modal-body"></div>
        </div>
    </div>
...
$(document).ready(function(){
	...
	$("#jam_ba2_umum").on('focus', function(){
	...
	/* START GET BA-1 UMUM */
	$("#btn-cari-ba1-umum").on('click', function(e){
		$("#get_ba1_umum_modal").find(".modal-body").html("");
		$("#get_ba1_umum_modal").find(".modal-body").load("/pidsus/pds-ba2-umum/getba1umum");
		$("#get_ba1_umum_modal").modal({backdrop:"static"});
	});
	$("#get_ba1_umum_modal").on('show.bs.modal', function(e){
		...
		insertToBa1u(param);
		$("#get_ba1_umum_modal").modal("hide");
	}).on('click', "#idPilihba1UModal", function(){
		var modal = $("#get_ba1_umum_modal").find("#tabel-ba1u-modal");
	...
	function insertToBa1u(param){
		$("#no_ba1_umum").val(decodeURIComponent(param[0]));
		$("#tgl_ba1_umum").val(decodeURIComponent(param[1]));
	...
	$(".form-yang-menyaksikan").find(".table-jpn-modal tr[data-id]").each(function(k, v){
		var idnya = $(v).data("id");
		formValuesBa2Umum[idnya] = idnya;
	});
	localStorage.setItem("formValuesBa2Umum", JSON.stringify(formValuesBa2Umum));

	/* START AMBIL JPN */
	$(".form-yang-menyaksikan").on("click", "#btn_popusul", function(){
		$("#tambah_usul_modal").find(".modal-body").load("/pidsus/pds-ba2-umum/getsaksi");
	...
	$("#tambah_usul_modal").on('show.bs.modal', function(e){
		...
		var id 	= [];
		var n 	= JSON.parse(localStorage.getItem('modalnyaDataBa2Umum')) || {};
		for(var x in n){ 
			id.push(n[x]);
		}
