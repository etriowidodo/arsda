...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			if($("input[name='saksi_ahli[]']").length == 0){
				bootbox.confirm({ 
	...
	function validasi_upload(){
		...
		var tgl_pidsus14_khusus 	= new Date(tgl_auto($("#tgl_pidsus14_khusus").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_khusus > tgl_pidsus14_khusus){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus14_khusus").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus-14 Khusus lebih kecil dari tanggal P-8 Khusus</i>');
			setErrorFocus($("#error_custom_tgl_pidsus14_khusus"), $("#role-form"), false);
			return false;
		} else if(tgl_pidsus14_khusus > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus14_khusus").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus-14 Khusus adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_pidsus14_khusus"), $("#role-form"), false);
			return false;
		} else{
