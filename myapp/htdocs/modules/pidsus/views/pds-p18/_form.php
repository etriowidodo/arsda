<?php
	...
	$linkBatal		= '/pidsus/pds-p18/index';
	$linkCetak		= '/pidsus/pds-p18/cetak?id1='.$model['no_berkas'];
	...
	$tgl_p24 		= ($model['tgl_ba'])?date('d-m-Y',strtotime($model['tgl_ba'])):'';
	$ttdJabatan 	= $model['penandatangan_status_ttd']." ".$model['penandatangan_jabatan_ttd'];
?>

<?php if($_SESSION['no_spdp'] && $_SESSION['tgl_spdp']){ ?>
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-p18/simpan" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <input type="text" name="instansi_penyidik" id="instansi_penyidik" class="form-control" value="<?php echo $model['instansi_ip'];?>" readonly/>
                                <div class="help-block with-errors"></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group form-group-sm">
                            <label class="control-label col-md-4">Tanggal P-24</label>        
                            <div class="col-md-8">
                                <div class="input-group">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                    <input type="text" name="tgl_p24" id="tgl_p24" class="form-control" value="<?php echo $tgl_p24;?>" readonly />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <?php
                        	if($model['id_p18'] == ''){
                        		$sqlx = "select no_urut, tembusan from pidsus.ms_template_surat_tembusan where kode_template_surat = 'P-18' order by no_urut";
                        		$resx = PdsP21::findBySql($sqlx)->asArray()->all();
                        	} else{
                        		$sqlx = "select a.no_urut as no_urut, a.tembusan as tembusan from pidsus.pds_p18_tembusan a
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['p18'].$model['file_upload_p18'];
                        $labelFile 	= 'Unggah File P-18';
                        if($model['file_upload_p18'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah File P-18';
                            $param1  	= chunk_split(base64_encode($pathFile));
                            $param2  	= chunk_split(base64_encode($model['file_upload_p18']));
                            $linkPt 	= "/pidsus/download-file/index?id=".$param1."&fn=".$param2;
                            $extPt		= substr($model['file_upload_p18'], strrpos($model['file_upload_p18'],'.'));
                            echo '<a href="'.$linkPt.'" title="'.$model['file_upload_p18'].'" style="float:left; margin-right:10px;">
