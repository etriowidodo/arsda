...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_p24 	= new Date(tgl_auto($("#tgl_p24").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			setErrorFocus($("#error_custom_file_template"), $("#role-form"), false);
			return false;
		} else if(tgldittd < tgl_p24){
			$("body").removeClass("loading");
			$("#error_custom_tgldittd").html('<i style="color:#dd4b39; font-size:12px;">Tanggal dikeluarkan tidak boleh lebih kecil daripada tanggal P-24</i>');
			...
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p18/cekp18'; ?>',
