...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_pidsus18    = new Date(tgl_auto($("#tgl_pidsus18").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_pidsus18 > tgl_p8_khusus){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p8_khusus").html('<i style="color:#dd4b39; font-size:12px;">Tanggal P-8 khusus lebih kecil dari tanggal Pidsus-18</i>');
			setErrorFocus($("#tgl_p8_khusus"), $("#role-form"), false);
			return false;
		} else if(tgl_p8_khusus > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p8_khusus").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal P-8 khusus adalah hari ini</i>');
			setErrorFocus($("#tgl_p8_khusus"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p8-khusus/cekp8khusus'; ?>',
