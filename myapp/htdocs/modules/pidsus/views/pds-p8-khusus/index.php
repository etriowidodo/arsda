				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn" name="btnSearch" id="btnSearch"><i class="fa fa-search jarak-kanan"></i>Cari</button>
                                                <a href="/pidsus/pds-p8-khusus/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-p8-khusus/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_p8_umum'])."#".rawurlencode($data['no_pidsus18'])."#".rawurlencode($data['no_p8_khusus']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_p8_khusus'].'</p>'.date("d-m-Y", strtotime($data['tgl_p8_khusus']));
					}, 
				],
				'2'=>[
					'headerOptions'=>['style'=>'width:20%', 'class'=>'text-center'],
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p8-khusus/update?id1="+tm[0]+"&id2="+tm[1]+"&id3="+tm[2];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p8-khusus/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
