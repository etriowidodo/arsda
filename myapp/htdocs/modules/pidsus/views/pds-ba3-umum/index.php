			...
			<div class="col-md-10">
				<div class="input-group">
					...
					<div class="input-group-btn">
                        ...
                        <a href="/pidsus/pds-ba3-umum/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-ba3-umum/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_ba3_umum']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Tanggal BA-3 Umum', 
					'value'=>function($data){
						return date("d-m-Y", strtotime($data['tgl_ba3_umum']))." Jam ".$data['jam_ba3_umum']."<br />".$data['tempat'];
					}, 
				],
				'2'=>[
...
$(document).ready(function(){
	...
	$("#idUbah").on("click", function(){
		...
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ba3-umum/update?id1="+tm[0];
	...
	$("#idCetak").on("click", function(){
		var id 	= $(".selection_one:checked").val();
		var tm 	= id.toString().split("#");
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ba3-umum/cetak?id1="+tm[0]+"&&id2=0";
		$(location).attr("href", url);
	});
		...
		var id 	= $(".selection_one:checked").val();
                ...
                if(id){
                    var tm = id.toString().split("#");
                    url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ba3-umum/cetak?id1="+tm[0]+"&&id2=1";
                }else{
                    url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ba3-umum/cetak?id1=null&&id2=1";
                }
		$(location).attr("href", url);
	});
		...
		bootbox.confirm({
			...
			callback: function (result) {
				if(result){
					...
					$.ajax({
						type	: "POST",
						url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-ba3-umum/hapusdata'; ?>",
						data	: { 'id' : id },
						cache	: false,
						dataType: "json",
