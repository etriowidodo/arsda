...
$(document).ready(function(){
	...
	$(".jpn-index").find(".selection_one_jpn").each(function(i, v){
		...
		(idnya[0] in formValuesBa3Umum)?$(v).prop({'checked':'checked', 'disabled':'disabled'}):$(v).removeProp('checked disabled');
	});
	...
	$('#myPjaxModalJpn').on('pjax:send', function(){
		...
		$(".jpn-index").find(".selection_one_jpn").each(function(i, v){
			var idnya = $(v).val().toString().split('#');
			(idnya[0] in formValuesBa3Umum)?$(v).prop({'checked':'checked', 'disabled':'disabled'}):$(v).removeProp('checked disabled');
			(idnya[0] in modalnyaDataBa3Umum)?$(v).prop({'checked':'checked'}):$(v);
		});
		$("input[type='checkbox']:not(.simple)").iCheck({checkboxClass: 'icheckbox_square-pink'});
	...
	$("#searchModalJpn").on("submit", function(){localStorage.removeItem("modalnyaDataBa3Umum");});
	$(".jpn-index").on("ifChecked", ".selection_one_jpn", function(){
		var nilai = $(this).val();
		var pecah = nilai.toString().split("#");
		modalnyaDataBa3Umum[pecah[0]] = nilai;
		localStorage.setItem("modalnyaDataBa3Umum", JSON.stringify(modalnyaDataBa3Umum));
	}).on("ifUnchecked", ".selection_one_jpn", function(){
		var nilai = $(this).val();
		var pecah = nilai.toString().split("#");
		delete modalnyaDataBa3Umum[pecah[0]];
		localStorage.setItem("modalnyaDataBa3Umum", JSON.stringify(modalnyaDataBa3Umum));
	});
