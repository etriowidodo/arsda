<?php   use app\modules\pidsus\models\PdsBa3Umum;
	require_once('./function/tgl_indo.php');
	require_once('./wordtest/classes/CreateDocx.inc');
	$docx = new CreateDocxFromTemplate('../modules/pidsus/template/BA-3-Umum.docx');
	...
	$almtSatker     = Yii::$app->inspektur->getLokasiSatker()->alamat;
        ...
        $no_ba3_umum    = ($model['isDraft'] && $model['no_ba3_umum']=="")?'null':"'".$model['no_ba3_umum']."'";
        $sql = "select * from pidsus.pds_ba3_umum_saksi where ".$whereDefault." and no_ba3_umum = ".$no_ba3_umum." order by no_urut_saksi";
	$res = PdsBa3Umum::findBySql($sql)->asArray()->all();
		...
		foreach($res as $data1){
			...
			$saksi .= '
				...
				<tr>
					...
					<td style="font-family:Times New Roman; font-size:12pt;">:</td>
					<td style="font-family:Times New Roman; font-size:12pt;">'.$data1['jabatan'].'</td>
				</tr>
	...
	$ttdsaksi .= '</tbody></table>';
        ...
        $hari_ba3       = ($model['isDraft'] && $model['tgl_ba3_umum']=="")?'........................................':Yii::$app->globalfunc->GetNamaHari($model['tgl_ba3_umum']);
        $tgl_ba3_umum   = ($model['isDraft'] && $model['tgl_ba3_umum']=="")?'........................................':tgl_indo($tgl_ba3_umum);
        $jam_ba3_umum   = ($model['isDraft'] && $model['jam_ba3_umum']=="")?'........................................':$model['jam_ba3_umum'];
	...
	$docx->replaceVariableByText(array('hari_ba3'=>$hari_ba3), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('tanggal_ba3'=>$tgl_ba3_umum), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('jam_ba3'=>$jam_ba3_umum), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('tempat_ba3'=>$tempat), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('agama'=>ucfirst(strtolower($model['agama']))), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('tgl_ba1'=>tgl_indo($tgl_keterangan)), array('parseLineBreaks'=>true));
        ...
        $docx->createDocx('template/pidsus/BA-3-Umum');
	$file = 'template/pidsus/BA-3-Umum.docx';
	if (file_exists($file)) {
