				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn" name="btnSearch" id="btnSearch"><i class="fa fa-search jarak-kanan"></i>Cari</button>
                                                <a href="/pidsus/ba-konsultasi/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/ba-konsultasi/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['id_ba_konsultasi']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'value'=>function($data){
						return ($data['tgl_pelaksanaan'])?date("d-m-Y", strtotime($data['tgl_pelaksanaan'])):'';
					}, 
				],
				'2'=>[
					'headerOptions'=>['style'=>'width:30%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Jaksa Penuntut Umum', 
					'value'=>function($data){
						return $data['nama_jaksa'];
					}, 
				],
                                '3'=>[
					'headerOptions'=>['style'=>'width:30%', 'class'=>'text-center'],
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/ba-konsultasi/update?id="+tm[0];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/ba-konsultasi/cetak?id="+tm[0];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/ba-konsultasi/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
