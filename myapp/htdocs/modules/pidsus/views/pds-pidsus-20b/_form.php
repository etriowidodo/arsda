<?php
	...
	use app\modules\pidsus\models\PdsPidsus20b;

	$this->title = 'Pidsus-20B';
	...
	$linkBatal		= '/pidsus/pds-pidsus-20b/index';
	$linkCetak		= '/pidsus/pds-pidsus-20b/cetak?id1='.rawurlencode($model['no_pidsus20a']);
	if($isNewRecord){
		$sqlCek = "select no_p8_khusus, tgl_p8_khusus from pidsus.pds_p8_khusus where ".$whereDefault;
		$model 	= PdsPidsus20b::findBySql($sqlCek)->asArray()->one();
	}

	$tgl_p8_khusus = ($model['tgl_p8_khusus'])?date('d-m-Y',strtotime($model['tgl_p8_khusus'])):'';
	$tgl_pidsus20b = ($model['tgl_pidsus20b'])?date('d-m-Y',strtotime($model['tgl_pidsus20b'])):'';
...
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-pidsus-20b/simpan" enctype="multipart/form-data">
...
<div class="box box-primary">
    <div class="box-body">
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Nomor Pidsus-20B</label>        
                    <div class="col-md-8">
                        <input type="text" name="no_pidsus20b" id="no_pidsus20b" class="form-control" value="<?php echo $model['no_pidsus20b'];?>" required data-error="Nomor Pidsus-20B Umum belum diisi" />
                        <div class="help-block with-errors" id="error_custom_no_pidsus20b"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Tanggal Pidsus-20B</label>        
                    <div class="col-md-4">
                        <div class="input-group">
                            <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                            <input type="text" name="tgl_pidsus20b" id="tgl_pidsus20b" class="form-control datepicker" value="<?php echo $tgl_pidsus20b;?>" required data-error="Tanggal Pidsus-20B belum diisi" />
                        </div>
                        <div class="help-block with-errors" id="error_custom_tgl_pidsus20b"></div>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <select name="sifat" id="sifat" class="select2" style="width:100%" required data-error="Sifat surat belum diisi">
                                    <option></option>
                                    <?php 
                                        $resOpt = PdsPidsus20b::findBySql("select distinct id, nama from ms_sifat_surat order by id")->asArray()->all();
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <tbody>
                        <?php
                        	if($model['no_pidsus20b'] == ''){
                        		$sqlx = "select no_urut, tembusan from pidsus.ms_template_surat_tembusan where kode_template_surat = 'Pidsus-20b' order by no_urut";
                        		$resx = PdsPidsus20b::findBySql($sqlx)->asArray()->all();
                        	} else{
                        		$sqlx = "select no_urut as no_urut, tembusan from pidsus.pds_pidsus20b_tembusan 
                                                where ".$whereDefault." and no_pidsus20b = '".$model['no_pidsus20b']."' order by no_urut";
                        		$resx = PdsPidsus20b::findBySql($sqlx)->asArray()->all();
                        	}
                        	$no = 1;
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['pidsus-20b'].$model['file_upload'];
                        $labelFile 	= 'Unggah Pidsus-20B';
                        if($model['file_upload_spdp_kembali'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah Pidsus-20B';
