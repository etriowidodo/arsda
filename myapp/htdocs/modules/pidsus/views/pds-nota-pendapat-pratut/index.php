				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn" name="btnSearch" id="btnSearch"><i class="fa fa-search jarak-kanan"></i>Cari</button>
                                                <a href="/pidsus/pds-nota-pendapat-pratut/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-nota-pendapat-pratut/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
		...
		echo GridView::widget([
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor Permintaan Perpanjangan', 
					'value'=>function($data){
						return $data['no_minta_perpanjang'];
					}, 
				],
				'2'=>[
					'headerOptions'=>['style'=>'width:14%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Tanggal Nota Pendapat', 
					'value'=>function($data){
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-nota-pendapat-pratut/update?id="+tm[0];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("|#|");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-nota-pendapat-pratut/cetak?id="+tm[0];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-nota-pendapat-pratut/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
