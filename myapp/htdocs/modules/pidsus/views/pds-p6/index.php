<?php
	...
	$this->title = 'P-6';
	$this->subtitle = 'Daftar Laporan Terjadinya Tindak Pidana';
			...
			<div class="col-md-10">
				<div class="input-group">
					...
					<div class="input-group-btn">
						...
						<a href="/pidsus/pds-p6/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-p6/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_urut_p6']).'|#|'.rawurlencode($data['tgl_p6']);
			...
			'columns' => [
				...
				'2'=>[
					...
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['nip_jaksa'].'<br/>'.$data['nama_jaksa'].'<br/>'.$data['pangkat_jaksa'].'</p>';
					}, 
				],
				'3'=>[
					...
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['melaporkan_dari'].'</p>';
					}, 
				],
				'4'=>[
					'headerOptions'=>['style'=>'width:20%', 'class'=>'text-center'],
					...
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['dilakukan_oleh'].'</p>';
					}, 
				],
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p6/update?no_urut_p6="+tm[0]+"&tgl_p6="+tm[1];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("|#|");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p6/cetak?no_urut_p6="+tm[0]+"&tgl_p6="+tm[1];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url	: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p6/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
