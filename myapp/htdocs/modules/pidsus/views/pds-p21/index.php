		...
		echo GridView::widget([
			...
			'columns' => [
				...
				'3'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal P-21', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_p21'].'</p>'.($data['tgl_p21']?$data['tgl_p21']:'');
					}, 
				],
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p21/create?id1="+tm[0]+"&id2="+tm[1];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p21/cetak?id1="+tm[0]+"&id2="+tm[1];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p21/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
