<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/pidsus/template/Pidsus-13-Umum.docx');
	...
	$tgl_pidsus13_umum= date('d-m-Y',strtotime($model['tgl_pidsus13_umum']));
	$tgl_p8_umum 	= date('d-m-Y',strtotime($model['tgl_p8_umum']));
        ...
        $kepada = ($model['tindak_pidana'] == 'Korupsi')?'KETUA KPK':'KOMNAS HAM';
        
        $sql2 = "select no_urut, tembusan as tembusan from pidsus.pds_pidsus13_umum_tembusan 
			where ".$whereDefault;
	$res2 = PdsPidsus13Umum::findBySql($sql2)->asArray()->all();
	if(count($res2) > 0){
		$nom2 = 0;
	...
	$docx->replaceVariableByText(array('tgl_surat'=>tgl_indo($tgl_pidsus13_umum)), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('no_pidsus13'=>$model['no_pidsus13_umum']), array('parseLineBreaks'=>true));
	...
	$docx->createDocx('template/pidsus/Pidsus-13-Umum');
	$file = 'template/pidsus/Pidsus-13-Umum.docx';
	if (file_exists($file)) {
