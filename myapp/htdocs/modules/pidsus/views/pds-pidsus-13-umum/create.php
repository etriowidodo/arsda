<?php
	...
	if(!$model['id_p13_umum']){
		$this->title = 'Pidsus-13 Umum';
		$isNewRecord = 1;
	} else{
		$this->title = 'Pidsus-13 Umum';
		$isNewRecord = 0;
...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_pidsus13_umum 	= new Date(tgl_auto($("#tgl_pidsus13_umum").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if($("#no_p8_umum").val() == "" || $("#tgl_p8_umum").val() == ""){
			$("body").removeClass("loading");
			$("#error_custom_no_p8_umum").html('<i style="color:#dd4b39; font-size:12px;">Nomor P-8 Umum Harus diisi</i>');
			setErrorFocus($("#error_custom_no_p8_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_p8_umum > tgl_pidsus13_umum){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus13_umum").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus 13 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_pidsus13_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_pidsus13_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus13_umum").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus 13 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_pidsus13_umum"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-13-umum/cekpidsus13umum'; ?>',
