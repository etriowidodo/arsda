...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_p10_khusus  = new Date(tgl_auto($("#tgl_p10_khusus").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_khusus > tgl_p10_khusus){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p10_khusus").html('<i style="color:#dd4b39; font-size:12px;">Tanggal P-10 Khusus lebih kecil dari tanggal P-8 Khusus</i>');
			setErrorFocus($("#error_custom_tgl_p10_khusus"), $("#role-form"), false);
			return false;
		} else if(tgl_p10_khusus > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p10_khusus").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal P-10 Khusus adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_p10_khusus"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p10-khusus/cekpdsp10khusus'; ?>',
