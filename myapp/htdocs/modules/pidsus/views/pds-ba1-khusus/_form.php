<?php
	...
	$linkBatal		= '/pidsus/pds-ba1-khusus/index';
	$linkCetak		= '/pidsus/pds-ba1-khusus/cetak?id1='.rawurlencode($model['no_ba1_khusus']);
	if($isNewRecord){
		$sqlCek = "select no_p8_khusus, tgl_p8_khusus from pidsus.pds_p8_khusus where ".$whereDefault;
		$model 	= PdsBa1Khusus::findBySql($sqlCek)->asArray()->one();
	}

	$tgl_p8_khusus 	= ($model['tgl_p8_khusus'])?date('d-m-Y',strtotime($model['tgl_p8_khusus'])):'';
	$tgl_ba1_khusus 	= ($model['tgl_ba1_khusus'])?date('d-m-Y',strtotime($model['tgl_ba1_khusus'])):'';
	$tgl_lahir 		= ($model['tgl_lahir'])?date('d-m-Y',strtotime($model['tgl_lahir'])):'';
	$readonly 		= (!$model['pns'])?'readonly':'';
	$checked 		= ($model['no_ba1_khusus'] && $model['pns'])?'checked':'';
?>
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-ba1-khusus/simpan" enctype="multipart/form-data">
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-4">
                                <div class="input-group">
                                    ...
                                    <input type="text" name="tgl_ba1_khusus" id="tgl_ba1_khusus" class="form-control datepicker" value="<?php echo $tgl_ba1_khusus;?>" required data-error="Tanggal BA-1 Khusus belum diisi" />
                                </div>
                            ...
                            <div class="col-md-8">
                                <select name="nama_jaksa_penyidik" id="nama_jaksa_penyidik" class="select2" style="width:100%" required data-error="Nama Jaksa belum dipilih">
                                    ...
                                    <?php
                                        ...
                                        $resOpt1 = PdsBa1Khusus::findBySql($sqlOpt1)->asArray()->all();
                ...
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-md-8">
                        <select name="status" id="status" class="form-control select2">
                            <option <?php echo ($model['status'] == 'Saksi')?'selected':'';?>>Saksi</option>
                            <option <?php echo ($model['status'] == 'Tersangka')?'selected':'';?>>Tersangka</option>
                        </select>
                        <div class="help-block with-errors"></div>
                    </div>
                </div>
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <select id="id_agama" name="id_agama" class="select2" style="width:100%;">
                                    <option></option>
                                    <?php 
                                         $agm = PdsBa1Khusus::findBySql("select * from public.ms_agama order by id_agama")->asArray()->all();
                                        foreach($agm as $ag){
                        ...
                        <div class="form-group form-group-sm">
                            <label class="control-label col-md-4">Pendidikan</label>
                                <div class="col-md-8">
                                    ...
                                    <option></option>
                                    <?php 
                                        $pdd = PdsBa1Khusus::findBySql("select * from public.ms_pendidikan order by id_pendidikan")->asArray()->all();
                                        foreach($pdd as $pd){
            ...
            <table id="table_pertanyaan" class="table table-bordered">
                ...
                <?php
                    if($model['no_ba1_khusus'] == ''){
                        ...
                        $sqlx = "select * from pidsus.pds_ba1_khusus_pertanyaan where ".$whereDefault." and no_ba1_khusus = '".$model['no_ba1_khusus']."' order by no_urut";
                        $resx = PdsBa1Khusus::findBySql($sqlx)->asArray()->all();
                    }
                    $nom = 0;
                ...
                <div class="form-group form-group-sm">
                    <div class="col-md-12">
                        ...
                        <label for="file_unggah_pertanyaan" class="label-inputfile">
                            <?php 
                                $pathFile 	= Yii::$app->params['ba1_khusus'].$model['file_unggah_pertanyaan'];
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['ba1_khusus'].$model['file_upload'];
                        $labelFile 	= 'Unggah BA-1 Khusus';
                        if($model['file_upload'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah BA-1 Khusus';
                            $param1  	= chunk_split(base64_encode($pathFile));
...
<div class="box-footer" style="text-align: center;"> 
    ...
    <input type="hidden" name="no_ba1_khusus" id="no_ba1_khusus" value="<?php echo $model['no_ba1_khusus']; ?>" />
...
$(document).ready(function(){
	$("#pilih_saksi").on("click", function(){
		...
		$("#list_saksi_modal").find(".modal-body").load("/pidsus/pds-ba1-khusus/getsaksi?perihal="+$("#status").val());
