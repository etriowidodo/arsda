...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			if($("input[name^='jpnsitaid']").length == 0 || $("input[name^='jpngldhid']").length == 0){
				bootbox.confirm({ 
					message: "Tabel Penyitaan/Penggeledahan masih kosong. Apakah anda ingin tetap menyimpan data?",
					size: "small",
					closeButton: false,
					buttons: {
	...
	function validasi_upload(){
		...
		var tgl_pidsus16_umum 	= new Date(tgl_auto($("#tgl_pidsus16_umum").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			setErrorFocus($("#error_custom_file_template"), $("#role-form"), false);
			return false;
		} else if(tgl_p8_umum > tgl_pidsus16_umum){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus16_umum").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus-16 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_pidsus16_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_pidsus16_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pidsus16_umum").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus-16 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_pidsus16_umum"), $("#role-form"), false);
			return false;
		} else{
