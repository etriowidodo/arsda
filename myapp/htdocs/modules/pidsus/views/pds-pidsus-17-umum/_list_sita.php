...
?>
<?php $form = ActiveForm::begin(['action'=>['listsita'], 'method'=>'get', 'id'=>'searchFormSita', 'options'=>['name'=>'searchFormSita']]); ?>
<div class="row">
...
<div class="get-sita-index">
	...
	<?php
        ...
        echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = $data['no_b4_umum'].$data['no_urut_penyitaan'].'|#|'.$data['nama_barang_disita'];
	...
	<div class="hide" id="filter-link"><?php echo '/pidsus/pds-pidsus-17-umum/listsita?q1='.htmlspecialchars($_GET['id'], ENT_QUOTES);?></div>
</div>
<style>
...
<script type="text/javascript">
	$(document).ready(function(){
            ...
            $(".get-sita-index").find(".selection_one_sita").each(function(i, v){
                    ...
                    (idnya[1] in formValuesSita)?$(v).prop({'checked':'checked', 'disabled':'disabled'}):$(v).removeProp('checked disabled');
            });
		...
		$("#myPjaxModalSita").on('pjax:beforeSend', function(a, b, c){
			...
			var idnya = $(v).val().toString().split('|#|');
			(idnya[1] in formValuesSita)?$(v).prop({'checked':'checked', 'disabled':'disabled'}):$(v).removeProp('checked disabled');
			(idnya[1] in modalnyaDataSITA)?$(v).prop({'checked':'checked'}):$(v);
                    });
