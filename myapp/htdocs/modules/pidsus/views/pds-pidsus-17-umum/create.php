...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			if($("input[name^='nama_barang_disita']").length == 0){
				bootbox.confirm({ 
					message: "Tabel barang yang disita masih kosong. Apakah anda ingin tetap menyimpan data?",
					size: "small",
					closeButton: false,
					buttons: {
	...
	function validasi_upload(){
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-17-umum/cekpidsus17umum'; ?>',
