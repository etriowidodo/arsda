...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/spdp/create"><i class="fa fa-plus jarak-kanan"></i>Tambah Spdp</a></div>
		...
		echo GridView::widget([
			...
			'columns' => [
				...
				'1'=>[
					...
					'value'=>function($data){
						return $data['no_surat'].'<br>'.date("d/m/Y", strtotime($data['tanggal_permohonan']));
					}, 
				],
				'2'=>[
					'headerOptions'=>['style'=>'width:23%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Pemohon', 
					'value'=>function($data){
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/datun/permohonan/update?id="+tm[0]+"&ns="+tm[1];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/datun/permohonan/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
