<?php
	...
	$this->title = 'B-2 Umum';
	$this->subtitle = 'Laporan Persetujuan Penggeledahan/Penyitaan';
	...
	$linkBatal		= '/pidsus/pds-b2-umum/index';
	$linkCetak		= '/pidsus/pds-b2-umum/cetak?id1='.rawurlencode($model['no_b2_umum']);
	if($isNewRecord){
		$sqlCek = "select a.no_p8_umum, a.tgl_p8_umum from pidsus.pds_p8_umum a where ".$whereDefault;
		$model 	= PdsB2Umum::findBySql($sqlCek)->asArray()->one();
	}
...
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-b2-umum/simpan" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Nomor B-2 Umum</label>
                    <div class="col-md-8">
                        <input type="text" name="no_b2_umum" id="no_b2_umum" class="form-control" value="<?php echo $model['no_b2_umum'];?>" required data-error="Nomor B-2 Umum belum diisi" maxlength="50" />
                        <div class="help-block with-errors" id="error_custom_no_b2_umum"></div>
                    </div>
            	</div>
            	<div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <select name="sifat" id="sifat" class="select2" style="width:100%" required data-error="Sifat surat belum diisi">
                                    <option></option>
                                    <?php 
                                        $resOpt = PdsB2Umum::findBySql("select distinct id, nama from ms_sifat_surat order by id")->asArray()->all();
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-sm-12">
                ...
                <a class="btn btn-success btn-sm" id="btn_popgeledahambil"><i class="fa fa-plus jarak-kanan"></i>Penggeledahan B-4 Umum</a>
            </div>		
        </div><br/>
        <div class="table-responsive">
            <table class="table table-bordered table-hover" id="tableGldh">
                ...
                <?php 
                    $sqlGeledah = "
                    select a.* from pidsus.pds_b2_umum_pengeledahan a 
                    where ".$whereDefault." and a.no_b2_umum = '".$model['no_b2_umum']."' order by a.no_urut_penggeledahan";
                    $resGeledah = ($model['no_b4_umum'])?PdsB2Umum::findBySql($sqlGeledah)->asArray()->all():array();
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-sm-12">
                ...
                <a class="btn btn-success btn-sm" id="btn_popsitaambil"><i class="fa fa-plus jarak-kanan"></i>Penyitaan B-4 Umum</a>
            </div>		
        </div><br/>
        <div class="table-responsive">
            <table class="table table-bordered table-hover" id="tableSita">
                ...
                <?php 
                    $sqlSita = "
                    select a.* from pidsus.pds_b2_umum_penyitaan a 
                    where ".$whereDefault." and a.no_b2_umum = '".$model['no_b2_umum']."' order by a.no_urut_penyitaan"; 
                    $resSita = ($model['no_b4_umum'])?PdsB2Umum::findBySql($sqlSita)->asArray()->all():array();
                    if(count($resSita) == 0)
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <?php
                        	if($model['no_b2_umum'] == ''){
                        		$sqlx = "select no_urut, tembusan from pidsus.ms_template_surat_tembusan where kode_template_surat = 'B-2-Umum' order by no_urut";
                        		$resx = PdsB2Umum::findBySql($sqlx)->asArray()->all();
                        	} else{
                        		$sqlx = "select no_urut as no_urut, tembusan from pidsus.pds_b2_umum_tembusan a 
                                                where ".$whereDefault." and a.no_b2_umum = '".$model['no_b2_umum']."' order by a.no_urut";
                        		$resx = PdsB2Umum::findBySql($sqlx)->asArray()->all();
                        	}
                        	$no = 1;
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['b2_umum'].$model['file_upload'];
                        $labelFile 	= 'Unggah B-2 Umum';
                        if($model['file_upload'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah B-2 Umum';
...
$(document).ready(function(){
	...
	$("#pilih_b4").on('click', function(e){
		...
		$("#b4_modal").find(".modal-body").load("/pidsus/pds-b2-umum/getb4");
	...
	$(".form-buat-penggeledahan").on("click", "#btn_popgeledah", function(){
                ...
                $("#tambah_gldh_modal").find(".modal-body").load("/pidsus/pds-b2-umum/getgldh",function(e){
		...
		$.post("/pidsus/pds-b2-umum/getgldh", objk, function(data){
			$("#tambah_gldh_modal").find(".modal-body").html("");
		...
		(n > 0)?$("#btn_hapusgeledah").removeClass("disabled"):$("#btn_hapusgeledah").addClass("disabled");
            ...
            var no_b4 = $("#no_b4_umum").val();
                if(no_b4==""){
                    bootbox.alert({message: "Nomor B-4 Umum Harus diisi!", size: 'small'});
                }else{
                    $("#tambah_gldh_modal").find(".modal-body").html("");
                    $("#tambah_gldh_modal").find(".modal-body").load("/pidsus/pds-b2-umum/listgldh?id1="+encodeURIComponent(no_b4));
                    $("#tambah_gldh_modal").modal({backdrop:"static"});
                }
	});
	...
	$("#tambah_gldh_modal").on('show.bs.modal', function(e){
		...
		if(arrnya['nurec_penggeledahan'] == 1){
			...
			$.post("/pidsus/pds-b2-umum/setgldh", frmnya, function(data){
				if(isNaN(rwNom)){
					rwTbl.remove();
	...
	$(".form-buat-penyitaan").on("click", "#btn_popsita", function(){
		...
		$("#tambah_sita_modal").find(".modal-body").load("/pidsus/pds-b2-umum/getsita",function(e){
		...
		objk.push({name: 'arr_id', value: trid});
		$.post("/pidsus/pds-b2-umum/getsita", objk, function(data){
			$("#tambah_sita_modal").find(".modal-body").html("");
		...
		tabel.find(".hRowCekSita:checked").each(function(k, v){
			...
			if(tabel.find("tr").length == 1){
				...
				nRow.append('<tr class="barisSitaan"><td colspan="5">Data tidak ditemukan</td></tr>');
		...
		(n > 0)?$("#btn_hapussita").removeClass("disabled"):$("#btn_hapussita").addClass("disabled");
            ...
            var no_b4 = $("#no_b4_umum").val();
                if(no_b4==""){
                    ...
                    $("#tambah_sita_modal").find(".modal-body").html("");
                    $("#tambah_sita_modal").find(".modal-body").load("/pidsus/pds-b2-umum/listsita?id1="+encodeURIComponent(no_b4));
                    $("#tambah_sita_modal").modal({backdrop:"static"});
                }
	});
	...
	$("#tambah_sita_modal").on('show.bs.modal', function(e){
		...
		if(arrnya['nurec_penyitaan'] == 1){
			...
			var newId 	= (isNaN(rwNom))?1:parseInt(rwNom + 1);
			frmnya.push({name:"arr_id", value:newId});
			$.post("/pidsus/pds-b2-umum/setsita", frmnya, function(data){
				if(isNaN(rwNom)){
					rwTbl.remove();
					rwTbl = tabel.find('tbody');
					rwTbl.append(data.hasil);
