			...
			<div class="col-md-10">
				<div class="input-group">
					...
					<div class="input-group-btn">
                        ...
                        <a href="/pidsus/pds-b2-umum/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-b2-umum/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_b2_umum']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal B-2 Umum', 
					'value'=>function($data){
						return $data['no_b2_umum'].'<br />'.date("d-m-Y", strtotime($data['tgl_dikeluarkan']));
					}, 
				],
				'3'=>[
					'headerOptions'=>['style'=>'width:45%', 'class'=>'text-center'],
...
$(document).ready(function(){
	...
	$("#idUbah").on("click", function(){
		...
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-b2-umum/update?id1="+tm[0];
	...
	$("#idCetak").on("click", function(){
		var id 	= $(".selection_one:checked").val();
		var tm 	= id.toString().split("#");
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-b2-umum/cetak?id1="+tm[0];
		$(location).attr("href", url);
	});
		...
		bootbox.confirm({
			...
			callback: function (result) {
				if(result){
					...
					$.ajax({
						type	: "POST",
						url	: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-b2-umum/hapusdata'; ?>",
						data	: { 'id' : id },
						cache	: false,
						dataType: "json",
