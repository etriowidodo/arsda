...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			$("body").removeClass("loading");
			$("#error_custom_no_p8_umum").html('<i style="color:#dd4b39; font-size:12px;">No P-8 Umum belum dipilih</i>');
			...
			$("body").removeClass("loading");
			$("#error_custom_tgl_dikeluarkan").html('<i style="color:#dd4b39; font-size:12px;">Maximal Tanggal dikeluarkan adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_dikeluarkan"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-18/cekpidsus18'; ?>',
