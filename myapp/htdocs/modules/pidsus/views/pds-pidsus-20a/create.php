...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_p9_umum  = new Date(tgl_auto($("#tgl_p9_umum").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
		} else if(tgl_p8_umum > tgl_p9_umum){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p9_umum").html('<i style="color:#dd4b39; font-size:12px;">Tanggal P-9 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_p9_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_p9_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_p9_umum").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal P-9 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_p9_umum"), $("#role-form"), false);
			return false;
		} else if(tgl_p9_umum > tgl_pemanggilan){
			$("body").removeClass("loading");
			$("#error_custom_tgl_pemanggilan").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pemanggilan lebih kecil dari tanggal P-9 Umum</i>');
			setErrorFocus($("#error_custom_tgl_pemanggilan"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p9-umum/cekpdsp9umum'; ?>',
