...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/ms-inst-pelak-penyidikan/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['kode_ip'])."|#|".rawurlencode($data['kode_ipp']);
			...
			'columns' => [
				...
				'2'=>[
					...
					'value'=>function($data){
						return $data['nama_inst_penyidik'];
					}, 
				],
				'3'=>[
					'headerOptions'=>['style'=>'width:25%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Instansi Pelaksana Penyidik', 
					'value'=>function($data){
						return $data['nama'];
					}, 
				],
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/ms-inst-pelak-penyidikan/update?kode_ip="+tm[0]+"&kode_ipp="+tm[1];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url	: "<?php echo Yii::$app->request->baseUrl.'/pidsus/ms-inst-pelak-penyidikan/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
