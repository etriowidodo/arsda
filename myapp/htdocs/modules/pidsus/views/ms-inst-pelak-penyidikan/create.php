<?php
	...
	if($model->isNewRecord){
		$this->title = 'Tambah Instansi Pelaksana Penyidik';
		$isNewRecord = 1;
		$model 	= array();
	} else{
		$this->title = 'Ubah Instansi Pelaksana Penyidik';
		$isNewRecord = 0;
	}
?>
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		if(!e.isDefaultPrevented()){
			...
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/ms-inst-pelak-penyidikan/cekttdjabatan'; ?>',
				...
				success : function(data){ 
					if(data.hasil){
						...
						setErrorFocus($("#kode_ipp"), $("#role-form"), true);
					} else{
