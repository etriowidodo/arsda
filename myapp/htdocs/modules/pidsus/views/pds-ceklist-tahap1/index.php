<?php
	...
	$this->title = 'Daftar CekList Berkas';
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                	<div class="text-right">
                    	<a class="btn btn-warning btn-sm disabled jarak-kanan" id="btn_ctkceklist">Cetak Ceklist</a>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		function showPengantar(param){
			$("body").addClass("loading");
			$.post("/pidsus/pds-ceklist-tahap1/getpengantar", {id1:param}, function(data){
		...
		$(".form-surat-pengantar").on("ifChecked", ".pilihPengantar", function(){
			...
			(n == 1)?$("#btn_ctkceklist, #btn_isipendapat").removeClass("disabled"):$("#btn_ctkceklist, #btn_isipendapat").addClass("disabled");
		}).on("click", "#btn_isipendapat", function(){
			$("body").addClass("loading");
			var id 	= $(".pilihPengantar:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ceklist-tahap1/create?id1="+tm[0]+"&id2="+tm[1];
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ceklist-tahap1/create?id1="+tm[0]+"&id2="+tm[1];
			$(location).attr("href", url);
		}).on("click", "#btn_ctkceklist", function(){
			var id 	= $(".pilihPengantar:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-ceklist-tahap1/cetak?id1="+tm[1];
			$(location).attr("href", url);
		});
