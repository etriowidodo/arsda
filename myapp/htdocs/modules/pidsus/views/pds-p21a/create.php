...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_p21 	= new Date(tgl_auto($("#tgl_p21").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			setErrorFocus($("#error_custom_file_template"), $("#role-form"), false);
			return false;
		} else if(tgldittd < tgl_p21){
			$("body").removeClass("loading");
			$("#error_custom_tgldittd").html('<i style="color:#dd4b39; font-size:12px;">Tanggal dikeluarkan tidak boleh lebih kecil daripada tanggal P-21</i>');
			...
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p21a/cekp21a'; ?>',
