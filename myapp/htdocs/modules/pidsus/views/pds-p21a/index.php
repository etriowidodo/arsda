	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_berkas'])."#".rawurlencode($data['no_pengantar']);
			...
			'columns' => [
				'1'=>[
					...
					'headerOptions'=>['style'=>'width:22%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal P21', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_p21'].'</p>'.date("d-m-Y", strtotime($data['tgl_p21']));
					}, 
				],
				'3'=>[
					'headerOptions'=>['style'=>'width:22%', 'class'=>'text-center'],
				...
				'4'=>[
					'headerOptions'=>['style'=>'width:22%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal P21A', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_p21a'].'</p>'.($data['tgl_p21a']?$data['tgl_p21a']:'');
					}, 
				],
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p21a/create?id1="+tm[0]+"&id2="+tm[1];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-p21a/cetak?id1="+tm[0]+"&id2="+tm[1];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-p21a/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
