			...
			<div class="col-md-10">
				<div class="input-group">
					...
					<div class="input-group-btn">
                        ...
                        <a href="/pidsus/pds-pengembalian-spdp/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-pengembalian-spdp/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
			...
			'columns' => [
				...
				'3'=>[
					...
					'value'=>function($data){
						...
						if($arrTmp[0] && $arrTmp[1]) $waktu = 'Jam '.$arrTmp[0].':'.$arrTmp[1];
				...
				'4'=>[
					...
					'header'=>'Tersangka', 
					'value'=>function($data){
						return ($data['tersangka_berkas'])?$data['tersangka_berkas']:$data['tersangka'];
					}, 
				],
				'5'=>[
					'headerOptions'=>['style'=>'width:20%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'No dan Tanggal Pengembalian SPDP', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_spdp_kembali'].'</p>'.date("d-m-Y", strtotime($data['tgl_spdp_kembali']));
					}, 
				],                
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-pengembalian-spdp/update?no_spdp="+tm[0]+"&tgl_spdp="+tm[1];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pengembalian-spdp/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
