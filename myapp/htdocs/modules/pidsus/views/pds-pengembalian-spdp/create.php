	...
	function validasi_upload(){
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
                        ...
                        $("body").removeClass("loading");
                        $("#error_custom_no_spdp").html('<i style="color:#dd4b39; font-size:12px;">Pilih SPDP terlebih dahulu</i>');
                        setErrorFocus($("#no_spdp"), $("#role-form"), false);
                        return false;
			...
			return false;
		} else if(tgl_terima > tgldittd){
			$("body").removeClass("loading");
			$("#error_custom_tgldittd").html('<i style="color:#dd4b39; font-size:12px;">Tanggal dikeluarkan lebih kecil dari tanggal terima SPDP</i>');
			...
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pengembalian-spdp/cekspdpkembali'; ?>',
