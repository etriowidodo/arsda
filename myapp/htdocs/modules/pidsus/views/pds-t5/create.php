...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		var tgl_resume 	= new Date(tgl_auto($("#tgl_resume").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			return false;
                } else if(tgl_minta_perpanjang > tgl_resume){
			$("body").removeClass("loading");
			$("#error_custom_tglresume").html('<i style="color:#dd4b39; font-size:12px;">Tanggal resume lebih kecil dari tanggal permintaan perpanjangan : '+$("#tgl_minta_perpanjang").val()+'</i>');
			setErrorFocus($("#tgl_resume"), $("#role-form"), false);
			return false;
                } else if(tgl_resume > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tglresume").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal resume adalah hari ini</i>');
			setErrorFocus($("#tgl_resume"), $("#role-form"), false);
			return false;
			...
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-t5/cek-t5'; ?>',
