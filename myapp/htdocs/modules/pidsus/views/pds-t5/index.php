...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-t5/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_minta_perpanjang']).'#'.rawurlencode($data['no_t5']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal Surat Penolakan', 
					'value'=>function($data){
						return '<p style="margin-bottom:5px;">'.$data['no_t5'].'</p>'.date("d-m-Y", strtotime($data['tgl_dikeluarkan']));
					}, 
				],
				'2'=>[
					...
					'header'=>'Kepada', 
					'value'=>function($data){
						return $data['kepada'].'<br />'.$data['di_kepada'];
					}, 
				],
				'3'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-t5/update?id1="+tm[0]+"&id2="+tm[1];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-t5/cetak?id1="+tm[0]+"&id2="+tm[1];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-t5/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
