...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			if($("input[name='jpnid[]']").length == 0){
				bootbox.confirm({ 
					message: "Tabel Jaksa Penelaah masih kosong. Apakah anda ingin tetap menyimpan data?",
					size: "small",
					closeButton: false,
					buttons: {
	...
	function validasi_upload(){
		...
		var tgl_p9_umum  = new Date(tgl_auto($("#tgl_dikeluarkan").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			$("body").removeClass("loading");
			$("#error_custom_tgl_dikeluarkan").html('<i style="color:#dd4b39; font-size:12px;">Tanggal Pidsus-6 Umum lebih kecil dari tanggal P-8 Umum</i>');
			setErrorFocus($("#error_custom_tgl_dikeluarkan"), $("#role-form"), false);
			return false;
		} else if(tgl_p9_umum > hariIni){
			$("body").removeClass("loading");
			$("#error_custom_tgl_dikeluarkan").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal Pidsus-6 Umum adalah hari ini</i>');
			setErrorFocus($("#error_custom_tgl_dikeluarkan"), $("#role-form"), false);
			return false;
		} else{
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pidsus-6-umum/cekpdspidsus6umum'; ?>',
