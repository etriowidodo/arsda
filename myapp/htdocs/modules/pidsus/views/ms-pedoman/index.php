...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/ms-pedoman/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['id'])."|#|".rawurlencode($data['id_pasal'])."|#|".rawurlencode($data['kategori']);
			...
			'columns' => [
				...
				'3'=>[
					...
					'value'=>function($data){
                                                $kategori="";
                                                if($data['kategori']=="1"){$kategori='I (Tidak ada hal yang meringankan)';}
                                                else if($data['kategori']=="2"){$kategori='II (Hal yang memberatkan lebih dominan tetapi ada hal yang meringankan)';}
                                                else if($data['kategori']=="3"){$kategori='III (Antara hal yang memberatkan dan meringankan sebanding)';}
                                                else if($data['kategori']=="4"){$kategori='IV (Lebih dominan hal yang meringankan tetapi ada hal yang memberatkan)';}
                                                else if($data['kategori']=="5"){$kategori='V (Tidak ada hal yang memberatkan)';}
						return $kategori;
					}, 
				],
                                '6'=>[
					...
					'headerOptions'=>['style'=>'width:15%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Tuntutan Pidana', 
					'value'=>function($data){
						return $data['tuntutan_pidana'];
					}, 
				],
				'aksi'=>[
					'headerOptions'=>['style'=>'width:5%', 'class'=>'text-center'],
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/ms-pedoman/update?id="+tm[0]+"&id2="+tm[1]+"&id3="+tm[2];
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url	: "<?php echo Yii::$app->request->baseUrl.'/pidsus/ms-pedoman/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
