...
<?php ActiveForm::begin(['action'=>['index'], 'method'=>'get', 'id'=>'searchModalTtd', 'options'=>['name'=>'searchModalTtd', 'class'=>'form-horizontal']]); ?>
<div class="row">
	<div class="col-md-2">
		...
		<div class="form-group">
			<div class="col-md-12">
				<select id="mttd_q3" name="mttd_q3" style="width:100%;">
					<option></option>
					<?php 
						$sqlnya = "select kode, deskripsi from pidsus.ms_penandatangan where id_kejati = '".$_SESSION['kode_kejati']."' 
								and id_kejari = '".$_SESSION['kode_kejari']."' and id_cabjari = '".$_SESSION['kode_cabjari']."' order by kode";
						$resOpt = GetTtd::findBySql($sqlnya)->asArray()->all();
				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn btn-warning" name="btnSearch" id="btnSearch">Cari</button>
                                                <a class="btn btn-info btn-flat reset-cari-ttd" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="modal-ttd-index">
	...
	<?php
		...
		Pjax::end();
    ?>
		<p style="margin-bottom:5px;" class="text-center"><a class="btn btn-warning btn-sm disabled" id="idPilihTtdModal"><i class="fa fa-table jarak-kanan"></i>Pilih</a></p>
    </div>
...
$(document).ready(function(){
	$("#mttd_q2").select2({placeholder: "Pilih Status", allowClear: true});
	$("#mttd_q3").select2({placeholder: "Pilih Jabatan", allowClear: true});
	...
	$(".modal-ttd-index").on("ifChecked", ".pilih-ttd-modal", function(){
        ...
        $(".reset-cari-ttd").on("click", function(){
		$("#mttd_q1").val("");
                $("#mttd_q2").val("").trigger("change");
                $("#mttd_q3").val("").trigger("change");
		$("#searchModalTtd").trigger("submit");
	});
});
</script>