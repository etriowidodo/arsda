<?php
	...
	if($model->isNewRecord){
		$this->title = 'Tambah Undang Undang';
		$isNewRecord = 1;
		$model 	= array();
	} else{
		$this->title = 'Ubah Undang Undang';
                $model['tanggal']= ($model['tanggal'])?date("d-m-Y", strtotime($model['tanggal'])):"";
		$isNewRecord = 0;
	}
?>
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		if(!e.isDefaultPrevented()){
			...
			$.ajax({
				type	: "POST",
				url	: '<?php echo Yii::$app->request->baseUrl.'/pidsus/ms-u-undang/cekttdjabatan'; ?>',
