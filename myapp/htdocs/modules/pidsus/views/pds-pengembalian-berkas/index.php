...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-pengembalian-berkas/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_spdp'])."#".rawurlencode($data['tgl_spdp'])."#".rawurlencode($data['no_berkas']);
			...
			'columns' => [
					...
					'value'=>function($data){
						...
						if(count($temp) > 0){
							foreach($temp as $idx=>$res){
								$isi = explode("--", $res);
								if($isi[0] && $isi[1]){
									$text .= '<div style="margin-bottom: 0px; width: 100%; display: table;">';
				...
				'3'=>[
					'headerOptions'=>['style'=>'width:25%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal Pengembalian Berkas', 
					'value'=>function($data){
						return '<p style="margin-bottom:0px;">'.$data['no_surat'].'</p>'.date("d-m-Y", strtotime($data['tgl_dikeluarkan']));
					}, 
				],
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-pengembalian-berkas/update?id1="+tm[0]+"&id2="+tm[1]+"&id3="+tm[2];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-pengembalian-berkas/cetak?id1="+tm[0]+"&id2="+tm[1]+"&id3="+tm[2];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-pengembalian-berkas/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
