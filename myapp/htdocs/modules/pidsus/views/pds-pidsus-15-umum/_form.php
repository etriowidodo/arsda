<?php
	...
	$linkBatal		= '/pidsus/pds-pidsus-15-umum/index';
	$linkCetak		= '/pidsus/pds-pidsus-15-umum/cetak?id1='.$model['no_pidsus15_umum'];
	...
	if($isNewRecord){
		...
		$model 	= PdsPidsus15Umum::findBySql($sqlCek)->asArray()->one();
	}

	$tgl_p8_umum = ($model['tgl_p8_umum'])?date('d-m-Y',strtotime($model['tgl_p8_umum'])):'';
	$tgl_pidsus15_umum 	= ($model['tgl_pidsus15_umum'])?date('d-m-Y',strtotime($model['tgl_pidsus15_umum'])):'';
	$model['perihal'] 	= ($model['perihal'])?$model['perihal']:'';
?>

<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-pidsus-15-umum/simpan" enctype="multipart/form-data">
...
<div class="box box-primary">
    <div class="box-body">
        <div class="row">
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Nomor Pidsus-15 Umum</label>        
                    <div class="col-md-8">
                        <input type="text" name="no_pidsus15_umum" id="no_pidsus15_umum" class="form-control" value="<?php echo $model['no_pidsus15_umum'];?>" required data-error="Nomor Pidsus-15 Umum belum diisi" />
                        <div class="help-block with-errors" id="error_custom_no_pidsus15_umum"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group form-group-sm">
                    <label class="control-label col-md-4">Tanggal Pidsus-15 Umum</label>        
                    <div class="col-md-4">
                        <div class="input-group">
                            <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                            <input type="text" name="tgl_pidsus15_umum" id="tgl_pidsus15_umum" class="form-control datepicker" value="<?php echo $tgl_pidsus15_umum;?>" required data-error="Tanggal Pidsus-15 Umum belum diisi" />
                        </div>
                        <div class="help-block with-errors" id="error_custom_tgl_pidsus15_umum"></div>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <select name="sifat" id="sifat" class="select2" style="width:100%" required data-error="Sifat surat belum diisi">
                                    <option></option>
                                    <?php 
                                        $resOpt = PdsPidsus15Umum::findBySql("select distinct id, nama from ms_sifat_surat order by id")->asArray()->all();
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <tbody>
                        <?php
                        	if($model['no_pidsus15_umum'] == ''){
                        		$sqlx = "select no_urut, tembusan from pidsus.ms_template_surat_tembusan where kode_template_surat = 'Pidsus-15-Umum' order by no_urut";
                        		$resx = PdsPidsus15Umum::findBySql($sqlx)->asArray()->all();
                        	} else{
                        		$sqlx = "select no_urut as no_urut, tembusan from pidsus.pds_pidsus15_umum_tembusan a 
										where ".$whereDefault." and a.no_pidsus15_umum = '".$model['no_pidsus15_umum']."' order by a.no_urut";
                        		$resx = PdsPidsus15Umum::findBySql($sqlx)->asArray()->all();
                        	}
                        	$no = 1;
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['pidsus_15umum'].$model['file_upload'];
                        $labelFile 	= 'Unggah Pidsus P-15 Umum';
                        if($model['file_upload_p16'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah Pidsus P-15 Umum';
...
<div class="modal fade" id="pidsus14_umum_modal" role="dialog" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" style="width:1100px;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Daftar Pidsus-14 Umum</h4>
            </div>
            <div class="modal-body"></div>
        </div>
    </div>
...
$(document).ready(function(){
	...
	$("#pilih_saksi").on('click', function(e){
		...
		if(keperluan){			
			...
			$("#saksi_modal").find(".modal-body").load("/pidsus/pds-pidsus-15-umum/getsaksi?keperluan="+encodeURIComponent(keperluan), function(){
	...
	}).on("dblclick", "#tabel-pidsus14-umum-modal td:not(.aksinya)", function(){
		var index = $(this).closest("tr").data("id");
		var param = index.toString().split('|#|');
		insertSaksi(param);
		$("#saksi_modal").modal("hide");
	}).on('click', "#idPilihPIDSUS14UModal", function(){
		var modal = $("#saksi_modal").find("#tabel-pidsus14-umum-modal");
		var index = modal.find(".pilih-pidsus14-umum-modal:checked").val();
		var param = index.toString().split('|#|');
		insertSaksi(param);
		$("#saksi_modal").modal("hide");
	...
	$("#pilih_pidsus14_umum").on('click', function(e){
		$("#pidsus14_umum_modal").find(".modal-body").html("");
		$("#pidsus14_umum_modal").find(".modal-body").load("/pidsus/pds-pidsus-15-umum/getpidsus14umum");
		$("#pidsus14_umum_modal").modal({backdrop:"static"});
	});
	$("#pidsus14_umum_modal").on('show.bs.modal', function(e){
		$("body").addClass("loading");
	}).on('shown.bs.modal', function(e){
		$("body").removeClass("loading");
	}).on("dblclick", "#tabel-pidsus14-umum-modal td:not(.aksinya)", function(){
		var index = $(this).closest("tr").data("id");
		var param = index.toString().split('|#|');
		$("#no_urut_pidsus14_umum").val(decodeURIComponent(param[0]));
		$("#tgl_pidsus14_umum").val(decodeURIComponent(param[1]));
		$("#pidsus14_umum_modal").modal("hide");
	}).on('click', "#idPilihPIDSUS14UModal", function(){
		var modal = $("#pidsus14_umum_modal").find("#tabel-pidsus14-umum-modal");
		var index = modal.find(".pilih-pidsus14-umum-modal:checked").val();
		var param = index.toString().split('|#|');
		$("#no_urut_pidsus14_umum").val(decodeURIComponent(param[0]));
		$("#tgl_pidsus14_umum").val(decodeURIComponent(param[1]));
