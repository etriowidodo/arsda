			...
			<div class="col-md-10">
				<div class="input-group">
					...
					<div class="input-group-btn">
                        ...
                        <a href="/pidsus/pds-b4-umum/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-b4-umum/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
		...
		echo GridView::widget([
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal B-4 Umum', 
					'value'=>function($data){
						return $data['no_b4_umum'].'<br />'.date("d-m-Y", strtotime($data['tgl_dikeluarkan']));
					}, 
				],
				'2'=>[
					'headerOptions'=>['style'=>'width:24%', 'class'=>'text-center'],
					'format'=>'raw',
					'header'=>'Jaksa Pelaksana', 
					'value'=>function($data){
						$temp = explode('#', $data['jpunya']);
						$text = '';
						if(count($temp) > 0 && count($temp) == 1){
							$isian = explode("--", $temp[0]);
							$text .= '<p style="margin-bottom:0px;">'.$isian[0].'<br />'.$isian[1].'</p>';
						} else if(count($temp) > 0 && count($temp) != 1){
							foreach($temp as $idx=>$res){
								$nom = $idx+1;
								$isian = explode("--", $res);
								$text .= '<div style="margin-bottom: 5px; width: 100%; display: table;">';
								$text .= '<div style="display: table-cell; width: 23px;">'.$nom.'.</div><div style="display: table-cell;">'.$isian[0].'<br />'.$isian[1].'</div>';
								$text .= '</div>';
							}
						}
						return $text;
				...
				'3'=>[
					...
					'format'=>'raw',
					'header'=>'Penggeledahan', 
					'value'=>function($data){
						$temp = explode('#', $data['geledahan']);
						$text = '';
						if(count($temp) > 0 && count($temp) == 1){
							$arrtm = explode("--", $temp[0]);
							$isian = ($arrtm[0] == 'Subyek')?$arrtm[1].'<br />'.$arrtm[2]:$arrtm[3].'<br />'.$arrtm[4];
							$text .= '<p style="margin-bottom:0px;">'.$isian.'</p>';
						} else if(count($temp) > 0 && count($temp) != 1){
							foreach($temp as $idx=>$res){
								$nom = $idx+1;
								$arrtm = explode("--", $res);
								...
								<div style="margin-bottom: 5px; width: 100%; display: table;">
									...
									<div style="display: table-cell;">'.$isian.'</div>
								</div>';
							}
						}
						return $text;
				...
				'4'=>[
					...
					'value'=>function($data){
						...
						if(count($temp) > 0 && count($temp) == 1){
							$isian = explode("--", $temp[0]);
							$text .= '<p style="margin-bottom:0px;">'.$isian[0].'<br />jenis : '.$isian[1].'<br />jumlah : '.$isian[2].'</p>';
						} else if(count($temp) > 0 && count($temp) != 1){
							foreach($temp as $idx=>$res){
								$nom = $idx+1;
								$isian = explode("--", $res);
								...
								<div style="margin-bottom: 5px; width: 100%; display: table;">
									<div style="display: table-cell; width: 23px;">'.$nom.'.</div>
									<div style="display: table-cell;">'.$isian[0].'<br />jenis : '.$isian[1].'<br />jumlah : '.$isian[2].'</div>
								</div>';
							}
						}
						return $text;
...
$(document).ready(function(){
	...
	$("#idUbah").on("click", function(){
		...
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-b4-umum/update?id1="+tm[0];
	...
	$("#idCetak").on("click", function(){
		var id 	= $(".selection_one:checked").val();
		var tm 	= id.toString().split("#");
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-b4-umum/cetak?id1="+tm[0];
		$(location).attr("href", url);
	});
		...
		bootbox.confirm({
			...
			callback: function (result) {
				if(result){
					...
					$.ajax({
						type	: "POST",
						url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-b4-umum/hapusdata'; ?>",
						data	: { 'id' : id },
						cache	: false,
						dataType: "json",
