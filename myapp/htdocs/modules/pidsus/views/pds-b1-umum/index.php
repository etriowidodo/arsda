				...
				<div class="input-group">
					...
					<div class="input-group-btn">
						<button type="submit" class="btn" name="btnSearch" id="btnSearch"><i class="fa fa-search jarak-kanan"></i>Cari</button>
                                                <a href="/pidsus/pds-b1-umum/index" class="btn btn-info btn-flat reset-cari" style="margin-left:10px;">Reset Pencarian</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="role-index">
    <div class="inline"><a class="btn btn-pidsus" href="/pidsus/pds-b1-umum/create"><i class="fa fa-plus jarak-kanan"></i>Tambah</a></div>
	...
	<?php
		...
		echo GridView::widget([
			...
			'rowOptions' => function($data){
				$idnya = rawurlencode($data['no_b1_umum']);
			...
			'columns' => [
				...
				'1'=>[
					...
					'format'=>'raw',
					'header'=>'Nomor dan Tanggal B-1 Umum', 
					'value'=>function($data){
						return $data['no_b1_umum'].'<br />'.date("d-m-Y", strtotime($data['tgl_dikeluarkan']));
					}, 
				],
                                '2'=>[
					...
					'header'=>'Benda/Barang', 
					'value'=>function($data){
						return 'Benda/Barang berupa : '.$data['berupa_barang'].
                                                        "<br />Diduga telah : ".$data['diduga_telah'].
                                                        "<br />Disimpan di : ".$data['simpan_di'].
                                                        "<br />Dikuasai oleh : ".$data['dikuasai'];
					}, 
				],
				'3'=>[
					'headerOptions'=>['style'=>'width:24%', 'class'=>'text-center'],
...
$(document).ready(function(){
	...
	$("#idUbah").on("click", function(){
		...
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-b1-umum/update?id1="+tm[0];
	...
	$("#idCetak").on("click", function(){
		var id 	= $(".selection_one:checked").val();
		var tm 	= id.toString().split("#");
		var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-b1-umum/cetak?id1="+tm[0];
		$(location).attr("href", url);
	});
		...
		bootbox.confirm({
			...
			callback: function (result) {
				if(result){
					...
					$.ajax({
						type	: "POST",
						url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-b1-umum/hapusdata'; ?>",
						data	: { 'id' : id },
						cache	: false,
						dataType: "json",
