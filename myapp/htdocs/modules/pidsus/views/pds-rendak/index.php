    ...
    <div class="row ">
        <div class="col-md-12">
            <div class="pull-right">
                <a class="btn btn-warning disabled" id="idCetak"><i class="fa fa-print jarak-kanan"></i>Unduh Blanko Rendak</a>&nbsp;
		...
		echo GridView::widget([
			...
			'columns' => [
				...
				'3'=>[
					...
					'format'=>'raw',
					'header'=>'Tanggal Rendak', 
					'value'=>function($data){
						return ($data['tgl_rendak']?$data['tgl_rendak']:'');
					}, 
				],
				'aksi'=>[
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#idUbah").on("click", function(){
			...
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-rendak/create?id1="+tm[0];
			...
			var id 	= $(".selection_one:checked").val();
			var tm 	= id.toString().split("#");
			var url = window.location.protocol + "//" + window.location.host + "/pidsus/pds-rendak/cetak?id1="+tm[0];
			$(location).attr("href", url);
		});
			...
			bootbox.confirm({
				...
				callback: function (result) {
					if(result){
						...
						$.ajax({
							type	: "POST",
							url		: "<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-rendak/hapusdata'; ?>",
							data	: { 'id' : id },
							cache	: false,
							dataType: "json",
