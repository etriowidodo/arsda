<?php
	...
	$linkBatal		= '/pidsus/pds-rendak/index';
	$linkCetak		= '/pidsus/pds-rendak/cetak?id1='.$model['no_berkas'];
...
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/pidsus/pds-rendak/simpan" enctype="multipart/form-data">
...
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title">Rencana Dakwaan</h3>
    </div>
    <div class="box-body">
        <div class="row">
            <div class="col-md-12">
                <div class="form-group form-group-sm">
                    <div class="col-md-12">
                        <textarea name="dakwaan" id="dakwaan" class="ckeditor"><?php echo $model['dakwaan']; ?></textarea>						
                    	<div class="help-block with-errors"></div>
                    </div>
            	</div>
            </div>
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="table_tembusan" class="table table-bordered">
                        ...
                        <?php
                        	if($model['id_rendak'] == ''){
                        		$sqlx = "select no_urut, tembusan from pidsus.ms_template_surat_tembusan where kode_template_surat = 'Rencana-Dakwaan' order by no_urut";
                        		$resx = PdsP21::findBySql($sqlx)->asArray()->all();
                        	} else{
                        		$sqlx = "select a.no_urut as no_urut, a.tembusan from pidsus.pds_rendak_tembusan a
        ...
        <div class="form-group form-group-sm">
            <div class="col-md-12">
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['rendak'].$model['file_upload_rendak'];
                        $labelFile 	= 'Unggah File Rendak';
                        if($model['file_upload_rendak'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah File Rendak';
                            $param1  	= chunk_split(base64_encode($pathFile));
                            $param2  	= chunk_split(base64_encode($model['file_upload_rendak']));
                            $linkPt 	= "/pidsus/download-file/index?id=".$param1."&fn=".$param2;
                            $extPt		= substr($model['file_upload_rendak'], strrpos($model['file_upload_rendak'],'.'));
                            echo '<a href="'.$linkPt.'" title="'.$model['file_upload_rendak'].'" style="float:left; margin-right:10px;">
