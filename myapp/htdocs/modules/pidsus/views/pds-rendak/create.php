<?php
	...
	$isNewRecord = (!$model['id_rendak'])?1:0;
?>
...
$(document).ready(function(){
	...
	function validasi_upload(){
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			setErrorFocus($("#error_custom_file_template"), $("#role-form"), false);
			return false;
		} else if(tgldittd < tgl_ba){
			$("body").removeClass("loading");
			$("#error_custom_tgldittd").html('<i style="color:#dd4b39; font-size:12px;">Tanggal dikeluarkan tidak boleh lebih kecil daripada tanggal P-24 ('+tgl_ba_txt+')</i>');
			...
			$.ajax({
				type	: "POST",
				url		: '<?php echo Yii::$app->request->baseUrl.'/pidsus/pds-rendak/cekrendak'; ?>',
