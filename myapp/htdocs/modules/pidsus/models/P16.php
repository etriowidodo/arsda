...
class P16 extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		where a.id_kejati = '".$id_kejati."' and a.id_kejari = '".$id_kejari."' and a.id_cabjari = '".$id_cabjari."' and a.no_spdp = '".$no_spdp."' 
			and a.tgl_spdp = '".$tgl_spdp."'";
                if($q1)
                $sql .= " and (upper(a.no_p16) like '%".strtoupper($q1)."%' or to_char(a.tgl_dikeluarkan, 'DD-MM-YYYY') = '".$q1."' 
    ...
    public function cekP16($post){
		...
		$count2 	= ($isNew)?$connection->createCommand($sqlCek2)->queryScalar():0;
                
		if($count1 > 0){
			$pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>No P16 dengan nomor '.$nom_p16.' sudah ada</i></p>';
	...
	public function simpanData($post){
		...
		try{
			if($isNew){
				...
				$sql1 = "update pidsus.pds_p16 set no_p16 = '".$no_p16."', tgl_dikeluarkan = '".$helpernya->tgl_db($tgl_dikeluarkan)."', penandatangan_nama = '".$penandatangan_nama."', 
						 ...
						 where ".$whereDef." and no_p16 = '".$sess_p16."'";
			}
