...
class PdsP10Khusus extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "select a.* from pidsus.pds_p10_khusus a 
		...
		if($q1)
			$sql .= " and (upper(a.no_p10_khusus) like '%".strtoupper($q1)."%' or to_char(a.tgl_p10_khusus, 'DD-MM-YYYY') = '".$q1."' 
				or upper(a.kepada_nama) like '%".strtoupper($q1)."%' or upper(a.keterangan_ahli) like '%".strtoupper($q1)."%')";

		$kueri = $this->db->createCommand("SELECT COUNT(*) FROM (".$sql.") a");
		$count = $kueri->queryScalar();
		$sql .= " order by a.tgl_p10_khusus desc, a.created_date desc";
    ...
    public function cekPdsP10Khusus($post){
        $connection  = $this->db;
        $id_kejati	= $_SESSION['kode_kejati'];
        $id_kejari	= $_SESSION['kode_kejari'];
        $id_cabjari	= $_SESSION['kode_cabjari'];
        ...
        $no_p10_khusus = htmlspecialchars($post['no_p10_khusus'], ENT_QUOTES);
        $whereDef = "id_kejati = '".$id_kejati."' and id_kejari = '".$id_kejari."' and id_cabjari = '".$id_cabjari."' and no_p10_khusus = '".$no_p10_khusus."'";
        
        $sql 	= "select count(*) from pidsus.pds_p10_khusus where ".$whereDef;
		if($isNew){
			$count 	= $connection->createCommand($sql)->queryScalar();
		} else{
			$id1 	= $_SESSION["pidsus_no_p10_khusus"];
			$count 	= ($id1 == $no_p10_khusus)?0:$connection->createCommand($sql)->queryScalar();
		}
        
        if($count > 0){
            $pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>P-10 Khusus dengan nomor '.$no_p10_khusus.' sudah ada</i></p>';
            return array("hasil"=>false, "error"=>$pesan, "element"=>"error_custom_no_p10_khusus");
        } else{
	...
	public function simpanData($post){
		...
		$no_p10_khusus      = htmlspecialchars($post['no_p10_khusus'], ENT_QUOTES);
		$tgl_p10_khusus     = htmlspecialchars($post['tgl_p10_khusus'], ENT_QUOTES);
		...
		$pathfile	= Yii::$app->params['p10_khusus'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_khusus);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_p10_khusus);
		$newPhoto 	= "p10khusus_".$clean1."-".$clean2.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_p10_khusus(id_kejati, id_kejari, id_cabjari, no_pidsus18, no_p8_khusus, no_p10_khusus, tgl_p10_khusus, sifat, lampiran, kepada_nama, di_tempat, 
						 ...
						 updated_nip, updated_nama, updated_ip, updated_date) values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_pidsus18."', '".$no_p8_khusus."', '".$no_p10_khusus."', 
						 '".$helpernya->tgl_db($tgl_p10_khusus)."', '".$sifat."', '".$lampiran."', '".$kepada_nama."', '".$di_tempat."', '".$keterangan_ahli."', 
				...
				$sql1 = "update pidsus.pds_p10_khusus set no_p10_khusus = '".$no_p10_khusus."', tgl_p10_khusus = '".$helpernya->tgl_db($tgl_p10_khusus)."', 
						 ...
						 where ".$whereDef." and no_p10_khusus = '".$_SESSION['pidsus_no_p10_khusus']."'";
			}
			$connection->createCommand($sql1)->execute();

			$sql4 = "delete from pidsus.pds_p10_khusus_tembusan where ".$whereDef." and no_p10_khusus = '".$no_p10_khusus."'";
			...
			if(count($post['nama_tembusan']) > 0){
				...
				foreach($post['nama_tembusan'] as $idx=>$val){
					...
					if($nama_tembusan){ 
						$noauto++; 
						$sql5 = "insert into pidsus.pds_p10_khusus_tembusan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_pidsus18."', '".$no_p8_khusus."', 
                                                        '".$no_p10_khusus."', '".$noauto."', '".$nama_tembusan."')";
			...
			if($upl1){
				$tmpPot = glob($pathfile."p10khusus_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload from pidsus.pds_p10_khusus where ".$whereDefault." and no_p10_khusus = '".rawurldecode($tmp[0])."'";
					$file = $connection->createCommand($kue)->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from pidsus.pds_p10_khusus where ".$whereDefault." and no_p10_khusus = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
