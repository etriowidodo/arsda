...
class PdsRendak extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "
			...
			select distinct a.id_kejati, a.id_kejari, a.id_cabjari, a.no_spdp, a.tgl_spdp, a.no_berkas, a.tgl_berkas, c.nama_tersangka, e.no_pengantar, 
			to_char(f.tgl_dikeluarkan, 'DD-MM-YYYY') as tgl_rendak  
    ...
    public function cekRendak($post){
		$connection = $this->db;
		...
		$sqlCek1 	= "select count(*) from pidsus.pds_rendak where ".$where." and no_berkas = '".$nom_p16."'";
		$count1 	= ($isNew)?$connection->createCommand($sqlCek1)->queryScalar():0;

		if($count1 > 0){
			$pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>Maaf 1 berkas hanya ada 1 Rendak</i></p>';
	...
	public function simpanData($post){
		...
		$dakwaan			= str_replace(array("'"), array("&#039;"), $post['dakwaan']);
		...
		$pathfile	= Yii::$app->params['rendak'];
		...
		$newPhoto 	= "rendak_".$clean1."-".$clean2."-".$clean3.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_rendak(id_kejati, id_kejari, id_cabjari, no_spdp, tgl_spdp, no_berkas, dakwaan, tgl_dikeluarkan, dikeluarkan, 
						 penandatangan_nama, penandatangan_nip, penandatangan_jabatan_pejabat, penandatangan_gol, penandatangan_pangkat, 
						 penandatangan_status_ttd, penandatangan_jabatan_ttd, file_upload_rendak, created_user, created_nip, created_nama, created_ip, created_date, 
						 updated_user, updated_nip, updated_nama, updated_ip, updated_date) values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_spdp."', 
						 '".$tgl_spdp."', '".$no_berkas."', '".$dakwaan."', '".$helpernya->tgl_db($tgl_dikeluarkan)."', '".$dikeluarkan."', 
				...
				if($filePhoto != ""){
					$upl1 = true;
					$filenya = ", file_upload_rendak = '".$newPhoto."'";
				} else{
					$upl1 = false;
					$filenya = "";
				}
				
				$sql1 = "update pidsus.pds_rendak set tgl_dikeluarkan = '".$helpernya->tgl_db($tgl_dikeluarkan)."', dakwaan = '".$dakwaan."', dikeluarkan = '".$dikeluarkan."', 
			...
			$sql4 = "delete from pidsus.pds_rendak_tembusan where ".$whereDef." and no_berkas = '".$no_berkas."'";
			...
			if(count($post['nama_tembusan']) > 0){
				...
				foreach($post['nama_tembusan'] as $idx=>$val){
					...
					if($nama_tembusan){ 
						$noauto++; 
						$sql5 = "insert into pidsus.pds_rendak_tembusan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_spdp."', '".$tgl_spdp."', 
			...
			if($upl1){
				$tmpPot = glob($pathfile."rendak_".$clean1."-".$clean2."-".$clean3.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload_rendak from pidsus.pds_rendak where ".$whereDefault." and no_berkas = '".rawurldecode($tmp[0])."'";
					$file = $connection->createCommand($kue)->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from pidsus.pds_rendak where ".$whereDefault." and no_berkas = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
