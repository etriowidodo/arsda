...
class PdsPidsus15Umum extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "select a.* from pidsus.pds_pidsus15_umum a 
		...
		$sql .= " order by a.tgl_pidsus15_umum desc, a.created_date desc";
    ...
    public function getSaksi($params){
        ...
        $sql = "select a.* from pidsus.pds_pidsus14_umum_saksi a where ".$def;
    ...
    public function cekPdsPidsus15Umum($post){
        $connection  = $this->db;
        $id_kejati	= $_SESSION['kode_kejati'];
        $id_kejari	= $_SESSION['kode_kejari'];
        $id_cabjari	= $_SESSION['kode_cabjari'];
        ...
        $no_pidsus15_umum = htmlspecialchars($post['no_pidsus15_umum'], ENT_QUOTES);
        $whereDef = "id_kejati = '".$id_kejati."' and id_kejari = '".$id_kejari."' and id_cabjari = '".$id_cabjari."' and no_pidsus15_umum = '".$no_pidsus15_umum."'";
        
        $sql 	= "select count(*) from pidsus.pds_pidsus15_umum where ".$whereDef;
		if($isNew){
			$count 	= $connection->createCommand($sql)->queryScalar();
		} else{
			$id1 	= $_SESSION["pidsus_no_pidsus15_umum"];
			$count 	= ($id1 == $no_pidsus15_umum)?0:$connection->createCommand($sql)->queryScalar();
		}
        
        if($count > 0){
            $pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>Pidsus-15 Umum dengan nomor '.$no_pidsus15_umum.' sudah ada</i></p>';
            return array("hasil"=>false, "error"=>$pesan, "element"=>"error_custom_no_pidsus15_umum");
        } else{
	...
	public function simpanData($post){
		...
		$no_pidsus15_umum	= htmlspecialchars($post['no_pidsus15_umum'], ENT_QUOTES);
		$tgl_pidsus15_umum	= htmlspecialchars($post['tgl_pidsus15_umum'], ENT_QUOTES);
		...
		$pathfile	= Yii::$app->params['pidsus_15umum'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_umum);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_pidsus15_umum);
		$newPhoto 	= "pidsus15umum_".$clean1."-".$clean2.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_pidsus15_umum(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_pidsus15_umum, tgl_pidsus15_umum, keperluan, sifat, lampiran, 
						 ...
						 values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$no_pidsus15_umum."', '".$helpernya->tgl_db($tgl_pidsus15_umum)."', 
				...
				$sql1 = "update pidsus.pds_pidsus15_umum set no_pidsus15_umum = '".$no_pidsus15_umum."', tgl_pidsus15_umum = '".$helpernya->tgl_db($tgl_pidsus15_umum)."', 
						 ...
						 where ".$whereDef." and no_pidsus15_umum = '".$_SESSION['pidsus_no_pidsus15_umum']."'";
			}
			$connection->createCommand($sql1)->execute();

			/*$sql2 = "delete from pidsus.pds_pidsus15_umum_uu_pasal where ".$whereDef." and no_pidsus15_umum = '".$no_pidsus15_umum."'";
			...
			if(count($post['undang_uu']) > 0){
				...
				foreach($post['undang_uu'] as $idx1=>$val1){
					...
					$sql3 = "
						insert into pidsus.pds_pidsus15_umum_uu_pasal(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_pidsus15_umum, id_uu_pasal, undang, pasal, dakwaan) 
						values ('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$no_pidsus15_umum."', '".$nom1."', '".$undang."', '".$pasal."', 
						'".$dakwaan."')";
			...
			$sql4 = "delete from pidsus.pds_pidsus15_umum_tembusan where ".$whereDef." and no_pidsus15_umum = '".$no_pidsus15_umum."'";
			...
			if(count($post['nama_tembusan']) > 0){
				...
				foreach($post['nama_tembusan'] as $idx=>$val){
					...
					if($nama_tembusan){ 
						$noauto++; 
						$sql5 = "insert into pidsus.pds_pidsus15_umum_tembusan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', 
								 '".$no_pidsus15_umum."', '".$noauto."', '".$nama_tembusan."')";
			...
			if($upl1){
				$tmpPot = glob($pathfile."pidsus15umum_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload from pidsus.pds_pidsus15_umum where ".$whereDefault." and no_pidsus15_umum = '".rawurldecode($tmp[0])."'";
					$file = $connection->createCommand($kue)->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from pidsus.pds_pidsus15_umum where ".$whereDefault." and no_pidsus15_umum = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
