...
class PdsP18 extends \yii\db\ActiveRecord{
    ...
    public function cekP18($post){
		...
		$sqlCek1 	= "select count(*) from pidsus.pds_p18 where ".$where." and no_berkas = '".$nom_p16."'";
		$count1 	= ($isNew)?$connection->createCommand($sqlCek1)->queryScalar():0;

		if($count1 > 0){
			$pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>Maaf 1 berkas hanya ada 1 P-18</i></p>';
	...
	public function simpanData($post){
		...
		$newPhoto 	= "p18_".$clean1."-".$clean2."-".$clean3.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_p18(id_kejati, id_kejari, id_cabjari, no_spdp, tgl_spdp, no_berkas, no_surat, sifat, lampiran, tgl_dikeluarkan, dikeluarkan, 
						 ...
						 penandatangan_status_ttd, penandatangan_jabatan_ttd, file_upload_p18, created_user, created_nip, created_nama, created_ip, created_date, 
				...
				if($filePhoto != ""){
					$upl1 = true;
					$filenya = ", file_upload_18 = '".$newPhoto."'";
				} else{
					$upl1 = false;
					$filenya = "";
				}
				
				$sql1 = "update pidsus.pds_p18 set no_surat = '".$no_surat."', tgl_dikeluarkan = '".$helpernya->tgl_db($tgl_dikeluarkan)."', sifat = '".$sifat."', 
			...
			$sql4 = "delete from pidsus.pds_p18_tembusan where ".$whereDef." and no_berkas = '".$no_berkas."'";
			...
			if(count($post['nama_tembusan']) > 0){
				...
				foreach($post['nama_tembusan'] as $idx=>$val){
					...
					if($nama_tembusan){ 
						$noauto++; 
						$sql5 = "insert into pidsus.pds_p18_tembusan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_spdp."', '".$tgl_spdp."', 
			...
			if($upl1){
				$tmpPot = glob($pathfile."p18_".$clean1."-".$clean2."-".$clean3.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload_p18 from pidsus.pds_p18 where ".$whereDefault." and no_berkas = '".rawurldecode($tmp[0])."'";
					$file = $connection->createCommand($kue)->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from pidsus.pds_p18 where ".$whereDefault." and no_berkas = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
