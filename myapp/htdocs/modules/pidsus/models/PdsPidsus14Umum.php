...
class PdsPidsus14Umum extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "
		with tbl_saksi as(
			select id_kejati, id_kejari, id_cabjari, no_p8_umum, no_urut_pidsus14_umum, 
			string_agg(nama||'#'||jabatan||'#'||waktu_pelaksanaan||'#'||status_keperluan, '|#|' order by no_urut_saksi) as saksinya 
			from pidsus.pds_pidsus14_umum_saksi 
			group by id_kejati, id_kejari, id_cabjari, no_p8_umum, no_urut_pidsus14_umum 
		)
		select b.saksinya, a.*  
		...
		$sql .= " order by a.no_urut_pidsus14_umum desc";//echo $sql;exit;
	...
	public function simpanData($post){
		...
		$no_urut_pidsus14_umum 	= ($post['no_urut_pidsus14_umum'])?htmlspecialchars($post['no_urut_pidsus14_umum'], ENT_QUOTES):0;
		$tgl_pidsus14_umum		= htmlspecialchars($post['tgl_pidsus14_umum'], ENT_QUOTES);
		...
		$sql0 = "select coalesce(max(no_urut_pidsus14_umum)+1,1) as nourut from pidsus.pds_pidsus14_umum 
		...
		$pathfile	= Yii::$app->params['pidsus_14umum'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_umum);
		$clean2 	= ($isNew)?$row0:$no_urut_pidsus14_umum;
		$newPhoto 	= "pidsus14umum_".$clean1."-".$clean2.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_pidsus14_umum(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_urut_pidsus14_umum, tgl_pidsus14_umum, lampiran, 
						 ...
						 updated_ip, updated_date) values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$row0."', 
						 '".$helpernya->tgl_db($tgl_pidsus14_umum)."', '".$lampiran."', '".$penandatangan_nama."', '".$penandatangan_nip."', '".$penandatangan_jabatan."', 
				...
				$sql1 = "update pidsus.pds_pidsus14_umum set tgl_pidsus14_umum = '".$helpernya->tgl_db($tgl_pidsus14_umum)."', lampiran = '".$lampiran."', 
						 ...
						 where ".$whereDef." and no_urut_pidsus14_umum = '".$no_urut_pidsus14_umum."'";
			}
			$connection->createCommand($sql1)->execute();

			$sql2 = "delete from pidsus.pds_pidsus14_umum_saksi where ".$whereDef." and no_urut_pidsus14_umum = '".$no_urut_pidsus14_umum."'";
			$connection->createCommand($sql2)->execute();
			if(count($post['saksi_ahli']) > 0){
				$nourut = ($isNew)?$row0:$no_urut_pidsus14_umum;
				foreach($post['saksi_ahli'] as $idx=>$val){
					...
					$sql3 = "insert into pidsus.pds_pidsus14_umum_saksi(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_urut_pidsus14_umum, no_urut_saksi, nama, jabatan, 
							 ...
							 ('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$nourut."', '".($idx+1)."', '".$nama."', '".$jabatan."', 
			...
			if($upl1){
				$tmpPot = glob($pathfile."pidsus14umum_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload from pidsus.pds_pidsus14_umum where ".$whereDefault." and no_urut_pidsus14_umum = '".rawurldecode($tmp[0])."'";
					$file = $connection->createCommand($kue)->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from pidsus.pds_pidsus14_umum where ".$whereDefault." and no_urut_pidsus14_umum = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
