...
class PdsP9Khusus extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "select a.* from pidsus.pds_p9_khusus a 
		...
		if($q1)
			$sql .= " and (upper(a.no_p9_khusus) like '%".strtoupper($q1)."%' or to_char(a.tgl_p9_khusus, 'DD-MM-YYYY') = '".$q1."' 
		...
		$sql .= " order by a.tgl_p9_khusus desc, a.created_date desc";
	...
	$q1  = htmlspecialchars($params['mb4_q1'], ENT_QUOTES);
        ...
        $sql = "select a.* from pidsus.pds_pidsus14_khusus_saksi_tersangka a where ".$def." and a.status_keperluan = '".$q0."'";
    ...
    public function cekPdsP9Khusus($post){
        $connection  = $this->db;
        $id_kejati	= $_SESSION['kode_kejati'];
        $id_kejari	= $_SESSION['kode_kejari'];
        $id_cabjari	= $_SESSION['kode_cabjari'];
        ...
        $no_p9_khusus = htmlspecialchars($post['no_p9_khusus'], ENT_QUOTES);
        $whereDef = "id_kejati = '".$id_kejati."' and id_kejari = '".$id_kejari."' and id_cabjari = '".$id_cabjari."' and no_p9_khusus = '".$no_p9_khusus."'";
        
        $sql 	= "select count(*) from pidsus.pds_p9_khusus where ".$whereDef;
		if($isNew){
			$count 	= $connection->createCommand($sql)->queryScalar();
		} else{
			$id1 	= $_SESSION["pidsus_no_p9_khusus"];
			$count 	= ($id1 == $no_p9_khusus)?0:$connection->createCommand($sql)->queryScalar();
		}
        
        if($count > 0){
            $pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>P-9 Khusus dengan nomor '.$no_p9_khusus.' sudah ada</i></p>';
            return array("hasil"=>false, "error"=>$pesan, "element"=>"error_custom_no_p9_khusus");
        } else{
	...
	public function simpanData($post){
		...
		$no_p9_khusus		= htmlspecialchars($post['no_p9_khusus'], ENT_QUOTES);
		$tgl_p9_khusus          = htmlspecialchars($post['tgl_p9_khusus'], ENT_QUOTES);
		...
		$pathfile	= Yii::$app->params['p9_khusus'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_khusus);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_p9_khusus);
		$newPhoto 	= "p9khusus_".$clean1."-".$clean2.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_p9_khusus(id_kejati, id_kejari, id_cabjari, no_pidsus18, no_p8_khusus, no_p9_khusus, tgl_p9_khusus, kepada_nama, kepada_jabatan, di_tempat, 
						 ...
						 ('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_pidsus18."', '".$no_p8_khusus."', '".$no_p9_khusus."', '".$helpernya->tgl_db($tgl_p9_khusus)."', '".$kepada_nama."', 
				...
				$sql1 = "update pidsus.pds_p9_khusus set no_p9_khusus = '".$no_p9_khusus."', tgl_p9_khusus = '".$helpernya->tgl_db($tgl_p9_khusus)."', 
						 ...
						 where ".$whereDef." and no_p9_khusus = '".$_SESSION['pidsus_no_p9_khusus']."'";
			}
			$connection->createCommand($sql1)->execute();

			if($upl1){
				$tmpPot = glob($pathfile."p9khusus_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload from pidsus.pds_p9_khusus where ".$whereDefault." and no_p9_khusus = '".rawurldecode($tmp[0])."'";
					$file = $connection->createCommand($kue)->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from pidsus.pds_p9_khusus where ".$whereDefault." and no_p9_khusus = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
