...
class PdsBa1Umum extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "select a.* from pidsus.pds_ba1_umum a 
	...
	public function simpanData($post){
		...
		$no_ba1_umum 	= ($post['no_ba1_umum'])?htmlspecialchars($post['no_ba1_umum'], ENT_QUOTES):0;
		$tgl_ba1_umum	= htmlspecialchars($post['tgl_ba1_umum'], ENT_QUOTES);
		...
		$sql0 = "select coalesce(max(no_ba1_umum)+1,1) as nourut from pidsus.pds_ba1_umum 
		...
		$pathfile	= Yii::$app->params['ba1_umum'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_umum);
		$clean2 	= ($isNew)?$row0:$no_ba1_umum;
		$newPhoto1 	= "ba1_umum_".$clean1."-".$clean2.$extPhoto1;
		$newPhoto2 	= "ba1_umum_pertanyaan_".$clean1."-".$clean2.$extPhoto2;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_ba1_umum(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_ba1_umum, tgl_ba1_umum, tempat, nip_jaksa, nama_jaksa, pangkat_jaksa, 
						 ...
						 values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$row0."', '".$helpernya->tgl_db($tgl_ba1_umum)."', '".$tempat."', 
						 '".$nip_jaksa."', '".$nama_jaksa."', '".$pangkat_jaksa."', '".$nama."', '".$tmpt_lahir."', '".$status."', '".$helpernya->tgl_db($tgl_lahir)."', 
				...
				$sql1 = "update pidsus.pds_ba1_umum set tgl_ba1_umum = '".$helpernya->tgl_db($tgl_ba1_umum)."', tempat = '".$tempat."', nip_jaksa = '".$nip_jaksa."', 
						 ...
						 where ".$whereDef." and no_ba1_umum = '".$no_ba1_umum."'";
			}
			$connection->createCommand($sql1)->execute();

			$sql2 = "delete from pidsus.pds_ba1_umum_pertanyaan where ".$whereDef." and no_ba1_umum = '".$no_ba1_umum."'";
			$connection->createCommand($sql2)->execute();
			if(count($post['pertanyaan']) > 0){
				$nourut = ($isNew)?$row0:$no_ba1_umum;
				foreach($post['pertanyaan'] as $idx=>$val){
					...
					$sql3 = "insert into pidsus.pds_ba1_umum_pertanyaan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$nourut."', 
							 '".($idx+1)."', '".$pertanyaan."', '".$jawaban."')";
					$connection->createCommand($sql3)->execute();
				}
			}

			if($upl1){
				$tmpPot = glob($pathfile."ba1_umum_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
			...
			if($upl2){
				$tmpPot = glob($pathfile."ba1_umum_pertanyaan_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
					foreach($tmpPot as $datj)
						if(file_exists($datj)) unlink($datj);
				}
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload, file_unggah_pertanyaan from pidsus.pds_ba1_umum where ".$whereDefault." and no_ba1_umum = '".rawurldecode($tmp[0])."'";
					...
					$sql1 = "delete from pidsus.pds_ba1_umum where ".$whereDefault." and no_ba1_umum = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
