...
class PdsPidsus12Khusus extends \yii\db\ActiveRecord{
    ...
    public function cekpidsus12khusus($post){
        $connection = $this->db;
        ...
        $no_pidsus12_khusus= htmlspecialchars($post['no_pidsus12_khusus'], ENT_QUOTES);

        $isNew 		= htmlspecialchars($post['isNewRecord'], ENT_QUOTES);

        $whereDef 	= "id_kejati = '".$id_kejati."' and id_kejari = '".$id_kejari."' and id_cabjari = '".$id_cabjari."' and no_pidsus18 = '".$no_pidsus18.
                            "' and no_p8_khusus = '".$no_p8_khusus."' and no_pidsus12_khusus = '".$no_pidsus12_khusus."'";

        $sqlCek = "select count(*) from pidsus.pds_pidsus12_khusus where ".$whereDef;
        
        if($isNew){
            $count 	= $connection->createCommand($sqlCek)->queryScalar();
        } else{
            $id1 	= $_SESSION["pidsus_no_pidsus12_khusus"];
            $count 	= ($id1 == $no_pidsus12_khusus)?0:$connection->createCommand($sqlCek)->queryScalar();
        }
        
        if($count > 0){
            $pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>Maaf Nomor Pidsus 12 Khusus dengan nomor '.$no_pidsus12_khusus.' sudah ada</i></p>';
            return array("hasil"=>false, "error"=>$pesan, "element"=>"error_custom_no_pidsus12_khusus");
        } else{
	...
	public function simpanData($post){
		...
		$no_p8_khusus		= htmlspecialchars($post['no_p8_khusus'], ENT_QUOTES);
		$no_pidsus12_khusus	= htmlspecialchars($post['no_pidsus12_khusus'], ENT_QUOTES);
		$tgl_pidsus12_khusus	= htmlspecialchars($post['tgl_pidsus12_khusus'], ENT_QUOTES);
		...
		$pathfile	= Yii::$app->params['pidsus_12khusus'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_khusus);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_pidsus12_khusus);
		$newPhoto 	= "pidsus12khusus_".$clean1."-".$clean2.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_pidsus12_khusus(id_kejati, id_kejari, id_cabjari, no_pidsus18, no_p8_khusus, no_pidsus12_khusus, tgl_pidsus12_khusus, lampiran, penandatangan_nama, 
						 ...
						 values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_pidsus18."' , '".$no_p8_khusus."', '".$no_pidsus12_khusus."', '".$helpernya->tgl_db($tgl_pidsus12_khusus)."', 
				...
				$sql1 = "update pidsus.pds_pidsus12_khusus set no_pidsus12_khusus = '".$no_pidsus12_khusus."', tgl_pidsus12_khusus = '".$helpernya->tgl_db($tgl_pidsus12_khusus)."', 
			...
			$sql4 = "delete from pidsus.pds_pidsus12_khusus_tembusan where ".$whereDef;
			...
			if(count($post['nama_tembusan']) > 0){
				...
				foreach($post['nama_tembusan'] as $idx=>$val){
					...
					if($nama_tembusan){ 
						$noauto++; 
						$sql5 = "insert into pidsus.pds_pidsus12_khusus_tembusan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_pidsus18."', '".$no_p8_khusus."', 
								'".$no_pidsus12_khusus."', '".$noauto."', '".$nama_tembusan."')";
			...
			if($upl1){
				$tmpPot = glob($pathfile."pidsus12khusus_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
    ...
    public function hapusData($id1, $id2, $id3){
		...
		try {
                        $kue = "select file_upload from pidsus.pds_pidsus12_khusus where ".$whereDef." and no_p8_khusus = '".rawurldecode($id2)."' 
                                and no_pidsus12_khusus = '".rawurldecode($id3)."'";
                        ...
                        $sql1 = "delete from pidsus.pds_pidsus12_khusus where ".$whereDef." and no_p8_khusus = '".rawurldecode($id2)."' 
                                 and no_pidsus12_khusus = '".rawurldecode($id3)."'";
                        $connection->createCommand($sql1)->execute();
			$transaction->commit();
			return true;
