...
class PdsBa3Umum extends \yii\db\ActiveRecord{
	...
	public function searchPer($params){
		...
		$sql = "select a.* from pidsus.pds_ba3_umum a 
				where a.id_kejati = '".$id_kejati."' and a.id_kejari = '".$id_kejari."' and a.id_cabjari = '".$id_cabjari."' and a.no_p8_umum = '".$no_p8_umum."'";

		if($q1)
			$sql .= " and (to_char(a.tgl_ba3_umum, 'DD-MM-YYYY') = '".$q1."' or upper(a.nip_jaksa) like '%".strtoupper($q1)."%' 
		...
		$sql .= " order by a.tgl_ba3_umum desc, a.created_date desc";
	...
	public function searchBa1Umum($get){
		...
		$sql = "select a.*, b.nama as kebangsaan from pidsus.pds_ba1_umum a left join public.ms_warganegara b on a.warganegara = b.id 
				where a.id_kejati = '".$id_kejati."' and a.id_kejari = '".$id_kejari."' and a.id_cabjari = '".$id_cabjari."' and a.no_p8_umum = '".$no_p8_umum."'";
	...
	public function simpanData($post){
		...
		$no_ba3_umum 	= ($post['no_ba3_umum'])?htmlspecialchars($post['no_ba3_umum'], ENT_QUOTES):0;
		$tgl_ba3_umum	= htmlspecialchars($post['tgl_ba3_umum'], ENT_QUOTES);
		$jam_ba3_umum	= htmlspecialchars($post['jam_ba3_umum'], ENT_QUOTES);
		...
		$sql0 = "select coalesce(max(no_ba3_umum)+1,1) as nourut from pidsus.pds_ba3_umum 
		...
		$pathfile	= Yii::$app->params['ba3_umum'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_umum);
		$clean2 	= ($isNew)?$row0:$no_ba3_umum;
		$newPhoto1 	= "ba3_umum_".$clean1."-".$clean2.$extPhoto1;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_ba3_umum(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_ba3_umum, tgl_ba3_umum, tempat, nip_jaksa, nama_jaksa, pangkat_jaksa, 
						 nama, tmpt_lahir, tgl_lahir, umur, id_jkl, warganegara, alamat, id_agama, pekerjaan, id_pendidikan, file_upload, created_user, 
						 created_nip, created_nama, created_ip, created_date, updated_user, updated_nip, updated_nama, updated_ip, updated_date, jam_ba3_umum) 
						 values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$row0."', '".$helpernya->tgl_db($tgl_ba3_umum)."', '".$tempat."', 
						 ...
						 '".$newPhoto1."', '".$created_user."', '".$created_nip."', '".$created_nama."', '".$created_ip."', NOW(), 
						 '".$updated_user."', '".$updated_nip."', '".$updated_nama."', '".$updated_ip."', NOW(), '".$jam_ba3_umum."')";
			}else{
				if($filePhoto1 != ""){
					$upl1 = true;
				...
				$sql1 = "update pidsus.pds_ba3_umum set tgl_ba3_umum = '".$helpernya->tgl_db($tgl_ba3_umum)."', tempat = '".$tempat."', nip_jaksa = '".$nip_jaksa."', 
						 ...
						 id_agama = '".$id_agama."', pekerjaan = '".$pekerjaan."', id_pendidikan = '".$id_pendidikan."', jam_ba3_umum = '".$jam_ba3_umum."', 
						 updated_user = '".$updated_user."', updated_nip = '".$updated_nip."', 
						 updated_nama = '".$updated_nama."', updated_ip = '".$updated_ip."', updated_date = NOW()".$filenya1." 
						 where ".$whereDef." and no_ba3_umum = '".$no_ba3_umum."'";
			}
			$connection->createCommand($sql1)->execute();

			$sql2 = "delete from pidsus.pds_ba3_umum_saksi where ".$whereDef." and no_ba3_umum = '".$no_ba3_umum."'";
			$connection->createCommand($sql2)->execute();
			if(count($post['jpnid']) > 0){
				$nourut = ($isNew)?$row0:$no_ba3_umum;
				foreach($post['jpnid'] as $idx=>$val){
					list($nip, $nama, $pangkat, $jabatan) = explode("#", $val);
					$sql3 = "insert into pidsus.pds_ba3_umum_saksi values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$nourut."', 
			...
			if($upl1){
				$tmpPot = glob($pathfile."ba3_umum_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$tmp = explode("#", $val);
					$kue = "select file_upload from pidsus.pds_ba3_umum where ".$whereDefault." and no_ba3_umum = '".rawurldecode($tmp[0])."'";
					...
					$sql1 = "delete from pidsus.pds_ba3_umum where ".$whereDefault." and no_ba3_umum = '".rawurldecode($tmp[0])."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
