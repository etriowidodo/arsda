...
class PdsPidsus13Umum extends \yii\db\ActiveRecord{
    public static function tableName(){
        return 'pidsus.pds_p16';
    }
    
    public function cekpidsus13umum($post){
		$connection = $this->db;
		...
		$whereDef 	= "id_kejati = '".$id_kejati."' and id_kejari = '".$id_kejari."' and id_cabjari = '".$id_cabjari."' and no_pidsus13_umum = '".$no_pidsus13_umum."'";

		$no_pidsus13_umum = htmlspecialchars($post['no_pidsus13_umum'], ENT_QUOTES);

        $sqlCek = "select count(*) from pidsus.pds_pidsus13_umum where ".$whereDef;
		$count 	= $connection->createCommand($sqlCek)->queryScalar();
        
        if($count > 0){
            $pesan = '<p style="color:#dd4b39; font-size:12px; margin-bottom:20px;"><i>Maaf Nomor Pidsus 13 Umum dengan nomor '.$no_pidsus13_umum.' sudah ada</i></p>';
            return array("hasil"=>false, "error"=>$pesan, "element"=>"error_custom_no_pidsus13_umum");
        } else{
	...
	public function simpanData($post){
		...
		$no_pidsus13_umum	= htmlspecialchars($post['no_pidsus13_umum'], ENT_QUOTES);
		$tgl_pidsus13_umum	= htmlspecialchars($post['tgl_pidsus13_umum'], ENT_QUOTES);
		...
		$pathfile	= Yii::$app->params['pidsus_13umum'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_p8_umum);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_pidsus13_umum);
		$newPhoto 	= "pidsus13umum_".$clean1."-".$clean2.$extPhoto;
		...
		try{
			if($isNew){
				...
				$sql1 = "insert into pidsus.pds_pidsus13_umum(id_kejati, id_kejari, id_cabjari, no_p8_umum, no_pidsus13_umum, tgl_pidsus13_umum, sifat, lampiran, 
						 ...
						 updated_date, file_upload) values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', '".$no_pidsus13_umum."', 
						 '".$helpernya->tgl_db($tgl_pidsus13_umum)."', '".$sifat."', '".$lampiran."', '".$penandatangan_nama."', '".$penandatangan_nip."', 
				...
				$sql1 = "update pidsus.pds_pidsus13_umum set no_pidsus13_umum = '".$no_pidsus13_umum."', tgl_pidsus13_umum = '".$helpernya->tgl_db($tgl_pidsus13_umum)."', 
			...
			$sql4 = "delete from pidsus.pds_pidsus13_umum_tembusan where ".$whereDef;
			...
			if(count($post['nama_tembusan']) > 0){
				...
				foreach($post['nama_tembusan'] as $idx=>$val){
					...
					if($nama_tembusan){ 
						$noauto++; 
						$sql5 = "insert into pidsus.pds_pidsus13_umum_tembusan values('".$id_kejati."', '".$id_kejari."', '".$id_cabjari."', '".$no_p8_umum."', 
								'".$no_pidsus13_umum."', '".$noauto."', '".$nama_tembusan."')";
			...
			if($upl1){
				$tmpPot = glob($pathfile."pidsus13umum_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
    ...
    public function hapusData($id1, $id2){
		...
		try {
			$kue = "select file_upload from pidsus.pds_pidsus13_umum where ".$whereDefault." and no_p8_umum = '".rawurldecode($id1)."' 
					and no_pidsus13_umum = '".rawurldecode($id2)."'";
			$file = $connection->createCommand($kue)->queryScalar();
			if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

			$sql1 = "delete from pidsus.pds_pidsus13_umum where ".$whereDefault." and no_p8_umum = '".rawurldecode($id1)."' and no_pidsus13_umum = '".rawurldecode($id2)."'";
			$connection->createCommand($sql1)->execute();
