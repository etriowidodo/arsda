                        ...
                        <div class="col-md-8">
							 ...
							 <?= $form->field($model, 'no_surat')->input('text',
                                ['oninput'  =>'
                                        ...
                                        this.value=str;
                                        ',  'value' =>  empty($model->no_surat) ? "PRINT-": $model->no_surat])  ?>
                        </div>
                    </div>
                </div>
            </div>
...
HTML;
      ...
      <div class="box-footer" style="text-align: center;">
        ...
        <?= Html::a('Batal', $model->isNewRecord ? ['index'] : ['../pdsold/p16/index'], ['class' => 'btn btn-danger']) ?>
        <!-- END CMS_PIDUM001_16 --> 
        </div>
        <div id="hiddenId">
...
$script1 = <<< JS
// $('#m_alert').load('/pdsold/p16/alert-sukses');
  ...
  $('#m_jpu').on('hidden.bs.modal', function () {
    ...
    $('#printToSave').click(function(){
        ...
        else
        {
             $.ajax({
                type        : 'POST',
                url         :'/pdsold/p16/cek-no-surat-p16',
                data        : 'no_surat='+print,                                
                                  ...
                                  else
                                  {
                                    $('input[name="printToSave"]').val('1');
                                         $.ajax({
                                            type: "POST",
                                            async:    false,
                                            url: '/pdsold/p16/create',
                                            data: $("form").serialize(),
                                            success:function(data){ 
                                            $('.box-footer').hide(); 
                                            var cetak   = '/pdsold/p16/cetak?id='+data;  
    ...
    $('#printToChange').click(function(){
        ...
        $('input[name="printToSave"]').val('2');
             $.ajax({
                type: "POST",
                async:    false,
                url: '/pdsold/p16/update?id='+'$model->id_p16',
                data: $("form").serialize(),
                success:function(data){
