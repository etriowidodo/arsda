...
<!--<button id="tambah" class='btn btn-warning btnUbahCheckboxIndex'>Tambah P21A</button>-->
    ...
    <?php
       $form = \kartik\widgets\ActiveForm::begin([
            'id' => 'hapus-index',
            'action' => '/pdsold/pdm-p21-a/delete/'
        ]);  
    ?>  
...
<?php
    ...
    $js = <<< JS
		...
		var url = window.location.protocol + "//" + window.location.host + "/pdsold/pdm-p21-a/create?id_pengantar="+idpengantar+"&idp21a="+idp21a;
...
$('#idUbah').click (function (e) {
		...
		var idpengantar =$('.checkHapusIndex:checked').val();
         var url = window.location.protocol + "//" + window.location.host + "/pdsold/pdm-p21-a/create?id_pengantar="+idpengantar;
        $(location).attr('href',url);
		}
    });
