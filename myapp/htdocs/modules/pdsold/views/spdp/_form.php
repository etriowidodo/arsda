...
<div class="box box-primary" style="border-color: #f39c12;padding: 13px;overflow: hidden;">
            ...
            <div class="form-group form-inline">
                ...
                <?php
                     ...
                     */
					 ...
					 /*}else{
						$pathfile = Url::to('/template/pdsold_surat/spdp/'. $model->file_upload);
						echo $form->field($model, 'file_upload')->widget(FileInput::classname(), [
	...
	<?php if($model->isNewRecord){ ?>
        <?= Html::SubmitButton($model->isNewRecord ? 'Simpan' : 'Ubah', ['class' => $model->isNewRecord ? 'btn btn-warning' : 'btn btn-warning']) ?>
    <?php } ?>
        <?= Html::a('Batal', $model->isNewRecord ? ['index'] : ['../pdsold/spdp/index'], ['class' => 'btn btn-danger']) ?>
        <!-- END CMS_PIDUM001_16 -->
    </div>
        <div id="hiddenId"></div>
    <?php ActiveForm::end(); ?>
...
$(document).ready(function(){
	...
	$.ajax({
		type: "POST",
		url: '/pdsold/spdp/showmstperkara',
    ...
    // Get the input box
        ...
        textInput.onkeyup = function (e) {
            ...
            timeout = setTimeout(function () {
                    $.ajax({
                                type        : 'POST',
                                url         :'/pdsold/spdp/cek-no-surat-spdp',
            ...
            }else
            {
                    ...
                    if ($('#pdmspdp-tgl_surat-disp').val() != '')
                    {
                            ...
                            {  
                              ...
                              {
                                $('#m_tersangka').html('');
                                $('#m_tersangka').load('/pdsold/spdp/show-tersangka?id_tersangka='+'$model->id_perkara|'+id_tersangka);
                                $('#m_tersangka').modal('show');
                              }
