...
use app\modules\pdsold\models\PdmP17Search;
use app\modules\pdsold\models\PdmSpdp;
use app\modules\pdsold\models\PdmTembusanP17;
...
use yii\web\Session;
use app\modules\pdsold\models\PdmSysMenu; //copy 1
...
from pidum.ms_tersangka where nama NOT IN (SELECT nama FROM pidum.ms_tersangka_pt WHERE id_perkara = '".$id."') AND id_perkara = '".$id."'";
        ...
        if ($model->load(Yii::$app->request->post())) {
			...
			if ($files != false && !empty($files) ) {
				if($file_lama !=''){
					...
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p17.'.$files->extension;
					$files->saveAs($path);
				}else{
					$model->file_upload = preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p17.'. $files->extension;
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p17.'.$files->extension;
    ...
    public function actionCetak($id){
        ...
        $connection = \Yii::$app->db;
        $odf = new \Odf(Yii::$app->params['report-path']."web/template/pdsold/p17.odt");
