...
use app\modules\pdsold\models\PdmT4Search;
use app\modules\pdsold\models\PdmTembusanT4;
use app\modules\pdsold\models\PdmStatusSurat;
use app\modules\pdsold\models\VwTersangkaPt;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                if ($files != false && !empty($files) ) {
                    ...
                    $path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/T4_'.$jml_pt['jml'].'.'. $files->extension;
                    $files->saveAs($path);
                }
            ...
            try {
                ...
                if ($files != false && !empty($files) ) {
                    if($file_lama !=''){
                        ...
                        $path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/T4_'.$jml_pt['jml'].'.'. $files->extension;
                        $files->saveAs($path);
                    }
                }else{
                    $model->file_upload = $file_lama;
...
//                echo '<pre>';print_r($model);exit();	
    ...
    public function actionCetak($id_t4) {
        $connection = \Yii::$app->db;
        $odf = new Odf(Yii::$app->params['report-path'] . "web/template/pdsold/t4.odt");
		$session = new Session();
        $id_perkara = $session->get('id_perkara');
