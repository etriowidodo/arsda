...
//        echo $berkasdanpengantar;exit();
        if($no_pengantar == ''){
            ...
            return $this->redirect(['../pdsold/pdm-p24/create?id_pengantar='.$berkasdanpengantar]);
        }
    }
...
//                echo '<pre>';print_r($model);exit();
            ...
            return $this->redirect(['../pdsold/pdm-p24/update?id_p24='.$id_p24->id_p24.'& id_pengantar='.$id_p24->id_p24]);
        }
    }
...
//                        echo '<pre>';print_r($model);exit();
    ...
    public function actionCetakdraft($id_pengantar){
        $odf    = new Odf(Yii::$app->params['report-path'] . "web/template/pdsold/p24-draf.odt");
        $id     = Yii::$app->session->get('id_perkara');
...
//        $dft_jaksaTtd->kepala('');
	...
	public function actionCetak($id_p24,$id_berkas)
	{
        ...
        $odf = new Odf(Yii::$app->params['report-path'] . "web/template/pdsold/p24.odt");
		  $id = Yii::$app->session->get('id_perkara');
