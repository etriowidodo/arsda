...
use app\modules\pdsold\models\PdmPengembalianBerkas;
use app\modules\pdsold\models\PdmPengembalianSearch;
use app\modules\pdsold\models\PdmBerkasTahap1;
use app\modules\pdsold\models\PdmSpdp;
use app\modules\pdsold\models\PdmTembusanPengembalian;
use app\modules\pdsold\models\PdmTembusanPengembalianBerkas;
...
class PdmPengembalianController extends Controller
{
    ...
    public function actionUpdate() // pengembalian SPDP
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) {
			...
			if ($files != false && !empty($files) ) {
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/pengembalianSPDP.'.$files->extension;
					$files->saveAs($path);
			}
            ...
            try{
				...
				if ($files != false && !empty($files) ) {
					if($file_lama !=''){
						...
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/pengembalianBerkas_'.$file_lama.'.'.$files->extension;
						$files->saveAs($path);
					}else{
						$model->file_upload = preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/pengembalianBerkas_'.$jml_pt['jml'].'.'. $files->extension;
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/pengembalianBerkas_'.$jml_pt['jml'].'.'.$files->extension;
						$files->saveAs($path);
					}
				}else{
				...
				if ($files != false && !empty($files) ) {
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/pengembalianBerkas_'.$id_berkas.'.'.$files->extension;
						$files->saveAs($path);
				}
	...
	public function actionCetakberkas($id) {
		...
		$odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pdsold/pengembalianBerkas.odt");
		$model = PdmPengembalianBerkas::findOne(['id_pengembalian' => $id]);
	...
	public function actionCetakspdp($id) {
		$connection = \Yii::$app->db;
		$odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pdsold/pengembalianSPDP.odt");
