...
use app\modules\pdsold\models\PdmBa10Search;
use Yii;
...
class PdmBa10Controller extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try{
               // $seq = Yii::$app->db->createCommand("select public.generate_pk('pidum.pdm_ba12', 'id_ba12', '" . \Yii::$app->globalfunc->getSatker()->inst_satkerkd . "', '" . date('Y') . "')")->queryOne();
                ...
                $model->nama = $_POST['PdmBa10']['nama'];
                $model->no_register_perkara = $no_register_perkara;
                $model->no_surat_t8 = $_POST['PdmBa10']['no_surat_t8'];
                $model->tgl_ba10 = date('Y-m-d', strtotime($_POST['PdmBa10']['tgl_ba10']));
                $model->nip_jaksa = $_POST['PdmBa10']['nip_jaksa'];
                $model->no_reg_tahanan_jaksa = $_POST['PdmBa10']['no_reg_tahanan_jaksa'];
                $model->id_tersangka = $_POST['PdmBa10']['id_tersangka'];
                $model->tgl_penahanan = date('Y-m-d', strtotime($_POST['tgl_mulai']));
                $model->tindakan = $_POST['PdmBa10']['tindakan'];
                $model->id_ms_loktahanan = $_POST['PdmBa10']['id_ms_loktahanan'];
                ...
                if($model->save()){
                    //var_dump($model->getErrors());echo "Ba10";exit;
    ...
    public function actionUpdate($id_tersangka,$no_register_perkara)
    {
        ...
        if(empty($model->no_register_perkara)){
            $this->redirect('/pdsold/pdm-Ba10/index');
        }
        $modeljaksiChoosen = PdmJaksaP16a::findAll(['no_register_perkara' => $no_register_perkara]);
        $modelLokTahanan = \yii\helpers\ArrayHelper::map(MsLoktahanan::find()->asArray()->all(),'id_loktahanan','nama');
        //$session = new Session();
...
public function actionJpu()
    ...
    public function actionCetak ($id,$tgl_surat) {		
        ...
        $ba10 = PdmBa10::findOne(['no_register_perkara' => $no_register_perkara, 'id_tersangka'=>$id, 'tgl_ba10'=>$tgl_surat]);
        ...
        $lokasi = MsLoktahanan::findOne(['id_loktahanan'=>$ba10->id_ms_loktahanan]);
        //echo '<pre>';print_r($lokasi);exit;
        return $this->render('cetak',['session'=>$_SESSION, 't8'=>$t8, 'ba10'=>$ba10, 'tersangka'=>$tersangka, 'listPasal'=>$listPasal, 'lokasi'=>$lokasi]);        
    ...
    public function actionDelete()
    {
        ...
        $id_tahap = $_POST['hapusIndex'][0];
            ...
            if($id_tahap=='all'){
                    $id_tahapx=PdmBa10::find()->select("id_tersangka, tgl_ba10")->where(['no_register_perkara'=>$no_register_perkara])->asArray()->all();
                    foreach ($id_tahapx as $key => $value) {
                        $arr[] = $value['id_tersangka'].'#'.$value['tgl_ba10'];
        ...
        $count = 0;
           foreach($id_tahap AS $key_delete => $delete){
             try{
                ...
                $id_tersangka = $ex[0];
                $tgl_ba10 = $ex[1];
                    PdmBa10::deleteAll(['no_register_perkara' => $no_register_perkara, 'id_tersangka'=>$id_tersangka, 'tgl_ba10'=>$tgl_ba10]);
    ...
    protected function findModel($id_tersangka, $no_register_perkara)
    {
        ...
        if (($model = PdmBa10::findOne(['no_register_perkara' => $no_register_perkara, 'id_tersangka' => $id_tersangka])) !== null) {
            return $model;
        }
    }
