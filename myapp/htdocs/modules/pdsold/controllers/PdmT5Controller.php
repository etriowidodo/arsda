...
use app\modules\pdsold\models\PdmT5Tersangka;
use app\modules\pdsold\models\PdmT5Search;
use app\modules\pdsold\models\PdmTembusanT5;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
				...
				if ($files != false && !empty($files) ) {
					...
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/T5_'.$jml_pt['jml'].'.'. $files->extension;
					$files->saveAs($path);
				}
...
//                echo '<pre>';print_r($model);exit();
	...
	public function actionCetak($id_t5)
	{
        ...
        $id_perkara = $session->get('id_perkara');
            ...
            $odf = new Odf(Yii::$app->params['report-path'] . "web/template/pdsold/t5.odt");
            $model = PdmT5::findOne(['id_t5' => $id_t5]);
