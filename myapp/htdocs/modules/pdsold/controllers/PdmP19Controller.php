...
use app\modules\pdsold\models\PdmP19Search;
...
use app\modules\pdsold\models\PdmTembusanP19;
...
GROUP BY a.id_berkas,coalesce(e.id_p19,'0'),e.id_p19,a.no_pengiriman ||' Tgl : '||tgl_pengiriman  ")->queryScalar();
    ...
    public function actionUpdate($id_p19,$id_pengantar,$id_berkas) {
        ...
        if ($model->load(Yii::$app->request->post())) {
			...
			if ($files1 != false && !empty($files1) ) {
				if($file_lama1 !=''){
					...
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/'.$file_lama1;
					$files1->saveAs($path);
				}else{
					$model->file_upload_petunjuk_p19 = preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p19Petunjuk_'.$jml_pt['jml'].'.'. $files1->extension;
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p19Petunjuk_'.$jml_pt['jml'].'.'.$files1->extension;
					$files1->saveAs($path);
				}
			}else{
				$model->file_upload_petunjuk_p19 = $file_lama1;
		...
		if ($files2 != false && !empty($files2) ) {
				if($file_lama2 !=''){
					...
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/'.$file_lama2;
					$files2->saveAs($path);
				}else{
					$model->file_upload_p19 = preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p19_'.$jml_pt['jml'].'.'. $files2->extension;
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/p19_'.$jml_pt['jml'].'.'.$files2->extension;
					$files2->saveAs($path);
				}
			}else{
				$model->file_upload_p19 = $file_lama2;
    ...
    public function actionCetak($id,$id_pengantar) {
        ...
        $odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pdsold/p19.odt");
