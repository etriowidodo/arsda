...
use app\modules\pdsold\models\PdmTembusanP21a;
...
class PdmP21AController extends Controller
{
    ...
    public function actionCreate($id_pengantar)
    {
	   ...
	   $transaction = Yii::$app->db->beginTransaction();
            try{
				...
				$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p21a_'.$jml_pt['jml'].'.'. $files->extension;
					$files->saveAs($path);
				}
			...
			$transaction->commit();
			 ...
			 return $this->redirect(['../pdsold/pdm-p21-a/index']);
			 }else{
                    Yii::$app->getSession()->setFlash('success', [
                        'type' => 'danger',
                        'duration' => 3000,
	...
	public function actionCetak($id_pengantar,$idp21a) {
		$id_perkara=Yii::$app->session->get('id_perkara');
                ...
                //....
                $odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pdsold/p21a.odt");
                $model = PdmP21A::findOne(['id_p21a' => $idp21a]);
    ...
    {	
		...
		PdmTembusan::deleteAll(['id_table' => $model1->id_p21a, 'kode_table' => GlobalConstMenuComponent::P21A]);
				...
				$filename = Yii::$app->basePath . '/web/template/pdsold_surat/' .preg_replace('/[^A-Za-z0-9\-]/', '',$model1->id_perkara). '/p21a_'.$model1->id_p21a.'.pdf';
