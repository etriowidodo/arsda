...
use app\modules\pdsold\models\PdmP23Search;
use yii\db\Query;
...
use app\modules\pdsold\models\PdmTembusanP23;
...
class PdmP23Controller extends Controller
{
    ...
    public function actionUpdate($id,$id_pengantar)
    {
        ...
        $searchModel = new PdmP23Search();
		$dataProvider = $searchModel->searchTersangka($modelPengantar->id_berkas);
    ...
    {	
		...
		PdmTembusan::deleteAll(['id_table' => $model1->id_p23, 'kode_table' => GlobalConstMenuComponent::P23]);
				...
				$filename = Yii::$app->basePath . '/web/template/pdsold_surat/' .preg_replace('/[^A-Za-z0-9\-]/', '',$model1->id_perkara). '/p23_'.$model1->id_p23.'.pdf';
    ...
    {
		$id_perkara=Yii::$app->session->get('id_perkara');
	$odf = new Odf(Yii::$app->params['report-path'] . "web/template/pdsold/p23.odt");
