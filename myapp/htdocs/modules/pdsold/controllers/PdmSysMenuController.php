...
use app\modules\pdsold\models\PdmSysMenuSearch;
use Yii;
use yii\db\Query;
use yii\web\Session;
...
class PdmSysMenuController extends Controller {
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->is_path = Yii::$app->basePath . "/web/template/pdsold/";

            if ($model->save()) {
                if ($files != false) {
                    $path = Yii::$app->basePath . '/web/template/pdsold/' . $files->name;
                    $files->saveAs($path);
                }
            }
    ...
    public function actionUpdate($id) {
        ...
        $oldFile = (isset($model->file_name) ? Yii::$app->basePath . '/web/template/pdsold/' . $model->file_name : null);
            ...
            try {
				...
				if ($files != false && !empty($files) ) {
					if($file_lama !=''){
						...
						$path = Yii::$app->basePath . '/web/template/pdsold/' . $file_lama;
						$files->saveAs($path);
					}else{
						$model->file_name = $file_lama;
						$path = Yii::$app->basePath . '/web/template/pdsold/' . $file_lama;
