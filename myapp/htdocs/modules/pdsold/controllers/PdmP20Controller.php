...
class PdmP20Controller extends Controller
{
           ...
           $jml_pt = Yii::$app->db->createCommand(" SELECT (count(*)+1) as jml FROM pidum.pdm_p20 WHERE id_berkas='".$modelPengantar->id_berkas."' AND (file_upload is NOT null OR file_upload <> '') ")->queryOne();
			...
			if ($files != false && !empty($files) ) {
				if($file_lama !=''){
					...
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p20_'.$file_lama.'.'.$files->extension;
					$files->saveAs($path);
				}else{
					$model->file_upload = preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p20_'.$jml_pt['jml'].'.'. $files->extension;
					$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p20_'.$jml_pt['jml'].'.'.$files->extension;
					$files->saveAs($path);
				}
			}else{
				...
				if ($files != false && !empty($files) ) {
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/p20_'.$model->id_p20.'.'. $files->extension;
						$files->saveAs($path);
				}
    ...
    public function actionCetak($id_p20) {
		 $id_perkara = Yii::$app->session->get('id_perkara');
        $connection = \Yii::$app->db;
        $odf = new \Odf(Yii::$app->params['report-path'] . "web/template/pdsold/p20.odt");
