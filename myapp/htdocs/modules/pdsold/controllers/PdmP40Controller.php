...
use Yii;
use app\modules\pdsold\models\PdmP40;
use app\modules\pdsold\models\PdmTembusanP40;
use app\modules\pdsold\models\PdmP40Search;
use yii\db\Query;
...
class PdmP40Controller extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->updated_ip = \Yii::$app->getRequest()->getUserIP();
            if($model->update()){
                PdmTembusanP40::deleteAll(['no_register_perkara'=>$no_register_perkara, 'no_surat_p40'=>$no_surat_p40]);
                if (isset($_POST['new_tembusan'])) {
                            for ($i = 0; $i < count($_POST['new_tembusan']); $i++) {
                                $modelNewTembusan = new PdmTembusanP40();
                                $modelNewTembusan->tembusan = $_POST['new_tembusan'][$i];
