...
use app\modules\pdsold\models\PdmPenyelesaianPratutSearch;
use app\modules\pdsold\models\PdmTembusanPratutLimpah;
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try{
				...
				if ($files != false && !empty($files) ) {
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id) . '/penyelesaianSPDP.'.$files->extension;
						$files->saveAs($path);
				}
			...
			$files = UploadedFile::getInstance($model, 'file_upload');
				...
				if ($files != false && !empty($files) ) {
					if($file_lama !=''){
						...
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/penyelesaianPratut_'.$file_lama.'.'.$files->extension;
						$files->saveAs($path);
					}else{
						$model->file_upload = preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/penyelesaianPratut_'.$jml_pt['jml'].'.'. $files->extension;
						$path = Yii::$app->basePath . '/web/template/pdsold_surat/' . preg_replace('/[^A-Za-z0-9\-]/', '',$id_perkara) . '/penyelesaianPratut_'.$jml_pt['jml'].'.'.$files->extension;
						$files->saveAs($path);
					}
				}else{
					$model->file_upload = $file_lama;
