...
use app\modules\datun\models\PutusanKasasi;

class PutusanKasasiController extends Controller{

    public function actionIndex(){
		if ($_SESSION['no_surat'] && $_SESSION['no_register_perkara']) {
		 	 $sqlnya="SELECT i.file_pts_kasasi,i.no_pts_kasasi,i.tanggal_pts_kasasi,i.amar_pts_kasasi, a.tanggal_panggilan_pengadilan, i.no_register_perkara as is_cek,a.no_register_perkara, a.no_surat,a.kode_jenis_instansi,
						...
						left join datun.putusan_kasasi i on i.no_register_perkara=a.no_register_perkara and i.no_surat=a.no_surat
			...
			$head 	= PutusanKasasi::findBySql($sqlnya)->asArray()->one();
    ...
    public function actionCek(){
		if (Yii::$app->request->isAjax) {
			$model = new PutusanKasasi;
			$nilai = $model->cekPtsKasasi(Yii::$app->request->post());
			\Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
			return ['hasil'=>$nilai['hasil'], 'error'=>$nilai['error'], 'element'=>$nilai['element']]; //asli
			//return array("hasil"=>false, "error"=>"tess", "element"=>"error_custom4");
		}    
	}
