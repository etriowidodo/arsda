...
use app\modules\datun\models\PutusanKontraBanding;

class PutusanKontraBandingController extends Controller{

    public function actionIndex(){
        if ($_SESSION['no_surat'] && $_SESSION['no_register_perkara'] && $_SESSION['no_register_skk'] && $_SESSION['tanggal_skk'] && $_SESSION['no_skks']) {
		$sqlnya="SELECT h.no_putusan,h.tanggal_putusan,i.no_pts_kontrabanding,i.tanggal_pts_kontrabanding,i.amar_pts_kontrabanding,i.file_pts_kontrabanding,
				...
				left join datun.pts_kontra_banding i on i.no_register_perkara=a.no_register_perkara and i.no_register_skks=d.no_register_skks
				...
				";
			$head 	= PutusanKontraBanding::findBySql($sqlnya)->asArray()->one();
    ...
    public function actionCek(){
		if (Yii::$app->request->isAjax) {
			$model = new PutusanKontraBanding;
			$nilai = $model->cekPtsKontraBanding(Yii::$app->request->post());
