...
use app\modules\datun\models\PutusanKontraKasasi;

class PutusanKontraKasasiController extends Controller{

    public function actionIndex(){
     if ($_SESSION['no_surat'] && $_SESSION['no_register_perkara'] && $_SESSION['no_register_skk'] && $_SESSION['tanggal_skk'] && $_SESSION['no_skks']) {
		$sqlnya="SELECT h.no_putusan,h.tanggal_putusan,i.no_pts_kontrakasasi,i.tanggal_pts_kontrakasasi,i.amar_pts_kontrakasasi,i.file_pts_kontrakasasi,
				...
				left join datun.pts_kontra_kasasi i on i.no_register_perkara=a.no_register_perkara and i.no_register_skks=d.no_register_skks
				...
				";
			$head 	= PutusanKontraKasasi::findBySql($sqlnya)->asArray()->one();
    ...
    public function actionCek(){
		if (Yii::$app->request->isAjax) {
			$model = new PutusanKontraKasasi;
			$nilai = $model->cekPtsKontraKasasi(Yii::$app->request->post());
