...
use app\modules\datun\models\PutusanBanding;

class PutusanBandingController extends Controller{

    public function actionIndex(){
		if ($_SESSION['no_surat'] && $_SESSION['no_register_perkara']) {
			$sqlnya="SELECT i.file_pts_banding,i.no_pts_banding,i.tanggal_pts_banding,i.amar_pts_banding,a.tanggal_panggilan_pengadilan, i.no_register_perkara as is_cek,a.no_register_perkara,a.no_surat,a.kode_jenis_instansi,
						...
						left join datun.putusan_banding i on i.no_register_perkara=a.no_register_perkara and i.no_surat=a.no_surat
			...
			$head 	= PutusanBanding::findBySql($sqlnya)->asArray()->one();
    ...
    public function actionCek(){
		if (Yii::$app->request->isAjax) {
			$model = new PutusanBanding;
			$nilai = $model->cekPtsBanding(Yii::$app->request->post());
