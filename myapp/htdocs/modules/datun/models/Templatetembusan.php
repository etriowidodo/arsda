...
class Templatetembusan extends \yii\db\ActiveRecord{
    ...
    public static function tableName(){
        return 'datun.template_tembusan';
    }
    ...
    public function searchCustom($get){
		...
		$sql = "select * from datun.template_tembusan where kode_template_surat = '".$id."'";
		if($q1)
    ...
    public function simpanData($post){
		...
		try{
			$sql1 = "delete from datun.template_tembusan where kode_template_surat = '".$kode."'";
			$connection->createCommand($sql1)->execute();
			foreach($post['tembusan'] as $idx=>$nilai){
				$tembus = htmlspecialchars($nilai, ENT_QUOTES);
				$nomor	= $idx+1;
				$sql2 	= "insert into datun.template_tembusan values('".$kode."', '".$nomor."', '".$tembus."')";
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$temp = explode("#", $val);
					$sql1 = "delete from datun.template_tembusan where kode_template_surat = '".$temp[0]."' and no_urut = '".$temp[1]."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
