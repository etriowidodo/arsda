...
class KontraBanding extends \yii\db\ActiveRecord{
	
    public static function tableName(){
        return 'datun.kontra_banding';
    }
	
   	public function cekKontraBanding($post){
		$connection 		= $this->db;
		$isNew 	        	= htmlspecialchars($post['isNewRecord'], ENT_QUOTES);
		$no_memori_banding  = htmlspecialchars($post['no_memori_banding'], ENT_QUOTES);
		if($isNew){
			$sqlCek = "select count(*) from datun.kontra_banding where kode_tk = '".$_SESSION['kode_tk']."' and kode_kejati = '".$_SESSION['kode_kejati']."' 
				   and kode_kejari = '".$_SESSION['kode_kejari']."' and kode_cabjari = '".$_SESSION['kode_cabjari']."' and no_memori_banding = '".$no_memori_banding."'";
			$count 	= $connection->createCommand($sqlCek)->queryScalar();
			if($count > 0){
				$pesan = '<i style="color:#dd4b39; font-size:12px;">No memori banding <b>'.$no_memori_banding.'</b> sudah ada</i>';
	...
	public function simpanData($post){
		...
		$tanggal_kontra_banding = htmlspecialchars($post['tanggal_kontra_banding']);		
		...
		$di_kontrabanding		= htmlspecialchars($post['di_kontrabanding'], ENT_QUOTES); 
		...
		$pathfile3	= Yii::$app->params['relas_kontra_banding'];
		$clean13 	= Yii::$app->inspektur->sanitize_filename($no_register_skks);
		$clean23 	= Yii::$app->inspektur->sanitize_filename($no_putusan);
		$newPhoto3 	= "relas_kontra_bdg_".$clean13."-".$clean23.$extPhoto3;
		
		$transaction = $connection->beginTransaction();
		try {
			
			if($isNewRecord){
				$sqlUpdate = "update datun.permohonan set status = 'KONTRA BANDING' where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
				$connection->createCommand($sqlUpdate)->execute();
				
				$upl1 = false;
				$upl2 = false;
				$upl3 = false;
				$sql1 = "insert into datun.kontra_banding(no_register_skks, no_putusan, no_register_perkara, kode_jenis_instansi, kode_instansi, kode_tk, 
						kode_kejati, kode_kejari, kode_cabjari, di_kontrabanding, kepada_yth, melalui,amar_putusan,
				...
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_kontrabanding";
				}
				if($filePhoto2 != ""){
					$upl2 = true;
					$sql1 .= ", file_memori_banding";
				}
				if($filePhoto3!= ""){
					$upl3 = true;
					$sql1 .= ", file_relas";
				...
				if ($tanggal_kontra_banding) $sql1 .=", tanggal_kontra_banding"; 
				...
				$sql1 .= ") values
						...
						'".$kode_kejati."', '".$kode_kejari."', '".$kode_cabjari."', '".$di_kontrabanding."', '".$kepada_yth."', '".$melalui."','".$amar."',
				...
				if ($tanggal_kontra_banding) $sql1 .=", '".$helpernya->tgl_db($tanggal_kontra_banding)."'";
				...
				$upl2 = false;
				$upl3 = false;
				$sql1 = "update datun.kontra_banding set  kepada_yth = '".$kepada_yth."', melalui = '".$melalui."',   
						di_kontrabanding = '".$di_kontrabanding."', amar_putusan='".$amar."', is_inkrah = '".$isInkrah."',
						updated_user = '".$updated_user."', updated_nip = '".$updated_nip."',updated_nama = '".$updated_nama."', 
						updated_ip = '".$updated_ip."', updated_date = NOW()";
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_kontrabanding = '".$newPhoto."'";
				}
				if($filePhoto2 != ""){
					$upl2 = true;
					$sql1 .= ", file_memori_banding = '".$newPhoto2."'";
				}
				if($filePhoto3 != ""){
					$upl3 = true;
				...
				if ($tanggal_kontra_banding) {
					$sql1 .=", tanggal_kontra_banding = '".$helpernya->tgl_db($tanggal_kontra_banding)."'";
				} else {
					$sql1 .=", tanggal_kontra_banding = NULL";
				}
				if ($tanggal_putusan) {
			...
			/* $asg2 = "delete from datun.kontra_banding_anak where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
			...
			if(count($post['jpnid']) > 0){
				foreach($post['jpnid'] as $idx=>$val){
					...
					$sql2 = "insert into datun.kontra_banding_anak (nip, nama, jabatan, pangkat, golongan, no_register_perkara,no_surat)
							values('".$nip."', '".$nama."', '".$jabatan."','".$pangkat."', '".$gol."', '".$no_register_perkara."','".$no_surat."')";
					$connection->createCommand($sql2)->execute();
				}
			}  */
			
			$sql3 = "delete from datun.anak_tergugat where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
			...
			if(count($post['nm_ins']) > 0){
				...
				foreach($post['nm_ins'] as $idx=>$val){
					...
					if($nama_tergugat){
						$noauto++;
						$sql4 = "insert into datun.anak_tergugat values('".$no_register_perkara."', '".$no_surat."', '".$nama_tergugat."', '".$jabatan."','', '".$noauto."')";
			...
			if($upl1){
				$tmpPot = glob($pathfile."kontra_bdg_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
			...
			if($upl2){
				$tmpPot2 = glob($pathfile2."memori_bdg_".$clean12."-".$clean22.".*");
				if(count($tmpPot2) > 0){
			...
			if($upl3){
				$tmpPot3 = glob($pathfile3."relas_kontra_bdg_".$clean13."-".$clean23.".*");
				if(count($tmpPot3) > 0){
