...
class PutusanKontraKasasi extends \yii\db\ActiveRecord{
	
    public static function tableName(){
        return 'datun.pts_kontra_kasasi';
    }
	
   	public function cekPtsKontraKasasi($post){
		$connection 			= $this->db;
		$isNew 	        		= htmlspecialchars($post['isNewRecord'], ENT_QUOTES);
		$no_pts_kontrakasasi  = htmlspecialchars($post['no_pts_kontrakasasi'], ENT_QUOTES);
		if($isNew){
			$sqlCek = "select count(*) from datun.pts_kontra_kasasi where kode_tk = '".$_SESSION['kode_tk']."' and kode_kejati = '".$_SESSION['kode_kejati']."' 
				   and kode_kejari = '".$_SESSION['kode_kejari']."' and kode_cabjari = '".$_SESSION['kode_cabjari']."' and no_pts_kontrakasasi = '".$no_pts_kontrakasasi."'";
			$count 	= $connection->createCommand($sqlCek)->queryScalar();
			if($count > 0){
				$pesan = '<i style="color:#dd4b39; font-size:12px;">No putusan kontra kasasi <b>'.$no_pts_kontrakasasi.'</b> sudah ada</i>';
	...
	public function simpanData($post){
		...
		$no_pts_kontrakasasi		= htmlspecialchars($post['no_pts_kontrakasasi'], ENT_QUOTES);
		$tanggal_pts_kontrakasasi	= htmlspecialchars($post['tanggal_pts_kontrakasasi'], ENT_QUOTES);				
		...
		$pathfile	= Yii::$app->params['pts_kontra_kasasi'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_putusan);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_pts_kontrakasasi);
		$newPhoto 	= "pts_kontra_kasasi_".$clean1."-".$clean2.$extPhoto;
		
		$transaction = $connection->beginTransaction();
		try {
			
			$sqlUpdate = "update datun.permohonan set status = 'PUTUSAN KONTRA KASASI' where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
			$connection->createCommand($sqlUpdate)->execute();
			
			if($isNewRecord){
				$upl1 = false;
				$sql1 = "insert into datun.pts_kontra_kasasi(no_register_skks,no_pts_kontrakasasi, tanggal_pts_kontrakasasi,no_putusan, tanggal_putusan, no_register_perkara, kode_jenis_instansi, kode_instansi, kode_tk, 
						kode_kejati, kode_kejari, kode_cabjari, amar_pts_kontrakasasi,
				...
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_pts_kontrakasasi";
				}
			
				$sql1 .= ") values
						('".$no_register_skks."', '".$no_pts_kontrakasasi."', '".$helpernya->tgl_db($tanggal_pts_kontrakasasi)."', '".$no_putusan."', '".$helpernya->tgl_db($tanggal_putusan)."', '".$no_register_perkara."', '".$kode_jenis_instansi."', '".$kode_instansi."', '".$kode_tk."', 
				...
				$upl1 = false;
				$sql1 = "update datun.pts_kontra_kasasi set  amar_pts_kontrakasasi = '".$amar."',
						updated_user = '".$updated_user."', updated_nip = '".$updated_nip."',updated_nama = '".$updated_nama."', 
						updated_ip = '".$updated_ip."', updated_date = NOW()";
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_pts_kontrakasasi = '".$newPhoto."'";
				}
			
				$sql1 .= " where no_pts_kontrakasasi = '".$no_pts_kontrakasasi."' and no_putusan = '".$no_putusan."' and no_register_skks = '".$no_register_skks."' 
							and tanggal_putusan = '".$helpernya->tgl_db($tanggal_putusan)."' and tanggal_pts_kontrakasasi = '".$helpernya->tgl_db($tanggal_pts_kontrakasasi)."'";
			}
			$connection->createCommand($sql1)->execute();
			
							
			if($upl1){
				$tmpPot = glob($pathfile."pts_kontra_kasasi_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
