...
class PutusanBanding extends \yii\db\ActiveRecord{
	
    public static function tableName(){
        return 'datun.putusan_banding';
    }
	
    public function cekPtsBanding($post){
		$connection = $this->db;
		$isNew 	                = htmlspecialchars($post['isNewRecord'], ENT_QUOTES);
		$no_pts_banding         = htmlspecialchars($post['no_pts_banding'], ENT_QUOTES);
		...
		if($isNew){
			$sqlCek = "select count(*) from datun.putusan_banding where kode_tk = '".$_SESSION['kode_tk']."' and kode_kejati = '".$_SESSION['kode_kejati']."' 
				   and kode_kejari = '".$_SESSION['kode_kejari']."' and kode_cabjari = '".$_SESSION['kode_cabjari']."' and no_pts_banding = '".$no_pts_banding."'";
			$count 	= $connection->createCommand($sqlCek)->queryScalar();
			if($count > 0){
				$pesan = '<i style="color:#dd4b39; font-size:12px;">No Putusan Banding <b>'.$no_pts_banding.'</b> sudah ada</i>';
	...
	public function simpanData($post){
		...
		$tanggal_pts_banding	= htmlspecialchars($post['tanggal_pts_banding'], ENT_QUOTES);
		...
		$pathfile	= Yii::$app->params['putusanBanding'];
		$clean1 	= Yii::$app->inspektur->sanitize_filename($no_putusan);
		$clean2 	= Yii::$app->inspektur->sanitize_filename($no_pts_banding);
		$newPhoto 	= "PtsBanding_".$clean1."-".$clean2.$extPhoto;
		
		$transaction = $connection->beginTransaction();
		try {
			
			if($isNewRecord){
				$sqlUpdate = "update datun.permohonan set status = 'Putusan PT' where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
				$connection->createCommand($sqlUpdate)->execute();
				
				$upl1 = false;
				$sql1 = "insert into datun.putusan_banding(no_pts_banding, tanggal_terima_jpn, tanggal_pts_banding, no_register_skks, no_register_perkara, kode_jenis_instansi, kode_instansi, kode_tk, 
						kode_kejati, kode_kejari, kode_cabjari, amar_pts_banding, created_user, created_nip, created_nama, created_ip,created_date, 
						updated_user, updated_nip, updated_nama, updated_date, updated_ip, no_register_skk, tanggal_skk, is_inkrah, no_surat";
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_pts_banding";
				}
				$sql1 .= ") values
						('".$no_pts_banding."', '".$helpernya->tgl_db($tanggal_terima_jpn)."', '".$helpernya->tgl_db($tanggal_pts_banding)."', '".$no_register_skks."', '".$no_register_perkara."', '".$kode_jenis_instansi."', '".$kode_instansi."', '".$kode_tk."', 
				...
				$upl1 = false;
				$sql1 = "update datun.putusan_banding set no_pts_banding = '".$no_pts_banding."',tanggal_pts_banding = '".$helpernya->tgl_db($tanggal_pts_banding)."',tanggal_terima_jpn = '".$helpernya->tgl_db($tanggal_terima_jpn)."', updated_user = '".$updated_user."', updated_nip = '".$updated_nip."',updated_nama = '".$updated_nama."', 
						updated_ip = '".$updated_ip."', updated_date = NOW(), is_inkrah = '".$isInkrah."'";
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_pts_banding = '".$newPhoto."'";
				}
			...
			if($upl1){
				$tmpPot = glob($pathfile."PtsBanding_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
