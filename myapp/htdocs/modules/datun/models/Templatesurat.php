...
class Templatesurat extends \yii\db\ActiveRecord{
    ...
    public static function tableName(){
        return 'datun.template_surat';
    }

    public function searchCustom($get){
		$q1  = htmlspecialchars($get['q1'], ENT_QUOTES);
		$sql = "select * from datun.template_surat where 1=1";
		if($q1)
    ...
    public function cekRole($post){
		...
		$sql 	= "select count(*) from datun.template_surat where upper(kode_template_surat) = '".strtoupper($q1)."'";
    ...
    public function simpanData($post){
		...
		try{
			if($isNew){
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 = "insert into datun.template_surat(kode_template_surat, deskripsi_template_surat, file_template_surat) 
							values('".$kode."', '".$deskripsi."', '".$newPhoto."')";
				} else{
					$upl1 = false;
					$sql1 = "insert into datun.template_surat(kode_template_surat, deskripsi_template_surat)values('".$kode."', '".$deskripsi."')";
				}
			}else{
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 = "update datun.template_surat set deskripsi_template_surat = '".$deskripsi."', file_template_surat = '".$newPhoto."' 
							where kode_template_surat = '".$kode."'";
				} else{
					$upl1 = false;
					$sql1 = "update datun.template_surat set deskripsi_template_surat = '".$deskripsi."' where kode_template_surat = '".$kode."'";
				}
			}
		...
		try {
			if(count($post['id']) > 0){
				foreach($post['id'] as $idx=>$val){
					$file = $connection->createCommand("select file_template_surat from datun.template_surat where kode_template_surat = '".$val."'")->queryScalar();
					if($file && file_exists($pathfile.$file)) unlink($pathfile.$file);

					$sql1 = "delete from datun.template_surat where kode_template_surat = '".$val."'";
					$connection->createCommand($sql1)->execute();
				}
			}
			$transaction->commit();
