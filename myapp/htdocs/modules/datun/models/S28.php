...
class S28 extends \yii\db\ActiveRecord{
	
    public static function tableName(){
        return 'datun.s28';
    }
	
    public function cekS28($post){ 
		$connection = $this->db;
		$isNew 	                = htmlspecialchars($post['isNewRecord2'], ENT_QUOTES);
		...
		$no_permohonan_pk 	    = htmlspecialchars($post['no_permohonan_pk'], ENT_QUOTES);
			
		if($isNew){
			$sqlCek = "select count(*) from datun.s28 where kode_tk = '".$_SESSION['kode_tk']."' and kode_kejati = '".$_SESSION['kode_kejati']."' 
				   and kode_kejari = '".$_SESSION['kode_kejari']."' and kode_cabjari = '".$_SESSION['kode_cabjari']."' and no_permohonan_pk = '".$no_permohonan_pk."'";
			$count 	= $connection->createCommand($sqlCek)->queryScalar();
			if($count > 0){
				$pesan = '<i style="color:#dd4b39; font-size:12px;">Nomor <b>'.$nomor.'</b> sudah ada</i>';
				return array("hasil"=>false, "error"=>$pesan, "element"=>"error_custom2");
			} else{
	...
	public function simpanData($post){
		...
		$tanggal_s28			= htmlspecialchars($post['tanggal_s28'], ENT_QUOTES);
		...
		$kepada_yth_s28			= htmlspecialchars($post['kepada_yth'], ENT_QUOTES);
		$di_s28					= htmlspecialchars($post['di_s28'], ENT_QUOTES); 
		$alasan					= str_replace(array("'"), array("&#039;"), $post['tab_alasan']);
		$isi_primer				= str_replace(array("'"), array("&#039;"), $post['tab_primair']);
		$isi_subsider			= str_replace(array("'"), array("&#039;"), $post['tab_subsidair']);
		$melalui				= htmlspecialchars($post['melalui'], ENT_QUOTES);
		$no_permohonan_pk		= htmlspecialchars($post['no_permohonan_pk'], ENT_QUOTES);		
		$tgl_permohonan_pk		= htmlspecialchars($post['tgl_permohonan_pk'], ENT_QUOTES);
		...
		$pathfile2	            = Yii::$app->params['permohonan_pk'];
		$clean12 	            = Yii::$app->inspektur->sanitize_filename($no_register_perkara);
		$clean22 	            = Yii::$app->inspektur->sanitize_filename($no_permohonan_pk);
		$newPhoto2 	            = "p_pk".$clean12."-".$clean22.$extPhoto2;
		...
		$pathfile3	            = Yii::$app->params['relas_s28'];
		$clean13 	            = Yii::$app->inspektur->sanitize_filename($no_register_perkara);
		$clean23 	            = Yii::$app->inspektur->sanitize_filename($no_putusan);
		$newPhoto3 	            = "relas_s28_".$clean13."-".$clean23.$extPhoto3;
		
		$transaction 			= $connection->beginTransaction();
		try {
			
			if($isNewRecord){
				$sqlUpdate = "update datun.permohonan set status = 'S28' where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
				$connection->createCommand($sqlUpdate)->execute();
			
				$upl1 = false;
				$upl2 = false;
				$upl3 = false;
				$sql1 = "insert into datun.s28(no_surat,no_register_skks, no_putusan, no_register_perkara, kode_jenis_instansi, kode_instansi, kode_tk, 
						kode_kejati, kode_kejari, kode_cabjari, di_s28, kepada_yth_s28, nama_jpn, alasan,  
						melalui,no_permohonan_pk,
						created_user, created_nip, created_nama, created_ip,created_date, 
						updated_user, updated_nip, updated_nama, updated_date, updated_ip,isi_petitum_primer,isi_petitum_subsider, no_register_skk, tanggal_skk, is_inkrah";
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_s28";
				}
				if($filePhoto2 != ""){
					$upl2 = true;
					$sql1 .= ", file_permohonan_pk ";
				}
				if($filePhoto3 != ""){
					$upl3 = true;
					$sql1 .= ", file_relas ";
				}
				if ($tanggal_s28) $sql1 .=", tanggal_s28";
				if ($tanggal_putusan) $sql1 .=", tanggal_putusan";
				if ($tgl_permohonan_pk) $sql1 .=", tgl_permohonan_pk";
				
				$sql1 .= ") values
						('".$no_surat."','".$no_register_skks."', '".$no_putusan."', '".$no_register_perkara."', '".$kode_jenis_instansi."', '".$kode_instansi."', '".$kode_tk."', 
						'".$kode_kejati."', '".$kode_kejari."', '".$kode_cabjari."', '".$di_s28."', '".$kepada_yth_s28."', '', '".$alasan."',
						'".$melalui."', '".$no_permohonan_pk."',
						'".$created_user."', '".$created_nip."', '".$created_nama."','".$created_ip."', NOW(), 
						'".$updated_user."', '".$updated_nip."', '".$updated_nama."', NOW(), '".$updated_ip."','".$isi_primer."','".$isi_subsider."', '".$no_register_skk."', '".$helpernya->tgl_db($tanggal_skk)."','".$isInkrah."'";

				if($filePhoto != "") $sql1 .= ", '".$newPhoto."'";
				if($filePhoto2 != "") $sql1 .= ", '".$newPhoto2."'";
				if($filePhoto3 != "") $sql1 .= ", '".$newPhoto3."'";
				
				if ($tanggal_s28) $sql1 .=", '".$helpernya->tgl_db($tanggal_s28)."'";
				if ($tanggal_putusan) $sql1 .=", '".$helpernya->tgl_db($tanggal_putusan)."'";
				if ($tgl_permohonan_pk) $sql1 .=", '".$helpernya->tgl_db($tgl_permohonan_pk)."'";
				$sql1 .= ")";
				...
				$upl2 = false;
				$upl3 = false;
				$sql1 = "update datun.s28 set  di_s28 = '".$di_s28."', kepada_yth_s28 = '".$kepada_yth_s28."', 
						melalui = '".$melalui."', no_permohonan_pk = '".$no_permohonan_pk."',
						alasan = '".$alasan."', updated_user = '".$updated_user."', updated_nip = '".$updated_nip."',updated_nama = '".$updated_nama."',
						isi_petitum_primer = '".$isi_primer."',isi_petitum_subsider = '".$isi_subsider."',
						updated_ip = '".$updated_ip."', updated_date = NOW(), is_inkrah = '".$isInkrah."'";
				if($filePhoto != ""){
					$upl1 = true;
					$sql1 .= ", file_s28 = '".$newPhoto."'";
				}
				if($filePhoto2 != ""){
					$upl2 = true;
					$sql1 .= ", file_permohonan_pk = '".$newPhoto2."'";
				} 
				if($filePhoto3 != ""){
					$upl3 = true;
				...
				if ($tanggal_s28) {
					$sql1 .=", tanggal_s28 = '".$helpernya->tgl_db($tanggal_s28)."'";
				} else {
					$sql1 .=", tanggal_s28 = NULL";
				}
				if ($tanggal_putusan) {
				...
				if ($tgl_permohonan_pk) {
					$sql1 .=", tgl_permohonan_pk = '".$helpernya->tgl_db($tgl_permohonan_pk)."'";
				} else {
					$sql1 .=", tgl_permohonan_pk = NULL";
				}
				$sql1 .= " where no_register_perkara = '".$no_register_perkara."' and no_surat='".$no_surat."'";
			}
			$connection->createCommand($sql1)->execute();
			
			$asg2 = "delete from datun.s28_anak where no_register_perkara = '".$no_register_perkara."' and no_surat = '".$no_surat."'";
			...
			if(count($post['jpnid']) > 0){
				foreach($post['jpnid'] as $idx=>$val){
					...
					$sql2 = "insert into datun.s28_anak (nip, nama, jabatan, pangkat, golongan, no_register_perkara,no_surat)
			...
			if($upl1){
				$tmpPot = glob($pathfile."s28_".$clean1."-".$clean2.".*");
				if(count($tmpPot) > 0){
			...
			if($upl2){
				$tmpPot2 = glob($pathfile2."p_pk".$clean12."-".$clean22.".*");
				if(count($tmpPot2) > 0){
			...
			if($upl3){
				$tmpPot3 = glob($pathfile3."relas_s28_".$clean13."-".$clean23.".*");
				if(count($tmpPot3) > 0){
