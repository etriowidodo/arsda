...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			...
			if(amar == ''){
				...
				msg ="Text editor masih kosong, Apakah anda tetap ingin menyimpan data?";
				bootbox.confirm({ 
					message: msg,
					size: "small",
					closeButton: false,
	...
	function booxqution2(){
		...
		if (cek2 == 0 && typeof(filenya2) == 'undefined') {
			bootbox.confirm({ 
			message: "File memori PK masih kosong, tetap simpan data tanpa melampirkan file memori PK?",
			size: "small",
			closeButton: false,
			buttons: {
				confirm: {label:'<div style="min-width:30px">Ya</div>', className:'btn-primary pull-right jarak-kanan'},
	...
	function booxqution3(){
		...
		if (cek == 0 && typeof(filenya) == 'undefined') {
			bootbox.confirm({ 
			message: "File kontra PK masih kosong, tetap simpan data tanpa melampirkan file kontra PK?",
			size: "small",
			closeButton: false,
			buttons: {
				confirm: {label:'<div style="min-width:30px">Ya</div>', className:'btn-primary pull-right jarak-kanan'},
	...
	function validasi_upload(){ 
		...
		var tgl_kontra_pk		= new Date(tgl_auto($("#tanggal_kontra_pk").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			$("body").removeClass("loading");
			$("#error_custom1").html('<i style="color:#dd4b39; font-size:12px;">Tanggal kontra PK harus lebih besar atau sama dengan tanggal '+war2+'</i>');
			setErrorFocus($("#tanggal_kontra_pk"), $("#role-form"), false);
			return false;
		} else if(tgl_kontra_pk > hariIni){
			$("body").removeClass("loading");
			$("#error_custom1").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal kontra PK adalah hari ini</i>');
			setErrorFocus($("#tanggal_kontra_pk"), $("#role-form"), false);
			return false;
		} else {
