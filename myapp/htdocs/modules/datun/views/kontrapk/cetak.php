<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/datun/template/kontra-pk.docx');
	...
	$tanggal_kontra_pk 		= date('d-m-Y',strtotime($model['tanggal_kontra_pk']));
	...
	$sql1 = "select a.nip, a.nama, a.jabatan from datun.kontra_memori_pk_anak a join datun.kontra_memori_pk b
			on a.no_register_skks = b.no_register_skks and a.no_putusan = b.no_putusan and a.tanggal_putusan = b.tanggal_putusan
			and a.tanggal_kontra_pk = b.tanggal_kontra_pk
			...
			and a.tanggal_kontra_pk = NULLIF('".$model['tanggal_kontra_pk']."',' ')::date order by a.nip";
	$res1 = KontraPK::findBySql($sql1)->asArray()->all();
	$jaksa_negara1	="<p> </p>";
	$kuasa_jpn		="..............";
	if(count($res1) > 0){
	...
	$sqla = "select a.nip, a.nama_tergugat as nama, a.jabatan from datun.anak_tergugat_kontra_pk a join datun.kontra_memori_pk b
			on a.no_register_skks = b.no_register_skks and a.no_putusan = b.no_putusan and a.tanggal_putusan = b.tanggal_putusan
			and a.tanggal_kontra_pk = b.tanggal_kontra_pk
			where a.no_register_skks = '".$model['no_register_skks']."' and a.no_putusan = '".$model['no_putusan']."' and a.tanggal_putusan = '".$model['tanggal_putusan']."'
			and a.tanggal_kontra_pk = NULLIF('".$model['tanggal_kontra_pk']."','')::date order by a.no_urut";
	$res1 = KontraPK::findBySql($sqla)->asArray()->all();
	$jaksa_negara2="<p> </p>";
	if(count($res1) > 0){
	...
	$docx->replaceVariableByText(array('tgl_pk'=>tgl_indo($tanggal_kontra_pk)), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('di'=>($model['di_kontrapk'])?$model['di_kontrapk']:' '), array('parseLineBreaks'=>true));
	...
	$docx->createDocx('template/datun/kontra-pk');
	$file = 'template/datun/kontra-pk.docx';
	if (file_exists($file)) {
