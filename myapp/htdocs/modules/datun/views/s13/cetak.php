<?php
	...
	$docx 		= new CreateDocxFromTemplate('../modules/datun/template/S-13.docx');
	...
	$ctanggal_diterima 	= ($model['tanggal_diterima'])?date("d-m-Y", strtotime($model['tanggal_diterima'])):"";		
	$ctanggal_skk 		= ($model['tanggal_skk'])?date("d-m-Y", strtotime($model['tanggal_skk'])):"";	
	$ctanggal_skks 		= ($model['tgl_skks'])?date("d-m-Y", strtotime($model['tgl_skks'])):"";	
	...
	$docx->replaceVariableByText(array('tanggal_diterima'=>tgl_indo($ctanggal_diterima)),array('parseLineBreaks'=>true));		
	...
	$docx->replaceVariableByHTML('jpn2', 'block', $jaksa_negara, $arrDocnya);	
	...
	$docx->replaceVariableByHTML('prim_konvensirekonvensi', 'block','<div style="margin-left:18px; line-height:30px; font-family:Trebuchet MS;">'.$cprim_konvensirekonvensi.'</div>', $arrDocnya);	
	...
	$docx->createDocx('template/datun/EKSEPSI DAN JAWABAN POKOK PERKARA');	
	$file = 'template/datun/EKSEPSI DAN JAWABAN POKOK PERKARA.docx';
	if (file_exists($file)) {
