...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			...
			if(amar == '' || (cek == 0 && typeof(filenya) == 'undefined')){
				...
				msg+=" putusan kontra banding masih kosong, Apakah anda tetap ingin menyimpan data?";
				bootbox.confirm({ 
					message: msg,
					size: "small",
					closeButton: false,
	...
	function validasi_upload(){ 
		...
		var tgl_kontra_banding	= new Date(tgl_auto($("#tanggal_pts_kontrabanding").val()));
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			$("body").removeClass("loading");
			$("#error_custom1").html('<i style="color:#dd4b39; font-size:12px;">Tanggal putusan kontra banding harus lebih besar atau sama dengan tanggal putusan</i>');
			setErrorFocus($("#tanggal_pts_kontrabanding"), $("#role-form"), false);
			return false;
		} else if(tgl_kontra_banding > hariIni){
			$("body").removeClass("loading");
			$("#error_custom1").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal putusan kontra banding adalah hari ini</i>');
			setErrorFocus($("#tanggal_pts_kontrabanding"), $("#role-form"), false);
			return false;
		} else {
				$.ajax({
						type	: "POST",
						url		: '<?php echo Yii::$app->request->baseUrl.'/datun/putusan-kontra-banding/cek'; ?>',
