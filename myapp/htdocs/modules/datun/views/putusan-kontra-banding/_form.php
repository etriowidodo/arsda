<?php
	...
	$this->title 	= 'Putusan Kontra Banding';
	$linkBatal		= '/datun/putusan/index';
	$tanggal_pts_kontrabanding = ($head['tanggal_pts_kontrabanding'])?date('d-m-Y',strtotime($head['tanggal_pts_kontrabanding'])):'';
    ...
    $isNewRecord 	= ($head['no_pts_kontrabanding'])?0:1; 
?>

<?php if($_SESSION['no_skks'] && $_SESSION['no_register_perkara']){ ?>
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/datun/putusan-kontra-banding/simpan" enctype="multipart/form-data">
...
<div class="row">  
    ...
    <div class="col-md-6">
        <div class="form-group form-group-sm">
            <label class="control-label col-md-4">Tanggal Pts Kontra Banding</label>        
            <div class="col-md-4">
                <div class="input-group date">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input type="text" class="form-control <?php echo ($isNewRecord)?'datepicker':''; ?>" id="tanggal_pts_kontrabanding"  name="tanggal_pts_kontrabanding" value="<?php echo $tanggal_pts_kontrabanding;?>" <?php echo ($isNewRecord)?'':'readonly'; ?> placeholder="DD-MM-YYYY" required data-error="Tanggal putusan kontra banding belum diisi"/>
                </div>                      
            </div>
...
<div class="row">  
    <div class="col-md-6">
        <div class="form-group form-group-sm">
            <label class="control-label col-md-4">Nomor Pts Kontra Banding</label>        
            <div class="col-md-8">
                <input type="text" name="no_pts_kontrabanding" id="no_pts_kontrabanding" <?php echo ($isNewRecord)?'':'readonly'; ?> class="form-control" value="<?php echo $head['no_pts_kontrabanding']; ?>" required data-error="Nomor putusan kontra banding belum diisi" maxlength="30"  />
                        ...
                        <div class="panel-body">
                            <div class="tab-content">
                                <div class="tab-pane fade in active" id="tab-amar">
                                    <textarea class="ckeditor" id="tab_amar" name="tab_amar"><?php echo $head['amar_pts_kontrabanding']; ?></textarea>
...
<div class="row">
    <div class="col-md-12">
        <div class="form-group form-group-sm">
            <div class="col-md-12">
				 <?php
					$pathFile 	= Yii::$app->params['pts_kontra_banding'].$head['file_pts_kontrabanding'];
					$cek=($head['file_pts_kontrabanding'] && file_exists($pathFile))?1:0;
				?>
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['pts_kontra_banding'].$head['file_pts_kontrabanding'];
                        $labelFile 	= 'Upload File Putusan Kontra Banding';
                        if($head['file_pts_kontrabanding'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah File Putusan Kontra Banding';
                            $param1  	= chunk_split(base64_encode($pathFile));
                            $param2  	= chunk_split(base64_encode($head['file_pts_kontrabanding']));
                            $linkPt 	= "/datun/download-file/index?id=".$param1."&fn=".$param2;
                            $extPt		= substr($head['file_pts_kontrabanding'], strrpos($head['file_pts_kontrabanding'],'.'));
                            echo '<a href="'.$linkPt.'" title="'.$head['file_pts_kontrabanding'].'" style="float:left; margin-right:10px;">
