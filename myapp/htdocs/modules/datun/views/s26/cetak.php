<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/datun/template/S26.docx');
	...
	$tanggalS26             = date('d-m-Y',strtotime($model['tanggal_s26']));
	$tanggalSkk             = date('d-m-Y',strtotime($model['tanggal_skk']));
	$tanggalSkks             = date('d-m-Y',strtotime($model['tanggal_skks']));
	$tgl_per_kasasi         = date('d-m-Y',strtotime($model['tgl_permohonan_kasasi']));  
	$tembusan_surat         = "";
	$jaksa_negara 	        = "";
	$amarPutusan 			= ($model['amar_putusan'])? '<div style=" line-height:30px; font-family:Trebuchet MS;"> '.$model['amar_putusan'].'</div>':'........................';
	$alasanKasasi 			= ($model['alasan_kasasi'])? '<div style=" line-height:30px; font-family:Trebuchet MS;"> '.$model['alasan_kasasi'].'</div>':'........................';
	...
	if($model['tanggal_s26']){
		$where = " and a.tanggal_s26 = '".$model['tanggal_s26']."' ";
	} else {
		$where ="";
	}
	
	$sql2 = "select a.nip, a.nama, a.jabatan from datun.s26_anak a
			inner join datun.s26 b on a.no_putusan=b.no_putusan and a.tanggal_putusan=b.tanggal_putusan 
			and a.no_register_skks=b.no_register_skks and a.tanggal_s26=b.tanggal_s26
			where a.no_register_skks='".$model['no_register_skks']."' and a.no_putusan = '".$model['no_putusan']."' 
			and a.tanggal_putusan = '".$model['tanggal_putusan']."'".$where." order by nip";
	$res2 = S26::findBySql($sql2)->asArray()->all();
	$kuasa_skk ="";
	...
	$res3 = S26::findBySql($sql3)->asArray()->all();
	$tgl_sidang="";
	if(count($res3) > 0){
		$nom2 = 0;
	...
	$docx->replaceVariableByText(array('tgl_s26'=>($tanggalS26)?tgl_indo($tanggalS26):' '), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('lokasi_keluar'=>$lokSatker), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('yth'=>($model['kepada_yth_s26'])?$model['kepada_yth_s26']:' '), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('diS26'=>($model['di_s26'])?$model['di_s26']:' '), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('melalui'=>($model['melalui'])?$model['melalui']:' '), array('parseLineBreaks'=>true));
	$docx->replaceVariableByText(array('jabatan'=>($model['jabatan_pegawai'])?$model['jabatan_pegawai']:' '), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('tgl_per_kasasi'=>($tgl_per_kasasi)?tgl_indo($tgl_per_kasasi):' '), array('parseLineBreaks'=>true)); 
	...
	$docx->replaceVariableByHTML('alasanKasasi', 'block', $alasanKasasi, $arrDocnya); 
	...
	$docx->createDocx('template/datun/S26');
	$file = 'template/datun/S26.docx';
	if (file_exists($file)) {
