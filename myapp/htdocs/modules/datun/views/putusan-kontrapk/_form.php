<?php
	...
	$this->title 	= 'Putusan Kontra PK';
	$linkBatal		= '/datun/putusan/index';
	$tanggal_pts_kontrapk = ($head['tanggal_pts_kontrapk'])?date('d-m-Y',strtotime($head['tanggal_pts_kontrapk'])):'';
    ...
    $isNewRecord 	= ($head['no_pts_kontrapk'])?0:1; 
?>

<?php if($_SESSION['no_surat'] && $_SESSION['no_register_perkara'] && $_SESSION['no_skks']){ ?>
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/datun/putusan-kontrapk/simpan" enctype="multipart/form-data">
...
<div class="row">  
    ...
    <div class="col-md-6">
        <div class="form-group form-group-sm">
            <label class="control-label col-md-4">Tanggal Pts Kontra PK</label>        
            <div class="col-md-4">
                <div class="input-group date">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input type="text" class="form-control <?php echo ($isNewRecord)?'datepicker':''; ?>" id="tanggal_pts_kontrapk"  name="tanggal_pts_kontrapk" value="<?php echo $tanggal_pts_kontrapk;?>" <?php echo ($isNewRecord)?'':'readonly'; ?> placeholder="DD-MM-YYYY" required data-error="Tanggal putusan kontra PK belum diisi"/>
                </div>                      
            </div>
...
<div class="row">  
    <div class="col-md-6">
        <div class="form-group form-group-sm">
            <label class="control-label col-md-4">Nomor Pts Kontra PK</label>        
            <div class="col-md-8">
                <input type="text" name="no_pts_kontrapk" id="no_pts_kontrapk" <?php echo ($isNewRecord)?'':'readonly'; ?> class="form-control" value="<?php echo $head['no_pts_kontrapk']; ?>" required data-error="Nomor putusan kontra PK belum diisi" maxlength="30"  />
                        ...
                        <div class="panel-body">
                            <div class="tab-content">
                                <div class="tab-pane fade in active" id="tab-amar">
                                    <textarea class="ckeditor" id="tab_amar" name="tab_amar"><?php echo $head['amar_pts_kontrapk']; ?></textarea>
...
<div class="row">
    <div class="col-md-12">
        <div class="form-group form-group-sm">
            <div class="col-md-12">
				 <?php
					$pathFile 	= Yii::$app->params['pts_kontra_pk'].$head['file_pts_kontrapk'];
					$cek=($head['file_pts_kontrapk'] && file_exists($pathFile))?1:0;
				?>
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['pts_kontra_pk'].$head['file_pts_kontrapk'];
                        $labelFile 	= 'Upload File Putusan Kontra PK';
                        if($head['file_pts_kontrapk'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah File Putusan Kontra PK';
                            $param1  	= chunk_split(base64_encode($pathFile));
                            $param2  	= chunk_split(base64_encode($head['file_pts_kontrapk']));
                            $linkPt 	= "/datun/download-file/index?id=".$param1."&fn=".$param2;
                            $extPt		= substr($head['file_pts_kontrapk'], strrpos($head['file_pts_kontrapk'],'.'));
                            echo '<a href="'.$linkPt.'" title="'.$head['file_pts_kontrapk'].'" style="float:left; margin-right:10px;">
