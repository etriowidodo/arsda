<?php
	...
	$linkCetak		= '/datun/kontra-banding/cetak';
	$tanggal_kontra_banding = ($head['tanggal_kontra_banding'])?date('d-m-Y',strtotime($head['tanggal_kontra_banding'])):'';
...
<form id="role-form" name="role-form" class="form-validasi form-horizontal" method="post" action="/datun/kontra-banding/simpan" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-sm">
                            <div class="col-md-12">
							<?php
								$pathFile3 	= Yii::$app->params['relas_kontra_banding'].$head['file_relas'];
							...
							<input type="hidden" name="cek_file3" id="cek_file3" value="<?php echo $cek3; ?>">
                                ...
                                <label for="file_template3" class="label-inputfile">
                                    <?php 
                                        $pathFile   = Yii::$app->params['relas_kontra_banding'].$head['file_relas'];
                                        $labelFile  = 'Upload File Relaas';
                                        if($head['file_relas'] && file_exists($pathFile)){
                                            $labelFile  = 'Ubah File Relaas';
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-sm">
                            <div class="col-md-12">
                            <?php
                                $pathFile2  = Yii::$app->params['memori_banding'].$head['file_memori_banding'];
                                $cek2=($head['file_memori_banding'] && file_exists($pathFile2))?1:0;
                            ?>
                            <input type="hidden" name="cek_file2" id="cek_file2" value="<?php echo $cek2; ?>">
                                ...
                                <label for="file_template2" class="label-inputfile">
                                    <?php 
                                        $pathFile   = Yii::$app->params['memori_banding'].$head['file_memori_banding'];
                                        $labelFile  = 'Upload File Memori Banding';
                                        if($head['file_memori_banding'] && file_exists($pathFile)){
                                            $labelFile  = 'Ubah File Memori Banding';
                                            $param1     = chunk_split(base64_encode($pathFile));
                                            $param2     = chunk_split(base64_encode($head['file_memori_banding']));
                                            $linkPt     = "/datun/download-file/index?id=".$param1."&fn=".$param2;
                                            $extPt      = substr($head['file_memori_banding'], strrpos($head['file_memori_banding'],'.'));
                                            echo '<a href="'.$linkPt.'" title="'.$head['file_memori_banding'].'" style="float:left; margin-right:10px;">
                                            <img src="'.Yii::$app->inspektur->getIconFile($extPt).'" /></a>';
                                        }
                                    ?>
                                    <div class="input-group">
                    ...
                    <div class="col-md-12">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-4">
                                <div class="input-group">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                     <input type="text" class="form-control datepicker" id="tanggal_kontra_banding"  name="tanggal_kontra_banding" value="<?php echo $tanggal_kontra_banding;?>" placeholder="DD-MM-YYYY"/>
                                </div>
                            </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <textarea id="kepada_yth" name="kepada_yth" class="form-control" style="height:80px;" maxlength="200"><?php echo $head['kepada_yth']; ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">        
                    <div class="col-md-12">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-md-8">
                                <input type="text" name="di_kontrabanding" id="di_kontrabanding" class="form-control" value="<?php echo $head['di_kontrabanding']; ?>" maxlength="30"/>
                                <div class="help-block with-errors" id="error_custom4"></div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="col-md-6">
                        ...
                        <?php 
                           /*  if($isNewRecord){
								...
								$sqlnya = "select nip, nama, jabatan as jabatan_jpn,pangkat as pangkat_jpn  , golongan as gol_jpn, pangkat||' ('||golongan||')' as pangkatgol   
                                        from datun.kontra_banding_anak where no_register_skks = '".$head['no_register_skks']."' and no_putusan = '".$head['no_putusan']."'
										and tanggal_putusan = '".$head['tanggal_putusan']."' and tanggal_kontra_banding = '".$head['tanggal_kontra_banding']."' order by nip";
                           	}
							$hasil = User::findBySql($sqlnya)->asArray()->all();
                            if(count($hasil) == 0)
        ...
        <div class="box box-primary" style="border-color:#f39c12; overflow:hidden;">
            <div class="box-header with-border" style="border-color:#c7c7c7; margin:10px 10px 0px;">
                <h3 class="box-title">Pembanding</h3>
            </div>
            <div class="box-body" style="padding:15px;">
                    ...
                    <table class="table table-bordered" id="table3">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" width="86%">Nama</th>
                        ...
                        <?php
                        	if (!$isNewRecord) {
								$sql2 = "select no_urut, nama_tergugat as nama from datun.anak_tergugat where no_register_perkara = '".$_SESSION['no_register_perkara']."' and no_surat = '".$_SESSION['no_surat']."' order by no_urut";
							} else {
								$sql2 = "select no_urut, nama_instansi as nama from datun.lawan_pemohon where no_register_perkara = '".$_SESSION['no_register_perkara']."' and no_surat = '".$_SESSION['no_surat']."'  order by no_urut";
							}							
							$arr2 = User::findBySql($sql2)->asArray()->all();
								if(count($arr2) > 0){
									$nom2 = 0;
									foreach($arr2 as $data2){
										...
										echo '<tr data-id="'.$nom2.'">
											...
											<td><input type="text" name="nm_ins[]" id="nm_ins'.$nom2.'" class="form-control input-sm" value="'.$data2['nama'].'" /></td>
                ...
                <div class="help-block with-errors" id=""></div>
            </div>         
        </div>
    </div>
</div>
                ...
                <div class="panel with-nav-tabs panel-default">
                    <div class="panel-heading single-project-nav">
                        <ul class="nav nav-tabs"> 
                            <li class="active"><a href="#tab-amar" data-toggle="tab">AMAR PUTUSAN KONTRA BANDING</a></li>  
                        </ul>
                    </div>
...
<div class="row">
    <div class="col-md-12">
        <div class="form-group form-group-sm">
            <div class="col-md-12">
				 <?php
					$pathFile 	= Yii::$app->params['kontra_banding'].$head['file_kontrabanding'];
					$cek=($head['file_kontrabanding'] && file_exists($pathFile))?1:0;
				?>
                ...
                <label for="file_template" class="label-inputfile">
                    <?php 
                        $pathFile 	= Yii::$app->params['kontra_banding'].$head['file_kontrabanding'];
                        $labelFile 	= 'Upload File Kontra Banding';
                        if($head['file_kontrabanding'] && file_exists($pathFile)){
                            $labelFile 	= 'Ubah File Kontra Banding';
                            $param1  	= chunk_split(base64_encode($pathFile));
                            $param2  	= chunk_split(base64_encode($head['file_kontrabanding']));
                            $linkPt 	= "/datun/download-file/index?id=".$param1."&fn=".$param2;
                            $extPt		= substr($head['file_kontrabanding'], strrpos($head['file_kontrabanding'],'.'));
                            echo '<a href="'.$linkPt.'" title="'.$head['file_kontrabanding'].'" style="float:left; margin-right:10px;">
                            <img src="'.Yii::$app->inspektur->getIconFile($extPt).'" /></a>';
                        }
                    ?>
                    <div class="input-group">
