<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/datun/template/SKKS.docx');
	...
	if(count($res1) > 0){
		...
		foreach($res1 as $data1){
			$jumlah++;
			if($jumlah == 1){
				$berdasarkan_skk .= $data1['tipenya'].(!in_array($model['kode_jenis_instansi'], array("01","06"))?' Nomor : '.$data1['no_register_skks']:'');
				$berdasarkan_skk .= ' tanggal '.tgl_indo(date('d/m/Y', strtotime($data1['tanggal_skks']))).' dari '.$data1['pemberi_kuasa'].' kepada '.$data1['penerima_kuasa'];
	...
	$docx->createDocx('template/datun/SKKS');
	$file = 'template/datun/SKKS.docx';
	if (file_exists($file)) {
