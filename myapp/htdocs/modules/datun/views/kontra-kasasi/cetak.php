<?php
	...
	$docx = new CreateDocxFromTemplate('../modules/datun/template/kontra-kasasi.docx');
	...
	$tanggal_kontra_kasasi	= date('d-m-Y',strtotime($model['tanggal_kontra_kasasi']));
	...
	$sql1 = "select a.nip, a.nama, a.jabatan from datun.kontra_kasasi_anak a join datun.kontra_kasasi b
			on a.no_register_skks = b.no_register_skks and a.no_putusan = b.no_putusan and a.tanggal_putusan = b.tanggal_putusan
			and a.tanggal_kontra_kasasi = b.tanggal_kontra_kasasi
			...
			and a.tanggal_kontra_kasasi = NULLIF('".$model['tanggal_kontra_kasasi']."',' ')::date order by a.nip";
	$res1 = KontraBanding::findBySql($sql1)->asArray()->all();
	$jaksa_negara1	="<p> </p>";
	$kuasa_jpn			="...............";
	if(count($res1) > 0){
		...
		//===================================================
		$nom2 = 0;
		$kuasa_jpn = '<table border="0" width="100%" style="border-collapse:collapse;"><tbody>';
		foreach($res1 as $data1){
			$nom2++;
			$kuasa_jpn .= '
				<tr>
					<td style="font-family:Trebuchet MS; font-size:11pt;" width="3%">'.$nom2.'. </td>
					<td style="font-family:Trebuchet MS; font-size:11pt;" width="97%">'.$data1['nama'].'</td>
				</tr>';
		}
		$kuasa_jpn .= '</tbody></table>';		
	}
	
	$sqla = "select a.nip, a.nama_tergugat as nama, a.jabatan from datun.anak_tergugat_kontra_kasasi a join datun.kontra_kasasi b
			on a.no_register_skks = b.no_register_skks and a.no_putusan = b.no_putusan and a.tanggal_putusan = b.tanggal_putusan
			and a.tanggal_kontra_kasasi = b.tanggal_kontra_kasasi
			where a.no_register_skks = '".$model['no_register_skks']."' and a.no_putusan = '".$model['no_putusan']."' and a.tanggal_putusan = '".$model['tanggal_putusan']."'
			and a.tanggal_kontra_kasasi = NULLIF('".$model['tanggal_kontra_kasasi']."',' ')::date order by a.no_urut";
	...
	$docx->replaceVariableByText(array('tgl_kasasi'=>tgl_indo($tanggal_kontra_kasasi)), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByText(array('di'=>($model['di_kontrakasasi'])?$model['di_kontrakasasi']:' '), array('parseLineBreaks'=>true));
	...
	$docx->replaceVariableByHTML('kuasa', 'block', $kuasa_jpn, $arrDocnya);
	...
	$docx->createDocx('template/datun/kontra-kasasi');
	$file = 'template/datun/kontra-kasasi.docx';
	if (file_exists($file)) {
