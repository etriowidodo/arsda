...
$(document).ready(function(){
	...
	$('#role-form').validator({disable:false}).on('submit', function(e){
		if(!e.isDefaultPrevented()){
			...
			var no_banding = $('#no_permohonan_banding').val(), tgl_banding = $('#tgl_permohonan_banding').val();
			...
			if(alasan == '' || primair == '' || subsidair == '' || no_banding == '' || tgl_banding == '' ){
				...
				if (no_banding ==''){ msg+=msg1+" Nomor akta pernyataan banding"; msg1="," } 
				if (tgl_banding ==''){ msg+=msg1+" Tanggal akta pernyataan banding";  msg1=","} 
	...
	function booxqution1(){
		...
		if (cek3 == 0 && typeof(filenya3) == 'undefined') {
			bootbox.confirm({ 
			message: "File akte penyerahan banding masih kosong, tetap simpan data tanpa melampirkan file akte penyerahan banding?",
			size: "small",
			closeButton: false,
			buttons: {
				confirm: {label:'<div style="min-width:30px">Ya</div>', className:'btn-primary pull-right jarak-kanan'},
	...
	function booxqution2(){
		...
		if (cek2 == 0 && typeof(filenya2) == 'undefined') {
			bootbox.confirm({ 
			message: "File akte pernyataan banding masih kosong, tetap simpan data tanpa melampirkan file akte pernyataan banding?",
			size: "small",
			closeButton: false,
			buttons: {
				confirm: {label:'<div style="min-width:30px">Ya</div>', className:'btn-primary pull-right jarak-kanan'},
	...
	function booxqution3(){
		...
		if (cek == 0 && typeof(filenya) == 'undefined') {
			bootbox.confirm({ 
			message: "File S-25 masih kosong, tetap simpan data tanpa melampirkan file S-25?",
			size: "small",
			closeButton: false,
			buttons: {
				confirm: {label:'<div style="min-width:30px">Ya</div>', className:'btn-primary pull-right jarak-kanan'},
	...
	/* 	var kepada = $('#kepada_yth_s25').val(), melalui = $('#melalui').val();
		$("#error_custom6, #error_custom7").html('');
		if(kepada==''){
			$("body").removeClass("loading");
			$("#error_custom6").html('<i style="color:#dd4b39; font-size:12px;">Kepada belum diisi</i>');
			setErrorFocus($("#kepada_yth_s25"), $("#role-form"), false);
			return false;
		}
		if(melalui==''){
			$("body").removeClass("loading");
			$("#error_custom7").html('<i style="color:#dd4b39; font-size:12px;">Melalui belum diisi</i>');
			setErrorFocus($("#melalui"), $("#role-form"), false);
			return false;
		} */
		...
		var tanggal_s25				= new Date(tgl_auto($("#tanggal_s25").val())); 
		var tgl_permohonan_banding	= new Date(tgl_auto($("#tgl_permohonan_banding").val()));
		var hariIni 				= new Date('<?php echo date('Y-m-d');?>');

		$("#error_custom1, #error_custom2,#error_custom3,#error_custom4,#error_custom5,#error_custom6,#error_custom7,#error_custom8,#error_custom9,#error_custom10").html('');
		...
		if(fname && $.inArray(extnya, arrExt) == -1){
			...
			setErrorFocus($("#error_custom10"), $("#role-form"), false);
			return false;
		} else if(tanggal_s25 < tanggal_putusan){
			$("body").removeClass("loading");
			$("#error_custom1").html('<i style="color:#dd4b39; font-size:12px;">Tanggal S25 harus lebih besar atau sama dengan tanggal '+war2+'</i>');
			setErrorFocus($("#tanggal_s25"), $("#role-form"), false);
			return false;
		} else if(tanggal_s25 > hariIni){
			$("body").removeClass("loading");
			$("#error_custom1").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal S25 adalah hari ini</i>');
			setErrorFocus($("#tanggal_s25"), $("#role-form"), false);
			return false;
		} else if (tgl_permohonan_banding && tgl_permohonan_banding < tanggal_putusan ) {
			$("body").removeClass("loading");
			$("#error_custom8").html('<i style="color:#dd4b39; font-size:12px;">Tanggal pernyataan banding harus lebih besar atau sama dengan tanggal '+war2+'</i>');
			setErrorFocus($("#tgl_permohonan_banding"), $("#role-form"), false);
			return false;
		} else if (tgl_permohonan_banding && tgl_permohonan_banding > hariIni ) {
			$("body").removeClass("loading");
			$("#error_custom8").html('<i style="color:#dd4b39; font-size:12px;">Maximal tanggal pernyataan banding adalah hari ini</i>');
			setErrorFocus($("#tgl_permohonan_banding"), $("#role-form"), false);
			return false;
		} else{
		/* 	$.ajax({
