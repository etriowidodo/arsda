...
<div class="row">
	...
	<div class="col-md-5">
		<div class="form-group">
			<div class="col-md-12">
				<select id="mjpn_q2" name="mjpn_q2" style="width:100%;">
					<option></option>
					<?php 
						$resOpt = MenuSearch::findBySql("select inst_satkerkd,inst_nama from kepegawaian.kp_inst_satker order by inst_satkerkd")->asArray()->all();
						foreach($resOpt as $dOpt){
							echo '<option value="'.$dOpt['inst_satkerkd'].'">'.$dOpt['inst_nama'].'</option>';
						}
					?>
				</select>
			</div>                    
		...
		<div class="form-group">
			<div class="col-md-12">
				<div class="input-group">
                    ...
                    <div class="input-group-btn"><button class="btn btn-warning" type="submit" name="btnCariM1" id="btnCariM1">Cari</button></div>
                </div>
			</div>                    
		</div>
	</div>
...
$(document).ready(function(){
	...
	$("#mjpn_q2").select2({placeholder: "Pilih wilayah kejaksaan", allowClear: true});
	...
	$('#myPjaxModalJpn').on('pjax:send', function(){
		...
		//var modalnyaDataJPN = JSON.parse(localStorage.getItem('modalnyaDataJPN')) || {};
		$(".jpn-index").find(".selection_one_jpn").each(function(i, v){
			var idnya = $(v).val().toString().split('#');
			(idnya[0] in formValues)?$(v).prop({'checked':'checked', 'disabled':'disabled'}):$(v).removeProp('checked disabled');
