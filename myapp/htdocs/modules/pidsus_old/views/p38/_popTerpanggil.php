...
$(document).ready(function() {
    ...
    // process the form
    $('#SuratPanggilan-form').submit(function(event) {

        // get the form data
        // there are many ways to get this data using jQuery (you can use the class or id also)
         var data=$("#SuratPanggilan-form").serialize();
        // process the form
        $.ajax({
            type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
            url         : 'p38/show-terpanggil', // the url where we want to POST
		  ...
		  dataType:'json'
            ...
            .done(function(data) {
                ...
                $('#tbody_tersangka').append(
                        '<tr id="tr_id'+data['id_pds_tut_surat_panggilan']+'">'+
                            '<td>' +
                            data['nama_terpanggil']+
                                '</td>' +
             				'<td>'+
             				$('#pdstutsuratpanggilan-keterangan').val()+
                            '</td>'+    
                            '<td><span class="glyphicon glyphicon-pencil" onclick="editTersangka(\''+data['id_pds_tut_surat_panggilan']+'\')"></span><a class="btn btn-danger btn-sm glyphicon glyphicon-remove hapus" onclick="hapusTersangka(\''+data['id_pds_tut_surat_panggilan']+'\')"></a></td>' +
                        '</tr>'
