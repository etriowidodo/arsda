...
use app\modules\pidsus\models\PdsLidSuratforPidsus6Lid;
 ...
 */
class Pidsus6Controller extends Controller
{
    ...
    public function actionIndex()
    {
    	...
    	$model= PdsLidSuratforPidsus6Lid::findBySql("select * from pidsus.pds_lid_surat where id_pds_lid='".$id."' and id_jenis_surat='pidsus6' and create_by='".(string)Yii::$app->user->identity->username."' order by create_date desc limit 1")->one();
    	if($model==null){
    		$model = new PdsLidSurat();
	        $model->id_jenis_surat='pidsus6';
	        $model->id_pds_lid=$id;
    ...
    {
       $model = PdsLidSuratforPidsus6Lid::findOne($id);
       $modelLid = $this->findModelLid($model->id_pds_lid);
       //$modelStatus=Status::findOne($modelLid->id_status);
       $_SESSION['startDatePidsus8']=new \DateTime($this->getStartDate('pidsus8'));
       $_SESSION['startDatePidsus8']=$_SESSION['startDatePidsus8']->format('d m Y');
    ...
    {
        if (($model = PdsLidSuratforPidsus6Lid::findOne($id)) !== null) {
            return $model;
        } else {
