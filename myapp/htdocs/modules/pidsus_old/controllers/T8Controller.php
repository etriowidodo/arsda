...
use app\modules\pidsus\models\PdsDikSuratforT8;
...
{
    public $idJenisSurat='t8';
    //public $perihalSurat='SURAT PENAHANAN/PENGALIHAN JENIS PENAHANAN';
    public $title='T8 - Surat Perintah Penangguhan Penahanan/Pengeluaran dari Tahanan';
    ...
    public function actionUpdate($id)
    {
        ...
        $countSuratDetailSyarat= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Syarat\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailSyarat'])){
                $modelDetailSyarat= $_POST['modelDetailSyarat'];
            }
            else $modelDetailSyarat=null;

            if($modelDetailSyarat!=null){
                for ($i = 0; $i < count($modelDetailSyarat); $i++) {
                    $countSuratDetailSyarat++;
                    $modelDetailNew= new PdsDikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat=$model->id_pds_dik_surat;
                    $modelDetailNew->no_urut=3;
                    $modelDetailNew->sub_no_urut=$countSuratDetailSyarat;
                    $modelDetailNew->tipe_surat_detail='Syarat';
                    $modelDetailNew->isi_surat_detail=$modelDetailSyarat[$i];
                    $modelDetailNew->save();
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/t8']);
            }
            else {
    ...
    {
        if (($model = PdsDikSuratforT8::find()->where(['id_pds_dik_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
