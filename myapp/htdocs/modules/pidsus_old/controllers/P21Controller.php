...
use app\modules\pidsus\models\PdsTutSuratforP21;
	...
	*/
    ...
    {	
    	if(isset($_SESSION['idPdsTut'])){
    		...
    		$modelPdsTut=PdsTut::find()->where(['id_pds_lid_parent'=>$_SESSION['idPdsDik']])->one();
        ...
        $model = $this->findModel($idPdsTut,'p21');
		$modelTut = $this->findModelTut($id);
		...
		if ($model->load(Yii::$app->request->post()) ) {
			...
			if($_POST['PdsTut']['is_tut']==1 ){
				$ip = (string)$_SERVER['REMOTE_ADDR'];
				$sql = "select * from pidsus.kirim_pratut_ke_tut('".$modelTut->id_pds_tut."','".(string)Yii::$app->user->identity->username."','".$ip."')";
        	...
        	if ($_POST['btnSubmit']=='simpan'){
        		...
        		$_SESSION['cetak']=1; return $this->refresh();   //return $this->redirect(['../pidsus/tutt/viewreporttut', 'id'=>$model->id_pds_tut_surat]);
        	}
    ...
    {
        if (($model = PdsTutSuratforP21::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Pemberitahuan hasil penyidikan perkara pidana a. n. .......... sudah lengkap';
			$model->save();
