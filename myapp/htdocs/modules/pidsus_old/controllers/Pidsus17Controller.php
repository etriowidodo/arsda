...
use app\modules\pidsus\models\PdsDikSuratforPidsus17;
 ...
 */
class Pidsus17Controller extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsDik,'pidsus17');
		$_SESSION['startDatePidsus17']=new \DateTime($this->getStartDate('pidsus20c'));
    	$_SESSION['startDatePidsus17']=$_SESSION['startDatePidsus17']->format('d m Y');
	 ...
	 public function getStartDate($jenisSurat){
    	$modelP17int=PdsDikSurat::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$_SESSION['idPdsDik'].'\'')->one();
    ...
    {
        if (($model = PdsDikSuratforPidsus17::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
            $model= new PdsDikSurat();
			$model->id_pds_dik=$id;
			$model->id_jenis_surat=$jenisSurat;
			//$model->kepada='Ketua Pengadilan Negeri';
			$model->create_by=(string)Yii::$app->user->identity->username;
			$model->create_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Pemberitahuan Tidak Dapat Dilakukan Penyitaan';
			$model->save();
