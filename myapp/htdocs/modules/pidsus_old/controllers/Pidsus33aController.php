...
use app\modules\pidsus\models\PdsDikSuratforPidsus33A;
 ...
 */
class Pidsus33aController extends Controller
{
	public $idJenisSurat='pidsus33a';
	public $perihalSurat='SURAT PENUNJUKAN PENASIHAT HUKUM UNTUK MENDAMPINGI TERSANGKA';
	public $title='Pidsus 33 A - Surat Penunjukan Penasihat Hukum Untuk Mendampingi Tersangka';
    ...
    public function actionUpdate($id)
    {
       ...
       if ($model->load(Yii::$app->request->post()) ) { 
			...
			if ($_POST['btnSubmit']=='simpan'){
				...
				return $this->redirect(['../pidsus/pidsus33a']);
        	}
        	else {
    ...
    {
        if (($model = PdsDikSuratforPidsus33A::find()->where(['id_pds_dik_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
