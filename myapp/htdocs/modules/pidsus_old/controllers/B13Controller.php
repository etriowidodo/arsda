...
use app\modules\pidsus\models\PdsDikSuratforB13;
 ...
 */
class B13Controller extends Controller
{
	public $idJenisSurat='b13';
	public $perihalSurat='Permohonan izin untuk melelang benda sitaan / barang bukti berdasarkan pasal 45 KUHAP';
	public $title='B13 - Permohonan Izin untuk Melelang Benda Sitaan / Barang Bukti';
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($id);
        $modelb12 = $this->findModel($idPdsDik,$modelDik,'b12');
        //echo $modelb12;
		$_SESSION['startDateB13']= date('d m Y', strtotime('-3 days'));
    	 $_SESSION['endDateB13']=date('d m Y', strtotime('+1 days'));
        ...
        $countSuratDetailAlasanLelang= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'AlasanLelang\'')->queryScalar();
        $modelSuratDetail=PdsdikSuratDetail::findBySql('select * from pidsus.select_surat_detail_dik(\''.$modelb12->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\')  order by no_urut,sub_no_urut')->all();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailAlasanLelang'])){
                $modelDetailAlasanLelang= $_POST['modelDetailAlasanLelang'];
            }
            else $modelDetailAlasanLelang=null;

            if($modelDetailAlasanLelang!=null){
                for ($i = 0; $i < count($modelDetailAlasanLelang); $i++) {
                    $countSuratDetailAlasanLelang++;
                    $modelDetailNew= new PdsDikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat=$model->id_pds_dik_surat;
                    $modelDetailNew->no_urut=1;
                    $modelDetailNew->sub_no_urut=$countSuratDetailAlasanLelang;
                    $modelDetailNew->tipe_surat_detail='AlasanLelang';
                    $modelDetailNew->isi_surat_detail=$modelDetailAlasanLelang[$i];
                    $modelDetailNew->save();
    ...
    {
        if (($model = PdsDikSuratforB13::find()->where(['id_pds_dik_surat'=>$id])->one()) !== null) {
            return $model;
        } else {
            return $this->redirect(['index']);
        }
