...
use app\modules\pidsus\models\PdsDikSuratforB7;
 ...
 */
class B7Controller extends Controller
{
	public $idJenisSurat='b7';
	public $perihalSurat='Copy Pemberitahuan Surat Perintah Penyitaan Barang Bukti oleh Kejaksaan';
	public $title='B7 - Copy Pemberitahuan Surat Perintah Penyitaan Barang Bukti oleh Kejaksaan';
	...
	*/
    ...
    {	
        ...
        $countSuratDetailBarang= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Barang\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) { 
            ...
            if(isset($_POST['modelDetailBarang'])){
                $modelDetailBarang= $_POST['modelDetailBarang'];
            }
            else $modelDetailBarang=null;

            if($modelDetailBarang!=null) {
                for ($i = 0; $i < count($modelDetailBarang); $i++) {
                    $countSuratDetailBarang++;
                    $modelDetailNew = new PdsDikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat = $model->id_pds_dik_surat;
                    $modelDetailNew->no_urut = 1;
                    $modelDetailNew->sub_no_urut = $countSuratDetailBarang;
                    $modelDetailNew->tipe_surat_detail = 'Barang';
                    $modelDetailNew->isi_surat_detail = $modelDetailBarang[$i];
                    $modelDetailNew->save();
                }
            }
    ...
    {
        if (($model = PdsDikSuratforB7::find()->where(['id_pds_dik_surat'=>$id])->one()) !== null) {
            return $model;
        } else {
        	return $this->redirect(['index']);
        }
