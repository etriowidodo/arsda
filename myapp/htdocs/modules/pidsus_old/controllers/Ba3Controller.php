...
use app\modules\pidsus\models\PdsDikSuratforBA3;
 ...
 */
class Ba3Controller extends Controller
{
    public $idJenisSurat='ba3';
    public $perihalSurat='BA Pengambilan Sumpah/Janji Orang Ahli';
    public $title='BA 3 - BA Pengambilan Sumpah/Janji Orang Ahli';
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelSuratSaksi->load(Yii::$app->request->post())  ) {
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/ba3']);
            }
            else {
    ...
    {
        if (($model = PdsDikSuratforBA3::find()->where(['id_pds_dik_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
