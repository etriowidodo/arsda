...
use app\modules\pidsus\models\PdsTutSuratforP20;
...
class P20Controller extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsTut,'p20');
    ...
    {
        if (($model = PdsTutSuratforP20::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Pemberitahuan bahwa waktu Penyidikan Tambahan perkara atas nama ..... yang melanggar pasal ..... sudah habis.';
			$model->save();
