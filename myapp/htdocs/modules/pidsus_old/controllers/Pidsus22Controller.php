...
use app\modules\pidsus\models\PdsDikSuratforPidsus22;
 ...
 */
class Pidsus22Controller extends Controller
{
    public $idJenisSurat='pidsus22';
    public $perihalSurat='Permohonan Tindakan';
    public $title='Pidsus 22 - Permohonan Tindakan';
    ...
    public function actionUpdate($id)
    {
        ...
        $countSuratDetailTindakanLain= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'TindakanLain\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailTindakanLain'])){
                $modelDetailTindakanLain= $_POST['modelDetailTindakanLain'];
            }
            else $modelDetailTindakanLain=null;

            if($modelDetailTindakanLain!=null){
                for ($i = 0; $i < count($modelDetailTindakanLain); $i++) {
                    $countSuratDetailTindakanLain++;
                    $modelDetailNew= new PdsDikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat=$model->id_pds_dik_surat;
                    $modelDetailNew->no_urut=1;
                    $modelDetailNew->sub_no_urut=$countSuratDetailTindakanLain;
                    $modelDetailNew->tipe_surat_detail='TindakanLain';
                    $modelDetailNew->isi_surat_detail=$modelDetailTindakanLain[$i];
                    $modelDetailNew->save();
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/pidsus22']);
            }
            else {
    ...
    {
        if (($model = PdsDikSuratforPidsus22::find()->where(['id_pds_dik_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
