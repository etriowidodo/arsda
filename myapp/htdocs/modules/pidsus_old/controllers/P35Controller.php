...
use app\modules\pidsus\models\PdsTutSuratforP35;
 ...
 */
class P35Controller extends Controller
{
    public $idJenisSurat='p35';
    public $perihalSurat='Laporan Pelimpahan Perkara atas nama terdakwa ... melanggar pasal ...';
    public $title='P35 - Laporan Pelimpahan Perkara';
    ...
    public function actionUpdate($id)
    {
        ...
        $countSuratDetailDakwaan= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_tut(\''.$model->id_pds_tut_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Dakwaan\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailDakwaan'])){
                  $modelDetailDakwaan= $_POST['modelDetailDakwaan'];
              }
              else $modelDetailDakwaan=null;

              if($modelDetailDakwaan!=null){
                  for ($i = 0; $i < count($modelDetailDakwaan); $i++) {
                      $countSuratDetailDakwaan++;
                      $modelDetailNew= new PdsTutSuratDetail();
                      $modelDetailNew->id_pds_tut_surat=$model->id_pds_tut_surat;
                      $modelDetailNew->no_urut=1;
                      $modelDetailNew->sub_no_urut=$countSuratDetailDakwaan;
                      $modelDetailNew->tipe_surat_detail='Dakwaan';
                      $modelDetailNew->isi_surat_detail=$modelDetailDakwaan[$i];
                      $modelDetailNew->save();
...
/*
  ...
  */
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/p35']);
            }
            else {
    ...
    {
        if (($model = PdsTutSuratforP35::find()->where(['id_pds_tut_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
