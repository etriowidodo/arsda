...
use app\modules\pidsus\models\PdsDikSuratforP14;
...
{
    public $idJenisSurat='p14';
    public $perihalSurat='SuratPerintah Penghentian Penyidikan';
    public $title='P14 - Surat Perintah Penghentian Penyidikan';
    ...
    {	
		...
		// $countSuratDetailMenimbang= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Menimbang\'')->queryScalar();
		...
		if ($model->load(Yii::$app->request->post()) ) {
			...
			else if (PdsDikSuratDetail::loadMultiple($modelSuratDetail, Yii::$app->request->post()) && PdsDikSuratDetail::validateMultiple($modelSuratDetail)){
				foreach($modelSuratDetail as $suratDetailRow){
					$suratDetailRow->save();
				}
			}
			...
			if ($_POST['btnSubmit']=='simpan'){
				...
				return $this->redirect(['../pidsus/p14']);
			}
			else {
       ...
       // $countSuratDetailMenimbang= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Menimbang\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
          ...
          /*  if(PdsDikSuratDetail::loadMultiple($modelSuratDetail, Yii::$app->request->post()) && PdsDikSuratDetail::validateMultiple($modelSuratDetail)){
            ...
            if(isset($_POST['modelDetailMenimbang'])){
                $modelDetailMenimbang= $_POST['modelDetailMenimbang'];
            }*/
            /*else $modelDetailMenimbang=null;

            if($modelDetailMenimbang!=null){
                for ($i = 0; $i < count($modelDetailMenimbang); $i++) {
                    $countSuratDetailMenimbang++;
                    $modelDetailNew= new PdsDikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat=$model->id_pds_dik_surat;
                    $modelDetailNew->no_urut=1;
                    $modelDetailNew->sub_no_urut=$countSuratDetailMenimbang;
                    $modelDetailNew->tipe_surat_detail='Menimbang';
                    $modelDetailNew->isi_surat_detail=$modelDetailMenimbang[$i];
                    $modelDetailNew->save();
...
*/
    ...
    {
    	if (($model = PdsDikSuratforP14::find()->where(['id_pds_dik'=>$_SESSION['idPdsDik'], 'id_jenis_surat'=>$this->idJenisSurat, 'flag'=>'1'])->one()) !== null) {
    		return $model->id_pds_dik_surat;
    	} else {
    		$idSurat=$this->getIdSurat();
    		return $idSurat;
    ...
    {
        if (($model = PdsDikSuratforP14::find()->where(['id_pds_dik_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
