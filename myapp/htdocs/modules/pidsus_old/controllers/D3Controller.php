...
use app\modules\pidsus\models\PdsTutSuratforD3;
...
{
    public $idJenisSurat='d3';
    public $perihalSurat='Tanda Terima Pembayaran Denda/Denda Ganti/Uang';
    public $title='D-3 : Tanda Terima Pembayaran Denda/Denda Ganti/Uang';
   ...
   //     $modelJaksa=KpPegawai::findBySql("select * from pidsus.get_jaksa_p8tut('".$model->id_pds_tut_surat."')")->all();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            */
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/d3']);
            }
            else {
    ...
    {
        if (($model = PdsTutSuratforD3::find()->where(['id_pds_tut_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
