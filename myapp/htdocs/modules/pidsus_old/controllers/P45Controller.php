...
use app\modules\pidsus\models\PdsTutSuratforP45;
 ...
 */
class P45Controller extends Controller
{
    public $idJenisSurat='p45';
    public $perihalSurat='Laporan putusan pengadilan dalam Perkara tindak pidana ............. atas nama Terdakwa ...........';
    public $title='P45 - Laporan Putusan Pengadilan';
    ...
    public function actionUpdate($id)
    {
        ...
        $countSuratDetailMenyatakan= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_tut(\''.$model->id_pds_tut_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Menyatakan\'')->queryScalar();
        $countSuratDetailMenuntut= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_tut(\''.$model->id_pds_tut_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Menuntut\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailMenyatakan'])){
                  $modelDetailMenyatakan= $_POST['modelDetailMenyatakan'];
              }
              else $modelDetailMenyatakan=null;

              if($modelDetailMenyatakan!=null){
                  for ($i = 0; $i < count($modelDetailMenyatakan); $i++) {
                      $countSuratDetailMenyatakan++;
                      $modelDetailNew= new PdsTutSuratDetail();
                      $modelDetailNew->id_pds_tut_surat=$model->id_pds_tut_surat;
                      $modelDetailNew->no_urut=1;
                      $modelDetailNew->sub_no_urut=$countSuratDetailMenyatakan;
                      $modelDetailNew->tipe_surat_detail='Menyatakan';
                      $modelDetailNew->isi_surat_detail=$modelDetailMenyatakan[$i];
                      $modelDetailNew->save();

                  }
              }

            if(isset($_POST['modelDetailMenuntut'])){
                $modelDetailMenuntut= $_POST['modelDetailMenuntut'];
            }
            else $modelDetailMenuntut=null;

            if($modelDetailMenuntut!=null){
                for ($i = 0; $i < count($modelDetailMenuntut); $i++) {
                    $countSuratDetailMenuntut++;
                    $modelDetailNew= new PdsTutSuratDetail();
                    $modelDetailNew->id_pds_tut_surat=$model->id_pds_tut_surat;
                    $modelDetailNew->no_urut=1;
                    $modelDetailNew->sub_no_urut=$countSuratDetailMenuntut;
                    $modelDetailNew->tipe_surat_detail='Menuntut';
                    $modelDetailNew->isi_surat_detail=$modelDetailMenuntut[$i];
                    $modelDetailNew->save();
...
/*
  ...
  */
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/p45']);
            }
            else {
    ...
    {
        if (($model = PdsTutSuratforP45::find()->where(['id_pds_tut_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
