...
use app\modules\pidsus\models\PdsTutSuratforP22;
 ...
 */
class P22Controller extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsTut,'p22');
    ...
    {
        if (($model = PdsTutSuratforP22::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Penyerahan Tersangka dan Barang Bukti';
			$model->save();
