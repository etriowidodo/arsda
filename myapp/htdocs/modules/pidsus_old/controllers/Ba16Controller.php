...
use app\modules\pidsus\models\PdsDikSuratforBA16;
 ...
 */
class Ba16Controller extends Controller
{
     public $idJenisSurat='ba16';
    public $perihalSurat='Berita Acara Penggeledahan/Penyitaan';
    public $title='BA-16  Berita Acara Penggeledahan/Penyitaan';
    ...
    public function actionUpdate($id)
    {
		...
		$countSuratDetailPenggeledahan= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Penggeledahan\'')->queryScalar();
        $countSuratDetailPenyitaan= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_dik(\''.$model->id_pds_dik_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'Penyitaan\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailPenggeledahan'])){
                $modelDetailPenggeledahan= $_POST['modelDetailPenggeledahan'];
            }
            else $modelDetailPenggeledahan=null;

            if($modelDetailPenggeledahan!=null){
                for ($i = 0; $i < count($modelDetailPenggeledahan); $i++) {
                    $countSuratDetailPenggeledahan++;
                    $modelDetailNew= new PdsDikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat=$model->id_pds_dik_surat;
                    $modelDetailNew->no_urut=1;
                    $modelDetailNew->sub_no_urut=$countSuratDetailPenggeledahan;
                    $modelDetailNew->tipe_surat_detail='Penggeledahan';
                    $modelDetailNew->isi_surat_detail=$modelDetailPenggeledahan[$i];
                    $modelDetailNew->save();

                }
            }

            if(isset($_POST['modelDetailPenyitaan'])){
                $modelDetailPenyitaan= $_POST['modelDetailPenyitaan'];
            }
            else $modelDetailPenyitaan=null;

            if($modelDetailPenyitaan!=null){
                for ($i = 0; $i < count($modelDetailPenyitaan); $i++) {
                    $countSuratDetailPenyitaan++;
                    $modelDetailNew= new PdsdikSuratDetail();
                    $modelDetailNew->id_pds_dik_surat=$model->id_pds_dik_surat;
                    $modelDetailNew->no_urut=1;
                    $modelDetailNew->sub_no_urut=$countSuratDetailPenyitaan;
                    $modelDetailNew->tipe_surat_detail='Penyitaan';
                    $modelDetailNew->isi_surat_detail=$modelDetailPenyitaan[$i];
                    $modelDetailNew->save();
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/ba16']);
            }
            else {
    ...
    {
        if (($model = PdsDikSuratforBA16::find()->where(['id_pds_dik_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
