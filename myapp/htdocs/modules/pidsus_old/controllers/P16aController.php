...
use app\modules\pidsus\models\PdsTutSuratforP16A;
 ...
 */
class P16aController extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsTut,'p16a');
        $model16 = $this->findModel($idPdsTut,'p16');
        $modelJaksa=PdsTutSuratJaksa::find()->where('id_pds_tut_surat=\''.$model16->id_pds_tut_surat.'\' order by no_urut')->all();
		...
		$modelSuratTersangka=$this->findModelSuratTersangka($model->id_pds_tut_surat);
		//$modelTersangka=$this->findModelTersangka($modelSuratTersangka);
    ...
    {
        if (($model = PdsTutSuratforP16A::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
    ...
    protected function findModelSuratTersangka($idPdsTutSurat){
    	if(($modelSuratTersangka=PdsTutSuratTersangka::find()->where(['id_pds_tut_surat'=>$idPdsTutSurat])->one())!==null){
    		return $modelSuratTersangka;
    	}
    	else {
    		$modelSuratTersangka = new PdsTutSuratTersangka();
    		$modelSuratTersangka->id_pds_tut_surat =  $idPdsTutSurat;  
			$modelSuratTersangka->create_by=(string)Yii::$app->user->identity->username;
        	$modelSuratTersangka->create_date=date('Y-m-d H:i:s');
    ...
    protected function findModelTersangka($modelSuratTersangka){
    	if (is_null($modelSuratTersangka->id_tersangka)){
    		$modelTersangka = new PdsTutTersangka();
    		$modelTersangka->id_pds_tut =  $_SESSION['idPdsTut'];
    		$modelTersangka->create_by=(string)Yii::$app->user->identity->username;
    		$modelTersangka->create_date=date('Y-m-d H:i:s');
    		$modelTersangka->save();
    		$modelSuratTersangka->id_tersangka=$modelTersangka->id_pds_tut_tersangka;
    		$modelSuratTersangka->save();
    ...
    $rawData=Yii::$app->db->createCommand("select * from pidsus.get_list_tersangkatut_dkk('".$_SESSION['idPdsTut']."')")->queryAll();  //idsatker
