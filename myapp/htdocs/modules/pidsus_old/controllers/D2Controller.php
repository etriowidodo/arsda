...
use app\modules\pidsus\models\PdsTutSuratforD2;
...
{
    public $idJenisSurat='d2';
    public $perihalSurat='Surat Pernyataan Kesanggupan Melunasi Pembayaran Denda';
    public $title='D-2 : Surat Pernyataan Kesanggupan Melunasi Pembayaran Denda';
   ...
   //     $modelJaksa=KpPegawai::findBySql("select * from pidsus.get_jaksa_p8tut('".$model->id_pds_tut_surat."')")->all();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            */
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/d2']);
            }
            else {
    ...
    {
        if (($model = PdsTutSuratforD2::find()->where(['id_pds_tut_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
