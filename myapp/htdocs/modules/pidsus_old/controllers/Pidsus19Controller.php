...
use app\modules\pidsus\models\PdsDikSuratforPidsus19;
 ...
 */
class Pidsus19Controller extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsDik,'pidsus19');
    ...
    {
        if (($model = PdsDikSuratforPidsus19::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Usul tindakan penahanan tersangka ......';
			$model->save();
