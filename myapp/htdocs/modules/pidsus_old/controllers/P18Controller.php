...
use app\modules\pidsus\models\PdsTutSuratforP18;
...
class P18Controller extends Controller
{
	...
	*/
    ...
    public function actionIndex()
    {
        ...
        $model = $this->findModel($idPdsTut,'p18');
		...
		if ($model->load(Yii::$app->request->post()) ) {
        	...
        	if ($_POST['btnSubmit']=='simpan'){
				...
				$_SESSION['cetak']=1; return $this->refresh(); //	return $this->redirect(['../pidsus/default/viewreporttut',  'id' => $model->id_pds_tut_surat, 'target' => '_blank']);
    ...
    {
        if (($model = PdsTutSuratforP18::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Hasil Penyidikan atas Nama .......... yang Disangka Melanggar Pasal .......... belum lengkap';
			$model->save();
