...
use app\modules\pidsus\models\PdsTutSuratforP17Int;
 ...
 */
class P17intController extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsTut,'p17int');
		$_SESSION['startDateP17int']=new \DateTime($this->getStartDate('p16'));
    	$_SESSION['startDateP17int']=$_SESSION['startDateP17int']->format('d m Y');
        ...
        if ($model->load(Yii::$app->request->post()) ) { 
        	...
        	if ($_POST['btnSubmit']=='simpan'){
        		return $this->redirect(['../pidsus/tut/viewlaporan','id'=>$idPdsTut]);
        	}
        	else {
	 ...
	 public function getStartDate($jenisSurat){
    	$modelP17int=PdsTutSurat::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$_SESSION['idPdsTut'].'\'')->one();
    	return $modelP17int->tgl_surat;
    }
    /**
    ...
    {
        if (($model = PdsTutSuratforP17Int::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Permintaan Perkembangan Hasil Penyidikan atas nama ...... yang Disangka Melanggar Pasal .....';
			$model->save();
