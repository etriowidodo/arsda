...
use app\modules\pidsus\models\PdsTutSuratforPidsus22Tut;
...
class Pidsus22Controller extends Controller
{
    ...
    public $title='Pidsus 22 - Permintaan Perpanjangan Penahanan';
   ...
   //     $modelJaksa=KpPegawai::findBySql("select * from pidsus.get_jaksa_p8tut('".$model->id_pds_tut_surat."')")->all();
        ...
        if(isset($_SESSION['cetak'])){
            $_SESSION['cetak']=null;
            $link = "<script>window.open(\"../../pidsus/default/viewreporttut?id=$model->id_pds_tut_surat\")</script>";
            echo $link;
        }
    ...
    {
        if (($model = PdsTutSuratforPidsus22Tut::find()->where(['id_pds_tut_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
