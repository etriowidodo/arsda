...
use app\modules\pidsus\models\PdsDikSuratforPidsus21;
 ...
 */
class Pidsus21Controller extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsDik,'pidsus21');
    ...
    {
        if (($model = PdsDikSuratforPidsus21::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Usul Tindakan Lain';
			$model->save();
