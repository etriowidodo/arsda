...
use app\modules\pidsus\models\PdsTutSuratforP40;
 ...
 */
class P40Controller extends Controller
{
	...
	*/
    ...
    {	
    	if(isset($_SESSION['idPdsTut'])){
    		...
    		$modelPdsTut=PdsTut::find()->where(['id_pds_dik_parent'=>$_SESSION['idPdsTut']])->one();
			$idPdsTut=$modelPdsTut->id_pds_tut;
    	}
    	else{
    		return $this->redirect(['../pidsus/tut/index']);
    	}
        //$modelLid = $this->findModelLid($idPdsTut);
        $model = $this->findModel($idPdsTut,'p40');
    ...
    {
        if (($model = PdsTutSuratforP40::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_tut=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Perlawanan Jaksa Penuntut Umum terhadap Penetapan Ketua Pengadilan Negeri/ Penetapan Hakim *)';
			$model->save();
