...
use app\modules\pidsus\models\PdsDikSuratforP12;
...
class P12Controller extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsDik,'p12');
    ...
    {
        if (($model = PdsDikSuratforP12::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Laporan Perkembangan Penyidikan';
			$model->save();
