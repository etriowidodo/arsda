...
use app\modules\pidsus\models\PdsDikSuratforPidsus20B;
 ...
 */
class Pidsus20bController extends Controller
{
	...
	*/
    ...
    {	
        ...
        $model = $this->findModel($idPdsDik,'pidsus20b');
    ...
    {
        if (($model = PdsDikSuratforPidsus20B::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
        	$modelDik=PdsDik::findOne($id);
        	$modelLid=PdsLid::findOne($modelDik->id_pds_lid_parent);
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Permohonan bantuan pengawalan akan dilakukan tindakan penahanan / pelaksanaan Penetapan Hakim / Putusan Pengadilan';
			$model->save();
