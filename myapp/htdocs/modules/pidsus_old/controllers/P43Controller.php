...
use app\modules\pidsus\models\PdsTutSuratforP43;
 ...
 */
class P43Controller extends Controller
{
    public $idJenisSurat='p43';
    public $perihalSurat='Laporan Tuntutan Pidana Perkara Tindak Pidana ............. atas nama Terdakwa ...........';
    public $title='P43 - Laporan Tuntutan Pidana';
    ...
    public function actionUpdate($id)
    {
        ...
        $countSuratDetailAmarTuntutan= Yii::$app->db->createCommand('select count(*) from pidsus.select_surat_detail_tut(\''.$model->id_pds_tut_surat.'\',\''.Yii::$app->user->id.'\') Where tipe_surat_detail=\'AmarTuntutan\'')->queryScalar();
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(isset($_POST['modelDetailAmarTuntutan'])){
                  $modelDetailAmarTuntutan= $_POST['modelDetailAmarTuntutan'];
              }
              else $modelDetailAmarTuntutan=null;

              if($modelDetailAmarTuntutan!=null){
                  for ($i = 0; $i < count($modelDetailAmarTuntutan); $i++) {
                      $countSuratDetailAmarTuntutan++;
                      $modelDetailNew= new PdsTutSuratDetail();
                      $modelDetailNew->id_pds_tut_surat=$model->id_pds_tut_surat;
                      $modelDetailNew->no_urut=1;
                      $modelDetailNew->sub_no_urut=$countSuratDetailAmarTuntutan;
                      $modelDetailNew->tipe_surat_detail='AmarTuntutan';
                      $modelDetailNew->isi_surat_detail=$modelDetailAmarTuntutan[$i];
                      $modelDetailNew->save();
...
/*
  ...
  */
            ...
            if ($_POST['btnSubmit']=='simpan'){
                ...
                return $this->redirect(['../pidsus/p43']);
            }
            else {
    ...
    {
        if (($model = PdsTutSuratforP43::find()->where(['id_pds_tut_surat'=>$id, 'id_jenis_surat'=>$this->idJenisSurat])->one()) !== null) {
            return $model;
        } else {
