...
use app\modules\pidsus\models\PdsDikSuratforP13Dik;
 ...
 */
class P13dikController extends Controller
{
	...
	*/
    ...
    {	
        ...
        //$modelDik = $this->findModelLid($idPdsDik);
        $model = $this->findModel($idPdsDik,'p13dik');
    ...
    {
        if (($model = PdsDikSuratforP13dik::find()->where('id_jenis_surat=\''.$jenisSurat.'\' and id_pds_dik=\''.$id.'\'')->one()) !== null) {
            return $model;
        } else {
			...
			$model->update_ip=(string)$_SERVER['REMOTE_ADDR'];
			$model->perihal_lap='Usulan Penghentian Penyidikan';
			$model->save();
