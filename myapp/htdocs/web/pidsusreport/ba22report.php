...
$reportFunction->getSuratJaksaSaksiTutNoJabatan($odf,$_GET['id'],$connection);
$reportFunction->getSuratJaksaSaksiTutNoJabatan($odf,$_GET['id'],$connection,'ttdsks');
//$reportFunction->getTtdSuratJaksa($odf,$_GET['id'],$connection);
//$reportFunction->getTtdSuratSaksi($odf,$_GET['id'],$connection);
...
// get info jaksa pertama
$query="select * from pidsus.get_data_jaksa('".$isi2."');";//query to get form's basic setup
//echo $query;
$row = $connection->createCommand($query)->queryOne();
$odf->setVars('nama1', $row['nama']);
$odf->setVars('nip1', $row['nip']);
$odf->setVars('pangkat1',$row['pangkat']);
$odf->setVars('jabatan1',$row['jabatan']);
$connection->close();
// end

// get info jaksa kedua
$query="select * from pidsus.get_data_jaksa('".$isi3."');";//query to get form's basic setup
//echo $query;
$row = $connection->createCommand($query)->queryOne();
$odf->setVars('nama2', $row['nama']);
$odf->setVars('nip2', $row['nip']);
$odf->setVars('pangkat2',$row['pangkat']);
$odf->setVars('jabatan2',$row['jabatan']);
$connection->close();
// end
...
$odf->exportAsAttachedFile('BA 22.odt');
