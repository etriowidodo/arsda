...
$odf = new odf("pidsus10report.odt");
...
$reportFunction->getSuratDetail($odf,$_GET['id'],'DaftarPaket',$connection);
...
$odf->setVars('tgl_surat', $row['tgl_surat_terbilang']);
...
QRcode::png(($row['id_satker'].'.PDS.Pidsus-10.'.$row['no_surat']), $qrtemplate, $errorCorrectionLevel, $matrixPointSize, 2);
...
//print_r($row);
// We export the file
$AwalKapital = ucfirst(strtolower($row['nama_ttd']));
$odf->setVars('nama', $AwalKapital);
$isi1hari =$reportFunction->getNamaHari($tgl_surat);
$odf->setVars('hari_surat', $isi1hari);
$odf->exportAsAttachedFile('Pidsus 10.odt');
