...
$odf->setVars('namaSatker', $row['inst_nama']);
$odf->setVars('noSurat', $row['no_surat']);
$odf->setVars('sifatSurat', $reportFunction->getParameterValue($row['sifat_surat'],$connection));
$odf->setVars('lampiranSurat', $row['lampiran_surat']);
$odf->setVars('perihalSurat', $row['perihal_surat']);
$odf->setVars('lokasiSurat', $row['lokasi_surat']);
$odf->setVars('tglSurat', $row['tgl_surat']);
$odf->setVars('penerimaSurat', $row['penerima']);
$odf->setVars('lokasiPenerimaSurat', $row['kepada_lokasi']);
$odf->setVars('jabatanTTD', $row['']);
$odf->setVars('pangkatTTD', $row['peg_golakhir']);
$odf->setVars('nipTTD', $row['peg_nip']);
...
//print_r($row);
// We export the file
$odf->exportAsAttachedFile('pidsus3a.doc');
