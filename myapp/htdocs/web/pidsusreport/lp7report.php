...
$query="select * from pidsus.rpt_pds_lp7 ('".$_GET['ID_SATKER']."','".$_GET['ID_JENIS_KASUS']."','".$_GET['BULAN']."','".$_GET['TAHUN']."')";//query to get form's basic setup

//echo $query;
//$rows = $connection->createCommand($query)->queryAll();
$odf = new odf("lp7report.odt");
/*

$i=1;
$lp7 = $odf->setSegment('lp7');
foreach($rows as $currentRow) {

 $lp7->nomor($currentRow['nomor']);
 $lp7->kejaksaan($currentRow['kejaksaan']);
 $lp7->sisa_bulan_lalu($currentRow['sisa_bulan_lalu']);
 $lp7->masuk_bulan_lap($currentRow['masuk_bulan_lap']);
 $lp7->jumlah($currentRow['jumlah']);
 $lp7->lengkap($currentRow['lengkap']);
 $lp7->berkas_kembali($currentRow['berkas_kembali']);
 $lp7->dpt_dilengkapi($currentRow['dpt_dilengkapi']);
 $lp7->tdk_dpt_dilengkapi($currentRow['tdk_dpt_dilengkapi']);
 $lp7->tdk_kembali_jpu($currentRow['tdk_kembali_jpu']);
 $lp7->jumlah2($currentRow['jumlah2']);
 $lp7->sisa($currentRow['sisa']);
 $lp7->keterangan($currentRow['keterangan']);
 $lp7->merge();
 $i++;
}
$odf->mergeSegment($lp7);
$reportFunction->closeConnection($connection);
$connection=$reportFunction->openConnection();
$query2="select * from pidsus.rpt_pds_lp7_sum ('".$_GET['ID_SATKER']."','".$_GET['ID_JENIS_KASUS']."','".$_GET['BULAN']."','".$_GET['TAHUN']."')";//query to get form's basic setup
...
$reportFunction->closeConnection($connection);
// We export the file
$odf->exportAsAttachedFile("LP 7.odt");
