...
$reportFunction->getSuratSaksiDik($odf,$_GET['id'],$connection);
//$reportFunction->getSuratJaksaDikNoJabatan($odf,$_GET['id'],$connection);
//$reportFunction->getSuratJaksaSaksiDikNoJabatan($odf,$_GET['id'],$connection);
//$reportFunction->getSuratJaksaSaksiDikNoJabatan($odf,$_GET['id'],$connection,'ttdsks');
...
$odf->setVars('tgl_surat_terbilang', $row['no_surat']);
$odf->setVars('jam_surat',$row['sifat_surat']);
...
$odf->setVars('isi4', '');
$odf->setVars('isi15', '');
//$odf->setVars('jenis_kasus', $row['jenis_kasus']);
...
// get info jaksa saksi1
$query="select * from pidsus.get_data_jaksa('".$isi3."');";//query to get form's basic setup
//echo $query;
$row = $connection->createCommand($query)->queryOne();
$odf->setVars('namasks1', $row['nama']);
$odf->setVars('nipsks1', $row['nip']);
$odf->setVars('pangkatsks1',$row['pangkat']);
$connection->close();
// end

// get info jaksa saksi2
$query="select * from pidsus.get_data_jaksa('".$isi4."');";//query to get form's basic setup
//echo $query;
$row = $connection->createCommand($query)->queryOne();
$odf->setVars('namasks2', $row['nama']);
$odf->setVars('nipsks2', $row['nip']);
$odf->setVars('pangkatsks2',$row['pangkat']);
$connection->close();
// end
...
$AwalKapital = ucfirst(strtolower($isi15));
$odf->setVars('Isi15', $AwalKapital);
// get umur
//kapital jenis ba
//$umurSaksi = $reportFunction->getUmur($isi_5);
//$odf->setVars('umur_saksi', $umurSaksi);
// set nama hari
//$isi1 =strtoupper($isi1);
//$odf->setVars('isi1', strtoupper($isi1));
