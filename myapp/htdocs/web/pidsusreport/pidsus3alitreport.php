...
$odf = new odf("pidsus3alitreport.odt");
...
QRcode::png(($row['id_satker'].'.PDS.PIDSUS-3A.'.$row['no_surat']), $qrtemplate, $errorCorrectionLevel, $matrixPointSize, 2);
$odf->setImage('QRCODE',$locationTempQR.md5($row['no_surat']).'.png');
// ~~~~~ end barcode ~~~~~~~~~~~

// untuk surat isi yang pakai format khusus bisa diubah disini
if ($isi4 != 'ditindaklanjuti dengan dikirim ke instansi atau unit lain') {
	$odf->setVars('isi5', '');
}
else {$odf->setVars('isi5', 'yaitu '.$isi5);}
...
//print_r($row);
// We export the file
//echo 'isi 4: '.$isi4.' isi5 ='.$isi5;
$odf->exportAsAttachedFile('Pidsus 3A Setelah Penelitian.odt');
