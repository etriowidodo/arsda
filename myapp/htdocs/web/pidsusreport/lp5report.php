...
$query="select * from pidsus.rpt_pds_lp5 ('".$_GET['ID_SATKER']."','".$_GET['ID_JENIS_KASUS']."','".$_GET['BULAN']."','".$_GET['TAHUN']."')";//query to get form's basic setup

//echo $query;
//$rows = $connection->createCommand($query)->queryAll();
$odf = new odf("lp5report.odt");

/*
$i=1;
$lp5 = $odf->setSegment('lp5');
foreach($rows as $currentRow) {

 $lp5->nomor($currentRow['nomor']);
 $lp5->kejaksaan($currentRow['kejaksaan']);
 $lp5->sisa_bulan_lalu($currentRow['sisa_bulan_lalu']);
 $lp5->masuk_bulan_lap($currentRow['masuk_bulan_lap']);
 $lp5->jumlah($currentRow['jumlah']);
 $lp5->lengkap($currentRow['lengkap']);
 $lp5->berkas_kembali($currentRow['berkas_kembali']);
 $lp5->dpt_dilengkapi($currentRow['dpt_dilengkapi']);
 $lp5->tdk_dpt_dilengkapi($currentRow['tdk_dpt_dilengkapi']);
 $lp5->tdk_kembali_jpu($currentRow['tdk_kembali_jpu']);
 $lp5->jumlah2($currentRow['jumlah2']);
 $lp5->sisa($currentRow['sisa']);
 $lp5->keterangan($currentRow['keterangan']);
 $lp5->merge();
 $i++;
}
$odf->mergeSegment($lp5);
$reportFunction->closeConnection($connection);
$connection=$reportFunction->openConnection();
$query2="select * from pidsus.rpt_pds_lp5_sum ('".$_GET['ID_SATKER']."','".$_GET['ID_JENIS_KASUS']."','".$_GET['BULAN']."','".$_GET['TAHUN']."')";//query to get form's basic setup
...
$reportFunction->closeConnection($connection);
// We export the file
$odf->exportAsAttachedFile("LP 5.odt");
