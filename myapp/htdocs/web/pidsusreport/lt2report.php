...
$query="select * from pidsus.rpt_pds_lt2 ('".$_GET['ID_SATKER']."','".$_GET['ID_JENIS_KASUS']."','".$_GET['BULAN']."','".$_GET['TAHUN']."')";//query to get form's basic setup

//echo $query;
//$rows = $connection->createCommand($query)->queryAll();
$odf = new odf("lt2report.odt");
/*

$i=1;
$lt2 = $odf->setSegment('lt2');
foreach($rows as $currentRow) {

 $lt2->nomor($currentRow['nomor']);
 $lt2->kejaksaan($currentRow['kejaksaan']);
 $lt2->sisa_bulan_lalu($currentRow['sisa_bulan_lalu']);
 $lt2->masuk_bulan_lap($currentRow['masuk_bulan_lap']);
 $lt2->jumlah($currentRow['jumlah']);
 $lt2->lengkap($currentRow['lengkap']);
 $lt2->berkas_kembali($currentRow['berkas_kembali']);
 $lt2->dpt_dilengkapi($currentRow['dpt_dilengkapi']);
 $lt2->tdk_dpt_dilengkapi($currentRow['tdk_dpt_dilengkapi']);
 $lt2->tdk_kembali_jpu($currentRow['tdk_kembali_jpu']);
 $lt2->jumlah2($currentRow['jumlah2']);
 $lt2->sisa($currentRow['sisa']);
 $lt2->keterangan($currentRow['keterangan']);
 $lt2->merge();
 $i++;
}
$odf->mergeSegment($lt2);
$reportFunction->closeConnection($connection);
$connection=$reportFunction->openConnection();
$query2="select * from pidsus.rpt_pds_lt2_sum ('".$_GET['ID_SATKER']."','".$_GET['ID_JENIS_KASUS']."','".$_GET['BULAN']."','".$_GET['TAHUN']."')";//query to get form's basic setup
...
$reportFunction->closeConnection($connection);
// We export the file
$odf->exportAsAttachedFile("LT 2.odt");
