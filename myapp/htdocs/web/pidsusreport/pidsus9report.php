...
$reportFunction->getSuratDetail($odf,$_GET['id'],'Berkas',$connection);
...
$query="select  i.inst_nama nama_satker ,initcap(i.inst_nama)::varchar nama_satker2,i.inst_lokinst lokasi_satker,initcap(i.inst_lokinst)::varchar  lokasi_satker2, s.no_surat , pd.nama_detail,
  s.lampiran_surat,s.perihal_lap perihal_surat ,  initcap(s.lokasi_surat)::varchar  ,pidsus.tanggal_tulisan(s.tgl_surat) tgl_surat, pidsus.tanggal_terbilang(s.tgl_surat) tgl_surat_terbilang
  ,s.kepada , initcap(s.kepada_lokasi)::varchar 
  ,pidsus.get_jabatan_ttd(s.jabatan,s.instansi) jabatan_ttd
  ,peg_nama nama_ttd, coalesce(pangkat_jaksa,pangkat) pangkat_ttd,peg_nip_baru nip_ttd,
  to_char(s.jam_surat, 'HH24:MI')::varchar
  ,s.dari,l.flag_dirahasiakan,s.status_lap
  ...
  ,s.peg_nama2 nama_ttd2,pidsus.get_pangkat_jaksa(s.peg_nik2),s.peg_nip_baru2 nip_ttd2
  ,replace(replace (replace(i.inst_nama,'CABANG KEJAKSAAN NEGERI ',''),'KEJAKSAAN TINGGI ',''),'KEJAKSAAN NEGERI ','')::varchar 
   ,initcap(replace(replace (replace(i.inst_nama,'CABANG KEJAKSAAN NEGERI ',''),'KEJAKSAAN TINGGI ',''),'KEJAKSAAN NEGERI ','')::varchar)::varchar
...
left join pidsus.parameter_detail pd on s.sifat_surat = pd.id_detail
where s.id_pds_lid_surat = '".$_GET['id']."'";
//echo $query;
$row = $connection->createCommand($query)->queryOne();
//CMS_PIDSUS001_2#bowo tanggal 16 mei 2016
...
$odf->setVars('nama_satker', $row['nama_satker']);
	...
	$odf->setVars('jabatan_ttd2', $row['jabatan_ttd2']);
	$odf->setVars('nama_ttd2', $row['nama_ttd2']);
	$odf->setVars('pangkat_ttd2', $row['pangkat_ttd2']);
	$odf->setVars('nip_ttd2', $row['nip_ttd2']);

//MSM Change Code CMS_PIDSUS001_06
// ~~~~~ QR Code by Danar 11 mei 2016 ~~~~~~
$qrtemplate = $PNG_TEMP_DIR.'QR_'.md5($row['no_surat']).'.png';	
QRcode::png(($row['id_satker'].'.PDS.Pidsus-9.'.$row['no_surat']), $qrtemplate, $errorCorrectionLevel, $matrixPointSize, 2);
...
//echo $isi1;
//print_r($row);
$isi1hari =$reportFunction->getNamaHari($tgl_surat);
$odf->setVars('hari_surat', $isi1hari);
// We export the file
$odf->exportAsAttachedFile('Pidsus 9.odt');
